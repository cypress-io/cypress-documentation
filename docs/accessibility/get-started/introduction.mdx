...
- **DOM snapshots**: Each detected violation is accompanied by a [full-page, inspectable DOM snapshot](/accessibility/guides/inspecting-violation-details), highlighting the exact location and context of the violation.
- **Report generation**: [Detailed reports](/accessibility/guides/run-level-reports) are generated, offering sortable and filterable views by page, component, or the entire test run. Rule-based summaries for an aggregated view of accessibility outcomes for the entire run are also available.
...
- **Deep linking and sharing**: Easily share whatever you are looking at with your team at any time by copying the link, or using built-in sharing functionality and [Jira issue creation](/accessibility/guides/inspecting-violation-details#Create-Jira-issue).
