...
This brings the results into CI, where you review them and determine if they are acceptable or need to be addressed before code changes can merge. It provides overall accessibility scores and violation details for the runs, as well as page- or component-level feedback. This supports a wide variety of needs related to alerting about failures in specific focus areas of the application or creating fine-grained regression monitoring according to the current standards being met by each page.
...
- The guide for [catching accessibility regressions](/accessibility/guides/detect-changes#enforce-a-standard-in-ci) shows some other common use cases.

:::

## Supported CI Providers

Fetching Accessibility results for a run supports fetching results for the following CI providers. See the docs below for information on general setup.
...
Reach out to Cypress Support to request support for a different provider.
...
```javascript
{
  ...
  // The App Quality configuration the report was processed with.
  // Useful for detecting configuration changes that alter results
  // without any application code changing.
  config?: {
    // When the configuration was last saved, as an ISO 8601 string
    updatedAt: string
    // The configuration values applied to this run
    value: object
  }
