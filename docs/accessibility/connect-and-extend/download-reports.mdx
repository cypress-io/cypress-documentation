...
The file can also be used as evidence for an for non-conformance in accessibility conformance report. Its **Tags** column maps each violation to WCAG success criteria, and its deep links let a reviewer verify each finding. See [Use results to inform a VPAT](/accessibility/guides/inform-a-vpat).
