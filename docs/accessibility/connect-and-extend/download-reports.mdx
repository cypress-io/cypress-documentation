...
The export is a portable snapshot of a single run. Unlike the [Results API](/accessibility/connect-and-extend/results-api) (built for CI assertions) and [Cypress Cloud MCP](/cloud/integrations/cloud-mcp) (built for live querying), a downloaded report needs no Cloud connection to work with after the fact—making it a good fit for offline analysis, spreadsheets and charts, sharing with people who don't use Cypress Cloud, and comparing two runs by downloading a file for each. For other ways to get results out of Cypress Cloud, see the [Connect & extend overview](/accessibility/connect-and-extend/overview).
...
| Status            | The element's status: `fail`, `incomplete`, or `ignore`.                                |
...
You must be signed in and a member of the project's organization. On public projects, signed-in non-members can view the report but cannot download it. The run's Accessibility report must also have finished processing.

:::info

The download button is rolling out gradually. If you don't see it on a run that has a finished Accessibility report, reach out to your Cypress representative.
