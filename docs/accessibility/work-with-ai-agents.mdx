...
If you ask an agent to draft VPAT or accessibility conformance report entries from the file, point it at [Use results to inform a VPAT](/accessibility/guides/inform-a-vpat) first. Violations can support _Partially Supports_ or _Does Not Support_, but a criterion with no automated violations still needs human evaluation before it can be marked _Supports_.
