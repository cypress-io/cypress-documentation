...
For optional **policies** that gate runs on Accessibility and UI Coverage thresholds in Cloud, see the dedicated [policies](/accessibility/configuration/policies) guide.
  ...
  "policies": [
    {
      ...
      "validations": [
        {
          "metric": "accessibility.score" | "uiCoverage.score" | "accessibility.failedElements" | "uiCoverage.untestedElements",
          "minScore": number,
          "maxCount": number,
          "viewMatch": string,
          "comment": string
...
Fields `minScore` and `maxCount` apply only to the score and count metrics respectively (the Cloud configuration editor validates allowed fields for each metric).

## Next steps

Check out the following configuration guides.

- [Views](/accessibility/configuration/views)
- [View Filters](/accessibility/configuration/viewfilters)
- [Element Filters](/accessibility/configuration/elementfilters)
- [Attribute Filters](/accessibility/configuration/attributefilters)
- [Significant Attributes](/accessibility/configuration/significantattributes)
- [Components](/accessibility/configuration/components)
- [Profiles](/accessibility/configuration/profiles)
- [policies](/accessibility/configuration/policies)
