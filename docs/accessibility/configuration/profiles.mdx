...
`viewFilters` applies the first matching rule, so the two `include: true` rules keep login and checkout while the catch-all excludes everything else. Regression runs record without this tag and use the base configuration, scanning every view. Pair the narrow report with the [Results API](/accessibility/connect-and-extend/results-api) to fail the build when a critical flow regresses.
...
A profile's `elementFilters` replaces the base list rather than adding to it, so the profile repeats the cookie-banner rule it still needs and omits the support-chat rule. In Cypress Accessibility an element is scanned unless an `include: false` rule matches it, so dropping the support-chat exclusion is all it takes to bring the widget back into this run's report; you don't add an `include: true` rule. On every other run, both widgets stay excluded.
...
- **In CI**, read it from the [Accessibility Results API](/accessibility/connect-and-extend/results-api), where `config.value` is the applied App Quality configuration and `config.updatedAt` is when it was last saved.
