...
1. **Displaying results**: Results are displayed in Cypress Cloud in the run details area as well as in [Branch Review](/accessibility/guides/compare-reports) for comparing runs.
1. **Retreiving results**: Results are accessible via the [Results API](/accessibility/connect-and-extend/results-api) for optional CI pipeline actions (e.g., failing builds based on unmet standards) and via Cloud MCP for agentic triage.
...
Cypress Accessibility uses data captured through Test Replay, ensuring comprehensive analysis while being subject to its limitations. Read [How Test Replay Works](/cloud/features/test-replay#How-Test-Replay-Works) to discover which data is captured and which browsers are supported.
