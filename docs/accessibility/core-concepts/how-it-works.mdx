...
At a high level stages of the Cypress Accessibility process are as follows:
...
1. When your run completes, the final specs are processed for accessibility checks.
...
1. The completed results become available in Cypress Cloud and are flagged in integrations such Slack as GitHub/Gitlab comments, and available CI via the Results API so that you can optionally fail your pipeline if your standards are not met.
