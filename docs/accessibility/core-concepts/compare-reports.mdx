...
- each report may test thousands of application snapshots for a single Cypress
...
- **Resolved elements**: Counts of elements that had failures detected in the Base run which are no longer detected Changed run. These are also listed as improvements in each View, under the accessibility rules that had improvements.
