...
- The `accessibility.components` configuration object adds `componentAttributes` and `componentAttributeFilters`, so you can require specific HTML attributes in stable violation-target selectors and exclude certain attribute values when needed. This makes it easier to tell apart repeated patterns (for example, the same `data-cy` in different component instances). See the [components](/accessibility/configuration/components) guide.
