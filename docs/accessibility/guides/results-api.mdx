...
The package exports three async functions with identical call signatures; `getAccessibilityResults`, documented
...
```javascript
{
  ...
  rules: [{
    ...
    // e.g. 'critical', 'serious', 'moderate', or 'minor'
    severity: string
    // The status of the rule for the run, e.g. 'violation'.
    status: string
  ...
  // The resolved Accessibility configuration that was applied to this run.
...
run. It uses the same call signature as `getAccessibilityResults` and is product-agnostic. A single policy can
check both Accessibility and UI Coverage metrics.
...
Reports are generated asynchronously after a run finishes. If you call `getAccessibilityResults` before the
report has finished processing, the package does not fail. It polls for you automatically:
...
This means you can safely call `getAccessibilityResults` immediately after your Cypress run completes, so you
don't need to add your own sleep/wait logic.
