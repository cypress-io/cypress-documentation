...
getAccessibilityResults().then((results) => {
  ...
  Object.entries(baseline.views).forEach(([displayName, baselineRules]) => {
    ...
    const resolvedRules = baselineRules.filter(rule => !currentRules.includes(rule));
  ...
  const countOfViewsWithNewFailedRules = Object.keys(viewsWithNewFailedRules).length;
