...
Use Branch review when reviewing a pull request made by another developer or before you create one of your own. You can compare the report from your changed branch with the equivalent report on the main branch. The diff allows you to see any issues that your changes have improved or fixed completely, as well as net-new issues introduced, and any new instances of existing failures.

Since this reduces the amount of violations to review to just what was affected by the PR code changes, you won't spend any time looking for what's relevant -- and if there is no diff, you'll know your PR does not affect the accessibility score and is safe to merge.
...
To get a high-quality diff for a small set of tests, record the tests locally from your main branch, and then on the branch with changes. In Cypress Cloud, you will be able to compare these two runs knowing the cover exactly the same surface area and see only the diff in that area.
...
Comparing reports on a regular schedule can let you see if accessibility issues have been introduced between this week and last week, for example, and then find the specific run where the changes happened. That diff can can be forwarded to engineering or product teams as the basis of a ticket.
...
The Results API provides a combined report with all the failing rules included. This means you can easily prepare a list of known, permitted failing rules to compare with the results from any run, and choose to fail only if a rule that used to be fully passing has started to fail. This is described in detail in the guide related to [blocking pull requests and setting policies](/accessibility/guides/block-pull-requests).
...
As an example, customers can use this functionality to trigger Slack webhooks that notify a particular channel of the issues in specific areas. The Results API provides direct links to the Views and to each failed rule within that View, making it easy to forward useful links to the correct places.
...
This level of reporting is ideal for documenting progress and spotting unexpected trends. When you see a project's score dropping over the long term, you can dig in further with Branch Review to understand what changes are driving the score. You can compare reports before and after any shifts in scores to determine which accessibility issues were introduced.
...
There are no one-size-fits all solutions in accessibility. The level of detail you need and the kind of action you will want to take depend on the situation at hand.

Cypress Accessibility supports multiple workflows and levels of detail to react to and manage changes in accessibility, and shift the detection of problems as far to the left of the development lifecycle as the rest of your tests.

When changes are easy to discover and understand, your team will be able to focus on dealing with the issues that matter most, and not become frustrated by having their attention called to problems their work did not introduce. As well as finding new problems, seeing progress and celebrating the wins fosters the sense of momentum and collective ownership that is essential for large scale accessibility programs to succeed.
