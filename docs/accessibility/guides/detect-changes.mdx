...
- **The Axe Core® version.** Cypress updates its internal version periodically, and a new version can add rules or refine how existing ones are detected. Compare `axeVersion` against the version your team calibrated its expectations against, so a jump in violations is explained rather than mysterious. There is a ready-made check in [Detect a change in the Axe Core® version](/accessibility/results-api#how-to-detect-a-change-in-the-axe-core-version), and the update policy is in [Axe Core® library version and updates](/accessibility/configuration/axe-core-configuration#axe-core-library-version-and-updates).
