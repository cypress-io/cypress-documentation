...
is the _official_ CircleCI Orb of Cypress.

An orb is a piece of configuration set in your `.circleci/config.yml` file to
correctly install, cache and run Cypress with very little effort. Using the orb
brings simplicity and static checks of parameters to the CircleCI configuration.

Although you do not need to use the orb to run your tests in CircleCI, the
benefit is that it abstracts common steps necessary for running your tests in
...
A more complex project that needs to install dependencies and run tests across 4
CI machines [in parallel](/guides/guides/parallelization) may have:
  ...
  build:
    jobs:
      - cypress/run:
          ...
          # name this group "all tests" on the dashboard
          cypress-command:
...
Cypress uses Electron by default to run your tests. The
[`install-browsers`](https://circleci.com/developer/orbs/orb/circleci/browser-tools#usage-install_browsers)
flag is used to install Firefox and Chrome to run your tests. This is only
needed if you are passing the `--browser` flag in your `cypress-command`
