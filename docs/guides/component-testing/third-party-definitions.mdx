...
The main things to note is the `name` property, which follows the `@org/cypress-ct-*` convention, and is the `exports` property. The `node` entry points to the Framework Definition, and the `default` entry points to the Mount Adapter:
...
If you want to share your Framework Definition with us, please send a Pull Request - we would be happy to feature a link to your package!