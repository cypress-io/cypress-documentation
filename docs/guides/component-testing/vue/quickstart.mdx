...
up previously. We chain `then()` off of `cy.get()` to make sure the event has
fired and registered as such, then grab the alias to the spy (by prepending an
