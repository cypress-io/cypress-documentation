...
1. A test runs for the first time. If the
   <Icon name="check-circle" color="green" /> test passes, Cypress will
...
## Configure Test Retries {#Configure-Test-Retries}
...
#### Configure retry attempts for all modes {#Configure-retry-attempts-for-all-modes}
...
#### Individual Test(s) {#Individual-Tests}
...
### Only upload videos for specs with failing or retried tests {#Only-upload-videos-for-specs-with-failing-or-retried-tests}
...
### Will retried tests be counted as more than one test result in my billing? {#Will-retried-tests-be-counted-as-more-than-one-test-result-in-my-billing}
...
### Can I access the current attempt counter from the test? {#Can-I-access-the-current-attempt-counter-from-the-test}
