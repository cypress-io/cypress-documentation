...
| `runnerUi`                | _boolean_              | Whether to display the Cypress Runner UI. Defaults to `true` when [Test Replay](TODO: add test replay link) is not enabled. Defaults to `false` when [Test Replay](TODO: add test replay link) is enabled. |
