...
| `runnerUi`                | _boolean_              | Whether to display the Cypress Runner UI. Defaults to `false` when [Test Replay](/guides/cloud/debugging/test-replay) is enabled. Otherwise defaults to `true`. |
