...
compression can be enabled and configured with the
[`videoCompression`](/guides/references/configuration#Videos) property in order
...
completely, so the file size of the video will be larger, but the processing should be faster.
