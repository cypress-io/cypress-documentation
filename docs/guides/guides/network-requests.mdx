...
By default Cypress logs all `XMLHttpRequest`s and `fetch`es made by the application under
...
**Note:** Logging can be disabled off by passing `{ log: false }` with your static response.
