...
<Badge type="danger">Before</Badge> Attaching a fixture from disk with <code>cypress-file-upload</code>

```js
cy.get('[data-cy="file-input"]').attachFile('myfixture.json')
```

<Badge type="success">After</Badge> Selecting a fixture from disk with <code>.selectFile()</code>. Cypress follows paths from your project root (same as <a href="/api/commands/readfile"><code>cy.readFile()</code></a>).
...
<Badge type="danger">Before</Badge> Dragging and dropping a file with <code>cypress-file-upload</code>
...
<Badge type="success">After</Badge> Selecting a fixture from disk with <code>.selectFile()</code>. Cypress follows paths from the root of your test folder (same as <a href="/api/commands/readfile"><code>cy.readFile()</code></a>).
...
<Badge type="danger">Before</Badge> Working with file contents before using using <code>cypress-file-upload</code>
...
<Badge type="danger">Before</Badge> Specifying the MIME type with <code>cypress-file-upload</code>
...
<Badge type="success">After</Badge> Specifying a MIME type with <code>.selectFile()</code>
.
...
<Badge type="success">After</Badge> Stylesheets are supported via an import and <code>mountingOptions.stylesheets</code> is not recommended
...
<Badge type="danger">Before</Badge> Failed assertion that <code>opacity: 0</code> element is not visible.
...
<Badge type="success">After</Badge> Passed assertion that <code>opacity: 0</code> element is not visible.
...
<Badge type="danger">Before</Badge> Setting retries with <code>cypress-plugin-retries</code> via env vars
...
<Badge type="danger">Before</Badge> Setting retries with <code>cypress-plugin-retries</code> via configuration
...
<Badge type="danger">Before</Badge> Setting retries with <code>cypress-plugin-retries</code> via the test
...
<Badge type="danger">Before</Badge> Cookies yielded before had no <code>sameSite</code> property.
...
<Badge type="success">After</Badge> Cookies yielded now have <code>sameSite</code> property if specified.
...
<Badge type="danger">Before</Badge> <code>__dirname</code> / <code>__filename</code>
...
<Badge type="success">After</Badge> <code>__dirname</code> / <code>__filename</code>
...
<Badge type="success">After</Badge> Update spies and stubs should now use <code>resetHistory()</code>
.
...
<Badge type="success">After</Badge> Access the <code>args</code> property off <code>launchOptions</code>
...
<Badge type="danger">Before</Badge> Passing BrowserWindow options on the <code>launchOptions</code> object is no longer supported.
...
<Badge type="success">After</Badge> Pass BrowserWindow options on the <code>options.preferences</code> object instead.
...
<Badge type="success">After</Badge> Use <code>browser.name</code> and <code>browser.family</code> to select non-Electron Chromium-based browsers
