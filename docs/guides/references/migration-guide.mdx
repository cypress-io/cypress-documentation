...
### `.should()` and `.and()` are now query commands

In Cypress 13, [assertions](/api/commands/should) are now queries. Most tests
should continue to operate as before, but there are some differences to the
previous behavior to be aware of.

#### Why Cypress made this change

Assertions being queries allows for extremely flexible and fluent chaining of
Cypress commands. See
[#25134](https://github.com/cypress-io/cypress/issues/25134) for an example.
This change also lays the groundwork for (freely chaining Cypress
commands)[https://github.com/cypress-io/cypress/issues/25655] and eventually
native [`async / await`](https://github.com/cypress-io/cypress/issues/1417)
support.

#### Assertions now retry after _all_ commands.

Previously, many commands would only execute their assertions once, failing
immediately if the assertion did not match. However, assertions now execute
independently of the previous command, meaning they always retry. Remember that
_only chains of queries are rerun_ - non-query commands still only execute once.

In this example, `.then()` is a non-query command. It returns a specific
subject, which updates after 1s.
...
The callback provided to `.then()` is only executed once. In Cypress 12, this
command would fail immediately, because `.then()` only checked assertions once,
but beginning in Cypress 13, `.should()` retries, waiting for the property to
exist.

#### It is safe to chain further queries after assertions

```js
cy.get('form').should('have.class', 'ready').find('button').click()
```

This test waits for a form to exist and have the "ready" class before clicking
on a button inside of it. In Cypress 12, this test was vulnerable to detached
DOM errors, where if React or another javascript library replaced elements on
the page, Cypress could 'lose track' of them. In Cypress 13, this operation is
reliable - because `.get().should().find()` is composed entirely of query
commands, the whole chain will be retried while while trying to find and click
on a button.

#### Assertions are now included in aliases

Previously, assertions were non-query commands, meaning that if `.as()` was used
after them, they did not rerun, and the alias was vulnerable to the element
becoming detached from the DOM if the page updated.

```js
cy.get('.modal').should('be.visible').as('myModal')

cy.get('@myModal').invoke('hide')
cy.get('@myModal').should('not.be.visible')
```

Beginning in Cypress 13, the above test will always fail. `.as()` is aliasing
`.get('.modal').should('be.visible')`, which re-runs whenever the alias is read.
This makes the last line impossible, since an element can never be both visible
and not visible at the same time.

You can resolve this by moving any assertions you don't want to re-run after
`.as()`:

```diff
- cy.get('.modal').should('be.visible').as('myModal')
+ cy.get('.modal').as('myModal').should('be.visible')
```

#### Chains that include assertions now retry "all the way through"

Previously, assertions were non-query commands, meaning that they served as
"breakpoints" in a chain of queries, and queries before them did not re-run.
...
In Cypress 12, this test would probably pass; because `.should()` was not a
query, it broke the chain, and the above test could be thought of as having two
parts:

1. `.get(.button').should('have.class', 'animating')`
2. `.get('button').should('have.class', 'done-animating')`

However, in Cypress 13, the chain is composed entirely of queries, meaning the
entire chain of all four commands must pass at the same time.

You can resolve this by breaking up chains of commands that you don't want to
execute together with `cy`.
...
This change also lays the groundwork for
[freely chaining Cypress commands](https://github.com/cypress-io/cypress/issues/25655)
and eventually native
[`async / await`](https://github.com/cypress-io/cypress/issues/1417) support.
