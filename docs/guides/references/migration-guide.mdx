...
Assertions being queries allows for extremely flexible and fluent chaining of Cypress commands. See [#25134](https://github.com/cypress-io/cypress/issues/25134) for an example. This change also lays the groundwork for (freely chaining Cypress commands)[https://github.com/cypress-io/cypress/issues/25655] and eventually native [`async / await`](https://github.com/cypress-io/cypress/issues/1417) support.

#### Assertions now retry after *all* commands.
...
independently of the previous command, meaning they always retry.

```js
cy.then(() => {
  const a = {}
  setTimeout(() => { a.foo = true }, 1000)

  return a
}).should('have.property', 'foo')
```

The callback provided to `.then()` is still only executed once, but
beginning in Cypress 13, `.should()` will wait for the property to exist
even after non-query commands.

#### Assertions are now included in aliases

Previously, assertions were non-query commands, meaning that if `.as()`
was used after them, they did not rerun, and the alias was vulnerable to
the element becoming detached from the DOM if the page updated.
...
`.get('.modal').should('be.visible')`, which re-runs whenever the alias is
read. This makes the last line impossible, since an element can never be both
visible and not visible at the same time.

You can resolve this by moving any assertions you don't want to re-run
after `.as()`:
...
`readFile()` will re-read the file from disk if any upcoming
...
This change also lays the groundwork for (freely chaining Cypress commands)[https://github.com/cypress-io/cypress/issues/25655] and eventually native [`async / await`](https://github.com/cypress-io/cypress/issues/1417) support.
