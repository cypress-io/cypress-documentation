...
[Test Replay](/guides/cloud/debugging/test-replay) is enabled by default in `v13` of the Cypress App.
...
You can continue recording video by setting `video` to `true` either in your Cypress configuration or via [overriding options](/guides/references/configuration#Overriding-Options). This can be useful if you want video locally or want video for some other reason, like in non-Chromium browsers where [Test Replay](/guides/cloud/debugging/test-replay) is not available.
