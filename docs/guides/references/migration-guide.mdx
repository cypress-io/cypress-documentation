...
## Migrating to Cypress version 10.0 {#Migrating-to-Cypress-version-100}
...
### Cypress App Changes {#Cypress-App-Changes}
...
### Plugins File Removed {#Plugins-File-Removed}
...
#### `experimentalStudio` {#experimentalStudio}
...
#### `ignoreTestFiles` → `excludeSpecPattern` {#ignoreTestFiles--excludeSpecPattern}
...
#### `testFiles` → `specPattern` {#testFiles--specPattern}
...
### Updated Test File Locations {#Updated-Test-File-Locations}
...
### Command / Cypress API Changes {#Command--Cypress-API-Changes}
...
#### `Cypress.Commands.add()` {#CypressCommandsadd}
...
### Component Testing Changes {#Component-Testing-Changes}
...
## Migrating from `cypress-file-upload` to [`.selectFile()`](/api/commands/selectfile) {#Migrating-from-cypress-file-upload-to-selectFile}
...
#### Read and attach a fixture {#Read-and-attach-a-fixture}
...
<Badge type="success">After</Badge> Selecting a fixture from disk with `.selectFile()`.
Cypress follows paths from your project root (same as [`cy.readFile()`](/api/commands/readfile)).
...
#### Using drag-n-drop {#Using-drag-n-drop}
...
<Badge type="success">After</Badge> Selecting a fixture from disk with `.selectFile()`.
Cypress follows paths from the root of your test folder (same as [`cy.readFile()`](/api/commands/readfile)).
...
#### Overriding the file name {#Overriding-the-file-name}
...
<Badge type="danger">Before</Badge> Working with file contents before using using
`cypress-file-upload`
...
#### Specifying a custom mimeType {#Specifying-a-custom-mimeType}
...
## Migrating to Cypress 8.0 {#Migrating-to-Cypress-80}

This guide details the changes and how to change your code to migrate to Cypress
8.0. [See the full changelog for 8.0](/guides/references/changelog#8-0-0).

### `cypress run` runs all browsers `--headless` {#cypress-run-runs-all-browsers---headless}
...
### Default screen size during `--headless` {#Default-screen-size-during---headless}
...
#### URL matching is stricter {#URL-matching-is-stricter}
...
#### Deprecated `cy.route2()` command removed {#Deprecated-cyroute2-command-removed}
...
#### `res.delay()` and `res.throttle()` have been renamed {#resdelay-and-resthrottle-have-been-renamed}
...
#### Falsy values are no longer dropped in `StaticResponse` bodies {#Falsy-values-are-no-longer-dropped-in-StaticResponse-bodies}
...
#### Errors thrown by request and response handlers are no longer wrapped {#Errors-thrown-by-request-and-response-handlers-are-no-longer-wrapped}
...
#### 1. Remove `experimentalComponentTesting` config {#1-Remove-experimentalComponentTesting-config}
...
#### 2. Install component testing dependencies {#2-Install-component-testing-dependencies}
...
#### 3. Update plugins file to use `dev-server:start` {#3-Update-plugins-file-to-use-dev-serverstart}
...
from [@cypress/react](https://www.npmjs.com/package/@cypress/react)
...
#### Expanded stylesheet support {#Expanded-stylesheet-support}
...
#### Desktop GUI no longer displays component tests {#Desktop-GUI-no-longer-displays-component-tests}
...
#### Executing all or some component tests {#Executing-all-or-some-component-tests}
...
#### cypress-react-selector {#cypress-react-selector}
...
#### HTML Side effects {#HTML-Side-effects}
...
#### Legacy `cypress-react-unit-test` and `cypress-vue-unit-test` packages {#Legacy-cypress-react-unit-test-and-cypress-vue-unit-test-packages}
...
### Uncaught exception and unhandled rejections {#Uncaught-exception-and-unhandled-rejections}
...
#### Turn off all uncaught exception handling {#Turn-off-all-uncaught-exception-handling}
...
#### Turn off uncaught exception handling unhandled promise rejections {#Turn-off-uncaught-exception-handling-unhandled-promise-rejections}
...
### Node.js 12+ support {#Nodejs-12-support}
...
## Migrating `cy.route()` to [`cy.intercept()`][intercept] {#Migrating-cyroute-to-cyinterceptintercept}
...
### Match simple route {#Match-simple-route}
...
### Match against `url` and `path` {#Match-against-url-and-path}
...
### Override intercepts {#Override-intercepts}
...
## Migrating to Cypress 6.0 {#Migrating-to-Cypress-60}

This guide details the changes and how to change your code to migrate to Cypress
6.0. [See the full changelog for 6.0](/guides/references/changelog#6-0-0).

### Non-existent element assertions {#Non-existent-element-assertions}
...
#### Assertions on non-existent elements {#Assertions-on-non-existent-elements}
...
#### Assert visibility of `opacity: 0` element {#Assert-visibility-of-opacity-0-element}
...
#### Perform actions on `opacity: 0` element {#Perform-actions-on-opacity-0-element}
...
### `cy.wait(alias)` type {#cywaitalias-type}
...
#### Restore old behavior {#Restore-old-behavior-1}
...
## Migrating to Cypress 5.0 {#Migrating-to-Cypress-50}

This guide details the changes and how to change your code to migrate to Cypress
5.0. [See the full changelog for 5.0](/guides/references/changelog#5-0-0).

### Tests retries {#Tests-retries}
...
#### Configure test retries via the CLI {#Configure-test-retries-via-the-CLI}
...
#### Configure test retries in the configuration file {#Configure-test-retries-in-the-configuration-file}
...
#### Configure test retries per test {#Configure-test-retries-per-test}
...
### Module API results {#Module-API-results}
...
### Cookies `whitelist` option renamed {#Cookies-whitelist-option-renamed}
...
### `blacklistHosts` configuration renamed {#blacklistHosts-configuration-renamed}
...
### Return type of `Cypress.Blob` changed {#Return-type-of-CypressBlob-changed}
...
### `cy.server()` `whitelist` option renamed {#cyserver-whitelist-option-renamed}
...
### Cookies `sameSite` property {#Cookies-sameSite-property}
...
### TypeScript esModuleInterop {#TypeScript-esModuleInterop}
...
### TypeScript 3.4+ support {#TypeScript-34-support}
...
### Node.js 10+ support {#Nodejs-10-support}
...
## Migrating to Cypress 4.0 {#Migrating-to-Cypress-40}

This guide details the changes and how to change your code to migrate to Cypress
4.0. [See the full changelog for 4.0](/guides/references/changelog#4-0-0).

### Mocha upgrade {#Mocha-upgrade}
...
#### <Icon name="exclamation-triangle" color="red" /> Breaking Change: invoke `done` callback and return a promise {#Icon-nameexclamation-triangle-colorred--Breaking-Change-invoke-done-callback-and-return-a-promise}
...
#### <Icon name="exclamation-triangle" color="red" /> Tests require a title {#Icon-nameexclamation-triangle-colorred--Tests-require-a-title}
...
#### <Icon name="exclamation-triangle" color="red" /> Breaking Change: assertions expecting numbers {#Icon-nameexclamation-triangle-colorred--Breaking-Change-assertions-expecting-numbers}
...
#### <Icon name="exclamation-triangle" color="red" /> Breaking Change: `empty` assertions {#Icon-nameexclamation-triangle-colorred--Breaking-Change-empty-assertions}
...
#### <Icon name="exclamation-triangle" color="red" /> Breaking Change: non-existent properties {#Icon-nameexclamation-triangle-colorred--Breaking-Change-non-existent-properties}
...
#### <Icon name="exclamation-triangle" color="red" /> Breaking Change: `include` checks strict equality {#Icon-nameexclamation-triangle-colorred--Breaking-Change-include-checks-strict-equality}
...
### Sinon.JS upgrade {#SinonJS-upgrade}
...
#### <Icon name="exclamation-triangle" color="red" /> Breaking Change: stub non-existent properties {#Icon-nameexclamation-triangle-colorred--Breaking-Change-stub-non-existent-properties}
...
#### <Icon name="exclamation-triangle" color="red" /> Breaking Change: `reset()` replaced by `resetHistory()` {#Icon-nameexclamation-triangle-colorred--Breaking-Change-reset-replaced-by-resetHistory}
...
### Plugin Event `before:browser:launch` {#Plugin-Event-beforebrowserlaunch}
...
### Electron options in `before:browser:launch` {#Electron-options-in-beforebrowserlaunch}
...
### Launching Chrome Canary with `--browser` {#Launching-Chrome-Canary-with---browser}
...
### Chromium-based browser `family` {#Chromium-based-browser-family}
...
#### Example #1 (Finding Electron) {#Example-1-Finding-Electron}
...
#### Example #2 (Finding Chromium-based browsers) {#Example-2-Finding-Chromium-based-browsers}
...
### `cy.writeFile()` yields `null` {#cywriteFile-yields-null}
...
### cy.contains() ignores invisible whitespaces {#cycontains-ignores-invisible-whitespaces}
...
### Node.js 8+ support {#Nodejs-8-support}
...
### CJSX is no longer supported {#CJSX-is-no-longer-supported}
