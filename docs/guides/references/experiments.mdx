...
and fail sometimes due to unpredictable conditions (e.g., temporary outages in
...
[Test retries](/guides/guides/test-retries) is a Cypress [Flake Detection](/guides/cloud/flaky-test-management#Flake-Detection) feature that _reactively_ finds flake on tests that initially failed. However, this offers no solution for a flaky test which initially passes.

Test Burn-in is an experimental feature for _proactively_ detecting flake on new, modified, and previously flaky or failing tests. These tests demonstrate the highest risk for flaky behavior, since they either represent code changes or have yet to pass consistently. Burn-in therefore increases your confidence in these test's flake status.
...
When enabled, burn-in will verify all new and modified tests will pass 3 consecutive attempts on a single run.

For example, here's how burn-in would execute on a **new** test:

**Burn-in Scenario 1: New Test**
...
Any update to these attributes counts as a new/modified test for the purposes of burn-in:

- Test title (full path)
- Spec title (full file path)
- Test body
- Any hooks before or after the test

:::

#### Previously flaky or failing tests

If a test failed in a previous run, you should be wary once it manages to pass. It's best to demonstrate consistently passing behavior, which is why burn-in also applies in this situation. Similarly, when a test was flaky in the previous run, it is burned-in with extra scrutiny to ensure that a passing result has overcome a previously flaky status.

Burn-in will verify all previously failing tests will pass 3 consecutive attempts on a single run, and that previously flaky tests will pass 5 consecutive attempts in a single run.

For example, here's how burn-in would execute on a **previously flaky** test:

**Burn-in Scenario 2: Previously Flaky Test**
...
Here's an example of how a new test might go through burn-in and experimental retries:

**Burn-in Scenario 3: New Test with Experimental Retries Configured**
...
- Attempt 1: <Icon name="check-circle" color="green" /> Pass
- Attempt 2: <Icon name="times" color="red" /> Fail (burn-in stops here on a failed attempt, and retries take-over with this 2nd burn-in attempt counting as the 1st retry attempt, so two retry attempts remains due to `maxRetries: 3`)
- Attempt 3: <Icon name="check-circle" color="green" /> Pass (retries stop here, since the test has met the `passesRequired: 2` threshold)

The new test did not pass all attempts, so it is **flaky**. However it did meet the `passesRequired: 2` threshold, so it is also **passing**. The test passed its 1st burn-in attempt and its 1st retry after the 2nd burn-in attempt failed. If the 1st retry after the failed attempt had also failed instead of passed, then there would be another attempt after that, because this 3rd attempt would have counted as the 2nd retry and `maxRetries` is `3`, while only the 1st burn-in attempt passed and `passesRequired` is `2`. A 4th attempt then would determine whether or not the final test result is pass or fail.

It's key to understand that all burn-in attempts _except_ the initial attempt count towards `retries`, or `maxRetries` in the case of experimental retries, once any burn-in attempt fails. Whereas any passing attempts _inclusive_ of the initial burn-in attempt count towards `passesRequired` and `stopIfAnyPassed` in experimental retries.

If a test that qualified for burn-in fails on the initial attempt, then test retries will also execute as normally.
...
If you would like to provide us with feedback about these levels of configuration for burn-in, please let us know. _(Feedback form coming soon)_ It's an experimental feature, so we certainly appreciate all the feedback we can get.

#### How is test run history taken into account when executing burn-in across branches?

If the test run is on the default branch, or if no default branch is set, then the test's run history on any other branch is completely ignored. For example, if a feature branch which adds a new test that was previously burned-in and passed is merged into the default branch, then that test will be burned-in again on the next run on this default branch because this test did not exist for any previous run on the default branch.

If the run is **not** on the default branch, then the run history extends beyond your branch to include runs on the default branch that preceded the first run on your branch.

:::info

**Note:** When enabling burn-in, any test that has **not** burned-in since it was first added, most recently modified, most recently failed, or most recently flaked is then also burned-in.

For example, let's say is burn-in is disabled while a test is found to be flaky (by test retries), and the test then passes without flake for multiple subsequent runs while burn-in is still disabled. Then if burn-in is enabled now, the test would be burned-in as if it was previously flaky. This is because the test has yet to demonstrate it can consistently pass in a controlled setting, i.e. on a single instance for any given test run.

:::

:::caution

When burn-in is first enabled, **every test is burned-in.** Take this into consideration when timing the enablement of burn-in, especially in your CI.
...
In the meantime, if you have any suggestions for burn-in features in Cloud, please let us know. _(Feedback form coming soon)_
...
Have any thoughts about this experimental retries feature? You're welcome to provide any feedback with [this form](https://share.hsforms.com/1pCBMIbQjQwKY_f2qcdLVzw3a4za).
