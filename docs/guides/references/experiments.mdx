...
Normally, [test retries](/guides/guides/test-retries) will simply stop on the first passing attempt. And then the final test result will be "passing", irregardless of how many prior attempts failed. The following experimental settings for retries give you more control over the retries process.
