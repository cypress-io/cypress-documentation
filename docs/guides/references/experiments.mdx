...
**Note:** When enabling burn-in, burn-in will execute for any test that has **not** burned-in since it was first added, most recently modified, most recently failed, or most recently flaked.

For example, let's say burn-in is disabled while a test is found to be flaky (by test retries), and the test then passes without flake for multiple subsequent runs while burn-in is still disabled. If burn-in is enabled after these runs, the test will be burned-in as previously flaky. This is because the test has yet to demonstrate it can consistently pass in a controlled setting, i.e. on a single instance in one test run.
