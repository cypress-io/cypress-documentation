...
- Aliases created with `.as()` now re-run queries leading up to them, which is equivalent to `.as({ type: 'query' })`. To opt in
  to the old behavior, where aliases are static once defined, use
  `.as('aliasName', { type: 'value' })`, introduced in Cypress
  [12.3.0](#12-3-0).
