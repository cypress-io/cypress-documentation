...
### <Icon name="exclamation-triangle" color="red" /> The `--ci-build-id`, `--group`, `--tag`, `--parallel`, or `--auto-cancel-after-failures` flags can only be used when recording {#record-params-without-recording}
...
The `--auto-cancel-after-failures` flag is only available in Cypress 12.x and
...
You passed in an invalid value for the `--auto-cancel-after-errors` flag. It
must be a numeric value or false.
...
You passed the `--auto-cancel-after-failures` flag, but your Cypress Cloud
account tier does not currently include this service. Please upgrade your
Cypress Cloud account to
[Business or Enterprise tier](https://www.cypress.io/pricing/) to gain access to
the Auto Cancellation service.
...
The first setting for any given run takes precedent, and therefore the previous
setting (either from this same CLI flag or the Project Settings, if the previous
setting you passed was null) of "disabled" will apply to this run's Auto
Cancellation setting. If this should be a separate run, ensure that the CI build
ID (--ci-build-id) is unique.
