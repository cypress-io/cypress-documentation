...
- Number arguments passed to `have.text`, `have.id`, `have.data`, `have.value`,
  and `have.attr`
  [assertions chainers](/api/assertions#Chai-jQuery) are now
...
- The [`isFinite` assertion](/api/assertions) is now supported.
...
- Fixed issue where support for `have.focus`, `not.have.focus`, `be.focused` and
  `not.be.focused` [assertions](/api/assertions) were not working.
...
- Support for `have.focus`, `not.have.focus`, `be.focused` and `not.be.focused`
  [assertions](/api/assertions) have been added. Addresses
...
- Added newly supported `focus` assertion to
  [Assertions](/api/assertions) doc. Addresses
...
- There is now a new
  [`CYPRESS_SKIP_BINARY_INSTALL`](/guides/references/advanced-installation#Environment-variables)
...
- [`chai-jQuery` assertions](/api/assertions#Chai-jQuery) no
...
- [`chai-jQuery` assertions](/api/assertions#Chai-jQuery) have
  ...
  [#605](https://github.com/cypress-io/cypress/issues/605).
- [`chai-jQuery` assertions](/api/assertions#Chai-jQuery) now
...
- [Updated "Assertions"](/api/assertions)
- [Updated "Interacting with Elements"](/guides/core-concepts/interacting-with-elements)
