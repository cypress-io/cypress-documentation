...
- When using `cy.route2()` the route handler timeouts will no longer
...
- The `request.body` is now available when using [cy.wait()](/api/commands/wait)
  on an aliased `cy.route2()` route which had not been intercepted.
...
- Input ranges are now more easily testable using the new
  ...
  [new recipe](/examples/recipes#Form-Interactions) for details on how.
...
- [cypress open](/guides/guides/command-line#cypress-open) no longer opens a
  ...
  [#531](https://github.com/cypress-io/cypress/issues/531).
- We have all new [docker examples](/examples/docker) you can check
  out.
