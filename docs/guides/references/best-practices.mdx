...
### <Icon name="graduation-cap" /> Real World Practices {#Icon-namegraduation-cap--Real-World-Practices}

The Cypress team maintains the <Icon name="github" inline="true" contentType="rwa" />, a
full stack example application that demonstrates **best practices and scalable strategies with Cypress in practical and realistic scenarios**.

The RWA achieves full [code-coverage](/guides/tooling/code-coverage) with
end-to-end tests [across multiple browsers](/guides/guides/cross-browser-testing) and
...
## Organizing Tests, Logging In, Controlling State {#Organizing-Tests-Logging-In-Controlling-State}

:::danger

‼️ **Anti-Pattern:** Sharing page
...
✅ **Best Practice:** Test specs in
isolation, programmatically log into your application, and take control of your application's
state.
...
#### <Icon name="graduation-cap" /> Real World Example {#Icon-namegraduation-cap--Real-World-Example}
...
## Assigning Return Values {#Assigning-Return-Values}

:::danger

‼️ **Anti-Pattern:** Trying to
...
✅ **Best Practice:** Use [closures
to access and store](/guides/core-concepts/variables-and-aliases) what Commands yield
you.
...
## Visiting external sites {#Visiting-external-sites}
...
✅ **Best Practice:** Only test what
you control. Try to avoid requiring a 3rd party server. When necessary, always use
[`cy.request()`](/api/commands/request) to talk to 3rd party servers via their APIs.
...
### 3rd party servers: {#3rd-party-servers}
...
### Verifying sent emails: {#Verifying-sent-emails}
...
## Having tests rely on the state of previous tests {#Having-tests-rely-on-the-state-of-previous-tests}

:::danger

‼️ **Anti-Pattern:** Coupling
...
✅ **Best Practice:** Tests should always
...
### 1. Combine into one test {#1-Combine-into-one-test}
...
### 2. Run shared code before each test {#2-Run-shared-code-before-each-test}
...
## Creating "tiny" tests with a single assertion <E2EOnlyBadge /> {#Creating-tiny-tests-with-a-single-assertion-E2EOnlyBadge-}

:::danger

‼️ **Anti-Pattern:** Acting like
...
✅ **Best Practice:** Add multiple assertions
...
## Using `after` or `afterEach` hooks {#Using-after-or-afterEach-hooks}

:::danger

‼️ **Anti-Pattern:** Using `after`
...
### Dangling state is your friend: {#Dangling-state-is-your-friend}
...
### It's all downside with no upside: {#Its-all-downside-with-no-upside}
...
### State reset should go before each test: {#State-reset-should-go-before-each-test}
...
### Is resetting the state necessary? {#Is-resetting-the-state-necessary}
...
#### <Icon name="graduation-cap" /> Real World Example {#Icon-namegraduation-cap--Real-World-Example-1}
...
## Unnecessary Waiting {#Unnecessary-Waiting}

:::danger

‼️ **Anti-Pattern:** Waiting for
...
✅ **Best Practice:** Use route aliases
or assertions to guard Cypress from proceeding until an explicit condition is met.
...
### Unnecessary wait for `cy.request()` {#Unnecessary-wait-for-cyrequest}
...
### Unnecessary wait for `cy.visit()` <E2EOnlyBadge /> {#Unnecessary-wait-for-cyvisit-E2EOnlyBadge-}
...
### Unnecessary wait for `cy.get()` {#Unnecessary-wait-for-cyget}
...
‼️ **Anti-Pattern:** Trying to
start a web server from within Cypress scripts with [`cy.exec()`](/api/commands/exec)
or [`cy.task()`](/api/commands/task).

:::

:::tip

✅ **Best Practice:** Start a web server
...
## Setting a global baseUrl {#Setting-a-global-baseUrl}

:::danger

‼️ **Anti-Pattern:** Using [cy.visit()](/api/commands/visit)
...
in your [Cypress configuration](/guides/references/configuration).
...
### Without `baseUrl` set, Cypress loads main window in `localhost` + random port {#Without-baseUrl-set-Cypress-loads-main-window-in-localhost--random-port}
...
### With `baseUrl` set, Cypress loads main window in `baseUrl` {#With-baseUrl-set-Cypress-loads-main-window-in-baseUrl}
...
<DocsImage
  ...
  alt="Cypress App with warning about how Cypress could not verify server set as the baseUrl is running"
></DocsImage>
...
### Usage of `baseUrl` in depth {#Usage-of-baseUrl-in-depth}
