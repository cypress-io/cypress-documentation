...
screenshot of the app UI and command log at the exact point of test failure. To see everything that happened prior to test failure, enable the `video` configuration option to record (as an MP4 file) the full test spec run.
