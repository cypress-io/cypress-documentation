...
[Burn-in](/guides/references/experiments#Experimental-burn-in) (an experimental feature) _proactively_ checks for flake on new, modified, or previously failing or flaky tests, by retrying these tests that meet these conditions.
...
[Test retries](/guides/guides/test-retries#Configure-Test-Retries) automatically retry failed tests to _reactively_ find flaky tests, by retrying any tests that fail on the first attempt.
