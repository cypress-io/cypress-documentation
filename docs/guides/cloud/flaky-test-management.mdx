...
tests by Cypress Cloud. [Burn-in](/guides/references/experiments#Experimental-burn-in) is primarily for "high risk" tests that might otherwise be mistaken for simply passing results. [Retries](/guides/guides/test-retries#Configure-Test-Retries) check only tests that initiated failed for flake. **Enabling burn-in and/or
test retries is required (preferably both are enabled)**
