...
GitHub Integration is dependent on your CI environment reliably providing a
commit SHA data (typically via an environment variable). This is not a problem
for most users, but if you are facing GitHub integration issues with your CI
...
- **Run failures:**
  ...
  - Links to output, screenshots and videos are also provided with each failure
    to conveniently provide context
