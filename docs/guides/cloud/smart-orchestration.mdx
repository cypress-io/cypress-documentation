...
The `auto-cancel-after-failure` flag in the CLI is only available in Cypress
v12.X and later. Including the flag in an older version will throw an error.
...
specifying it will default to the value in the project settings. When using the
flag, make sure you have Auto Cancellation
[enabled](#Activate-Auto-Cancellation-in-Cypress-Cloud) for the project, or the
command will fail.

To set it, pass the `--auto-cancel-after-failure` flag to the `cypress run`
command, and give it a numeric value:

```bash
npx cypress run --record --key <<your_record_key>> --auto-cancel-after-failure 1
```
...
```bash
npx cypress run --record --key <<your_record_key>> --auto-cancel-after-failure false
```
