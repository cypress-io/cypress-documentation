...
The `auto-cancel-after-failure` flag in the CLI is only available in Cypress v12.X
and later. Including the flag in an older version will throw an error.
...
When using the flag, make sure you have Auto Cancellation
