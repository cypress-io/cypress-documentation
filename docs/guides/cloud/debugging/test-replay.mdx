...
Because of this, as of Cyrpess `v13`, the Cypress command log is no longer rendered during `cypress run` for users using Test Replay and `video: false`, with an option to opt out of this behavior.

When `video: true` is set, the Cypress command log will be captured as part of that video recording. You can still keep the command log enabled for test runs and artifacts in non-Chromium browsers.
