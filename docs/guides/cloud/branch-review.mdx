...
If a run is missing on either the merge base or feature commit of the PR, you may also find a helpful banner that will specify which commit is missing the run and link out to it.

Recording exactly one run for every commit is the best/only way to ensure complete and accurate comparisons.
