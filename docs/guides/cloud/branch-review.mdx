...
## How Does Branch Review Work?

Branch Review works by leveraging the Cypress Cloud Github integration and queries the Github API for open pull requests on active branches.
...
Most importanly, unless a PR is open (not closed or merged), the branch review is not guaranteed to show you the latest run recorded for the branch. When a PR transitions into a closed/merged state, it does not consider any subsequent commits pushed to the branch as part of the PR itself.

Additionally, whether or not a branch has a run recorded to Cypress Cloud will affect what is displayed. The table below captures each scenario:
...
</svg>&#8239;total count, for example, <em>3 new and 1 existing</em>
