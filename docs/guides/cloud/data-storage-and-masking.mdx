...
- What controls exist to control data sent to and viewed in Cypress Cloud
...
When [Test Replay](#Test-Replay) capturing is enabled, the additional data outlined below will be sent to the Cypress Cloud. You can disable capturing: TODO link this data.
...
You can disable capturing data in Test Replay per project by following this guide: TODO: Test Replay guide link. This means Test Replay will not be available for viewing or debugging in the project. This will prevent Cypress Cloud from capturing the following data:
...
Note that when `--no-runner-ui` is set, data from the Cypress Command Log will still be sent to Cypress Cloud to be generated and displayed in Test Replay if it is enabled. See [Disable Test Replay](#Disable-Test-Replay) for more information.
