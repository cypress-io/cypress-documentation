...
- &nbsp;<Icon name="check-circle" color="green" /> `cy.get('nav header [data-testid="user"]').as('user')`
- &nbsp;<Icon name="exclamation-triangle" color="red" /> `cy.get('nav').find('header').find('[data-testid="user"]').as('user')`
