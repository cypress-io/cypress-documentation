...
#### Avoiding the use of `this` {#Avoiding-the-use-of-this}
...
- <Icon name="check-circle" color="green" /> <code>cy.get('nav header [data-testid="user"]').as('user')</code>
- <Icon name="exclamation-triangle" color="red" /> <code>cy.get('nav').find('header').find('[data-testid="user"]').as('user')</code>
...
### Aliases are reset before each test {#Aliases-are-reset-before-each-test}
