...
For example, there are 7 queries (2 of them assertions) and 2 non-queries in the test below.
...
second statement is never reached. The `.and()` command fails after timing out, and the
...
_waiting_ and assertions. For example, as described in the "Assertions" section of
...
[Action commands](/guides/core-concepts/interacting-with-elements) - such as `.click()` - automatically wait until multiple
built-in assertions like these pass, and then it will attempt the action once.
