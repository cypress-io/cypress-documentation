...
general, there are only a handful of very specific situations where you can or should
...
asynchronously, after an indeterminate period of time or even if the button is removed from the DOM entirely for a while (replaced with a waiting spinner, for example).
...
Implicit Assertions which offer you a high level of
...
Using `expect` allows you to assert on any javascript object, not just the current subject. This is probably how you're used to seeing assertions written in unit
tests:
