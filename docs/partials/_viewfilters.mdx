The `viewFilters` configuration excludes URLs from your reports. By default, every URL your tests visit is included. A `viewFilters` rule with `include: false` removes matching URLs before any analysis happens: their snapshots don't appear in your report, and in UI Coverage, links pointing to them stop counting against your coverage score. The result is a report and score that reflect the parts of your application you own and intend to test.
...
- **Stop irrelevant links from lowering your UI Coverage score**: Every link to an excluded URL is also removed from UI Coverage reports and scores, so links to pages you never plan to test stop appearing as untested elements.
...
What exclusion affects differs by product:

- **In UI Coverage**, snapshots from an excluded URL are not analyzed and don't appear in the report. Link elements are also filtered by their destination: any link whose resolved URL matches an excluded pattern is removed from the report and the coverage score, and excluded URLs never appear as [untested links](/ui-coverage/core-concepts/interactivity#Untested-Links).
- **In Cypress Accessibility**, snapshots from an excluded URL are not scanned and don't appear in the report. There are no other effects.
