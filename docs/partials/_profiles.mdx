...
You can list `policies` at the root of your App Quality configuration or inside a profile's `config` object. When a profile applies to a run, profile `config` follows the same override behavior described above—use profiles when different run tags should use different policy sets or thresholds.
