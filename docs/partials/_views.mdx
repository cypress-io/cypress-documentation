...
In the examples below, the views shown in the UI do not include the protocol or hostname. When every view in a report shares the same protocol, host, and port, they are omitted from view names for readability.
...
When grouping by a query parameter, the view name keeps the pattern's search syntax with the matched value substituted in, so it's clear which parameter created the grouping.
...
All of the views share the `*.my-app.com` hostname pattern, so it is omitted from the view names, and URLs from every matching subdomain are grouped together.
