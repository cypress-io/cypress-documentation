...
| [`.closest()`](/api/commands/closest)                           | Get the element itself or its closest ancestor that matches a selector.                                    |
...
| [`.session()`](/api/commands/session)                               | Cache and restore cookies, localStorage, and sessionStorage (i.e. session data) to recreate a consistent browser context between tests.                                         |
...
| [`.spy()`](/api/commands/spy)                                       | Wrap a method in a spy to record calls to the function.                                                                                                                         |
...
| [`.then()`](/api/commands/then)                                     | Invoke a callback function with the current subject to run code at specific points during a Cypress test.                                                                       |
...
| [`Cypress.require`](/api/cypress-api/require)                      | Use dependencies within the [cy.origin()](/api/commands/origin) callback function.                                             |
...
The Node Events hook into and extend Cypress behavior. For more
