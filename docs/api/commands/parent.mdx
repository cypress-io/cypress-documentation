...
- `.parent()` is a query command, and will not break a chain of
  [retry-ability](/guides/core-concepts/retry-ability#Commands-Queries-and-Assertions).
  It is _safe_ to chain further commands that use the subject after `.parent()`.
- `.parent()` yields the immediate parent of each element in the subject.
