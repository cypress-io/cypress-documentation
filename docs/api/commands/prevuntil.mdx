...
- `.prevUntil()` is a query command, and will not break a chain of
  ...
  `.prevUntil()`.
- `.prevUntil()` yields the previous siblings of each DOM element in the subject
  up to, but not including, the provided selector.
