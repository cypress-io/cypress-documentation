...
- `.blur()` yields the same subject it was given from the previous
command.
...
#### Blur the comment input {#Blur-the-comment-input}
...
#### Blur the first input {#Blur-the-first-input}
...
#### Blur is not an action command {#Blur-is-not-an-action-command}
...
#### `.blur()` can time out because your browser did not receive any blur events {#blur-can-time-out-because-your-browser-did-not-receive-any-blur-events}
...
- `.blur()` requires being chained off a command that yields DOM
...
- `.blur()` will automatically wait for assertions you have chained to
pass
...
- `.blur()` can time out waiting for assertions you've added to
pass.
