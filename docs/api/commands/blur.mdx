...
- `.blur()` requires being chained off a command that yields a DOM element or
  the window.
- `.blur()` requires the subject to contain a single element.
- `.blur()` requires the element to currently have focus. The window and the
  `body` are exempt, since neither reports as the focused element.
