...
<List>
  <li>`cy.request()` requires being chained off of `cy`.</li>
  <li>`cy.request()` requires that the server sends a response.</li>
  <li>
...
<List>
  <li>`cy.request()` can time out waiting for the server to respond.</li>
</List>
