...
- `cy.request()` ignores the subject it is given.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- `cy.request()` will automatically [retry](/guides/core-concepts/retry-ability)
  until there is no network error unless [`retryOnNetworkFailure`](#Options) is
  `false`.
- `cy.request()` will automatically [retry](/guides/core-concepts/retry-ability)
  until there the response status code is a success if
  [`retryOnStatusCodeFailure`](#Options) is `true`.

### Timeouts [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Timeouts)

- `cy.request()` can time out waiting retrying if there are network errors.
- `cy.request()` can time out waiting for the server to respond.
- `cy.request()` can time out waiting for a successful status code.

### Yields [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Subject-Management)

- `cy.request()` is not a query. Once a request has succeeded, it
  [will not make further requests](/guides/core-concepts/retry-ability#Only-queries-are-retried)
  even if chained assertions fail.
- `cy.request()` yields the `response` as an object literal containing
  properties such as:
  - `status`
  - `body`
  - `headers`
...
[Check out our example recipe using `cy.request()` for HTML web forms](/examples/recipes#Logging-In)
...
- [Recipe: Logging In - Single Sign on](/examples/recipes#Logging-In)
- [Recipe: Logging In - HTML Web Form](/examples/recipes#Logging-In)
- [Recipe: Logging In - XHR Web Form](/examples/recipes#Logging-In)
- [Recipe: Logging In - CSRF Tokens](/examples/recipes#Logging-In)
