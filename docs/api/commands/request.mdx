...
such as:
  ...
  - `headers`
  - `duration`
- `cy.request()` is not a query. It [only makes the request once](/guides/core-concepts/retry-ability#Only-queries-are-retried).
...
- `cy.request()`  will automatically [retry](/guides/core-concepts/retry-ability)
  until there is no network error unless [`retryOnNetworkFailure`](#Options) is `false`.
- `cy.request()`  will automatically [retry](/guides/core-concepts/retry-ability)
  until there the response status code is a success if [`retryOnStatusCodeFailure`](#Options) is `true`.
