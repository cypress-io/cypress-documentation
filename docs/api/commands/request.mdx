...
#### Issue an HTTP request {#Issue-an-HTTP-request}
...
#### Alias the request using [.as()](/api/commands/as) {#Alias-the-request-using-as}
...
### Method, URL, and Body {#Method-URL-and-Body}

#### Send a `POST` request with a JSON body {#Send-a-POST-request-with-a-JSON-body}
...
#### Request a page while disabling auto redirect {#Request-a-page-while-disabling-auto-redirect}
...
#### Get Data URL of an image {#Get-Data-URL-of-an-image}
...
#### HTML form submissions using form option {#HTML-form-submissions-using-form-option}
...
#### Using `cy.request()` for HTML Forms {#Using-cyrequest-for-HTML-Forms}
...
### Request Polling {#Request-Polling}

#### Call `cy.request()` over and over again {#Call-cyrequest-over-and-over-again}
...
#### Request is not displayed in the Network Tab of Developer Tools {#Request-is-not-displayed-in-the-Network-Tab-of-Developer-Tools}
...
#### CORS is bypassed {#CORS-is-bypassed}
...
#### Cookies are automatically sent and received {#Cookies-are-automatically-sent-and-received}
...
### [`cy.intercept()`](/api/commands/intercept), [`cy.server()`](/api/commands/server), and [`cy.route()`](/api/commands/route) {#cyintercept-cyserver-and-cyroute}

#### `cy.request()` sends requests to actual endpoints, bypassing those defined using `cy.route()` or `cy.intercept()` {#cyrequest-sends-requests-to-actual-endpoints-bypassing-those-defined-using-cyroute-or-cyintercept}
