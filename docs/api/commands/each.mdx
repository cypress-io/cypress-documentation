...
- `value` - the current item in the collection. DOM elements are passed
  wrapped in jQuery, not as raw elements.
...
| --------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `timeout` | [`defaultCommandTimeout`](/app/references/configuration#Timeouts) | Time to wait for each callback's returned promise to resolve before [timing out](#Timeouts) |

<HeaderYields />

- `.each()` yields the same subject it was given, even if the callback returns
  a value or runs other Cypress commands.
...
before iterating over the next element in the collection.
...
**Wait longer for each promise to resolve**

The `timeout` applies to each iteration separately. Here, every returned
promise has up to 10 seconds to resolve.

```javascript
cy.get('ul>li').each({ timeout: 10000 }, ($li) => {
  return new Cypress.Promise((resolve) => {
...
Return `false` from the callback function to stop the `.each()` loop early. The
callback is not invoked for any remaining items, and `.each()` still yields the
original collection.

```javascript
cy.wrap([1, 2, 3, 4]).each((num) => {
  if (num === 3) {
    return false // 1 and 2 are logged, then the loop stops
  }

  cy.log(`Processing ${num}`)
})
```

`.each()` only checks the value the callback returns directly. A `Promise` that
resolves to `false` does not stop the loop.

### Empty collections

If the subject has a `length` of `0`, `.each()` never invokes the callback and
yields the empty subject.
...
- `.each()` requires its subject to be array-like, with a `length` property.
...
- `.each()` can time out waiting for a promise you've returned to resolve. The
  timeout applies to each iteration separately.
