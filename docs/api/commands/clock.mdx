...
#### Create a clock and use it to trigger a `setInterval` {#Create-a-clock-and-use-it-to-trigger-a-setInterval}
...
#### Access the clock object to synchronously move time {#Access-the-clock-object-to-synchronously-move-time}
...
#### Access the clock object to restore native functions {#Access-the-clock-object-to-restore-native-functions}
...
#### Specify a now timestamp {#Specify-a-now-timestamp}
...
#### Specify which functions to override {#Specify-which-functions-to-override}
...
#### `Using cy.clock()` with `cy.tick()` {#Using-cyclock-with-cytick}
...
#### iframes not supported {#iframes-not-supported}
...
#### clock behavior before `cy.mount()` {#clock-behavior-before-cymount}
...
#### clock behavior before `cy.visit()` {#clock-behavior-before-cyvisit}
