...
- `cy.getAllCookies()`
  [only reads the data once](/guides/core-concepts/retry-ability#Only-queries-are-retried), so any assertions chaining the command will not retry because the yielded value is static.
