...
#### Get the siblings of each `li` {#Get-the-siblings-of-each-li}
...
#### Get siblings of element with class `active` {#Get-siblings-of-element-with-class-active}
...
- `.siblings()` requires being chained off a command that yields DOM
element(s).
