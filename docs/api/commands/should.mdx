...
```javascript
cy.should('eq', '42') // Can not be chained off 'cy'
...
Chain assertions together with [`.and()`](/api/commands/and), which is identical
to `.should()`.
...
Cypress doesn't resolve your commands until all of its assertions pass.
...
- `.should()` [retries](/app/core-concepts/retry-ability) its specified
  assertions until they pass or it times out.
