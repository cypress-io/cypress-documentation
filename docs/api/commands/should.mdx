...
- `.should()` will assert on the subject it receives. If chained directly off
  `cy`, the subject will be `undefined`, which is usually not useful.

### Timeouts [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Timeouts)

- `.should()` uses the timeout value of the previous command. If none exists, it
  will use the default command timeout.
- `.should()` can time out waiting for the assertion to pass.

### Yields [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Subject-Management)

- `.should()` is a query command, and will not break a chain of
  [retry-ability](/guides/core-concepts/retry-ability#Commands-Queries-and-Assertions).
  It is _safe_ to chain further commands that use the subject after `.should()`.
...
```javascript
cy.wrap([1, 2, 3])
  .should((array) => array[1]) // Return value is ignored
  .and('have.length', 3) // The array is still the subject
```

However, some chainers change the subject. In the example below, `.should()`
yields the string `sans-serif` because the chainer `have.css, 'font-family'`
changes the subject.
