...
- `.parentsUntil()` will automatically
  [retry](/guides/core-concepts/retry-ability) until it
...
- `.parentsUntil()` is a query command, and will not break a chain of
  ...
  `.parentsUntil()`.
- `.parentsUntil()` yields the parents of each DOM element in the subject up to,
  but not including, the provided selector.
