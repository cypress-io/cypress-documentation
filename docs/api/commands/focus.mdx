...
[focusable element](https://www.w3.org/TR/html5/editing.html#focusable). The
window and the `body` are the two exceptions, so `cy.window().focus()` fires a
`focus` event on the window. Focus itself does not move: whichever element held
