...
- `.focus()` yields the same subject it was given from the previous
command.
...
#### Focus on an input {#Focus-on-an-input}

```javascript
cy.get('[type="input"]').focus()
```

#### Focus, type, and blur a textarea {#Focus-type-and-blur-a-textarea}
...
#### Focus is not an action command {#Focus-is-not-an-action-command}
...
#### Cypress blurs other focused elements first {#Cypress-blurs-other-focused-elements-first}
...
#### Can only be called on a valid focusable element {#Can-only-be-called-on-a-valid-focusable-element}
...
#### Can time out because your browser did not receive any focus events {#Can-time-out-because-your-browser-did-not-receive-any-focus-events}
...
- `.focus()` requires being chained off a command that yields DOM
...
- `.focus()` will automatically wait for assertions you have chained to
pass
...
- `.focus()` can time out waiting for assertions you've added to
pass.
