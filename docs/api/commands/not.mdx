---
title: 'cy.not()'
description: Filter DOM element(s) from a set of DOM elements in Cypress.
sidebar_label: not
slug: /api/commands/not
---

<ProductHeading product="app" />

# not

Filter DOM element(s) from a set of DOM elements.

:::info

Opposite of [`.filter()`](/api/commands/filter)

:::

:::info

The querying behavior of this command matches exactly how
[`.not()`](https://api.jquery.com/not/) works in jQuery.

:::

## Syntax

```javascript
.not(selector)
.not(selector, options)
.not(function(index, element))
.not(function(index, element), options)
```

### Usage

<Icon name="check-circle" color="green" /> **Correct Usage**

```javascript
cy.get('input').not('.required') // Yield all inputs without class '.required'
cy.get('input').not((index, el) => el.disabled) // Yield all inputs that are not disabled
```

<Icon name="exclamation-triangle" color="red" /> **Incorrect Usage**

```javascript
cy.not('.icon') // Errors, cannot be chained off 'cy'
cy.clock().not() // Errors, 'clock' does not yield DOM elements
```

### Arguments

<Icon name="angle-right" /> **selector _(String selector)_**

A selector used to remove matching DOM elements.

<Icon name="angle-right" /> **function(index, element) _(Function)_**

A function called for each element in the subject, with the element's index and
the element itself. `.not()` removes the elements for which the function returns
a truthy value.

<Icon name="angle-right" /> **options _(Object)_**

Pass in an options object to change the default behavior of `.not()`.

| Option    | Default                                                           | Description                                                                         |
| --------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `log`     | `true`                                                            | Displays the command in the [Command log](/app/core-concepts/open-mode#Command-Log) |
| `timeout` | [`defaultCommandTimeout`](/app/references/configuration#Timeouts) | Time to wait for `.not()` to resolve before [timing out](#Timeouts)                 |

<HeaderYields />

- `.not()` yields the new DOM element(s) it found.
- `.not()` is a query, and it is _safe_ to chain further commands.

## Examples

### Selector

#### Yield the elements that do not have class `active`

```html
<ul>
  <li>Home</li>
  <li class="active">About</li>
  <li>Services</li>
  <li>Pricing</li>
  <li>Contact</li>
</ul>
```

```javascript
cy.get('ul>li').not('.active').should('have.length', 4) // true
```

### Function

#### Remove elements by a custom condition

Pass a function when a selector can't express the condition, such as a numeric
comparison against an attribute value.

```html
<ul>
  <li data-stock="0">Pencil</li>
  <li data-stock="12">Notebook</li>
  <li data-stock="3">Backpack</li>
</ul>
```

```javascript
// yields <li>Notebook</li> and <li>Backpack</li>
cy.get('li')
  .not((index, el) => Number(el.dataset.stock) === 0)
  .should('have.length', 2)
```

`.not()` is a query, so Cypress calls the function again on each retry until the
chained assertions pass. Keep the function free of side effects.

### Contains

#### Yield the elements that do not contain some text

<InvertedContainsSelection />

## Rules

<HeaderRequirements />

- `.not()` requires being chained off a command that yields DOM element(s) or a
  document.

<HeaderAssertions />

- `.not()` will automatically [retry](/app/core-concepts/retry-ability) until
  the element(s)
  [exist in the DOM](/app/core-concepts/introduction-to-cypress#Implicit-Assertions).
- `.not()` will automatically [retry](/app/core-concepts/retry-ability) until
  all chained assertions have passed.

<HeaderTimeouts />

- `.not()` can time out waiting for the element(s) to
  [exist in the DOM](/app/core-concepts/introduction-to-cypress#Implicit-Assertions).
- `.not()` can time out waiting for assertions you've added to pass.

## Command Log

**_Find all buttons that are not of type submit_**

```javascript
cy.get('form').find('button').not('[type="submit"]')
```

The commands above will display in the Command Log as:

<DocsImage
  src="/img/api/not/filter-elements-with-not-and-optional-selector.png"
  alt="Command Log not"
/>

When clicking on `not` within the command log, the console outputs the
following:

<DocsImage
  src="/img/api/not/log-elements-found-when-using-cy-not.png"
  alt="Console Log not"
/>

## See also

- [`cy.contains()`](/api/commands/contains)
- [`.filter()`](/api/commands/filter)
