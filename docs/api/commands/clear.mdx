...
Browsers fire `change` only when an edited field loses focus, so `.clear()`
alone doesn't trigger it. If your app validates or saves on `change`, follow
`.clear()` with [`.blur()`](/api/commands/blur):
