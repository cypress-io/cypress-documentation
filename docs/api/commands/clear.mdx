...
Chain an assertion directly off `.clear()`. `.clear()` retries the assertion
until it passes, so this is the reliable way to check the result.
...
cy.get('#signup-form input[type="text"]').each(($input) => {
  cy.wrap($input).should('have.value', '')
})
```

A `have.value` assertion on several elements checks only the first one's value,
so assert on each element to confirm they're all empty.
