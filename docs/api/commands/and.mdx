...
```javascript
cy.and('eq', '42') // Cannot be chained off 'cy'
...
- `.and()` will assert on the subject it receives. If chained directly off `cy`,
  the subject will be `undefined`, which is usually not useful.

### Timeouts [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Timeouts)

- `.and()` uses the timeout value of the previous command. If none exists, it
  will use the default command timeout.
- `.and()` can time out waiting for the assertion to pass.

### Yields [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Subject-Management)

- `.and()` is a query command, and will not break a chain of
  [retry-ability](/guides/core-concepts/retry-ability#Commands-Queries-and-Assertions).
  It is _safe_ to chain further commands that use the subject after `.and()`.
