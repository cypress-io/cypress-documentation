---
title: 'cy.and()'
description: 'Create an assertion in Cypress, chained off a previous command and retried until it passes or times out.'
sidebar_label: and
...
cy.get('p').and(($p) => {
  cy.log($p) // Can not invoke a Cypress command inside the callback
})
```
...
function. Cypress retries the callback until no assertions within it throw.

You cannot invoke Cypress commands inside of an `.and()` callback function. Use
Cypress commands before or after `.and()` instead.
...
`.and()` retries as part of the command that precedes it. Once every assertion
in the chain passes, Cypress locks in the subject at that point, so queries that
come after it retry from that subject rather than from the top of the chain. If
your application re-renders the DOM in that window, those later queries run
...
- `.and()` [retries](/app/core-concepts/retry-ability) its specified assertions
  until they pass or it times out.
...
| [0.5.4](/app/references/changelog#0-5-4)   | `.and()` repurposed as an alias of `.should()`  |
