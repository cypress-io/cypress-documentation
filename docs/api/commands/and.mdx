...
- `.and()` is an assertion, and it is _safe_ to chain further commands that use
  the subject. Queries chained after it do retry from the subject it locks in
  rather than from the top of the chain, which matters when the DOM re-renders.
  See [Retry boundaries](#Retry-boundaries).
...
```javascript
// run the command after the assertion
cy.get('p')
  .should('not.be.empty')
  .and(($p) => {
    // ...
...
// or use .then() instead, which does not retry
cy.get('p').then(($p) => {
  // ...
  cy.log($p)
})
