...
request or its response in any way. This alone is powerful, since those aliases
support [waiting](#Waiting-on-a-request) on these requests, resulting in more
reliable tests.
...
```ts
{
   ...
   * Not reported in Chrome, Chromium, or Edge, where the browser negotiates the
   * protocol with the origin directly and it is not known when the request is
   * intercepted. Firefox, WebKit, and Electron still report it.
   */
  httpVersion?: string
  /**
...
`req.httpVersion` is not reported in Chrome, Chromium, or Edge, where Cypress intercepts
traffic inside the browser and the protocol your application negotiates with your server
is not known at the point the request is intercepted. Firefox, WebKit, and Electron route
traffic through the Cypress HTTP/1.1 proxy and still report it. See
...
This overrides existing `cy.intercept()` declarations when you define an
overlapping `cy.intercept()`:
