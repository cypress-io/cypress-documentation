...
#### **<Icon name="angle-right" /> method** **_(String)_** {#Icon-nameangle-right--method-_String_}
...
#### **<Icon name="angle-right" /> url** **_(String, Glob, RegExp)_** {#Icon-nameangle-right--url-_String-Glob-RegExp_}
...
#### **<Icon name="angle-right" /> routeMatcher** **_(`RouteMatcher`)_** {#Icon-nameangle-right--routeMatcher-_RouteMatcher_}
...
#### <Icon name="angle-right" /> staticResponse (<code>[StaticResponse][staticresponse]</code>) {#Icon-nameangle-right--staticResponse-codeStaticResponsestaticresponsecode}
...
#### <Icon name="angle-right" /> routeHandler (<code>Function</code>) {#Icon-nameangle-right--routeHandler-codeFunctioncode}
...
### Matching with [RouteMatcher](#routeMatcher-RouteMatcher) {#Matching-with-RouteMatcher}
...
### Aliasing an intercepted route {#Aliasing-an-intercepted-route}
...
### Aliasing individual requests {#Aliasing-individual-requests}
...
### Waiting on a request {#Waiting-on-a-request}
...
#### With [RouteMatcher](#routeMatcher-RouteMatcher) {#With-RouteMatcher}
...
#### Using the yielded object {#Using-the-yielded-object}
...
#### Waiting on errors {#Waiting-on-errors}
...
### Stubbing a response {#Stubbing-a-response}
...
#### With a `StaticResponse` object {#With-a-StaticResponse-object}
...
### Using the **`routeHandler`** function {#Using-the-routeHandler-function}
...
#### Asserting on a request {#Asserting-on-a-request}
...
#### Modifying an outgoing request {#Modifying-an-outgoing-request}
...
#### Adding a header to an outgoing request {#Adding-a-header-to-an-outgoing-request}
...
#### Waiting on the intercept {#Waiting-on-the-intercept}
...
#### Add, modify or delete a header to all outgoing requests {#Add-modify-or-delete-a-header-to-all-outgoing-requests}
...
#### Dynamically stubbing a response {#Dynamically-stubbing-a-response}
...
#### Passing a request to the next request handler {#Passing-a-request-to-the-next-request-handler}
...
#### Asserting on a response {#Asserting-on-a-response}
...
#### Returning a Promise {#Returning-a-Promise-1}
...
#### Throttle or delay response all incoming responses {#Throttle-or-delay-response-all-incoming-responses}
...
#### Asserting on a request {#Asserting-on-a-request-1}
...
#### Controlling the outgoing request {#Controlling-the-outgoing-request}
...
#### Verifying the request modification {#Verifying-the-request-modification}
...
#### Returning a Promise {#Returning-a-Promise-2}
...
#### Stubbing a response with a string {#Stubbing-a-response-with-a-string}
...
## Intercepted requests {#Intercepted-requests}
...
### Controlling the outbound request with `req.continue()` {#Controlling-the-outbound-request-with-reqcontinue}
...
#### `req.reply()` shorthand {#reqreply-shorthand}
...
## Intercepted responses {#Intercepted-responses}
...
### Ending the response with `res.send()` {#Ending-the-response-with-ressend}
...
#### `res.send()` shorthand {#ressend-shorthand}
...
## `StaticResponse` objects {#StaticResponse-objects}
...
## Interception lifecycle {#Interception-lifecycle}
...
### Cypress.minimatch {#Cypressminimatch}
...
## Comparison to `cy.route()` {#Comparison-to-cyroute}
...
## `cy.intercept()` and request caching {#cyintercept-and-request-caching}
