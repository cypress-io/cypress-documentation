...
- `.uncheck()` requires the element to have type `checkbox` or `radio`.
...
- After executing, `.uncheck()` [implicitly asserts](/guides/core-concepts/introduction-to-cypress#Implicit-Assertions) that the element(s) still
