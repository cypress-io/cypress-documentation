...
cy.get('[type="radio"]').uncheck() // Errors, radios cannot be unchecked
```
...
```javascript
// Unchecks and yields only the checkbox with the value 'ga'
cy.get('[type="checkbox"]').uncheck(['ga', 'nowhere'])
```
