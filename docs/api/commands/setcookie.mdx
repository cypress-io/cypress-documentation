...
| `domain`   | Hostname of the application under test                      | The domain the cookie is visible to. By default the resulting cookie is **not** host-only; it is matched for this host _and all of its subdomains_. To restrict the cookie to a single exact host, pass `hostOnly: true`. See [Cookie scope](#Cookie-scope-host-only-vs-domain-cookies). |
...
- `cy.setCookie()` can time out waiting for the browser to set the cookie.
  It waits up to the [`responseTimeout`](/app/references/configuration#Timeouts)
  (or the `timeout` option, when passed) before failing.
- If the application under test is on a different origin, `cy.setCookie()`
