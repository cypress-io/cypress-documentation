...
- `.next()` is a query command, and will not break a chain of
  [retry-ability](/guides/core-concepts/retry-ability#Commands-Queries-and-Assertions).
  It is _safe_ to chain further commands that use the subject after `.next()`.
- `.next()` yields the immediately following sibling of each element in the
  subject.
