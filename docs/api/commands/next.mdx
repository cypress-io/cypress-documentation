...
#### Find the element next to `.second` {#Find-the-element-next-to-second}
...
#### Testing a datalist {#Testing-a-datalist}
...
#### Find the very next sibling of each li. Keep only the ones with a class `selected` {#Find-the-very-next-sibling-of-each-li-Keep-only-the-ones-with-a-class-selected}
...
- `.next()` requires being chained off a command that yields DOM
element(s).
