...
- `cy.route()` yields `null`.</li><li>`cy.route()` can be aliased, but
otherwise cannot be chained further.

## Examples {#Examples}

### Without Stubbing {#Without-Stubbing}
...
#### Wait on XHR `GET` request matching `url` {#Wait-on-XHR-GET-request-matching-url}
...
#### Wait on XHR's matching `method` and `url` {#Wait-on-XHRs-matching-method-and-url}
...
#### Setup route to `POST` to login {#Setup-route-to-POST-to-login}
...
#### Wait on `url` matching glob {#Wait-on-url-matching-glob}
...
#### Match route against any UserId {#Match-route-against-any-UserId}
...
#### Use glob to match all segments {#Use-glob-to-match-all-segments}
...
#### Override `url` glob matching options {#Override-url-glob-matching-options}
...
#### `url` as a RegExp {#url-as-a-RegExp}
...
#### Matching requests and routes {#Matching-requests-and-routes}
...
#### Making multiple requests to the same route {#Making-multiple-requests-to-the-same-route}
...
#### Pass in an options object {#Pass-in-an-options-object}
...
#### Simulate a server redirect {#Simulate-a-server-redirect}
...
#### Setup route to error on `POST` to login {#Setup-route-to-error-on-POST-to-login}
...
#### Use delays for responses {#Use-delays-for-responses}
...
#### Set the routing options using a callback function {#Set-the-routing-options-using-a-callback-function}
...
#### Functions that return promises are awaited {#Functions-that-return-promises-are-awaited}
...
#### Understanding stubbed vs regular XHRs {#Understanding-stubbed-vs-regular-XHRs}
...
### `cy.route()` cannot be debugged using [`cy.request()`](/api/commands/request) {#cyroute-cannot-be-debugged-using-cyrequest}

#### `cy.request()` sends requests to actual endpoints, bypassing those defined using `cy.route()` {#cyrequest-sends-requests-to-actual-endpoints-bypassing-those-defined-using-cyroute}
...
#### Matching origins and non origin URL's {#Matching-origins-and-non-origin-URLs}
...
#### Requests that don't match any routes {#Requests-that-dont-match-any-routes}
