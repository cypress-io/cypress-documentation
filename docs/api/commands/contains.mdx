...
Get the DOM element containing the text. The element may contain _more_ than
the desired text and still be considered a match.

When multiple elements contain the text, Cypress returns a signle element, determined by
- Element preference order
- DOM order 
See the [preferences](#Preferences) section for details.
