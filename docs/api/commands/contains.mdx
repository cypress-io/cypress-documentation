...
tree than the deepest one containing the text. Cypress yields the deepest
matching element, and applies its [element preference order](#Preferences) only
when the selector itself matches a `button`, `a`, `label`, or
`input[type='submit']` element. A comma-separated selector, such as
...
```javascript
// clicks the Edit button in Jane Lane's row
cy.contains('tr', 'Jane Lane').contains('button', 'Edit').click()
```
...
Where one of these examples takes an action, such as `.click()`, the action ends
the chain and the next step starts from `cy`. That way every step queries the
page again, so a re-render between steps does not leave a test holding a
detached element. Read
...
When the attribute is omitted, the `value` is an empty string, and there is no
programmatic way for Cypress to filter elements by the label displayed by the
user agent. This can cause unexpected failures when using `cy.contains()` with
submit buttons.
