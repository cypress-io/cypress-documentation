...
when the selector itself matches one of those elements. A comma-separated
selector, such as `'a, button'`, searches for every listed selector.
...
Each of these examples ends a chain at an action, such as `.click()`, and starts
the next step from `cy`. That way every step queries the page again, so a
re-render between steps does not leave a test holding a detached element. Read
