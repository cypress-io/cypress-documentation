...
- `.check()` yields the same subject it was given from the previous
command.
...
#### Select all radios {#Select-all-radios}

```javascript
cy.get('[type="radio"]').check()
```

#### Check the element with id of 'saveUserName' {#Check-the-element-with-id-of-saveUserName}
...
#### Select the radio with the value of 'US' {#Select-the-radio-with-the-value-of-US}
...
#### Check the checkboxes with the values 'subscribe' and 'accept' {#Check-the-checkboxes-with-the-values-subscribe-and-accept}
...
#### Check an invisible checkbox {#Check-an-invisible-checkbox}
...
### Find checked option {#Find-checked-option}
...
- `.check()` requires being chained off a command that yields DOM
