...
Setting `force` to `true` skips the
[actionability](/app/core-concepts/interacting-with-elements) checks, so Cypress
clicks the element without first requiring it to be visible, enabled, and
unobstructed.
...
```javascript
// If no checkbox has the value 'purple', this checks and yields only 'blue'
