...
- `cy.fixture()` yields the contents of the file. Formatting is determined by
  its file extension.
- `cy.fixture()` is not a query. It [only reads the data once](/guides/core-concepts/retry-ability#Only-queries-are-retried).
...
- `cy.fixture()`  will automatically [retry](/guides/core-concepts/retry-ability)
  until the file [exists on disk](/guides/core-concepts/introduction-to-cypress#Default-Assertions).
