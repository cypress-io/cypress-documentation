...
- **Memory pressure**: The complete file must fit in available RAM. Cypress
  ...
  as images and `.zip` archives are also converted to a `base64` string in the
  browser, which is about a third larger than the file itself.
- **Single-message transfer**: File contents are sent as a single message over
  the internal socket, so the whole payload is buffered at once.
