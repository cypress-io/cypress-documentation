...
`cy.clearAllSessionStorage()` yields `null`, so there's nothing to assert on. To confirm session storage is cleared, assert on [`cy.getAllSessionStorage()`](/api/commands/getallsessionstorage) afterward, for example `cy.getAllSessionStorage().should('be.empty')`.
