...
`cy.clearAllSessionStorage()` runs once, and any assertions you chain run against the `null` it yields. Cypress doesn't re-run `cy.clearAllSessionStorage()` to make an assertion pass.
