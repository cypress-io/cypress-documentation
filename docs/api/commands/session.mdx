...
### Asserting the session inside setup {#Asserting-the-session-inside-setup}
...
### Conditionally caching a session {#Conditionally-caching-a-session}
...
### Switching sessions inside tests {#Switching-sessions-inside-tests}
...
### Modifying session data before caching {#Modifying-session-data-before-caching}
...
### Multiple login commands {#Multiple-login-commands}
...
### Where to call `cy.mount()` <ComponentOnlyBadge /> {#Where-to-call-cymount-ComponentOnlyBadge-}
...
### Where to call `cy.visit()` <E2EOnlyBadge /> {#Where-to-call-cyvisit-E2EOnlyBadge-}
...
### Updating a login function that returns a value {#Updating-a-login-function-that-returns-a-value}
...
### Cross-domain sessions <E2EOnlyBadge /> {#Cross-domain-sessions-E2EOnlyBadge-}
...
### When the page and active session data are cleared {#When-the-page-and-active-session-data-are-cleared}
...
### Explicitly clearing sessions {#Explicitly-clearing-sessions}
...
### Where to call `cy.session()` {#Where-to-call-cysession}
...
#### Why are all my Cypress commands failing after calling `cy.session()`? {#Why-are-all-my-Cypress-commands-failing-after-calling-cysession}
...
#### Why am I seeing `401` errors after calling `cy.session()`? {#Why-am-I-seeing-401-errors-after-calling-cysession}
...
- A saved session was found, and used:
  ...
  <DocsImage
    src="/img/api/session/session-collapsed-saved.png"
