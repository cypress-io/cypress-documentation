---
title: 'cy.session()'
description: Cache and restore cookies, localStorage, and sessionStorage to recreate a consistent browser context between tests in Cypress.
...
(i.e. session data) to recreate a consistent browser context between
tests.
...
simple cases, a `String` value is sufficient. To simplify generation of
...
Large or cyclical data structures may be slow or difficult to
...
`setup`, you can use it to switch between sessions without first needing
...
reuse the session through the run.
...
Tests will be faster if you call `cy.visit()` only when necessary.
You can do this by
...
wrapping it with `cy.session()` will break that test. You can usually solve this
...
It's possible to switch domains while caching sessions. Be sure to explicitly
visit the domain in your login command before calling `cy.session()`.
...
and switches sessions without first having to explicitly log out.
...
To reduce development time, when running Cypress in "open" mode,
...
a name, a token, and a password), to be able to log in many different
...
In cases where the `validate` function fails immediately after `setup`
