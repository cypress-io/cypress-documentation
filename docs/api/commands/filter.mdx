...
.filter(function(index, element), options)
```
...
cy.get('li').filter((index, el) => el.textContent.includes('Sale')) // Yield all li's whose text includes 'Sale'
```
...
<Icon name="angle-right" /> **function(index, element) _(Function)_**

A function called for each element in the subject, with the element's index and
the element itself. `.filter()` keeps the elements for which the function
...
#### Filter by a custom condition

Pass a function when a selector can't express the condition, such as a numeric
comparison against an attribute value.

```html
<ul>
  <li data-price="5">Pencil</li>
  <li data-price="25">Notebook</li>
  <li data-price="40">Backpack</li>
</ul>
```

```javascript
// yields <li>Notebook</li> and <li>Backpack</li>
cy.get('li')
  .filter((index, el) => Number(el.dataset.price) > 20)
  .should('have.length', 2)
```

`.filter()` is a query, so Cypress calls the function again on each retry until
the chained assertions pass. Keep the function free of side effects.
