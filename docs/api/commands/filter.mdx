...
#### Filter the current subject to the elements with the class 'active' {#Filter-the-current-subject-to-the-elements-with-the-class-active}
...
- `.filter()` requires being chained off a command that yields DOM
element(s).
