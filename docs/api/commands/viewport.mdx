...
### `Cypress.config()` cannot change the viewport during a test

Starting in Cypress [16.0.0](/app/references/changelog#16-0-0),
`viewportHeight` and `viewportWidth` cannot be set with
[`Cypress.config()`](/api/cypress-api/config) while a test is executing. Setting
them this way applied to the _next_ test rather than the current one, so Cypress
now throws instead.

Use `cy.viewport()` to change the viewport during a test, or set the values in
the [test configuration](#Set-viewport-in-the-test-configuration) of a
...
- `cy.viewport()` runs once, and any assertions you chain run against the `null` it yields. Cypress doesn't re-run `cy.viewport()` to make an assertion pass.
...
| Version                                    | Changes                                                                                                  |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------- |
| [16.0.0](/app/references/changelog#16-0-0) | `viewportHeight` and `viewportWidth` can no longer be set with `Cypress.config()` during test execution. |
...
- Read blog post
  [Use meaningful smoke tests](https://www.cypress.io/blog/2019/12/06/use-meaningful-smoke-tests/?utm_source=docs.cypress.io&utm_medium=api-viewport)
  where we run the same test with different viewport resolutions
- Read blog post
  [Generate High-Resolution Videos and Screenshots](https://www.cypress.io/blog/2021/03/01/generate-high-resolution-videos-and-screenshots/?utm_source=docs.cypress.io&utm_medium=api-viewport)
