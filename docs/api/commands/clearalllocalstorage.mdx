...
- `cy.clearAllLocalStorage()` yields `null`, so there's nothing to assert on. To confirm local storage is cleared, assert on [`cy.getAllLocalStorage()`](/api/commands/getalllocalstorage) afterward, for example `cy.getAllLocalStorage().should('be.empty')`.
