...
- `cy.clearAllLocalStorage()` runs once, and any assertions you chain run against the `null` it yields. Cypress doesn't re-run `cy.clearAllLocalStorage()` to make an assertion pass.
