...
#### When given a `key` argument: {#When-given-a-key-argument}
...
#### Make assertions about every location property {#Make-assertions-about-every-location-property}
...
#### Check location for query params and pathname {#Check-location-for-query-params-and-pathname}
...
#### Assert that a redirect works {#Assert-that-a-redirect-works}
...
#### No need to use `window.location` {#No-need-to-use-windowlocation}
...
#### Console output of `window.location` {#Console-output-of-windowlocation}
...
#### Console output of `.location()` {#Console-output-of-location}
...
- `cy.location()` can time out waiting for assertions you've added to
pass.
