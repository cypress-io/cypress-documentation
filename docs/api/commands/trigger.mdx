...
- `.trigger()` yields the same subject it was given from the previous
command.
...
#### Trigger a `mouseover` on the button {#Trigger-a-mouseover-on-the-button}
...
#### Simulate a "long press" event {#Simulate-a-long-press-event}
...
#### Trigger a `mousedown` from a specific mouse button {#Trigger-a-mousedown-from-a-specific-mouse-button}
...
#### Interact with a range input (slider) {#Interact-with-a-range-input-slider}
...
#### Trigger a `mousedown` on the top right of a button {#Trigger-a-mousedown-on-the-top-right-of-a-button}
...
#### Specify explicit coordinates relative to the top left corner {#Specify-explicit-coordinates-relative-to-the-top-left-corner}
...
#### Specify that the event should not bubble {#Specify-that-the-event-should-not-bubble}
...
#### Specify the exact `clientX` and `clientY` the event should have {#Specify-the-exact-clientX-and-clientY-the-event-should-have}
...
### Fire other Event types. {#Fire-other-Event-types}
...
#### The element must first reach actionability {#The-element-must-first-reach-actionability}
...
#### What event should I fire? {#What-event-should-I-fire}
...
#### Why should I manually set the event type? {#Why-should-I-manually-set-the-event-type}
...
#### What's the difference between triggering and event and calling the corresponding cypress command? {#Whats-the-difference-between-triggering-and-event-and-calling-the-corresponding-cypress-command}
...
- `.trigger()` requires being chained off a command that yields DOM
element(s).
