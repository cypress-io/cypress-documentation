...
- If given a path, `.selectFile()` requires the file exist.
- If given an alias, `.selectFile()` requires that the subject of the alias not
...
- `.selectFile()` will automatically wait for the a file to exist on disk or for
  an alias to resolve to a non-null value.
- After executing, `.selectFile()` [implicitly asserts](/guides/core-concepts/introduction-to-cypress#Implicit-Assertions) that the element(s) still
