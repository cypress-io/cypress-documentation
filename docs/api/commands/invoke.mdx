...
#### Assert on a function's return value {#Assert-on-a-functions-return-value}
...
#### Use `.invoke()` to test HTML content {#Use-invoke-to-test-HTML-content}
...
#### Properties that are functions are invoked {#Properties-that-are-functions-are-invoked}
...
#### Use `.invoke('show')` and `.invoke('trigger')` {#Use-invokeshow-and-invoketrigger}
...
#### Send specific arguments to the function {#Send-specific-arguments-to-the-function}
...
#### Use `cy.invoke('removeAttr', 'target')` to get around new tab {#Use-cyinvokeremoveAttr-target-to-get-around-new-tab}
...
#### Arguments are automatically forwarded to the function {#Arguments-are-automatically-forwarded-to-the-function}
...
### Invoking an async function {#Invoking-an-async-function}
...
#### Using a Kendo DropDown {#Using-a-Kendo-DropDown}
...
- `.invoke()` will wait for the `function` to exist on the subject
...
- `.invoke()` can time out waiting for assertions you've added to
