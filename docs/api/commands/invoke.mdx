...
- `.invoke()` requires being chained off a previous command that yields an a
  value with `functionName` as a property.
- The called function must not return a promise.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- `.invoke()` will automatically [retry](/guides/core-concepts/retry-ability)
  until `functionName` exists on the subject, is a function and does not throw
...
- `.invoke()` can
  ...
  for `functionName` to exist on the subject, be a function, and not throw an
  error.

### Yields [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Subject-Management)

- `.invoke()` is a query command, and is
  [retry-able](/guides/core-concepts/retry-ability#Commands-Queries-and-Assertions).
  It is _safe_ to chain further commands that use the subject after `.invoke()`.
