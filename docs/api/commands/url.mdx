...
#### Assert the URL is `http://localhost:8000/users/1/edit` {#Assert-the-URL-is-httplocalhost8000users1edit}
...
### Href Shorthand {#Href-Shorthand}

#### URL is an alias for `cy.location('href')` {#URL-is-an-alias-for-cylocationhref}
...
#### URL versus href {#URL-versus-href}
...
#### Hardcoded versus using the configuration object {#Hardcoded-versus-using-the-configuration-object}
...
#### Assert that the url contains "#users/new" {#Assert-that-the-url-contains-usersnew}
...
- `cy.url()` can time out waiting for assertions you've added to
pass.
