...
This means `cy.getCookies().should('have.length', 1)` no longer fails on the
first read when the cookie hasn't been written yet — it retries until a cookie is
present or the command times out.
