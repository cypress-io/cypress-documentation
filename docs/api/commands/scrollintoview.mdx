...
- After executing, `.scrollIntoView()`
...
- `.scrollIntoView()` can only time out if the element disappears while
  scrolling. The `timeout` option is mainly useful to set the timeout of
