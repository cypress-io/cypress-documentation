...
- `.scrollIntoView()` yields the same subject it was given from the
previous command.
...
#### Scroll to element over 2000ms {#Scroll-to-element-over-2000ms}

```javascript
cy.get('footer').scrollIntoView({ duration: 2000 })
```

#### Scroll 150px below an element {#Scroll-150px-below-an-element}
...
- `.scrollIntoView()` requires being chained off a command that yields
DOM element(s).
...
- `.scrollIntoView()` will automatically wait for assertions you have
chained to pass
...
- `.scrollIntoView()` can time out waiting for assertions you've added
to pass.

## Command Log {#Command-Log}

#### Assert element is visible after scrolling it into view {#Assert-element-is-visible-after-scrolling-it-into-view}
