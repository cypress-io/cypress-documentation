...
- `cy.clearAllCookies()` yields `null`, so there's nothing to assert on. To confirm the cookies are cleared, assert on [`cy.getAllCookies()`](/api/commands/getallcookies) afterward, for example `cy.getAllCookies().should('be.empty')`.
