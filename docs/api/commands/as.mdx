---
title: 'cy.as()'
description: Assign an alias in Cypress so you can reference a DOM element, intercept, spy, stub, or value later with an @ prefix.
sidebar_label: as
...
[`.selectFile()`](/api/commands/selectfile) command using an `@` prefix, or on
`this` inside a test written with the standard `function` syntax.
...
you read it, so it reflects the current state of the application. Pass
`{ type: 'static' }` to capture the value at the moment the alias is created
instead.
