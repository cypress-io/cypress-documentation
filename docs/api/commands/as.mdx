...
- `.as()` yields the same subject it was given from the previous
command.
...
### Aliases are reset {#Aliases-are-reset}
...
#### Alias names cannot match some reserved words. {#Alias-names-cannot-match-some-reserved-words}
...
### `as` is asynchronous {#as-is-asynchronous}
