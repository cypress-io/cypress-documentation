...
- `.its()` requires being chained off a previous command that yields an a value
  with `propertyName` as a property.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- `.its()` will automatically [retry](/guides/core-concepts/retry-ability) until
  `propertyName` exists on the subject.

### Timeouts [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Timeouts)

- `.its()` can
  [time out waiting](/guides/core-concepts/introduction-to-cypress#Default-Assertions)
  for `propertyName` to exist on the subject.

### Yields [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Subject-Management)

- `.its()` is a query command, and is
  [retry-able](/guides/core-concepts/retry-ability#Commands-Queries-and-Assertions).
  It is _safe_ to chain further commands that use the subject after `.its()`.
...
[Check out our example recipe on testing `window.fetch` using `.its()`](/examples/recipes#Stubbing-and-spying)
