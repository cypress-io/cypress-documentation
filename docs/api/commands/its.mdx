...
#### Get the `length` property of a DOM element {#Get-the-length-property-of-a-DOM-element}
...
#### Get the `user` object of the response's `body` {#Get-the-user-object-of-the-responses-body}
...
#### Get `length` of title {#Get-length-of-title}
...
#### Get function as property {#Get-function-as-property}
...
#### Access function properties {#Access-function-properties}
...
#### Use `.its()` to test `window.fetch` {#Use-its-to-test-windowfetch}
...
#### Wait for some property to exist on `window` {#Wait-for-some-property-to-exist-on-window}
...
#### Assert that a property does not exist on `window` {#Assert-that-a-property-does-not-exist-on-window}
...
- `.its()` can time out waiting for the property to
