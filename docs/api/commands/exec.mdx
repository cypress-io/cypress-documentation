...
#### Seed the database and assert it was successful {#Seed-the-database-and-assert-it-was-successful}

```javascript
cy.exec('rake db:seed').its('code').should('eq', 0)
```

#### Run an arbitrary script and assert its output {#Run-an-arbitrary-script-and-assert-its-output}
...
#### Write to a file to create a fixture from response body {#Write-to-a-file-to-create-a-fixture-from-response-body}
...
#### Choose to not fail on non-zero exit and assert on code and stderr {#Choose-to-not-fail-on-non-zero-exit-and-assert-on-code-and-stderr}
...
### Commands Must Exit {#Commands-Must-Exit}

#### Commands that do not exit are not supported {#Commands-that-do-not-exit-are-not-supported}
...
### Reset timeout via `Cypress.config()` {#Reset-timeout-via-Cypressconfig}
...
### Set timeout in the test configuration {#Set-timeout-in-the-test-configuration}
...
- `cy.exec()` requires being chained off of `cy`.</li><li>`cy.exec()`
...
- `cy.exec()` will only run assertions you have chained once, and will
...
- `cy.exec()` can time out waiting for the system command to
exist.
