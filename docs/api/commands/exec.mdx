...
- `cy.exec()` ignores the subject it is given.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- `cy.exec()` waits for the command to exit.
- `cy.exec()` asserts that the exit code be `0` when `failOnNonZeroExit` is
  `true` (the default). If the command exits with a non-zero exit code,
  `cy.exec()` will fail the test.

### Timeouts [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Timeouts)

- `cy.exec()` can time out waiting for the system command to exit.
...
- `cy.exec()` yields an object with the following properties:
  ...
  - `stdout`
  - `stderr`
- `cy.exec()` is not a query. It
  [only executes the command once](/guides/core-concepts/retry-ability#Only-queries-are-retried).
