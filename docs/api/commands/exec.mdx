...
- `cy.exec()` yields an object with the following properties:
  ...
  - `stdout`
  - `stderr`
- `cy.exec()` is not a query. It [only executes the command once](/guides/core-concepts/retry-ability#Only-queries-are-retried).
...
- `cy.exec()` asserts that the command exits successfully. If the command exits with a non-zero exit code, `cy.exec()` will fail the test.
