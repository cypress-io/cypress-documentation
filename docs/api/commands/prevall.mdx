...
#### Find all of the element's siblings before `.third` {#Find-all-of-the-elements-siblings-before-third}
...
#### Find all of the previous siblings of each `li`. Keep only the ones with a class `selected` {#Find-all-of-the-previous-siblings-of-each-li-Keep-only-the-ones-with-a-class-selected}
...
- `.prevAll()` requires being chained off a command that yields DOM
element(s).
