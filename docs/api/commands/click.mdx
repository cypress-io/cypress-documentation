...
Register an [`cy.intercept()`](/api/commands/intercept) before the click, then
...
cy.get('[data-cy=todo]').eq(2).find('.delete').click()
...
#### Keep a sticky header from covering the element

Cypress scrolls the element into view before clicking it. With the default
`scrollBehavior` of `top`, a sticky header can end up covering the element and
fail the [covering check](/app/core-concepts/interacting-with-elements#Covering).
Scroll the element to the center of the viewport instead, or set each axis
...
#### Dismiss what covers an element before forcing a click

When another element covers the one you want to click, deal with it the way a
user would before reaching for `{ force: true }`. Forcing skips the checks that
would catch a real bug, such as a banner that blocks users from checking out.
...
Cypress starts from the element at the click coordinates, which can differ from
the subject when a descendant is positioned outside of its parent (per the CSS
Object Model), and walks up its ancestors to find a focusable element. If none
is focusable, Cypress blurs the currently focused element instead, which matches
...
(`e.preventDefault()`), the element does not receive focus, as per the spec.
