...
- `.click()` yields the same subject it was given from the previous
command.
...
#### Click a link in a nav {#Click-a-link-in-a-nav}
...
#### Specify a corner of the element to click {#Specify-a-corner-of-the-element-to-click}
...
#### Force a click regardless of its actionable state {#Force-a-click-regardless-of-its-actionable-state}
...
#### Force a click with position argument {#Force-a-click-with-position-argument}

```javascript
cy.get('#collapse-sidebar').click('bottomLeft', { force: true })
```

#### Force a click with relative coordinates {#Force-a-click-with-relative-coordinates}

```javascript
cy.get('#footer .next').click(5, 60, { force: true })
```

#### Click all elements with id starting with 'btn' {#Click-all-elements-with-id-starting-with-btn}
...
#### Click with key combinations {#Click-with-key-combinations}
...
#### Focus is given to the first focusable element {#Focus-is-given-to-the-first-focusable-element}
...
#### Mousedown cancellation will not cause focus {#Mousedown-cancellation-will-not-cause-focus}
...
- `.click()` requires being chained off a command that yields DOM
element(s).
