...
- `.pause()` yields the same subject it was given from the previous
command.
...
#### Pause after assertion {#Pause-after-assertion}
...
- `.pause()` can be chained off of `cy` or off another
command.
...
- `.pause()` is a utility command.</li><li>`.pause()` will not run
...
### Pause and step through each `.click()` command {#Pause-and-step-through-each-click-command}
