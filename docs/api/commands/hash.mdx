...
#### When the current URL contains a hash: {#When-the-current-URL-contains-a-hash}

- `cy.hash()` "yields the current URL's hash (including the `#`
character)" 

#### When the current URL does not contain a hash: {#When-the-current-URL-does-not-contain-a-hash}
...
#### Assert that hash is `#/users/1` given remote URL: `http://localhost:8000/app/#/users/1` {#Assert-that-hash-is-users1-given-remote-URL-httplocalhost8000appusers1}
...
#### Assert that the hash matches via RegExp {#Assert-that-the-hash-matches-via-RegExp}
...
- `cy.hash()` can time out waiting for assertions you've added to
pass.
