...
// TypeScript: type the yielded object
cy.env < T > keys
cy.env < T > (keys, options)
```
...
```javascript
cy.env('apiUrl') // Errors, keys must be an array
cy.env([]) // Errors, the array cannot be empty
cy.env(['']) // Errors, each key must be a non-empty string
cy.env(['environment']) // Yields { environment: undefined }, values in `expose` are read with Cypress.expose()
```
...
`cy.env()` reads only from the resolved `env` configuration. Values set in `expose` are not returned; read those with [`Cypress.expose()`](/api/cypress-api/expose).
...
| `timeout` | `4000`  | Time to wait for `cy.env()` to resolve before timing out. This default does not follow `defaultCommandTimeout`.                        |

<HeaderYields />

`cy.env()` yields an object with one property for each requested key. A key that isn't set has the value `undefined`.
...
In TypeScript, the yielded object is typed as `Record<string, any>`. Pass a type argument to type it yourself:

```typescript
cy.env<{ apiUrl: string; retries: number }>(['apiUrl', 'retries']).then(
  ({ apiUrl, retries }) => {
...
- Environment variables are not synced into [`cy.origin()`](/api/commands/origin) contexts; `cy.env()` called inside a `cy.origin()` callback requests them the same way
...
### Where you can call cy.env()

In end-to-end testing, `cy.env()` must be invoked from the spec file or the support file, including custom commands and functions that those files import. A call made from code running in the application under test fails with an error.
...
- `cy.env()` requires an array of one or more non-empty string keys.
- In end-to-end testing, `cy.env()` requires being invoked from the spec file or support file.

<HeaderAssertions />

- `cy.env()` will only run assertions you have chained once, and will not
  [retry](/app/core-concepts/retry-ability).

<HeaderTimeouts />

- `cy.env()` can time out waiting for the Cypress server to return the requested values.
