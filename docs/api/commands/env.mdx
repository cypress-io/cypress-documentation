...
description: Get environment variables in your tests.
sidebar_label: env
---
...
Retrieve environment variables set in your Cypress configuration.

:::caution

**Read-Only Command**

`cy.env()` is **read-only** and can only retrieve environment variables. It cannot be used to set or modify environment variables. To set environment variables, use one of the methods described in the [Environment Variables](/app/references/environment-variables) guide.
...
Once you've migrated all `Cypress.env()` calls to `cy.env()`, you can enable the `disallowCypressEnv` configuration option to prevent future use of `Cypress.env()`:
...
Please see the [Migration Guide](/app/guides/migration/cypress-env-to-cy-env) for detailed migration instructions.
...
Environment variables cannot be set using [test configuration](/app/references/configuration#Test-Configuration) when `disallowCypressEnv` is set to `true`. This is a security feature to prevent exposing sensitive data through test configuration.
...
| Version                                    | Changes                  |
| ------------------------------------------ | ------------------------ |
| [15.9.0](/app/references/changelog#15-9-0) | `cy.env()` command added |

## See also

- [Migration Guide: `Cypress.env()` to `cy.env()`](/app/guides/migration/cypress-env-to-cy-env)
