...
- `.type()` yields the same subject it was given from the previous
command.
...
#### Type into a textarea. {#Type-into-a-textarea}

```javascript
cy.get('textarea').type('Hello world') // yields <textarea>
```

#### Type into a login form {#Type-into-a-login-form}
...
#### Mimic user typing behavior {#Mimic-user-typing-behavior}
...
#### 'Selecting' an option from datalist {#Selecting-an-option-from-datalist}
...
#### Type into a non-input or non-textarea element with `tabindex` {#Type-into-a-non-input-or-non-textarea-element-with-tabindex}
...
#### Type a key combination {#Type-a-key-combination}
...
#### Type literal `{` or `}` characters {#Type-literal--or--characters}
...
#### Hold down modifier key and type a word {#Hold-down-modifier-key-and-type-a-word}
...
#### Use keyboard shortcuts in body {#Use-keyboard-shortcuts-in-body}
...
#### Do a shift + click {#Do-a-shift--click}
...
#### Force typing regardless of its actionable state {#Force-typing-regardless-of-its-actionable-state}
...
#### When element is not in focus {#When-element-is-not-in-focus}
...
#### Events that fire {#Events-that-fire}
...
#### Preventing `mousedown` does not prevent typing {#Preventing-mousedown-does-not-prevent-typing}
...
#### Key Events Table {#Key-Events-Table}
...
#### Typing `tab` key does not work {#Typing-tab-key-does-not-work}
...
#### Implicit form submission behavior {#Implicit-form-submission-behavior}
...
- `.type()` requires being chained off a command that yields DOM
element(s).
