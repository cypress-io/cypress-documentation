...
- `.type()` requires the element to be an `input` or `textarea`.
...
- After executing, `.type()` [implicitly asserts](/guides/core-concepts/introduction-to-cypress#Implicit-Assertions) that the element(s) still
