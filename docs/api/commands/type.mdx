...
| Version                                      | Changes                                                                                                                                |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| [13.14.0](/app/changelog#13-14-0) | Added support for `.type({upArrow})` and `.type({downArrow})` to work on date, month, week, time, datetime-local and range input types |
| [6.1.0](/app/changelog#6-1-0)     | Added option `scrollBehavior`                                                                                                          |
| [5.6.0](/app/changelog#5-6-0)     | Support single key combination syntax                                                                                                  |
| [5.5.0](/app/changelog#5-5-0)     | Support `beforeinput` event                                                                                                            |
| [3.4.1](/app/changelog#3-4-1)     | Added `parseSpecialCharSequences` option                                                                                               |
| [3.3.0](/app/changelog#3-3-0)     | Added `{insert}`, `{pageup}` and `{pagedown}` character sequences                                                                      |
| [3.2.0](/app/changelog#3-2-0)     | Added `{home}` and `{end}` character sequences                                                                                         |
| [0.20.0](/app/changelog#0-20-0)   | Supports for typing in inputs of type `date`, `time`, `month`, and `week`                                                              |
| [0.17.1](/app/changelog#0-17-1)   | Added `ctrl`, `cmd`, `shift`, and `alt` keyboard modifiers                                                                             |
| [0.16.3](/app/changelog#0-16-3)   | Supports for typing in elements with `tabindex` attribute                                                                              |
| [0.16.2](/app/changelog#0-16-2)   | Added `{downarrow}` and `{uparrow}` character sequences                                                                                |
| [0.8.0](/app/changelog#0-8-0)     | Outputs Key Events Table to console on click                                                                                           |
| [0.8.0](/app/changelog#0-8-0)     | Added `{selectall}`, `{del}`, `{backspace}`, `{esc}`, `{{}`, `{enter}`, `{leftarrow}`, `{rightarrow}` character sequences              |
| [0.8.0](/app/changelog#0-8-0)     | Added small delay (10ms) between each keystroke during `cy.type()`                                                                     |
| [0.6.12](/app/changelog#0-6-12)   | Added option `force`                                                                                                                   |
