...
- `cy.visit()` ignores the subject it is given.
- After visiting a cross-origin site, you must use a
  [`cy.origin()`](/api/commands/origin) block to interact with the content. When
  visiting a cross-origin site, the `onBeforeLoad` and `onLoad` options are not
  supported.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- `cy.visit()` asserts that response has `content-type: text/html`.
- `cy.visit()` asserts that the response code is `2xx` after following
  redirects.
- `cy.visit()` waits for the `load` event fire.
...
- [Bootstrapping your App](/examples/recipes#Server-Communication)
- [Set a token to `localStorage` for login during Single Sign On](/examples/recipes#Logging-In)
- [Stub `window.fetch`](/examples/recipes#Stubbing-and-spying)
...
After visiting a cross-origin site, you must use a
[`cy.origin()`](/api/commands/origin) block to interact with the content.
...
- [Recipe: Bootstrapping your App](/examples/recipes#Server-Communication)
- [Recipe: Logging In - Single Sign on](/examples/recipes#Logging-In)
- [Recipe: Stubbing `window.fetch`](/examples/recipes#Stubbing-and-spying)
