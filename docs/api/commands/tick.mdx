...
#### Create a clock and move time to trigger a `setTimeout` {#Create-a-clock-and-move-time-to-trigger-a-setTimeout}
...
- `cy.tick()` requires being chained off of `cy`.</li><li>`cy.tick()`
requires that `cy.clock()` be called before it.
...
- `cy.tick()` is a utility command.</li><li>`cy.tick()` will not run
