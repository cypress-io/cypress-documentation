...
see [Callback restrictions](#Callback-restrictions) section below for a
full list.
...
between tests. So to avoid this overhead we recommend you use the
[`cy.session()`](/api/commands/session) command, which caches session
information and reuses it across tests. So now let's enhance our
...
[`Cypress.require()`](/api/cypress-api/require) can be used to include
...
that share the same origin. This can be used to share code between
