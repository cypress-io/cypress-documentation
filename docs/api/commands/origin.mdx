...
// Won't work because still on example.cypress.io
cy.get('h1').contains('My cool site under test')
```
...
cy.origin('example.cypress.io', () => {
  // Visit https://example.cypress.io/commands/querying
  cy.visit('/commands/querying')
  cy.get('h1').contains('Querying')
})
```
...
`/commands/querying`, the three are concatenated to make
