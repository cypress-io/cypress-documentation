...
- `cy.reload()` ignores the subject it is given.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- `cy.reload()` asserts that response has `content-type: text/html`.
- `cy.reload()` asserts that the response code is `2xx` after following
  redirects.
- `cy.reload()` waits for the `load` event fire.
