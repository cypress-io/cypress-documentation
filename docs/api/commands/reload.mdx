...
- `cy.reload()` 'yields the `window` object after the page finishes
loading'
...
#### Reload the page as if the user clicked 'Refresh' {#Reload-the-page-as-if-the-user-clicked-Refresh}
...
- `cy.reload()` requires being chained off of
...
- `cy.reload()` will automatically wait for assertions you have chained
to pass
...
- `cy.reload()` can time out waiting for the page to fire its `load`
