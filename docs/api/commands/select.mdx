...
- `.select()` yields the same subject it was given from the previous
command.
...
#### Select the `option` with the text `apples` {#Select-the-option-with-the-text-apples}
...
#### Select the `option` with the value "456" {#Select-the-option-with-the-value-456}
...
#### Select the `option` with index 0 {#Select-the-option-with-index-0}
...
#### Select the options with the texts "apples" and "bananas" {#Select-the-options-with-the-texts-apples-and-bananas}
...
#### Select the options with the values "456" and "457" {#Select-the-options-with-the-values-456-and-457}
...
#### Select the options with the indexes 0 and 1 {#Select-the-options-with-the-indexes-0-and-1}
...
#### Force select a hidden `<select>` {#Force-select-a-hidden-select}
...
#### Force select a disabled `<select>` {#Force-select-a-disabled-select}
...
- `.select()` requires being chained off a command that yields DOM
