...
- `.select()` requires the element to have type `select`.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- Before executing, `.select()` will wait for the element to reach an
  [actionable state](/guides/core-concepts/interacting-with-elements).
- After executing, `.select()` [implicitly asserts](/guides/core-concepts/introduction-to-cypress#Implicit-Assertions) that the element(s) still
