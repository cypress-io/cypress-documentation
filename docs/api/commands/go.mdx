...
- `cy.go()` ignores the subject it is given.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- `cy.go()` asserts that response has `content-type: text/html`.
- `cy.go()` asserts that the response code is `2xx` after following redirects.
- `cy.go()` waits for the `load` event fire.
