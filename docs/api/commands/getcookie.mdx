...
#### When a cookie matching the name could not be found: {#When-a-cookie-matching-the-name-could-not-be-found}
...
#### Get `session_id` cookie after logging in {#Get-session_id-cookie-after-logging-in}
...
#### Using `cy.getCookie()` to test logging in {#Using-cygetCookie-to-test-logging-in}
