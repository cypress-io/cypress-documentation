...
| --------- | ----------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| `domain`  | Hostname of the application under test                            | Retrieves the cookie from the specified domain                                      |
...
assertions have passed, or until it times out, re-reading the cookie on each
retry so the yielded value stays up to date. Chained assertions are what drive
those retries: with no assertions chained, `cy.getCookie()` yields whatever it
reads on the first attempt, which is `null` if the cookie does not exist yet.
Chain [`.should('exist')`](/api/commands/should) to wait for a cookie that is
set asynchronously.

:::caution

Because `cy.getCookie()` is a query, it re-reads the cookie on every retry, and
assertions you chain with [`.should()`](/api/commands/should) drive those retries
until they pass or the command times out. A [`.then()`](/api/commands/then)
callback, by contrast, runs only once after the command settles and is not
retried — so prefer chaining [`.should()`](/api/commands/should) assertions over
asserting inside `.then()` when you need Cypress to wait for the cookie to be
set.
...
assertions run against. If an assertion fails, either because the cookie is `null`, or
