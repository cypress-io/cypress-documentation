...
#### Get the children of the `.secondary-nav` {#Get-the-children-of-the-secondary-nav}
...
#### Get the children with class `active` {#Get-the-children-with-class-active}
...
- `.children()` requires being chained off a command that yields DOM
element(s).
