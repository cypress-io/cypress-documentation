...
#### When given a `time` argument: {#When-given-a-time-argument}

- `cy.wait()` yields the same subject it was given from the previous
command.

#### When given an `alias` argument: {#When-given-an-alias-argument}
...
#### Wait for an arbitrary period of milliseconds: {#Wait-for-an-arbitrary-period-of-milliseconds}
...
#### Wait for a specific request to respond {#Wait-for-a-specific-request-to-respond}
...
#### Wait automatically increments responses {#Wait-automatically-increments-responses}
...
#### You can pass an array of aliases that will be waited on before resolving. {#You-can-pass-an-array-of-aliases-that-will-be-waited-on-before-resolving}
...
#### Using [`.spread()`](/api/commands/spread) to spread the array into multiple arguments. {#Using-spread-to-spread-the-array-into-multiple-arguments}
...
#### `requestTimeout` and `responseTimeout` {#requestTimeout-and-responseTimeout}
...
#### Using an Array of Aliases {#Using-an-Array-of-Aliases}
...
- When passed a `time` argument `cy.wait()` can be chained off of `cy`
...
- `cy.wait()` will only run assertions you have chained once, and will
...
- `cy.wait()` can time out waiting for the request to go
