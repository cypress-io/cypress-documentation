...
// As templated types:
cy.wait<CustomRequest, CustomResponse>('@getUsers').then(({ request, response }) => {
  request.body // will be of type CustomRequest
  response.body // will be of type CustomResponse
})

// As inferred types, with type `Interception` available in `cypress/types/net-stubbing`
cy.wait('@getUsers').then(({ request, response }: Interception<CustomRequest, CustomResponse>) => {
  request.body // will be of type CustomRequest
  response.body // will be of type CustomResponse
})
```
...
In Typescript, you are able specify an array of types for the request and response bodies.
...
cy.wait(['@getUsers', '@getActivities']).then(
  (interceptions: Array<Interception<GetUsersReq | GetActiviesReq, GetUsersRes | GetActiviesRes>>) => {
    interceptions.forEach(({ request, response }) => {
      request.body // Is of type GetUsersReq | GetActivitiesReq
      response.body // Is of type GetUsersRes | GetActivitiesRes
    })
  }
)
...
| Version                                         | Changes                                             |
| ----------------------------------------------- | --------------------------------------------------- |
| [13.11.0](/guides/references/changelog#13-11-0) | Added examples for Typescript type parameters       |
