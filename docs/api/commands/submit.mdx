...
- `.submit()` requires being chained off a command that yields a DOM element.
- `.submit()` requires the element to have type `form`.

### Assertions [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Assertions)

- After executing, `.submit()` [implicitly asserts](/guides/core-concepts/introduction-to-cypress#Implicit-Assertions) that the element(s) still
  [exist in the DOM](/guides/core-concepts/introduction-to-cypress#Default-Assertions).

### Timeouts [<Icon name="question-circle"/>](/guides/core-concepts/introduction-to-cypress#Timeouts)

- `.submit()` can time out if the form element is removed from the page after
  submitting. The `timeout` argument is mostly useful to set the timeout for
