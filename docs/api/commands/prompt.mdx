...
`cy.prompt` turns plain-English test steps into real, executable Cypress commands using AI. Describe what you want to test the way you'd explain it to a teammate (for example, `'click the login button'` or `'verify the dashboard loads'`), and `cy.prompt` figures out the selectors and commands for you.
