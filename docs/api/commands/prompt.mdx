...
- **Specify Cypress options**: Some Cypress command options can be specified directly in your prompts using natural language. This allows you to customize `timeout` and `force` options without changing your test code. For example: `'go to "/users" with a timeout of 10 seconds'` or `'force click the submit button'`. Not all Cypress command options can be specified this way.
