...
#### Simulating `mouseover` event to get popover to display {#Simulating-mouseover-event-to-get-popover-to-display}
...
#### Example of showing an element in order to perform action {#Example-of-showing-an-element-in-order-to-perform-action}
...
#### Example of clicking on a hidden element {#Example-of-clicking-on-a-hidden-element}

```javascript
cy.get('.hidden').click({ force: true })
```

#### Example of checking a hidden element {#Example-of-checking-a-hidden-element}

```javascript
cy.get('.checkbox').check({ force: true })
```

### Add as a custom command {#Add-as-a-custom-command}
