...
### Read a file that might not exist {#Read-a-file-that-might-not-exist}
...
### Return a Promise from an asynchronous task {#Return-a-Promise-from-an-asynchronous-task}
...
### Save a variable across non same-origin URL visits {#Save-a-variable-across-non-same-origin-URL-visits}
...
### Tasks must end {#Tasks-must-end}

#### Tasks that do not end are not supported {#Tasks-that-do-not-end-are-not-supported}
...
### Tasks are merged automatically {#Tasks-are-merged-automatically}
...
### Allows a single argument only {#Allows-a-single-argument-only}
...
### Argument should be serializable {#Argument-should-be-serializable}
...
- `cy.task()` requires being chained off of `cy`.</li><li>`cy.task()`
...
- `cy.task()` will only run assertions you have chained once, and will
