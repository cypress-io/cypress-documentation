...
`cy.task()` is the recommended replacement for
[`cy.exec()`](/api/commands/exec), which was deprecated in Cypress `15.21.0`.
Tasks run in the Node process that loads your Cypress configuration, so they
don't depend on the operating system, shell, or terminal of the machine running
Cypress. See
[Run an external command or CLI](#Run-an-external-command-or-CLI) for tasks that
need to shell out to a non-Node tool.
...
Tasks are a good fit for file system work, such as emptying the downloads folder
before a test and then finding the file the test just downloaded.
...
```ts
const downloads = path.join(config.projectRoot, 'cypress', 'downloads')
...
When setup depends on a tool that isn't a Node module — a Docker container, a
database CLI, or a cloud provider's command line tool — spawn it from the task
with
[`child_process.execFileSync()`](https://nodejs.org/api/child_process.html#child_processexecfilesynccommand-args-options).
Passing the arguments as an array avoids the shell entirely, so quoting and
`PATH` resolution behave the same way on every platform.
...
- [`cy.exec()`](/api/commands/exec) <Badge type="caution">deprecated</Badge>
