...
#### The following options are merged in as default options to [`cy.route()`](/api/commands/route) {#The-following-options-are-merged-in-as-default-options-to-cyroute}
...
#### The following options control the behavior of the server affecting all requests {#The-following-options-control-the-behavior-of-the-server-affecting-all-requests}
...
- `cy.server()` yields `null`.</li><li>`cy.server()` cannot be chained
further.
...
#### After starting a server: {#After-starting-a-server}
...
#### Change defaults for [`cy.route()`](/api/commands/route) {#Change-defaults-for-cyroute}
...
#### Change the default delay for all routes {#Change-the-default-delay-for-all-routes}
...
#### Send 404s on unmatched requests {#Send-404s-on-unmatched-requests}
...
#### Change the default response headers for all routes {#Change-the-default-response-headers-for-all-routes}
...
#### Set a custom request header for all requests {#Set-a-custom-request-header-for-all-requests}
...
#### Change the default filtering {#Change-the-default-filtering}
...
#### Turn off the server after you've started it {#Turn-off-the-server-after-youve-started-it}
...
#### Server persists until the next test runs {#Server-persists-until-the-next-test-runs}
...
#### Outstanding requests are automatically aborted between tests {#Outstanding-requests-are-automatically-aborted-between-tests}
...
#### Server can be started before you [`cy.visit()`](/api/commands/visit) {#Server-can-be-started-before-you-cyvisit}
...
### `cy.request()` {#cyrequest}

#### `cy.server()` does not effect [cy.request()](/api/commands/request) {#cyserver-does-not-effect-cyrequest}
