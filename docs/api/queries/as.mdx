...
cy.get('input.username').invoke('val').as('username', { type: 'static' }) // Alias a static (not re-queried) value
...
- `.as()` is a utility command.</li><li>`.as()` will not run assertions.
  Assertions will pass through as if this command did not exist.
