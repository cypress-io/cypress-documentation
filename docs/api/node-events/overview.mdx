...
Some plugins may use or require these values, so they can take certain
...
[Check out our API docs for modifying configuration here.](/api/node-events/configuration-api)

## Use Cases

### Configuration

Hooking into node events programmatically alters the resolved configuration and
...
[`cy.task()`](/api/commands/task) command. It runs arbitrary code in Node to
accomplish tasks that aren't possible in the browser.
