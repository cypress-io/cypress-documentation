...
Cypress.config() // => {defaultCommandTimeout: 10000, pageLoadTimeout: 60000, ...}
```
...
A few options can be set in a suite or test configuration override, but not with
`Cypress.config()` while a test is executing, because doing so would affect the
_next_ test rather than the current one. Starting in Cypress
[16.0.0](/app/references/changelog#16-0-0), `Cypress.config()` throws for these
options during test execution:

- `blockHosts`
- `viewportHeight` - use [`cy.viewport()`](/api/commands/viewport) to change the
  viewport during a test
- `viewportWidth` - use [`cy.viewport()`](/api/commands/viewport) to change the
  viewport during a test
...
`Cypress.config()` executes synchronously and will
...
| Version                                    | Changes                                                                                                                 |
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| [16.0.0](/app/references/changelog#16-0-0) | `blockHosts`, `viewportHeight`, and `viewportWidth` can no longer be set with `Cypress.config()` during test execution. |
