...
The Keyboard API exposes the available `Keys` for use with [`cy.press()`](/api/commands/press) and sets the default values for how the
...
`keystrokeDelay` can also be set in the [Cypress configuration](/app/references/configuration#Keyboard). The resolved value is read from the configuration first, then from `Cypress.Keyboard.defaults()`, and falls back to `0` if neither is set. A per-command `delay` passed to [`.type()`](/api/commands/type) is independent of this and overrides the resolved value for that single call.
