...
`Cypress.env()` has been deprecated in Cypress `15.10.0` and will be removed in a future major version.

Cypress recommends migrating to using [`cy.env()`](/api/commands/env) or [`Cypress.expose()`](/api/cypress-api/expose) instead.
