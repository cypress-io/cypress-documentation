...
be asynchronous, or if it shouldn't be called more than once.
...
Cypress.Commands.addQuery('get', function (originalFn, ...args) {
...
commands. If you want to modify the behavior of a non-query command, you'll
need to use [`Cypress.Commands.overwrite`](/api/cypress-api/custom-commands)
instead.
