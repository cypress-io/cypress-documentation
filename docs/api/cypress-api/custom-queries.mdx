...
This is a general pattern: when something goes wrong, queries throw an
...
When defining `focused2()`, we used `function`,
...
or you can perform your own validation and throw an error:
...
There's nothing special about these functions - they validate their
...
Both of these are wrapping `cy.get(selector)`. It's completely unnecessary. Call
`.get('button')` or `.get('tr:first')` instead.
