...
#### Click link containing text {#Click-link-containing-text}
...
#### Commands to work with `sessionStorage` {#Commands-to-work-with-sessionStorage}
...
#### Log in command using UI {#Log-in-command-using-UI}
...
#### Log in command using request {#Log-in-command-using-request}
...
#### Log out command using UI {#Log-out-command-using-UI}
...
#### Log out command using `localStorage` <E2EOnlyBadge /> {#Log-out-command-using-localStorage-E2EOnlyBadge-}
...
#### Custom `console` command {#Custom-console-command}
...
#### Custom Dual Command {#Custom-Dual-Command}
...
#### Overwrite `visit` command {#Overwrite-visit-command}
...
#### Overwrite `type` command {#Overwrite-type-command}
...
<strong>Keep passwords secret blog</strong>
...
#### Overwrite `screenshot` command {#Overwrite-screenshot-command}
...
#### Overwrite `contains` command {#Overwrite-contains-command}
...
### Allow Multiple Types {#Allow-Multiple-Types}
...
### Optional with Types {#Optional-with-Types}
...
### `cy.hover()` and `cy.mount()` {#cyhover-and-cymount}
...
#### 1. Don't make everything a custom command {#1-Dont-make-everything-a-custom-command}
...
#### 2. Don't overcomplicate things {#2-Dont-overcomplicate-things}
...
#### 3. Don't do too much in a single command {#3-Dont-do-too-much-in-a-single-command}
...
#### 4. Skip your UI as much as possible {#4-Skip-your-UI-as-much-as-possible}
...
#### 5. Write TypeScript definitions {#5-Write-TypeScript-definitions}
