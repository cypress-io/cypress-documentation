...
**<Icon name="exclamation-triangle" /> `Cypress.Cookies.preserveOnce()` and `Cypress.Cookies.defaults()` are deprecated in Cypress 9.7.0.**

In a future release, support for
...
#### Log when cookie values are created, modified or deleted {#Log-when-cookie-values-are-created-modified-or-deleted}
...
#### Turn off verbose debugging output {#Turn-off-verbose-debugging-output}
...
### Preserve Once {#Preserve-Once}

#### Preserve cookies through multiple tests {#Preserve-cookies-through-multiple-tests}
...
#### Set global default cookies {#Set-global-default-cookies}
...
#### `preserve` accepts: {#preserve-accepts}
...
#### Preserve RegExp {#Preserve-RegExp}
