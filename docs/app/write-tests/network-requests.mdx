...
description: 'Observe, wait on, and stub your application network requests with cy.intercept(), including fixtures, aliases, assertions, and GraphQL queries and mutations.'
sidebar_label: Intercepting network requests
sidebar_position: 20
slug: /app/guides/network-requests
---
...
This guide covers the requests _your application_ makes, which Cypress can
observe, wait on, and stub with [`cy.intercept()`](/api/commands/intercept).

To call an endpoint _from your test_ and assert on what it returns, see
...
complex JSON objects. Cypress integrates fixture syntax directly into responses.
...
This following section uses a concept known as
...
With Cypress, by adding a [`cy.wait()`](/api/commands/wait), you can pinpoint
your specific problem. If the response never came back, you'll receive
an error like this:
...
it yields the actual request object. This is useful when you want to make
assertions about this object.
...
stubbed. The alias for the `cy.intercept()` is also now displayed on
...
- [API Testing](/app/guides/api-testing) - calling endpoints directly with
  `cy.request()`
...
- Read the blog post
  [Asserting Network Calls from Cypress Tests](https://www.cypress.io/blog/2019/12/23/asserting-network-calls-from-cypress-tests/?utm_source=docs.cypress.io&utm_medium=network-requests)
- If you want to test the application in offline mode, read
  [Testing an Application in Offline Network Mode](https://www.cypress.io/blog/2020/11/12/testing-application-in-offline-network-mode/?utm_source=docs.cypress.io&utm_medium=network-requests)
