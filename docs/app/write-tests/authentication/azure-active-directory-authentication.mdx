...
sidebar_position: 30
slug: /app/guides/authentication-testing/azure-active-directory-authentication
---
...
against an Azure Active Directory web app. Clone the
...
- Remove any
  ...
  [framebusting code](/app/references/configuration#modifyObstructiveCode). The
  recommended way to do this is to enable the
  [`removeSRIAttributes`](/app/references/configuration#removeSRIAttributes)
  configuration option, which strips these attributes automatically without
  editing the application's source. If you'd rather not change your Cypress
  configuration, you can instead remove the `integrity` attributes from the
  `<script>` tags manually. Either approach is recommended only when an
...
following this guide, or have any feedback, let us know and submit a
