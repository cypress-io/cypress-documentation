...
sidebar_position: 20
slug: /app/guides/authentication-testing/auth0-authentication
---
...
`auth0_client_secret` is only needed for
...
Auth0 authentication. If you are
...
To validate API requests from the frontend, we install
...
Cypress.Commands.add('loginByAuth0Api', (username, password) => {
  ...
  cy.request('https://ipv4.icanhazip.com')
    .its('body')
    .then((ip) => {
