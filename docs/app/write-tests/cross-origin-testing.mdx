...
Cypress must assign and manage browser certificates to modify the
...
CA authority and issue certificates dynamically to intercept requests
otherwise impossible to access. We only do this for the origin currently under
test, and bypass other traffic.

Note, that Cypress optionally accepts CA / client certificate
...
Although we do our **very best** to ensure your
...
**Same-origin** iframes are not affected by this restriction and can be
...
If for any reason you can't use `cy.origin`, programmatic authentication
...
Setting `chromeWebSecurity` to `false` in Chrome-based browsers does the
following:

- Displays insecure content
- Navigates to any origin without cross-origin errors with or without
  `cy.origin`
- Accesses cross-origin iframes that are embedded in your application
...
- Removes
  ...
  from modified third-party scripts as they will not execute otherwise. To also
  remove SRI from first-party resources (and from `integrity` set via a runtime
  DOM property assignment), use the
  [`removeSRIAttributes`](/app/references/configuration#removeSRIAttributes)
