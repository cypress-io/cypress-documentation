...
sidebar_label: Vue examples
sidebar_position: 20
slug: /app/component-testing/vue/examples
---
...
To encourage interoperability between your existing component tests and
...
`mount` command and use `wrapper.emitted()` to gain access to Native
...
Notice that we're using the `'should'` function signature to take
...
to make sure the DOM has updated or any reactive events have fired.
...
This registers `cy.mount()` for any test without having to import the `mount()`
