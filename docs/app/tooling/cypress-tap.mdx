...
| [`specs`](p#cypress-tap-specs)      | Lists the specs the session can run.                                                                                                                                                           |
...
| Option            | Description                                                               |
| ----------------- | ------------------------------------------------------------------------- |
| `--session <pid>` | Use to target a specific running Cypress instance by its process id (pid) |
...
polling, see [Polling for a verdict]().
...
EMMILY -- FIX THIS I DONT UNDERSTAND IT
...
### `cypress tap run <spec>` {#cypress-tap-run}
...
[`status`](#cypress-tap-status) command for progress; see [Polling for a verdict]().
...
| ------------------------- | ----------------------------------------------------------- | ----------------------- |
| `-t, --testId <testId>`   | Test id, as listed by the spec overview this command prints | The spec-level overview |
| `-a, --attempt <attempt>` | 1-based attempt, where attempt 1 is the first run           | The latest attempt      |
...
| Option                          | Description                                                                                                                                                           | Default            |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| `-t, --test-id <test-id>`       | Test id, as listed by the [`reporter`](#cypress-tap-reporter) command                                                                                                 | Required           |
| `-c, --command-id <command-id>` | Command id, as listed by the `reporter` command: a row number, an e-prefixed event id, or hook-qualified like `h1:3`                                                  | Required           |
| `-a, --attempt <attempt>`       | 1-based attempt, where attempt 1 is the first run                                                                                                                     | The latest attempt |
| `-d, --depth <depth>`           | How many levels of nested console properties to expand before summarizing the rest: a number or `all`. A section over 8 rows folds at any depth unless this is passed | `3`                |
...
| `-t, --test-id <test-id>`       | Test id, as listed by the [`reporter`](#cypress-tap-reporter) command                                                | Required                                     |
| `-c, --command-id <command-id>` | Command id, as listed by the `reporter` command: a row number, an e-prefixed event id, or hook-qualified like `h1:3` | Required                                     |
| `--at <at>`                     | Which snapshot to pin, either a name like `before`/`after`, or a 1-based index                                       | The last snapshot, the command's final state |
| `-a, --attempt <attempt>`       | 1-based attempt, where attempt 1 is the first run                                                                    | The latest attempt                           |
...
### `cypress tap dom [selector]` {#cypress-tap-dom}
...
| Option                        | Description                                                                       | Default |
| ----------------------------- | --------------------------------------------------------------------------------- | ------- |
| `-e, --selector <selector>`   | CSS selector matching exactly one element                                         | `body`  |
| `-m, --max-chars <max-chars>` | Cap on returned HTML characters                                                   | `30000` |
| `--at <at>`                   | When a selector matches multiple elements, the 0-based index of the match to read | `0`     |
...
### `cypress tap aria [selector]` {#cypress-tap-aria}
...
| ----------------------------- | --------------------------------------------------------------------------------- | ------- |
| `-e, --selector <selector>`   | CSS selector matching exactly one element                                         | `body`  |
| `-m, --max-nodes <max-nodes>` | Cap on the number of accessibility nodes returned                                 | `200`   |
| `--at <at>`                   | When a selector matches multiple elements, the 0-based index of the match to read | `0`     |
...
### `cypress tap inspect <selector>` {#cypress-tap-inspect}
...
### Tap CLI vs Cloud MCP vs Cloud CLI
...
The `cypress tap` CLI is available for use for all Cypress test authors using Cypress version 15.22.0+ at no cost. Additionally, there are no usage limits.
...
- You must be on Cypress v15.22.0+ or higher to use the CLI. You cannot use `cypress tap` to interact with older versions.
