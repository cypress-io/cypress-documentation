...
If your project sets `"type": "module"` in `package.json`, Node.js treats `.js`
files as ES modules. In this case, add `"module": "nodenext"` to your Cypress
`tsconfig.json` so TypeScript generates module-compatible output:
...
For bundler-based projects (such as Vite), your framework's `tsconfig.json`
likely already sets a compatible `module` and `moduleResolution` (e.g.
`"moduleResolution": "bundler"`), so the Cypress `tsconfig.json` can extend it
or rely on those settings.
