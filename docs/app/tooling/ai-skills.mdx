...
description: Give your AI tool expert-level Cypress knowledge so it writes, reviews, and explains tests using Cypress best practices.
...
AI coding tools can write Cypress tests in seconds, but without guidance they
often reach for brittle selectors, outdated APIs, and patterns that don't match
your project. Cypress AI Skills close that gap. They give your AI tool
expert-level Cypress knowledge, so the tests it authors, reviews, and explains
follow the same best practices the Cypress team recommends and stay consistent
across your team.

Skills are published in the open-source
[Cypress AI Toolkit](https://github.com/cypress-io/ai-toolkit) and work with any
AI coding tool that supports custom instructions, including Cursor, Claude Code,
and GitHub Copilot.

## What are AI Skills?

Think of a generic AI as a brilliant but inexperienced new hire. Without
...
Cypress workflow: what conventions to follow, how to reason about the task, and
...
- **Grounds answers in docs, not training data.** If a claim cannot be verified in official docs, it says so rather than inventing an API or behavior, reducing the chance of a confident but incorrect answer.
- **Stays version-aware.** When a Cypress version is detected in your project, it scopes answers to what is actually available in that version and calls out differences explicitly.
...
You can invoke skills explicitly with `/cypress-author` or `/cypress-explain`, or your AI tool can load them automatically when relevant based on the language used in your prompt.

### Scaffold from requirements

This prompt describes acceptance criteria for the "Mark all complete" behavior of the TODO app in our [Kitchen Sink example](https://github.com/cypress-io/cypress-example-kitchensink).
...
Each agent is different, but you can usually look in the conversation history to confirm your agent invoked the skill based on your prompt. The agent will typically print out that fact.
...
2. Skills listen for keywords and concepts, but depending on your agent, model, and other installed skills, they may not always trigger. You can improve your odds by using the word "Cypress" in your prompt.
3. Most agents let you verify a skill is installed and/or force a skill to be used with a slash command, i.e. `/cypress-author`.
