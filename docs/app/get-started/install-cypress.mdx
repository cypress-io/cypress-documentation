...
sidebar_position: 30
slug: /app/get-started/install-cypress
---
...
- **Linux** _(x64 or arm64)_ see also [Linux Prerequisites](#Linux-Prerequisites) down below
  ...
  - Debian >=12 _(arm64 requires Debian >=13)_
  - Fedora >=43
...
The [Node.js Snap for Linux](https://github.com/nodejs/snap) version manager is not recommended for use with Cypress.
Attempting to use it as a non-root user may result in permissions errors.

:::

If you're using a [Cypress Docker image](/app/continuous-integration/overview#Cypress-Docker-variants), you'll find a fixed version of Node.js is pre-installed in the image. You select the Node.js version using the Docker image tag.
...
Each Cypress release also includes a bundled, Electron-based Chromium
[browser](/app/references/launching-browsers#Electron-Browser).
...
so your runs don't rely on the bundled Electron default. See [Migrating away
from the Electron
browser](/app/references/migration-guide#Migrating-away-from-the-Electron-browser).
...
[Cypress Docker images](/app/continuous-integration/overview#Cypress-Docker-variants) are available from
...
generates your own customized Docker image.
