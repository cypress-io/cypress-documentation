...
spec. Accept the default name for now.
...
The newly-generated spec is displayed in a confirmation dialog. Close it with
the ✕ button.
...
we click a link on the page? Let's go look for one we like... how about `type`?
...
The [App](/app/core-concepts/open-mode)
has also updated after the click, following the link and showing the
destination page:
