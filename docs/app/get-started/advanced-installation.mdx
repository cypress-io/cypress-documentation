...
sidebar_position: 40
slug: /app/references/advanced-installation
---
...
on its own with [debug logging](/app/references/troubleshooting#Log-sources) enabled so you can
...
To set environment variables `CYPRESS_INSTALL_BINARY` and `DEBUG` in Windows CMD or PowerShell terminals, refer to examples in [Print DEBUG Logs](/app/references/troubleshooting#Print-DEBUG-logs).
...
The npm config is used as a replacement, and the node environment
...
[`cypress cache`](/app/references/command-line#cypress-cache-command) command lets you inspect and
prune it. Run these with the appropriate package manager prefix described in
[How to run commands](/app/references/command-line#How-to-run-commands).

| Command                                                                   | Description                                                                         |
| ------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| [`cypress cache list`](/app/references/command-line#cypress-cache-list)   | Lists the cached Cypress versions. Add `--size` to also report each version's size. |
| [`cypress cache path`](/app/references/command-line#cypress-cache-path)   | Prints the path to the Cypress binary cache.                                        |
| [`cypress cache prune`](/app/references/command-line#cypress-cache-prune) | Deletes every cached binary except the version currently in use.                    |
| [`cypress cache clear`](/app/references/command-line#cypress-cache-clear) | Deletes all cached Cypress binaries.                                                |
...
The Cypress commands [open](/app/references/command-line#cypress-open), [run](/app/references/command-line#cypress-run), and [verify](command-line#cypress-verify)
will then launch the provided binary.

The following example [cypress run](/app/references/command-line#cypress-run) commands assume that you have first
...
See [How to run commands](/app/references/command-line#How-to-run-commands).
...
If you encounter a `permission denied` failure message from [cypress verify](/app/references/command-line#cypress-verify),
...
Cypress.io does not specifically support the use of Cypress under Windows Subsystem for Linux (WSL). If you want to report an issue, ensure that you can reproduce it without using WSL on one of the Cypress [supported operating systems](/app/get-started/install-cypress#Operating-System).
...
Cypress.io does not specifically support the use of Cypress in Dev Containers or GitHub Codespaces. If you want to report an issue, ensure that you can reproduce it without a containerized environment on one of the Cypress [supported operating systems](/app/get-started/install-cypress#Operating-System).
...
To uninstall all cached Cypress binary versions, use the [cypress cache clear](/app/references/command-line#cypress-cache-clear) command with the appropriate package manager prefix described in [How to run commands](/app/references/command-line#How-to-run-commands).
To remove only the older cached versions while keeping the one currently in use, use [cypress cache prune](/app/references/command-line#cypress-cache-prune) instead.
...
**To delete cached [Cypress App Data](/app/references/troubleshooting#Clear-App-Data), manually delete the following directories / folders:**
