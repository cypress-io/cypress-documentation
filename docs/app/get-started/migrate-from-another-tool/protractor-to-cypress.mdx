...
sidebar_label: Migrate from Protractor
sidebar_position: 30
slug: /app/guides/migration/protractor-to-cypress
---
...
under test looked like at the time of the test's execution. This shows the
**real rendered UI** and the behavior of the app under **real user
...
is critical to iterating quickly and with confidence. With Cypress, your
...
provides an instant feedback loop that keeps each iteration fast and
confident.
...
[retries failed tests](/app/guides/test-retries). Sometimes tests
...
Both of these commands launch Cypress. To run in a specific browser, specify an
installed browser with `--browser` (for example, `--browser chrome`) or set a
[`defaultBrowser`](/app/references/configuration#Browser).
...
you can use the [intercept API](/api/commands/intercept) to spy on and
...
Cypress's [intercept API](/api/commands/intercept) also stubs any network
request for your app under test. You can use the
...
This keeps your tests deterministic since all of your commands are executed
serially, enabling your tests to run predictably each time.
...
Protractor's Control Flow can be disabled, so test cases are written as
...
[Cypress Cloud](/cloud/get-started/introduction) runs your test files in
