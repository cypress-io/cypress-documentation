...
sidebar_label: Migrate from Playwright
sidebar_position: 10
slug: /app/guides/migration/playwright-to-cypress
---
...
<CopyPrompt
  ...
  title="Migrate from Playwright to Cypress with your AI assistant"
  prompt={`Migrate this project's tests from Playwright to Cypress. First read https://docs.cypress.io/app/guides/migration/playwright-to-cypress.md for the syntax mappings and patterns, then work through these steps:\n\n1. Take inventory. Detect my package manager, find my Playwright config and test files, and summarize the patterns I use: locators, assertions, network mocking, authentication, fixtures/page objects, web server startup, and CI configuration.\n\n2. Agree on a selector strategy. If my tests rely on getByRole/getByLabel/getByText and I have no test attributes, recommend installing @testing-library/cypress for a near 1:1 mapping instead of editing my app to add data-testid attributes.\n\n3. Install Cypress alongside Playwright. Configure them to coexist, and do not modify or delete my Playwright config or specs until the Cypress equivalents pass.\n\n4. Migrate one spec at a time. Translate async/await and the locator API to Cypress command chaining and retry-ability (never assign a cy.* return value to a variable), map expect() assertions to Cypress assertions, convert page.route() mocks to cy.intercept(), and port authentication, fixtures, and page objects. If my Playwright config uses a webServer block, set up start-server-and-test so Cypress has a running app.\n\n5. Flag gaps. Call out anything with no direct Cypress equivalent (visual snapshots, soft assertions, test.step) instead of guessing.\n\n6. Verify. Show me the proposed changes before applying them, then install and run eslint-plugin-cypress and my migrated Cypress tests to confirm they pass.\n\nFinish with a summary of what changed and anything you couldn't safely automate.`}
/>
...
Unlike Playwright, Cypress does not download its own browser binaries. It uses the browsers already installed on your machine, such as Chrome.
...
If your Playwright tests are written in TypeScript, add a `tsconfig.json` inside
the `cypress/` folder so Cypress globals (`cy`, `Cypress`) type-check:
...
| <span className="sr-only">Configuration</span> | Playwright                       | Cypress                  |
| ---------------------------------------------- | -------------------------------- | ------------------------ |
...
| `--shard=1/4`                      | `--record --parallel` (requires [Cypress Cloud](/cloud/get-started/introduction))                                                |
| `--fully-parallel`                 | `--record --parallel` (requires [Cypress Cloud](/cloud/get-started/introduction))                                                |
| `--workers=4`                      | `--record --parallel` (requires [Cypress Cloud](/cloud/get-started/introduction))                                                |
...
URLs, API versions). These are set via the top-level `env` and `expose` keys in
your Cypress configuration file. Both APIs require Cypress 15.10.0 or newer.
...
Playwright's config can start your dev server automatically through the
`webServer` option. Cypress does not start your application for you — it assumes
the app is already running at the `baseUrl` you configured.

The standard solution is
[`start-server-and-test`](https://github.com/bahmutov/start-server-and-test),
which boots your server, waits for the URL to respond, runs Cypress, then shuts
the server down.
...
Playwright suites lean on semantic locators — `getByRole()`, `getByLabel()`,
and `getByText()`. Before translating tests, decide how these map to Cypress,
because the choice affects nearly every test.

**Option A — Cypress Testing Library (lowest friction, no app changes).**
Install
[`@testing-library/cypress`](https://github.com/testing-library/cypress-testing-library)
for a near 1:1 replacement of Playwright's semantic locators:

| Playwright                      | Cypress (Testing Library)         |
| ------------------------------- | --------------------------------- |
| `getByRole('button', { name })` | `findByRole('button', { name })`  |
| `getByLabel('Email')`           | `findByLabelText('Email')`        |
| `getByText('Welcome')`          | `findByText('Welcome')`           |
| `getByPlaceholder('Search')`    | `findByPlaceholderText('Search')` |
| `getByTestId('submit')`         | `findByTestId('submit')`          |

**Option B — `data-*` attributes (Cypress best practice for new code).**
Add `data-testid` (or `data-cy`) attributes to your application and select with
`cy.get('[data-testid="..."]')`, as described in
[Best Practices](/app/core-concepts/best-practices#Selecting-Elements). This is
the most resilient long-term strategy, but it requires editing your application
source.

The examples in this guide use `data-testid` for brevity. If your Playwright
suite relies on semantic locators and you don't already have test attributes,
Option A is the fastest path to parity without touching application code.
...
Both tools change the current time without triggering timers. In Playwright this is `page.clock.setSystemTime()`. In Cypress it is `clock.setSystemTime()` on the yielded clock object.
...
Cypress replaces only the native functions you name. This is useful when you only need to control `Date` without affecting timer behavior, or vice versa.
...
| <span className="sr-only">Capability</span> | Playwright Component Testing                     | Cypress Component Testing                                                                                                                                                                                                            |
| ------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
...
A few Playwright features have no built-in Cypress counterpart. Identify these
up front so you can plan alternatives rather than discovering them mid-migration.

| Playwright feature                                          | Cypress equivalent                                                                                                                                   |
| ----------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `toHaveScreenshot()` / `toMatchSnapshot()` visual snapshots | No built-in visual diffing. See [Visual Testing](/app/tooling/visual-testing) for plugin options.                                                    |
| `expect.soft()` (soft assertions)                           | No equivalent. Cypress fails on the first failed assertion; split into separate assertions or tests.                                                 |
| `test.step()`                                               | No direct equivalent. Group related commands and annotate with [`cy.log()`](/api/commands/log) or nested `context()` blocks.                         |
| `toMatchAriaSnapshot()`                                     | No equivalent. Assert roles and labels individually, or use [Cypress Accessibility](/accessibility/get-started/introduction) for automated scanning. |
...
| `page.locator(selector).press('Tab')`                    | `cy.get(selector).focus(); cy.press(Cypress.Keyboard.Keys.TAB)` |
| `page.locator(selector).selectOption('blue')`            | `cy.get(selector).select('blue')`                               |
...
| `expect(page).toHaveURL(/re/)`                           | `cy.url().should('match', /re/)`                                |
...
| `expect(locator).toBeAttached()`                         | `cy.get(selector).should('exist')`                              |
...
| `page.locator(selector).first()`                         | `cy.get(selector).first()`                                      |
| `page.locator(selector).last()`                          | `cy.get(selector).last()`                                       |
| `page.locator(selector).nth(n)`                          | `cy.get(selector).eq(n)`                                        |
| `page.locator(selector).filter({ hasText: 'x' })`        | `cy.get(selector).filter(':contains("x")')`                     |
| `page.waitForURL('**/dashboard')`                        | `cy.url().should('include', '/dashboard')`                      |
