---
title: 'Upgrade Cypress: version migration guide'
description: 'A guide to help you upgrade to the latest version of Cypress.'
sidebar_label: Upgrade Cypress
sidebar_position: 20
slug: /app/references/migration-guide
---
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| [Native browser network in Chrome, Chromium, and Edge](#Chrome-Chromium-and-Edge-use-the-native-browser-network)                                                              | Usually none. Update tests that assert on `req.httpVersion`, compression headers, or a `304` status. |
| [Modern visibility algorithm on by default](#experimentalFastVisibility-replaced-by-visibilityStrategy)                                                                       | Usually none. Update tests that relied on legacy-only visibility semantics.                          |
| [Browser memory management on by default](#experimentalMemoryManagement-replaced-by-manageBrowserMemory)                                                                      | Only if you set `experimentalMemoryManagement: false`. Replace it with `manageBrowserMemory: false`. |
| [`keystrokeDelay` default changed from `10` to `0`](#Default-keystrokeDelay-changed-from-10-to-0)                                                                             | Only if a test depended on the implicit delay.                                                       |
| [Cookie and storage reads are now queries](#cygetCookie-cygetCookies-and-cygetAllCookies-are-now-queries)                                                                     | Usually none. Move assertions out of `.then()` and onto `.should()` to get retries.                  |
| [Node.js 20 and 25 dropped](#Nodejs-20-and-25-no-longer-supported)                                                                                                            | Yes, if you run Cypress on either version.                                                           |
| [`Cypress.env()` removed](#Cypressenv-removed)                                                                                                                                | Yes. Migrate to `cy.env()` or `Cypress.expose()`.                                                    |
| [`cy.end()` removed](#cyend-removed)                                                                                                                                          | Yes. Delete the `.end()` calls.                                                                      |
| [`cy.exec()` removed](#cyexec-removed)                                                                                                                                        | Yes. Replace the calls with `cy.task()`.                                                             |
| [`viewportWidth`, `viewportHeight`, and `blockHosts` via `Cypress.config()`](#viewportWidth-and-viewportHeight-can-no-longer-be-set-with-Cypressconfig-during-test-execution) | Yes, if you set them while a test is executing.                                                      |
| [`experimentalSourceRewriting` removed](#experimentalSourceRewriting-config-option-removed)                                                                                   | Yes. Remove the option, and enable `removeSRIAttributes` if you used it for SRI.                     |
| [CoffeeScript support removed](#CoffeeScript-support-removed)                                                                                                                 | Yes, if you have `.coffee` specs, support files, or fixtures.                                        |
| [Angular, Vite, and Next.js minimums raised for component testing](#Angular-18-19-and-20-are-no-longer-supported-for-Component-Testing)                                       | Yes, if you are below Angular 21, Vite 8, or Next.js 15.0.4.                                         |
| [Electron deprecated as a test browser](#Electron-browser-deprecated)                                                                                                         | Not yet, but plan to switch to an installed browser.                                                 |
...
If setting `forceHttp1` to `true` allows your suite to pass, and the difference is not one of the documented behaviors, [open an issue](https://github.com/cypress-io/cypress/issues/new/choose) with what you are experiencing so it can be investigated.

<ForceHttp1Deprecation />

### Default `keystrokeDelay` changed from `10` to `0`

The default delay between keystrokes in [`cy.type()`](/api/commands/type) has been changed from `10` to `0` milliseconds to improve performance of typing-heavy test runs.

If your tests relied on the implicit 10ms delay between keystrokes, you can restore the previous behavior using any of these approaches. The resolved `keystrokeDelay` is read from your Cypress [configuration](/app/references/configuration#Keyboard) first, then [`Cypress.Keyboard.defaults()`](/api/cypress-api/keyboard-api), and falls back to the built-in default of `0` if neither is set. A per-command `delay` passed to [`.type()`](/api/commands/type) is independent of this and overrides the resolved value for that single call.
...
The experimental `experimentalFastVisibility` boolean flag has been removed. Its successor is the new [`visibilityStrategy`](/app/references/configuration#Actionability) configuration option, which accepts `'legacy'` or `'modern'` and defaults to `'modern'`. The modern algorithm is now the default for **all** users, not just those who opted into the experiment.

**If you were setting `experimentalFastVisibility: true`:** remove it. The modern algorithm is now the default.

**If you were setting `experimentalFastVisibility: false` (or relying on the 15.x default) and a test now fails:** the modern algorithm intentionally drops some behaviors the legacy algorithm had (ancestor `overflow` clipping, transform-based hiding, and coverage detection for fixed/sticky elements). Prefer rewriting the affected assertions to verify the same user-visible behavior in an algorithm-agnostic way. See [Visibility Strategy](/app/core-concepts/interacting-with-elements#Visibility-Strategy) for what each algorithm checks and recommended rewrite patterns.
...
The experimental `experimentalMemoryManagement` configuration option has been replaced by the new [`manageBrowserMemory`](/app/references/configuration#manageBrowserMemory) configuration option, which defaults to **`true`**. Browser memory management is now on by default for **all** users running Chromium-based browsers.
...
See [manageBrowserMemory](/app/references/configuration#manageBrowserMemory) for more information.

### `cy.getCookie()`, `cy.getCookies()`, and `cy.getAllCookies()` are now queries

[`cy.getCookie()`](/api/commands/getcookie),
[`cy.getCookies()`](/api/commands/getcookies), and
[`cy.getAllCookies()`](/api/commands/getallcookies) are now retry-able
[query commands](/app/core-concepts/retry-ability), just like
[`cy.get()`](/api/commands/get) and [`cy.contains()`](/api/commands/contains).

Previously they were one-shot commands that resolved a single value and ran any
chained assertions only once. As of Cypress 16 they retry their built-in and
chained assertions until they pass or time out.
...
- **Query commands** (`cy.getCookie()`, `cy.getCookies()`, and
  `cy.getAllCookies()`) are now governed by
  [`defaultCommandTimeout`](/app/references/configuration#Timeouts) (default
  `4000` ms), the same timeout used by every other query.
- **Cookie action commands** ([`cy.setCookie()`](/api/commands/setcookie),
  [`cy.clearCookie()`](/api/commands/clearcookie),
  [`cy.clearCookies()`](/api/commands/clearcookies), and
  [`cy.clearAllCookies()`](/api/commands/clearallcookies)) are unchanged. They
  make a single automation round-trip, do not retry assertions, and remain
  governed by [`responseTimeout`](/app/references/configuration#Timeouts)
  (default `30000` ms).

The new retry behavior applies only to assertions chained directly onto the
command. A [`.then()`](/api/commands/then) callback still runs only once, after
...
[`.should()`](/api/commands/should) assertions directly instead:
...
[`.then()`](/api/commands/then) callback, which runs once and is not retried:
...
See [Overwriting Existing Queries](/api/cypress-api/custom-queries#Overwriting-Existing-Queries) for more details.

### `cy.getAllLocalStorage()` and `cy.getAllSessionStorage()` are now queries

[`cy.getAllLocalStorage()`](/api/commands/getalllocalstorage) and
[`cy.getAllSessionStorage()`](/api/commands/getallsessionstorage) are now
retry-able [query commands](/app/core-concepts/retry-ability), just like
...
Both commands are now [`Timeoutable`](/api/commands/getalllocalstorage#Arguments)
...
[`viewportWidth`](/app/references/configuration#Viewport) and [`viewportHeight`](/app/references/configuration#Viewport) can no longer be set with [`Cypress.config()`](/api/cypress-api/config) while a test is executing. Doing so applied to the _next_ test rather than the current one, and the change was not reflected in Test Replay, so Cypress now throws instead.

Use [`cy.viewport()`](/api/commands/viewport) to change the viewport during a test, or set the values in the [test configuration](/app/references/configuration#Test-specific-Configuration) of a `describe`, `context`, or `it` block.
...
[`blockHosts`](/app/references/configuration#blockHosts) can no longer be set with [`Cypress.config()`](/api/cypress-api/config) while a test is executing. Doing so applied to the _next_ test rather than the current one, and the value leaked into every test that followed, so Cypress now throws instead.

Set `blockHosts` in the [test configuration](/app/references/configuration#Test-specific-Configuration) of a `describe`, `context`, or `it` block instead, which takes effect for the suite or test where it is set and is restored afterwards.
...
Use [`cy.task()`](/api/commands/task) instead. A task runs in the Node.js
...
on('task', {
  resetDatabase() {
    execFileSync('docker', ['exec', 'db', 'reset-script'], { stdio: 'pipe' })
...
The [`cy.task()` examples](/api/commands/task#Examples) cover the patterns
`cy.exec()` was commonly used for, including
[cleaning up and verifying downloaded files](/api/commands/task#Clean-up-and-verify-downloaded-files),
[running an external command or CLI](/api/commands/task#Run-an-external-command-or-CLI),
and [seeding a database](/api/commands/task#Seed-a-database).
...
[15.21.0](/app/references/changelog#15-21-0) and removed in 16.0.

### `Cypress.env()` removed

`Cypress.env()` has been removed in Cypress 16.0. Use [`cy.env()`](/api/commands/env) for sensitive values (API keys, tokens, secrets) or [`Cypress.expose()`](/api/cypress-api/expose) for non-sensitive configuration that needs synchronous browser access.

See [Migrating away from Cypress.env()](#Migrating-away-from-Cypressenv) below for detailed before/after examples.
...
The `allowCypressEnv` configuration option has been removed. If your `cypress.config` still includes it, Cypress will print a warning telling you to remove it. There is no behavior change, so remove the key.
...
[`@cypress/webpack-batteries-included-preprocessor`](/api/node-events/preprocessors-api)
...
register it with
[`@cypress/webpack-preprocessor`](/api/node-events/preprocessors-api#Examples):
...
The command still reports the details it is usually run for: detected browsers,
application data, browser profile and binary cache paths, the Cypress version,
and system platform and memory.
...
Additionally, the [`@cypress/angular-zoneless`](https://www.npmjs.com/package/@cypress/angular-zoneless) npm package is deprecated and no longer supported. It has been merged into [`@cypress/angular`](https://www.npmjs.com/package/@cypress/angular).

#### To continue using Angular below 21.0.0

If you haven't been able to migrate away from an older Angular version and still need that test harness, it can be installed independently via the [`@cypress/angular`](https://www.npmjs.com/package/@cypress/angular) `4.x.x` package from `npm`.

This test harness version is deprecated and no longer supported by Cypress. This version is intended to serve as a temporary workaround to migrate your project to Angular v21.0.0+.
...
`@cypress/vite-dev-server@7` is deprecated and no longer supported by Cypress. It is intended as a temporary workaround until you can migrate to Vite `8`. More information on configuring a custom dev server can be found in the [Cypress Vite Dev Server documentation](https://github.com/cypress-io/cypress/blob/@cypress/vite-dev-server-v7.2.1/npm/vite-dev-server/README.md).
...
Upgrade Next.js in your project before upgrading Cypress. See the
[Next.js upgrade guide](https://nextjs.org/docs/app/guides/upgrading) for the
steps that apply to your application, and
[Next.js component testing](/app/component-testing/react/overview#Nextjs) for
the Cypress-side configuration.

### Electron browser deprecated

The bundled Electron browser is deprecated as a test browser in Cypress 16.0.0 and will be removed in a future major version. If your runs rely on the implicit Electron default, set a [`defaultBrowser`](/app/references/configuration#Browser) (for example, `'chrome'`) and/or pass [`--browser`](/app/references/command-line#cypress-run) to switch to an installed browser. See [Migrating away from the Electron browser](#Migrating-away-from-the-Electron-browser) for the full guide.
...
<CopyPrompt
  defaultCollapsed
  excludeFromLlmExport
  title="Migrate off Cypress.env() with your AI assistant"
  prompt={`Read https://docs.cypress.io/app/references/migration-guide/migrating-away-from-cypress-env.md, then migrate this project off Cypress.env() by working through these steps:\n\n1. Check my Cypress version. cy.env() and Cypress.expose() require Cypress 15.10.0 or newer, so check my installed version first: if it's on an older 15.x release, detect my package manager and update the cypress dependency to the latest 15.x; if it's below Cypress 15, stop and tell me, since majors should be upgraded one at a time first.\n\n2. Classify each value. Find every Cypress.env() usage and classify each value as sensitive or public.\n\n3. Migrate the values. Migrate sensitive reads to cy.env() and public values to Cypress.expose() with a top-level expose block in my Cypress config. Check for --env CLI flags and plugins that rely on Cypress.env(), and show me the proposed changes before applying them.\n\n4. Lock it down and verify. Once every Cypress.env() call is gone, add allowCypressEnv: false to my config and run my Cypress tests to confirm nothing broke.\n\nFinish with a summary of what changed and anything you couldn't safely automate.`}
/>

`Cypress.env()` was deprecated in Cypress 15.10.0 and removed in Cypress 16.0. This guide helps you migrate to [`cy.env()`](/api/commands/env) (for sensitive values) or [`Cypress.expose()`](/api/cypress-api/expose) (for public/non-sensitive values), depending on your use case.
...
1. **All-or-nothing exposure**: All environment variables were serialized and sent to the browser where they could be accessed via `Cypress.env()` or inspected in the browser's developer tools.
1. **Cross-origin risks**: When using [`cy.origin()`](/api/commands/origin) for cross-origin testing, all environment variables were automatically passed to the cross-origin context, potentially exposing sensitive data to untrusted origins.
1. **Application code access**: Since environment variables were available in the browser context when tests ran, application code or third-party scripts could potentially access them by checking for `window.Cypress` or similar patterns.
...
<Badge type="success">After: Cypress.expose()</Badge>
...
<Badge type="danger">Before: Cypress.env() CLI flags</Badge>

```shell
cypress run --env FEATURE_FLAG=true,API_VERSION=v2,PUBLIC_API_URL=https://api.example.com
```

<Badge type="success">After: Cypress.expose() CLI flags</Badge>
...
You can set exposed values at runtime using `Cypress.expose(key, value)` or `Cypress.expose(object)`. These changes only persist for the remainder of the current spec file:
...
| -------------------------------------------------------------------------------------------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------- |
| [@badeball/cypress-cucumber-preprocessor](https://github.com/badeball/cypress-cucumber-preprocessor)                 | 27.0.0+ | Use `-x` or `--expose` command line options for configuration overrides, not `-e` or `--env` as previously |
| [@bahmutov/cy-api](https://github.com/bahmutov/cy-api)                                                               | 2.3.0+  | Move options from `env` to `expose`                                                                        |
| [@chromatic-com/cypress](https://github.com/chromaui/chromatic-e2e)                                                  | 0.12.5+ | Move options from `env` to `expose`; requires Cypress 15.10+                                               |
| [@frsource/cypress-plugin-visual-regression-diff](https://github.com/FRSOURCE/cypress-plugin-visual-regression-diff) | 4.1.0+  | Move options from `env` to `expose`                                                                        |
| [@percy/cypress](https://github.com/percy/percy-cypress)                                                             | 3.1.8+  | Move `PERCY_*` values from `env` to `expose` (unexposed still warns)                                       |
| [@simonsmith/cypress-image-snapshot](https://github.com/simonsmith/cypress-image-snapshot)                           | 10.1.1+ | Move options from `env` to `expose` (unexposed still warns)                                                |
| [cypress-data-session](https://github.com/bahmutov/cypress-data-session)                                             | 3.0.0+  | Move options from `env` to `expose`; requires Cypress 15.10+                                               |
| [cypress-fail-fast](https://github.com/javierbrea/cypress-fail-fast)                                                 | 8.0.0+  | Move options from `env` to `expose`; renamed (`FAIL_FAST_STRATEGY` → `failFastStrategy`)                   |
| [cypress-firebase](https://github.com/prescottprue/cypress-firebase)                                                 | 4.4.0+  | Set `allowCypressEnv: false` to disable its `Cypress.env()` fallback                                       |
| [cypress-image-diff-js](https://github.com/uktrade/cypress-image-diff)                                               | 2.7.0+  | None                                                                                                       |
| [cypress-localstorage-commands](https://github.com/javierbrea/cypress-localstorage-commands)                         | 2.3.0+  | None                                                                                                       |
| [cypress-maildev](https://github.com/Clebiez/cypress-maildev)                                                        | 2.0.0+  | Move `MAILDEV_*` values from `env` to `expose`                                                             |
| [cypress-mailosaur](https://github.com/mailosaur/cypress-mailosaur)                                                  | 4.0.0+  | None                                                                                                       |
| [cypress-mailpit](https://github.com/pushpak1300/cypress-mailpit)                                                    | 2.0.0+  | Move `MAILPIT_URL` from `env` to `expose`; keep credentials in `env`                                       |
| [cypress-mongodb](https://github.com/Zaista/cypress-mongodb)                                                         | 7.0.0+  | None                                                                                                       |
| [cypress-network-idle](https://github.com/bahmutov/cypress-network-idle)                                             | 2.0.1+  | None; requires Cypress 15.10+                                                                              |
| [cypress-ntlm-auth](https://github.com/bjowes/cypress-ntlm-auth)                                                     | 4.2.4+  | None                                                                                                       |
| [cypress-plugin-api](https://github.com/filiphric/cypress-plugin-api)                                                | 2.12.0+ | Move options from `env` to `expose` (unexposed options still warn)                                         |
| [cypress-plugin-grep-boxes](https://github.com/dennisbergevin/cypress-plugin-grep-boxes)                             | 3.0.0+  | Move options from `env` to `expose`                                                                        |
| [cypress-plugin-last-failed](https://github.com/dennisbergevin/cypress-plugin-last-failed)                           | 3.0.0+  | Requires Cypress 15.10+; also update `@cypress/grep` to 6.0.0+                                             |
| [cypress-plugin-xhr-toggle](https://github.com/filiphric/cypress-plugin-xhr-toggle)                                  | 1.3.0+  | Move `hideXhr` from `env` to `expose` (unexposed still warns)                                              |
| [cypress-runner-themes](https://github.com/dingraham/cypress-runner-themes)                                          | 1.1.0+  | Move `theme` from `env` to `expose` (unexposed still warns)                                                |
| [cypress-visual-regression](https://github.com/cypress-visual-regression/cypress-visual-regression)                  | 6.0.0+  | Move options from `env` to `expose`                                                                        |
| [cypress-voice-plugin](https://github.com/dennisbergevin/cypress-voice-plugin)                                       | 2.0.0+  | Move options from `env` to `expose`                                                                        |
| [cypress-watch-and-reload](https://github.com/bahmutov/cypress-watch-and-reload)                                     | 2.1.0+  | Requires Cypress 15.11+                                                                                    |
...
2. `env` test configuration overrides are disabled
...
1. ✅ **Cypress ^15.10.0 only**: Set `allowCypressEnv: false` in your Cypress configuration and verify no errors are thrown
1. ✅ **Cypress 16.0+**: Remove `allowCypressEnv` from your configuration if present

## Migrating away from the Electron browser

Starting with Cypress 16.0.0, the bundled Electron browser is deprecated as a
test browser and will be removed in a future major version. This guide helps you
move your local and CI runs to an installed browser, such as Chrome, before the
removal.

### Why Electron is being removed

The Electron browser ships inside Cypress as a version of Chromium bundled with
[Electron](https://www.electronjs.org/). It was convenient because it required no
separate install, but it no longer gives you a representative testing
environment:

- **It lags Chromium.** Electron's Chromium version is tied to the Electron
  release Cypress bundles, so it trails the current stable Chrome by weeks or
  months and misses recent web-platform features and fixes.
- **Its behavior diverges from Chrome.** Because it is an embedded Chromium
  rather than the browser your users run, test behavior in Electron can differ
  from real Chrome, which undermines confidence in the results.

Standardizing on installed browsers means your tests run in the same engines
your users do, on a predictable, evergreen update cadence.
...
1. **Install a supported browser** in your local and CI environments (for
   example, [Google Chrome](/app/references/launching-browsers#Chrome-Browsers)).
   If you don't want to install one yourself, our
   [Continuous Integration](/app/continuous-integration/overview) guides cover
   browser options for CI, including the Cypress
   [Docker images](/app/continuous-integration/overview#Cypress-Docker-Images),
   the [GitHub Action](https://github.com/cypress-io/github-action), and the
   [CircleCI Orb](https://github.com/cypress-io/circleci-orb).
1. **Set a `defaultBrowser`** so Cypress no longer falls back to Electron when no
...
1. **Or pass `--browser` explicitly** wherever you launch Cypress:

   ```shell
   cypress run --browser chrome
   ```

1. **Update your CI configuration.** If your pipeline relied on the implicit
   Electron default, update any jobs that previously ran without a browser
   argument to target an installed browser. See
   [Continuous Integration](/app/continuous-integration/overview) for
   provider-specific setup details.
1. **Update your Docker image, if you use one.** If you use the `cypress/base`
   [Docker image](/app/continuous-integration/overview#Cypress-Docker-variants)
   and don't explicitly install a browser, you'll need to switch to
   `cypress/browsers` or install a browser yourself.
1. **Remove any explicit Electron usage**: `--browser electron`,
   `defaultBrowser: 'electron'`, or `{ browser: 'electron' }` test/suite
   overrides. Replace it with an installed browser.

### Migration checklist

1. ✅ Install a supported browser locally and in CI
1. ✅ Set `defaultBrowser` (or pass `--browser`) so runs don't fall back to Electron
1. ✅ Replace any `--browser electron` / `defaultBrowser: 'electron'` usage
1. ✅ Update your CI config (GitHub Action, Docker) to target an installed browser
1. ✅ Verify your runs no longer emit the Electron deprecation warning
...
Cypress requires [Node.js](https://nodejs.org/en) to install the Cypress binary and the supported versions are now Node.js 20, 22, 24, and above.
...
The deprecated 3 argument signature of [`cy.stub()`](/api/commands/stub), `cy.stub(object, name, fn)`, is no longer supported. Use `cy.stub(object, name).callsFake(fn)` instead.
...
<PackageManagerTabs install="@cypress/webpack-dev-server@4" dev />
...
<PackageManagerTabs install="@cypress/webpack-preprocessor@6" dev />
...
`@cypress/vite-dev-server` no longer supports Vite `4` for component testing. The minimum supported Vite version is now `5`. Review the [Vite migration guide](https://vite.dev/guide/migration) for any changes needed in your own Vite configuration when upgrading.
...
This test harness version is deprecated and no longer supported by Cypress. This version is intended to serve as a temporary workaround to migrate your project to Angular v18.0.0+.

<PackageManagerTabs install="@cypress/angular@3" dev />
...
`@cypress/angular` now requires a minimum [zone.js](https://www.npmjs.com/package/zone.js) version of `0.14.0`. If your project uses Angular component testing and resolves a `zone.js` version below `0.14.0`, update it:
...
[`Cypress.ElementSelector`](/api/cypress-api/element-selector-api). Additionally, the `getSelector` method has been removed, and the `onElement` function has been removed as an option to the `defaults` method.

This change was made to reflect its use in features beyond just the Selector Playground - like Cypress Studio.
...
<CopyPrompt
  defaultCollapsed
  excludeFromLlmExport
  title="Upgrade to Cypress 14 with your AI assistant"
  prompt={`Read https://docs.cypress.io/app/references/migration-guide/migrating-to-cypress-14-0.md, then upgrade this project from Cypress 13 to 14 by working through these steps:\n\n1. Confirm the starting point. If the project isn't on Cypress 13.x yet, stop and tell me; majors should be upgraded one at a time.\n\n2. Check requirements. Verify my environment meets the requirements in that section (Node.js 18+, macOS 11+, a supported Linux distribution, and, if I use component testing, that my framework and bundler versions are still supported) and flag anything unsupported.\n\n3. Update Cypress. Detect my package manager and update the cypress dependency.\n\n4. Fix breaking changes. Fix every breaking change in that section that affects my code and config.\n\n5. Verify. Run npx cypress verify and my Cypress tests.\n\nFinish with a summary of what changed and anything you couldn't safely automate.`}
/>
...
may cause certain sites to stop working in Cypress. Read the [configuration
notes](/app/references/configuration#injectDocumentDomain) before use.
...
Our intention is to replace essential functionality dependent on the `resourceType` within Cypress in a future version (like [hiding network logs that are not fetch/xhr](/api/commands/intercept#Disabling-logs-for-a-request)). If you're using `resourceType` in your tests, leave feedback on which `resourceType` values are important to you in this [GitHub issue](https://github.com/cypress-io/cypress/issues/30447).

### `experimentalFetchPolyfill` removed

The `experimentalFetchPolyfill` configuration option, deprecated since Cypress [6.0.0](/app/references/changelog#6-0-0), has been removed. Remove it from your configuration and use [`cy.intercept()`](/api/commands/intercept) to handle `fetch` requests.

### `before:browser:launch` browser arguments

The second argument of the [`before:browser:launch`](/api/node-events/browser-launch-api) node event is no longer yielded as an array of browser arguments. This behavior had been deprecated since Cypress [4.0.0](/app/references/changelog#4-0-0). Update any handler that treats the second argument as an array to use the `launchOptions` object (for example, `launchOptions.args`) instead.

### `open-ct` and `run-ct` commands removed

The `cypress open-ct` and `cypress run-ct` CLI commands, deprecated since Cypress [10.0](#Migrating-to-Cypress-100), have been removed. Update any npm scripts or CI jobs to use [`cypress open --component`](/app/references/command-line#cypress-open) or [`cypress run --component`](/app/references/command-line#cypress-run) respectively.
...
It is no longer possible to make a `fetch` or `XMLHttpRequest` request from the `about:blank` page in Electron (for example, `cy.window().then((win) => win.fetch('<some-url>'))` before any navigation). Use [`cy.request()`](/api/commands/request) instead, or perform an initial navigation with [`cy.visit()`](/api/commands/visit) first.
...
This test harness version is deprecated and no longer supported by Cypress. This version is intended to serve as a temporary workaround to migrate your project to React v18+.

<PackageManagerTabs install="@cypress/react@8" dev />
...
With [LTS ending](https://angular.dev/reference/releases#actively-supported-versions) for Angular 16, the minimum required Angular version for component testing is now `17.2.0` to support [signals](/app/component-testing/angular/examples#Signals) as a first class citizen.
...
To migrate, change the test harness from `cypress/angular-signals` to `cypress/angular`.
...
This test harness version is deprecated and no longer supported by Cypress. This version is intended to serve as a temporary workaround to migrate your project to Angular v17.2.0+.

<PackageManagerTabs install="@cypress/angular@2" dev />
...
<PackageManagerTabs install="@cypress/vue2" dev />
...
If you haven't been able to migrate away from Nuxt 2, the underlying Vue 2 harness can still be installed independently via the [@cypress/vue2](https://www.npmjs.com/package/@cypress/vue2) package, as described in [To continue using Vue 2](#Vue-2-CT-no-longer-supported). This package is deprecated and no longer supported by Cypress, and is intended only as a temporary workaround while you migrate to Nuxt 3+.
...
- Use [vite](https://vite.dev/guide/) to bundle your component tests (quick setup with [create-vite](https://github.com/vitejs/vite/tree/main/packages/create-vite)).
...
- Use [vite](https://vite.dev/guide/). The Vue team recommends migrating to using `create-vue` to scaffold a [Vite](https://vite.dev/)-based project.
...
<CopyPrompt
  defaultCollapsed
  excludeFromLlmExport
  title="Upgrade to Cypress 13 with your AI assistant"
  prompt={`Read https://docs.cypress.io/app/references/migration-guide/migrating-to-cypress-13-0.md, then upgrade this project from Cypress 12 to 13 by working through these steps:\n\n1. Confirm the starting point. If the project isn't on Cypress 12.x yet, stop and tell me; majors should be upgraded one at a time.\n\n2. Check requirements. Verify my Node.js version is 16+ (18+ recommended) and, if I use TypeScript, that it's 4.x or newer, and flag anything unsupported.\n\n3. Update Cypress. Detect my package manager and update the cypress dependency.\n\n4. Fix breaking changes. Fix every breaking change in that section that affects my code and config, calling out behavior changes like video recording being off by default.\n\n5. Verify. Run npx cypress verify and my Cypress tests.\n\nFinish with a summary of what changed and anything you couldn't safely automate.`}
/>
...
Node.js 14 support has been removed and Node.js 16 support has been deprecated. Node.js 16 may continue to work with Cypress `v13`, but is not supported moving forward, to closer coincide with [Node.js 16's end-of-life](https://nodejs.org/en/blog/announcements/nodejs16-eol) schedule. It is recommended that users update to at least Node.js 18.
...
The deprecated `nodeVersion` configuration option has been removed. Remove it from your configuration; Cypress now always uses the Node.js version it was launched with to execute your [setupNodeEvents](/app/references/configuration#setupNodeEvents).
...
The properties and values returned by the [Module API](/app/references/module-api) and included in the arguments of handlers for the [`after:run`](/api/node-events/after-run-api) and [`after:spec`](/api/node-events/after-spec-api) events have been changed to be more consistent. If you use the Module API (the return value of `cypress.run()`) or read the result arguments in `after:run` or `after:spec` handlers within [setupNodeEvents](/app/references/configuration#setupNodeEvents), review that code against the linked API documentation and update any property access to the new result shape.
...
<CopyPrompt
  defaultCollapsed
  excludeFromLlmExport
  title="Upgrade to Cypress 12 with your AI assistant"
  prompt={`Read https://docs.cypress.io/app/references/migration-guide/migrating-to-cypress-12-0.md, then upgrade this project from Cypress 11 to 12 by working through these steps:\n\n1. Confirm the starting point. If the project isn't on Cypress 11.x yet, stop and tell me; majors should be upgraded one at a time.\n\n2. Check requirements. Verify my Node.js version is 14, 16, or 18+ and flag anything unsupported.\n\n3. Update Cypress. Detect my package manager and update the cypress dependency.\n\n4. Fix breaking changes. Fix every breaking change in that section that affects my code and config, paying special attention to test isolation being enabled by default: audit my tests for state shared between tests and fix them or configure testIsolation as that section describes.\n\n5. Verify. Run npx cypress verify and my Cypress tests.\n\nFinish with a summary of what changed and anything you couldn't safely automate.`}
/>
...
[`cy.origin()`](/api/commands/origin) is not enabled by default. To use
...
Node.js 12, 15, and 17 have reached their end of life and are no longer
supported when installing Cypress.
[See Node's release schedule](https://github.com/nodejs/Release). Installing
Cypress now requires Node.js 14, 16, or 18+.
...
isolation means Cypress resets the browser context _before_ each test by:
...
local storage, and session storage between tests to re-establish the
...
While disabling test isolation may improve the
...
clean up. Explicitly setting test isolation to enabled or disabled puts that
behavior under your control.
...
[`cy.clearCookies()`](/api/commands/clearcookies). If your tests depend on
...
<CopyPrompt
  defaultCollapsed
  excludeFromLlmExport
  title="Upgrade to Cypress 11 with your AI assistant"
  prompt={`Read https://docs.cypress.io/app/references/migration-guide/migrating-to-cypress-11-0.md, then upgrade this project from Cypress 10 to 11 by working through these steps:\n\n1. Confirm the starting point. If the project isn't on Cypress 10.x yet, stop and tell me; majors should be upgraded one at a time.\n\n2. Update Cypress. Detect my package manager and update the cypress dependency.\n\n3. Fix breaking changes. Fix every breaking change in that section that affects my code and config. If I use component testing, update my devServer configuration and any removed mounting APIs (mountHook, unmount, mountCallback) as that section describes.\n\n4. Verify. Run npx cypress verify and my Cypress tests.\n\nFinish with a summary of what changed and anything you couldn't safely automate.`}
/>
...
Read more about the rationale in
[Upcoming Changes to Component Testing](https://www.cypress.io/blog/2022/11/04/upcoming-changes-to-component-testing/?utm_source=docs.cypress.io&utm_medium=migration-guide).
...
`mountHook` from `cypress/react` has been removed. Read more about the
rationale in
[Upcoming Changes to Component Testing](https://www.cypress.io/blog/2022/11/04/upcoming-changes-to-component-testing/?utm_source=docs.cypress.io&utm_medium=migration-guide).

We recommend replacing it with `mount` and a component.
...
See a concrete example in
[Upcoming Changes to Component Testing](https://www.cypress.io/blog/2022/11/04/upcoming-changes-to-component-testing/?utm_source=docs.cypress.io&utm_medium=migration-guide#angularproviders-priority).
...
[`cy.session()`](/api/commands/session) now requires a `setup` function to be
...
<CopyPrompt
  defaultCollapsed
  excludeFromLlmExport
  title="Upgrade to Cypress 10 with your AI assistant"
  prompt={`Read https://docs.cypress.io/app/references/migration-guide/migrating-to-cypress-10-0.md, then upgrade this project from Cypress 9 to 10 by working through these steps:\n\n1. Confirm the starting point. If the project isn't on Cypress 9.x yet, stop and tell me; majors should be upgraded one at a time.\n\n2. Update Cypress. Detect my package manager and update the cypress dependency.\n\n3. Apply the configuration migration. Apply the configuration migration that section describes: convert cypress.json to a cypress.config file, move the plugins file into setupNodeEvents, rename support files and spec file locations to the new defaults, and update any removed config options or APIs my code uses.\n\n4. Verify. Run npx cypress verify and my Cypress tests.\n\nFinish with a summary of what changed and anything you couldn't safely automate.`}
/>
...
The default values are unchanged (`10000` for `e2e` and `250` for
`component`).
