...
| [Third-party plugins may need an update](#Third-party-plugins-may-need-an-update)                                                                                             | Yes, if a plugin you use lacks a Cypress 16 release. `cypress-real-events` needs `1.15.1`+.          |
...
### Third-party plugins may need an update

Plugins that declare a `cypress` peer dependency, or that reach into the Cypress
app's internals, often need a new release before they work on Cypress 16. Update
your plugins alongside Cypress, and check each one's release notes for a version
that lists Cypress 16 support.

[`cypress-real-events`](https://github.com/dmtrKovalenko/cypress-real-events)
requires **`1.15.1` or newer**. Versions up to `1.15.0` cap their `cypress` peer
dependency at `^15.x`, so installing them next to Cypress 16 fails peer
dependency resolution. `1.15.1` widens that range to include `^16.x`, and also
looks up the application under test by its `aut-iframe` selector rather than
taking the first `<iframe>` it finds, which keeps `realClick`, `realType`,
`realHover`, and the other `real*` commands pointed at the right frame.

```shell
npm install --save-dev cypress-real-events@^1.15.1
```

If you use plugins that call `Cypress.env()`, see
[Migrate plugins that use `Cypress.env()`](#Migrate-plugins-that-use-Cypressenv)
for the versions that adopted [`cy.env()`](/api/commands/env) and
