...
- Fixed an issue where adding a `--disable-features` argument in [`before:browser:launch`](/api/node-events/browser-launch-api) silently dropped every feature Cypress disables in Chrome, Chromium, and Edge, because the browser honors only the last occurrence of that argument. Cypress now merges its own values with yours. Fixes [#34775](https://github.com/cypress-io/cypress/issues/34775).
...
- Fixed a regression in [12.0.0](#12-0-0) where an assertion on a [`cy.contains()`](/api/commands/contains) command that matched no element did not say what was searched for, reporting `expected undefined not to exist in the DOM` in the Command Log. The searched content is now shown, such as `expected Saving not to exist in the DOM`. Fixes [#25962](https://github.com/cypress-io/cypress/issues/25962).
- Fixed a regression in [16.0.0](#16-0-0) where [`blockHosts`](/app/references/configuration#blockHosts) was not enforced in Chrome, Chromium, and Edge. Requests to a blocked host reached the network instead of failing with a `503` status. Scripts and other resources from those hosts still loaded. Fixes [#34785](https://github.com/cypress-io/cypress/issues/34785).
- Fixed a regression in [16.0.0](#16-0-0) where the headless Electron browser window was sized smaller than requested on Windows during `cypress run`, so failure screenshots and recorded videos were smaller than the expected 1280x720. Setting `preferences.width` and `preferences.height` in [`before:browser:launch`](/api/node-events/browser-launch-api) had no effect on the result. Fixes [#34771](https://github.com/cypress-io/cypress/issues/34771).

**Misc:**

- Fixed an issue where setting [`keystrokeDelay`](/app/references/configuration#Keyboard) in a TypeScript configuration file failed to compile with `'keystrokeDelay' does not exist in type 'ConfigOptions'`, even though Cypress read and validated the option at runtime. Fixes [#34796](https://github.com/cypress-io/cypress/issues/34796). Addressed in [#34798](https://github.com/cypress-io/cypress/pull/34798).
...
_Faster tests, less flake, and a few breaking changes that may require updates._

Most of this release's speed arrives without you editing a single test: requests run over HTTP/2 by default in Chrome, Chromium, and Edge browsers, `cy.type()` drops its 10ms keystroke delay, visibility checks use a faster algorithm, cookie and storage commands now retry the way queries do, and browser memory management is on by default so long runs stop crashing partway through. Read the full [Cypress 16 announcement](https://on.cypress.io/cypress-16-blog?utm_source=docs.cypress.io&utm_medium=app-changelog) for what changed and why.

**Breaking Changes:**

:::info

Refer to the [16 Migration Guide](/app/references/migration-guide#Migrating-to-Cypress-160) for help migrating your code. The upgrade guide also includes a ready-made prompt for your AI assistant to assist you with the upgrade.

:::

- Chrome, Chromium, and Edge intercept test traffic on the native browser network rather than routing it through Cypress. A small number of `cy.intercept()` behaviors differ as a result. Most notably, `req.httpVersion` is no longer reported, and compression headers are no longer present on an intercepted response. Firefox, WebKit, and Electron continue to use the legacy network path, and [`forceHttp1`](/app/references/configuration#forceHttp1) routes every browser back through it while you migrate. See [Native network interception](/app/guides/native-network-interception) and the [Migration Guide](/app/references/migration-guide#Chrome-Chromium-and-Edge-use-the-native-browser-network). Addresses [#3708](https://github.com/cypress-io/cypress/issues/3708).
- Removed support for Node.js 20 and Node.js 25. Cypress now requires Node.js 22.x, 24.x, or >=26.x. Addresses [#33705](https://github.com/cypress-io/cypress/issues/33705) and [#33778](https://github.com/cypress-io/cypress/issues/33778).
- Linux arm64 now requires Ubuntu >=24.04, or another distribution providing `GLIBCXX_3.4.32` or later. On Ubuntu 22.04 arm64 — including the `ubuntu-22.04-arm` GitHub Actions runner — Cypress exits at startup with ``version `GLIBCXX_3.4.32' not found``. Linux x64 is unaffected and continues to support Ubuntu >=22.04. See [Operating System](/app/get-started/install-cypress#Operating-System). Addresses [#34792](https://github.com/cypress-io/cypress/issues/34792).
- Upgraded the bundled Node.js version from `22.19.0` to `24.15.0`. Addressed in [#33494](https://github.com/cypress-io/cypress/pull/33494).
- Removed `Cypress.env()`. Use [`Cypress.expose()`](/api/cypress-api/expose) for non-sensitive values that need to be readable in the browser, and [`cy.env()`](/api/commands/env) for sensitive values that must remain in the Node process. The `env` key is no longer supported in per-test or suite config overrides; use `expose: { KEY: value }` instead if absolutely necessary. The `allowCypressEnv` configuration option has also been removed. See the [Migration Guide](/app/references/migration-guide#Migrating-away-from-Cypressenv). Addresses [#33889](https://github.com/cypress-io/cypress/issues/33889). Addressed in [#33963](https://github.com/cypress-io/cypress/pull/33963).
...
- Removed the `cy.end()` command, which ended a chain of commands by yielding `null`. A Cypress chain is already terminated when the next `cy.<command>()` starts a new chain, so existing `.end()` calls can simply be removed. See the [Migration Guide](/app/references/migration-guide#cyend-removed). Addressed in [#33696](https://github.com/cypress-io/cypress/pull/33696).
- The `experimentalMemoryManagement` configuration option has graduated out of experimental status and been replaced by [`manageBrowserMemory`](/app/references/configuration#manageBrowserMemory), which defaults to `true`. Memory management is now enabled by default in Chromium-based browsers. Setting `experimentalMemoryManagement` prints a warning and is otherwise ignored. If you previously set `experimentalMemoryManagement: false` to opt out, you must now set `manageBrowserMemory: false`. See the [Migration Guide](/app/references/migration-guide#experimentalMemoryManagement-replaced-by-manageBrowserMemory). Addresses [#34385](https://github.com/cypress-io/cypress/issues/34385).
- Removed the `experimentalSourceRewriting` configuration option. The default regex-based source rewriting handles every case the AST-based rewriter did, so default behavior is unchanged and the option can be deleted from your config. If you were using it to work around Subresource Integrity errors, enable [`removeSRIAttributes`](/app/references/configuration#removeSRIAttributes) instead. See the [Migration Guide](/app/references/migration-guide#experimentalSourceRewriting-config-option-removed). Addresses [#34213](https://github.com/cypress-io/cypress/issues/34213).
...
- The experimental `experimentalFastVisibility` flag has been replaced by a new [`visibilityStrategy`](/app/references/configuration#Actionability) configuration option that accepts `'legacy'` or `'modern'` and defaults to `'modern'`. The modern visibility algorithm (based on the browser's native `Element.checkVisibility()` API) is now the default for all users. `visibilityStrategy` is deprecated, so set `visibilityStrategy: 'legacy'` only as a temporary migration path. See the [Visibility Strategy](/app/core-concepts/interacting-with-elements#Visibility-Strategy) guide for migration help. Addressed in [#33794](https://github.com/cypress-io/cypress/pull/33794).
- [`cy.getCookie()`](/api/commands/getcookie), [`cy.getCookies()`](/api/commands/getcookies), and [`cy.getAllCookies()`](/api/commands/getallcookies) are now retry-able [query commands](/app/core-concepts/retry-ability). They re-read cookies and retry any chained assertions until they pass or time out, are governed by [`defaultCommandTimeout`](/app/references/configuration#Timeouts), and can no longer be overwritten with [`Cypress.Commands.overwrite()`](/api/cypress-api/custom-commands#Overwrite-Existing-Commands). Use [`Cypress.Commands.overwriteQuery()`](/api/cypress-api/custom-queries#Overwriting-Existing-Queries) instead. See the [Migration Guide](/app/references/migration-guide#cygetCookie-cygetCookies-and-cygetAllCookies-are-now-queries). Addresses [#4802](https://github.com/cypress-io/cypress/issues/4802).
- [`viewportWidth`](/app/references/configuration#Viewport) and [`viewportHeight`](/app/references/configuration#Viewport) can no longer be set via [`Cypress.config()`](/api/cypress-api/config) during test execution. Setting them this way affected the _next_ test rather than the current one and was not reflected in Test Replay. Use [`cy.viewport()`](/api/commands/viewport) or set them in the test configuration of a `describe`, `context`, or `it` block instead. See the [Migration Guide](/app/references/migration-guide#viewportWidth-and-viewportHeight-can-no-longer-be-set-with-Cypressconfig-during-test-execution). Addresses [#31592](https://github.com/cypress-io/cypress/issues/31592). Addressed in [#34262](https://github.com/cypress-io/cypress/pull/34262).
- [`blockHosts`](/app/references/configuration#blockHosts) can no longer be set via [`Cypress.config()`](/api/cypress-api/config) during test execution. Setting it this way affected the _next_ test rather than the current one. Set `blockHosts` in the test configuration of a `describe`, `context`, or `it` block instead. See the [Migration Guide](/app/references/migration-guide#blockHosts-can-no-longer-be-set-with-Cypressconfig-during-test-execution). Addressed in [#34553](https://github.com/cypress-io/cypress/pull/34553).
- [`cy.getAllLocalStorage()`](/api/commands/getalllocalstorage) and [`cy.getAllSessionStorage()`](/api/commands/getallsessionstorage) are now retry-able [query commands](/app/core-concepts/retry-ability). They re-read storage from all origins and retry any chained assertions until they pass or time out, accept a `timeout` option governed by [`defaultCommandTimeout`](/app/references/configuration#Timeouts), and can no longer be overwritten with [`Cypress.Commands.overwrite()`](/api/cypress-api/custom-commands#Overwrite-Existing-Commands). Use [`Cypress.Commands.overwriteQuery()`](/api/cypress-api/custom-queries#Overwriting-Existing-Queries) instead. See the [Migration Guide](/app/references/migration-guide#cygetAllLocalStorage-and-cygetAllSessionStorage-are-now-queries). Addresses [#26422](https://github.com/cypress-io/cypress/issues/26422). Addressed in [#34318](https://github.com/cypress-io/cypress/pull/34318).
...
- Removed built-in CoffeeScript support. The default `@cypress/webpack-batteries-included-preprocessor` no longer bundles `coffee-loader` or `coffeescript`, and `.coffee` is no longer resolved for specs, support files, or [`cy.fixture()`](/api/commands/fixture). Use JavaScript or TypeScript, or add CoffeeScript to your own webpack config via [`@cypress/webpack-preprocessor`](https://github.com/cypress-io/cypress/tree/develop/npm/webpack-preprocessor). See the [Migration Guide](/app/references/migration-guide#CoffeeScript-support-removed). Addressed in [#33654](https://github.com/cypress-io/cypress/pull/33654).
- **Component Testing breaking changes:**
  ...
  - **Next.js**
    - Removed support for Next.js 14. Next.js 15.0.4+ or 16+ is now required. Addressed in [#33641](https://github.com/cypress-io/cypress/pull/33641).

**Deprecations:**

- Deprecated the Electron browser as a test browser. The Electron browser will be removed in a future major version of Cypress; switch to Chrome or another installed browser to avoid a breaking change when you upgrade. See [Migrating away from the Electron browser](/app/references/migration-guide#Migrating-away-from-the-Electron-browser). Addresses [#33560](https://github.com/cypress-io/cypress/issues/33560). Addressed in [#34074](https://github.com/cypress-io/cypress/pull/34074).
- Added the [`forceHttp1`](/app/references/configuration#forceHttp1) configuration option, which routes every browser through the legacy network path. It is deprecated at introduction and prints a warning when set: it exists to give suites time to migrate to the native browser network, and will be removed once that path is supported long term in both Chrome and Firefox. Addresses [#33847](https://github.com/cypress-io/cypress/issues/33847).

**Features:**

- Cypress now supports HTTP/2/3 in Chrome, Chromium, and Edge. Your application is tested over the same protocol it uses in production, with multiplexed requests and no six-connection ceiling, so modern apps load faster under test and behave the way your users actually experience them. Cypress also stops terminating TLS for the application under test, so the browser validates your origin's real certificate instead of one Cypress generates. See [Native network interception](/app/guides/native-network-interception) and the [Migration Guide](/app/references/migration-guide#Chrome-Chromium-and-Edge-use-the-native-browser-network). Addresses [#3708](https://github.com/cypress-io/cypress/issues/3708).
...
- Fixed an issue where [`experimentalMemoryManagement`](/app/references/experiments) could fail to prevent the browser from running out of memory and crashing when Cypress was running inside a memory-limited container. Memory is now managed correctly in these environments. Fixes [#34104](https://github.com/cypress-io/cypress/issues/34104). Addressed in [#34123](https://github.com/cypress-io/cypress/pull/34123).

**Misc:**

- Corrected the `@default` value in the TypeScript type definitions for the [`videoCompression`](/app/references/configuration#Videos) configuration option from `32` to `false`. Addressed in [#34198](https://github.com/cypress-io/cypress/pull/34198).
...
- Fixed an issue where, during `cypress run` with [`experimentalSingleTabRunMode`](/app/references/experiments) and the experimental WebKit browser, component test runs with video recording enabled would only record the first spec's video and could then hang without exiting. Each spec is now recorded to its own video and the run exits normally. Fixes [#23815](https://github.com/cypress-io/cypress/issues/23815).
- Fixed an issue where running component tests with the [`experimentalSingleTabRunMode`](/app/references/experiments) experiment enabled could cause a spec to fail intermittently after many other specs had run in the same browser tab, even though that spec passed when run on its own. Fixes [#24146](https://github.com/cypress-io/cypress/issues/24146).
- Fixed an issue where passing a [`cy.fixture()`](/api/commands/fixture) alias to [`cy.selectFile()`](/api/commands/selectfile) attached the file without its name in a second or later test that loaded the same fixture (for example, when the same fixture is reused across multiple tests), causing servers to reject the upload as a missing file. The file name is now preserved every time the fixture is used. Fixes [#21936](https://github.com/cypress-io/cypress/issues/21936).
...
- Fixed an issue where the [`clientCertificates`](/app/references/client-certificates) config option failed to load ECDSA (EC) PEM or PKCS#12 client certificates. Fixes [#33767](https://github.com/cypress-io/cypress/issues/33767). Fixed in [#33799](https://github.com/cypress-io/cypress/pull/33799).
...
- Fixed an intermittent `ENOENT: no such file or directory, open <path>/bundle.tar-<rand>` error during [`cy.prompt`](/api/commands/prompt) and Studio bundle initialization. Fixed in [#33748](https://github.com/cypress-io/cypress/pull/33748).
...
- Fixed an issue where [`cy.wait`](/api/commands/wait) on multiple aliases could surface an unhandled `Cannot read properties of undefined (reading 'routeId')` rejection when a retry short-circuited during runnable teardown. Fixed in [#33651](https://github.com/cypress-io/cypress/pull/33651).
...
`Cypress.env()` is now deprecated and will be removed in a future major release of Cypress. To understand why, and how to migrate, read our [Migration Guide](https://on.cypress.io/cypress-env-migration). Addressed in [#33181](https://github.com/cypress-io/cypress/pull/33181).

**Features:**

- Introduced a new [`cy.env()`](/api/commands/env) command that can be used to asynchronously and securely access Cypress environment variables. Addressed in [#33181](https://github.com/cypress-io/cypress/pull/33181).
- Added a [`allowCypressEnv`](/app/references/configuration#Global) configuration option that disallows use of the deprecated `Cypress.env()` API. Addressed in [#33181](https://github.com/cypress-io/cypress/pull/33181).
...
- Introduced a new `experimentalFastVisibility` experiment. Enabling this experiment changes how Cypress performs visibility checks and assertions. Read more about the [visibility strategy](/app/core-concepts/interacting-with-elements#Visibility-Strategy) that replaced this experiment in 16.0.0. Addresses [#33044](https://github.com/cypress-io/cypress/issues/33044). Addressed in [#32801](https://github.com/cypress-io/cypress/pull/32801).
...
- Added Escape key support to [`cy.press()`](/api/commands/press). Addresses [#32429](https://github.com/cypress-io/cypress/issues/32429). Addressed in [#32545](https://github.com/cypress-io/cypress/pull/32545).
...
- `@cypress/webpack-preprocessor` no longer supports `webpack` version 4. Addresses [#31344](https://github.com/cypress-io/cypress/issues/31344). If you still need to use `webpack` version 4, see our [migration guide](/app/references/migration-guide#Migrating-to-Cypress-150).
- To better align with best practices, `@cypress/webpack-batteries-included-preprocessor` no longer includes certain browser built-ins that were automatically provided by Webpack 4. The removed built-ins are `assert`, `constants`, `crypto`, `domain`, `events`, `http`, `https`, `punycode`, `querystring`, `string_decoder`, `sys`, `timers`, `tty`, `url`, `util`, `vm`, and `zlib`. However, we know that certain built-ins are popular, given that many users have files that are shared between their Cypress tests and node context. Because of this, `@cypress/webpack-batteries-included-preprocessor` will ship with built-in support for `buffer`, `path`, `process`, `os`, and `stream`. If there is a built-in that isn't supported by default and you need to add support, refer to the Webpack [resolve.fallback](https://webpack.js.org/configuration/resolve/#resolvefallback) documentation and the [`@cypress/webpack-batteries-included-preprocessor` README](https://github.com/cypress-io/cypress/blob/develop/npm/webpack-batteries-included-preprocessor/README.md). Addresses [#31039](https://github.com/cypress-io/cypress/issues/31039).
...
- Updated `execa` from `1.0.0` to `4.1.0`. This changes the `code` property returned by `cy.exec()` to `exitCode`. Addressed in [#32238](https://github.com/cypress-io/cypress/pull/32238).
- **Component Testing breaking changes:**
  ...
  - `@cypress/webpack-dev-server` no longer supports `webpack-dev-server` version 4. Addresses [#31605](https://github.com/cypress-io/cypress/issues/31605). If you still need to use `webpack-dev-server` version 4, see our [migration guide](/app/references/migration-guide#Migrating-to-Cypress-150).
...
- [`Cypress.stop()`](/api/cypress-api/stop) is now available to stop the Cypress App on the current machine while tests are running. This can be useful for stopping test execution upon failures or other predefined conditions. Addresses [#518](https://github.com/cypress-io/cypress/issues/518). Addressed in [#31225](https://github.com/cypress-io/cypress/pull/31225).
...
tests still run as expected. As always, [open a bug report](https://github.com/cypress-io/cypress/issues/new?assignees=&labels=&projects=&template=1-bug-report.yml) for any issues you find.
...
- Cypress Component Testing no longer supports:
  ...
  - `Angular` versions 13, 14, 15, and 16. The minimum supported version is now `17.2.0` to fully support Angular [signals](https://angular.dev/guide/signals). Addresses [#29582](https://github.com/cypress-io/cypress/issues/29582). Addressed in [#30539](https://github.com/cypress-io/cypress/pull/30539).
...
- The `delayMs` option of `cy.intercept()` has been removed. This option was deprecated in Cypress [6.4.0](#6-4-0). Use the `delay` option instead. Addressed in [#30463](https://github.com/cypress-io/cypress/pull/30463).
...
- The `cypress open-ct` and `cypress run-ct` CLI commands were removed. Use `cypress open --component` or `cypress run --component` respectively instead. Addressed in [#30456](https://github.com/cypress-io/cypress/pull/30456).
...
- `injectDocumentDomain`, a new configuration option, can be set to `true` to re-enable the injection of `document.domain` setters in Cypress. Addressed in [#30770](https://github.com/cypress-io/cypress/pull/30770).
...
- Added shadow DOM snapshot support within Test Replay to highlight elements correctly within the Cypress reporter. Addressed in [#28823](https://github.com/cypress-io/cypress/pull/28823).
...
- Correctly sync `Cypress.currentRetry` with secondary origin so test retries that use [`cy.origin()`](/api/commands/origin) render logs as expected. Fixes [#28574](https://github.com/cypress-io/cypress/issues/28574).
...
- Fixed an issue to account for cancelled and failed downloads to correctly reflect these status in Command log as a download failure where previously it would be pending. Fixed in [#28222](https://github.com/cypress-io/cypress/pull/28222).
...
Read more about v13.0.0 in [our blog post](https://on.cypress.io/cypress-13-release?utm_source=docs.cypress.io&utm_medium=app-changelog).
...
- Resolve the `process/browser` global inside `@cypress/webpack-batteries-included-preprocessor` to resolve to `process/browser.js` to explicitly provide the file extension. File resolution must include the extension for `.mjs` and `.js` files inside ESM packages to resolve correctly. Fixes [#27599](https://github.com/cypress-io/cypress/issues/27599).
...
- It is now possible to control the number of connection attempts to the browser
  using the `CYPRESS_CONNECT_RETRY_THRESHOLD` Environment Variable. Learn
  more about
  [Cypress environment variables](/app/references/advanced-installation#Environment-variables).
...
- Added a Public API for third party component libraries to define a Framework
  ...
  Learn more about
  [custom component testing frameworks](/app/component-testing/custom-frameworks).
...
- Easily debug failed CI test runs recorded to the Cypress Cloud from your local
  Cypress app with the new Debug page. Leave any feedback in the
  [Debug page discussion](https://github.com/cypress-io/cypress/discussions/25649).
  Your feedback will help us make decisions to improve the Debug experience.
  For more details,
  see [our blog post](https://on.cypress.io/debug-page-release?utm_source=docs.cypress.io&utm_medium=app-changelog). Addressed in
...
- Added new
  ...
  experienced "Out of Memory" issues. Leave any feedback in the
  [`experimentalMemoryManagement` discussion](https://github.com/cypress-io/cypress/discussions/25557).
  Your feedback will help us make decisions to improve memory issues. Addresses
  [#23391](https://github.com/cypress-io/cypress/issues/23391).
- Added new
  `experimentalSkipDomainInjection`
  configuration option to disable Cypress from setting `document.domain` on
  injection, allowing users to test Salesforce domains. If you believe you are
  having `document.domain` issues, see the
  `experimentalSkipDomainInjection`
...
- [`component.specPattern`](/app/references/configuration#component) now
  uses a JSX/TSX file extension when generating a new empty spec file if
...
[Retry-ability Guide](/app/core-concepts/retry-ability) with all the details
if you'd like to learn more.

Read more about 12.0 in
[our blog post](https://on.cypress.io/cypress-12-release?utm_source=docs.cypress.io&utm_medium=app-changelog).
...
- Cypress has always recommended writing tests in a clean context. In Cypress
  ...
  understand the full impact of this change, review the
  [migration guide](/app/references/migration-guide#Test-Isolation).
...
- The `experimentalSessionAndOrigin` configuration option has been removed and
  ...
  the [`cy.origin()`](/api/commands/origin) command. To use external
  dependencies in `cy.origin()` callbacks, set the new
  [`e2e.experimentalOriginDependencies`](/app/references/experiments#End-to-End-Testing)
...
- The previously experimental Cypress command,
  ...
  and
  [`sessionStorage`](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage)
  to recreate a consistent browser context between tests. Addresses
...
- Added a new configuration option called
  [`experimentalOriginDependencies`](/app/references/experiments#End-to-End-Testing),
...
- When
  [`experimentalRunAllSpecs=true`](/app/references/experiments#End-to-End-Testing),
...
- Re-implemented the 'Run All Specs' for End to End Testing for running multiple
  ...
  ['Run All Specs' discussion](https://github.com/cypress-io/cypress/discussions/21628).
  Your feedback will help us make product decisions around the future of this
  feature.
...
Component tests render and exercise your application's components in a real
browser as you work. You can use your favorite Cypress commands and
features to develop your components without running your whole app. Learn more
in our [blog](https://on.cypress.io/cypress-11-release?utm_source=docs.cypress.io&utm_medium=app-changelog) post.
...
**Read our
[Migration Guide](/app/references/migration-guide#Migrating-to-Cypress-110)
...
- Removed `cssFile`, `cssFiles`, `style`, `styles`, `stylesheet`, and
  ...
  [migration guide](/app/references/migration-guide#Changes-to-Mounting-Options).
...
- `mountHook` from `cypress/react` has been removed. We recommend replacing it
  ...
  [migration guide](/app/references/migration-guide#React---mountHook-Removed).
  Addresses [#24328](https://github.com/cypress-io/cypress/issues/24328).
- `unmount` from `cypress/react` has been removed. We recommend using the API
  ...
  See
  [migration guide](/app/references/migration-guide#React---unmount-Removed).
  Addresses [#24328](https://github.com/cypress-io/cypress/issues/24328).
- `mountCallback` from `cypress/vue` has been removed. We recommend using
  `mount`. See
  [migration guide](/app/references/migration-guide#Vue---mountCallback-Removed).
  Addresses [#24328](https://github.com/cypress-io/cypress/issues/24328).
...
- When providing an inline `viteConfig` inside of `cypress.config`, any
  ...
  [migration guide](/app/references/migration-guide#Vite-Dev-Server-cypressvite-dev-server).
...
- Angular providers passed as part of the mounting options will be assigned at
  ...
  [migration guide](/app/references/migration-guide#Angular---Providers-Mounting-Options-Change).
...
- When using dependencies within the `cy.origin()` callback, errors now show
...
- [`cy.session()`](/api/commands/session) is now supported when using
  [WebKit (Experimental)](/app/references/launching-browsers#WebKit-Experimental)
...
- The way that Cypress handles
  ...
  - The default mode is `on` when
    [`experimentalSessionAndOrigin`](/app/references/experiments) is enabled.
...
- Launch options returned from the
  [`before:browser:launch`](/api/node-events/browser-launch-api) event can now
...
- You can now run tests in WebKit, Safari's browser engine, by enabling the
  [`experimentalWebKitSupport`](/app/references/experiments) experiment and
  installing
  [`playwright-webkit`](https://www.npmjs.com/package/playwright-webkit). For
  more details, read
  [our blog post](https://www.cypress.io/blog/2022/09/13/cypress-10-8-experimental-run-tests-in-webkit?utm_source=docs.cypress.io&utm_medium=app-changelog).
...
- The `cypress/react18` [`cy.mount()`](/app/component-testing/react/overview)
...
- The `Sec-Fetch-Dest`
  ...
  Test when the
  [`experimentalModifyObstructiveThirdPartyCode`](/app/references/experiments#Configuration)
...
- Correctly override the Electron `userAgent`when the
  [`userAgent`](/app/references/configuration#Browser) configuration option
  is provided or
  [`experimentalModifyObstructiveThirdPartyCode`](/app/references/experiments#Configuration)
...
- Fix regression introduced by Cypress [10.0.0](#10-0-0) where the `before:spec`
  ...
  [`experimentalInteractiveRunEvents`](/app/references/experiments#Configuration)
...
- Fixed a bug where the `cypress/svelte`
  [`cy.mount()`](/app/component-testing/svelte/overview) command was not
  logging by default. Fixed in
  [#23771](https://github.com/cypress-io/cypress/pull/23771).
- Fixed an issue where setting the
  [`testIsolation`](/app/references/configuration#Global) configuration at
...
- Added [`modifyObstructiveCode`](/app/references/configuration#Browser)
...
- Re-introduces the experimental
  [Cypress Studio feature](/app/guides/cypress-studio). Cypress Studio
  ...
  [our blog post](https://www.cypress.io/blog/2022/08/30/how-to-use-studio-in-cypress-10-7?utm_source=docs.cypress.io&utm_medium=app-changelog).
...
- Angular component testing projects can provide a custom configuration to the
  ...
  [`projectConfig`](/app/component-testing/angular/overview#Options-API) key.
...
- Fixed an issue where an incomplete
  [`component.devServer`](/app/references/configuration#devServer)
...
- Introduced a new Cypress experiment,
  [`experimentalSingleTabRunMode`](/app/references/experiments), for
...
- Enhanced the
  [`CYPRESS_DOWNLOAD_PATH_TEMPLATE`](/app/references/advanced-installation#Environment-variables)
...
- Cypress Dashboard test data is now available real-time within Cypress. For
  more details, read
  [our blog post](https://www.cypress.io/blog/2022/06/28/cypress-10-3-0-speed-up-testing-workflows-with-improved-visibility-into-your-test-results/?utm_source=docs.cypress.io&utm_medium=app-changelog).
...
- Cypress is now available natively on Apple silicon, including the M1 and M2
  ...
  [our blog post](https://www.cypress.io/blog/2022/06/21/cypress-10-2-0-run-tests-up-to-2x-faster-on-apple-silicon-m1/?utm_source=docs.cypress.io&utm_medium=app-changelog).
...
<Icon name="exclamation-triangle" color="red" /> Run `cypress open` to go
...
- We introduced several breaking changes to the **Cypress configuration file**
  ...
  - A default configuration file will no longer be auto-generated by Cypress
    ...
    generated. This customizes the configuration setup by testing-type for the
    language and dependencies used in your project.
...
- Cypress has made several **configuration option changes** including renaming
  ...
  testing type. Run `cypress open` to have our automated migration update
  ...
  - Previously the `supportFile` option defaulted to `cypress/support/index.js`.
    Now the [`e2e.supportFile`](/app/references/configuration#e2e) option
    defaults to `cypress/support/e2e.{js,jsx,ts,tsx}` and the
    [`component.supportFile`](/app/references/configuration#component) option
  ...
  - The `pluginsFile` option was removed. This option should be replaced with
    the new
    [`setupNodeEvents()`](/app/references/configuration#setupNodeEvents) and
    [`devServer()`](/app/references/configuration#devServer) configuration
  ...
  - The `experimentalStudio` option was removed with the removal of the
    ...
    project, a warning message will be displayed. Leave any feedback
    in the
    [Cypress Studio discussion](https://github.com/cypress-io/cypress/discussions/21561).
...
- During `cypress open`, the ability to "Run all specs" and "Run filtered specs"
  has been removed. Leave feedback about the removal in the
  ['Run all specs' discussion](https://github.com/cypress-io/cypress/discussions/21628).
  Your feedback will help us make product decisions around the future of this
  feature.

- The experimental Cypress Studio feature has been removed. Leave
  feedback about the removal in the
  [Cypress Studio discussion](https://github.com/cypress-io/cypress/discussions/21561).
  Your feedback will help us make product decisions around the future of this
...
- You will no longer be able to run your tests during `cypress run` or
  `cypress open` in
  [browser versions that are not supported by Cypress](/app/references/launching-browsers#Browser-versions-supported).
...
- We've added several **new configuration options** detailed below:
  - The [`setupNodeEvents()`](/app/references/configuration#setupNodeEvents)
    ...
    [`component`](/app/references/configuration#component) configurations.
    This option modifies your loaded configuration, listens to events and
    registers plugins. This option replaces the `pluginsFile` option.
    Addressed in [#18798](https://github.com/cypress-io/cypress/pull/18798) and
    [#20560](https://github.com/cypress-io/cypress/pull/20560).

  - The [`devServer`](/app/references/configuration#devServer) option is a
    new component testing specific option that must be defined within the
    [`component`](/app/references/configuration#component) configuration.
  ...
  - The `specPattern` option is a new testing-type specific configuration option
    ...
    [`e2e`](/app/references/configuration#e2e) and/or
    [`component`](/app/references/configuration#component) configurations.
    ...
    - The [`e2e.specPattern`](/app/references/configuration#e2e) default
      ...
      existing projects, run `cypress open` to have your spec files
      automatically migrated to match this `specPattern`. Addressed in
      [#21193](https://github.com/cypress-io/cypress/pull/21193).
    - The [`component.specPattern`](/app/references/configuration#component)
  ...
  - The `excludeSpecPattern` option is a new testing type specific configuration
    ...
    [`e2e`](/app/references/configuration#e2e) and/or
    [`component`](/app/references/configuration#component) configurations.
    This option accepts a glob or an array of globs, and ignores the matching
    ...
    [#20853](https://github.com/cypress-io/cypress/pull/20853).
    - The [`e2e.excludeSpecPattern`](/app/references/configuration#e2e)
      default value is the same as the replaced `ignoreTestFiles` glob pattern
      of `*.hot-update.js`.
    - The
      [`component.excludeSpecPattern`](/app/references/configuration#component)
...
- The Plugins concept in Cypress still exists with the intent to tap into,
  ...
  option, which was named to reflect that Plugins use Cypress's node
  events.
...
- Now you can easily test multi-domain workflows by using the experimental
  [`cy.origin()`](/api/commands/origin) command. This feature tests across
  different origins or domains within a single test. This experiment was
  ...
  [`experimentalSessionAndOrigin`](/app/references/experiments) configuration
  ...
  - The `experimentalSessionSupport` configuration option has been removed and
    replaced by the `experimentalSessionAndOrigin` configuration option. Update
    your configuration to use the new `experimentalSessionAndOrigin` option to
  ...
  - When `experimentalSessionAndOrigin` is enabled, cross-origin requests will
    ...
    [`pageLoadTimeout`](/app/references/configuration#Timeouts)
...
- Updates were made to decrease the length of the Cypress cache path for Windows
  to ensure
  [installing pre-release versions](/app/references/advanced-installation#Install-pre-release-version)
...
- Updates were made to correctly escape dollar signs in the
  ...
  using this template when the env was provided from a package manager file.
...
- Updates were made to the pre-release build setup such that Cypress will use a
  ...
  [pre-release installation](/app/references/advanced-installation#Install-pre-release-version)
...
- Made a change to the DNS lookup to improve test performances and reliability
  on MacOS machines that use the VMWare CBCloud SaaS solution. Before this
...
- Added the `CYPRESS_DOWNLOAD_PATH_TEMPLATE` environment variable which can be
  ...
  [Install Binary](/app/references/advanced-installation#Install-binary)
  documentation. Fixes
...
- Cypress will throw an error when a user attempts to update a configuration
  ...
  [readonly option](/app/references/configuration#Test-Configuration).
...
- There are changes to how "slow" tests are determined and displayed in Cypress.
  ...
  - Added a [`slowTestThreshold`](/app/references/configuration#Timeouts)
...
- Pinned resolution of `ua-parser-js` to `0.7.24` to avoid dependency resolution
  to the compromised package versions. Read the
...
- You can now cache and restore [cookies](/api/cypress-api/cookies),
  ...
  and
  [`sessionStorage`](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage)
  to reduce test setup times by using
  [`cy.session()`](/api/commands/session). The `session` API is experimental and
  can be enabled by setting the
  [`experimentalSessionSupport`](/app/references/experiments) flag to `true`
...
- When using `experimentalStudio`, you can now generate assertions in
  [Cypress Studio](/app/guides/cypress-studio) by right clicking on an
...
<Icon name="exclamation-triangle" color="red" /> Read our [Migration
...
- When running the `--headless` Chrome browser via `cypress run`, the device
  ...
  [browser launch API](/api/node-events/browser-launch-api#Override-the-device-pixel-ratio).
...
- You can now configure certificate authority (CA) and client certificates to
  ...
  option. See [Client certificates](/app/references/client-certificates) for
  more detail.
...
- `console.log` and `console.error` called within the
  [plugins file](/app/core-concepts/writing-and-organizing-tests#Node-events-setupNodeEvents)
...
- When using the experimental
  [Cypress Studio](/app/guides/cypress-studio), there should be a reduced
...
- When using `experimentalStudio`,
  [Cypress Studio](/app/guides/cypress-studio) can now copy all commands
...
- Component tests can now be launched via the
  [Module API](/app/references/module-api) by passing `testingType: 'component'`
...
- Cypress can now use the certificate authority specified in npm config if
  ...
  ["Using a custom CA"](/app/references/advanced-installation#Using-a-custom-certificate-authority-CA)
...
[Component Testing Guide](/app/component-testing/get-started) for more details.
...
- We introduced several breaking changes to
  ...
  - `cy.route2()` was previously aliased to `cy.intercept()`. Now the alias
    `cy.route2()` has been removed. Update usage of `cy.route2()` to
...
- Component tests have been migrated to use a dedicated test runner and are
  ...
  [Migration Guide](/app/references/migration-guide#Component-Testing) for
...
- Cypress now throws an error if the application under test redirects more than
  ...
  [`redirectionLimit`](/app/references/configuration#Global) config. Addresses
...
- `Cypress.moment()` has been removed. Migrate to a different datetime
  formatter. See
...
- The default headless browser window size has been increased to 1920x1080
  pixels to capture
  [High-definition videos and screenshots](https://www.cypress.io/blog/2021/03/01/generate-high-resolution-videos-and-screenshots/?utm_source=docs.cypress.io&utm_medium=app-changelog).
...
- You can now pass runner specific configuration to pass different
...
- Improved the specs list user interface with a new icon set and accessibility
  features when in [Component Testing](/app/component-testing/get-started) mode
  (alpha). Addressed in
...
- When using `experimentalStudio`,
  [Cypress Studio](/app/guides/cypress-studio) the icon to add commands
...
- The `experimentalRunEvents` configuration flag has been removed. You can now
  ...
  [experimentalRunEvents](/app/references/experiments) configuration option.
  See the [`before:run`](/api/node-events/before-run-api),
  [`after:run`](/api/node-events/after-run-api),
  [`before:spec`](/api/node-events/before-spec-api) and
  [`after:spec`](/api/node-events/after-spec-api) docs for more information.
  Addressed in [#15276](https://github.com/cypress-io/cypress/pull/15276).
- When cancelling a run from the Dashboard, previously only parallelized runs
...
- When using `experimentalStudio`,
  [Cypress Studio](/app/guides/cypress-studio) now records typing by
...
- There's now a [Command Log](/app/core-concepts/open-mode#Command-Log)
...
- We addressed several issues with the experimental
  [Cypress Studio](/app/guides/cypress-studio). To enable the Cypress
...
- [**Cypress Studio**](/app/guides/cypress-studio) provides a visual way
  ...
  be enabled by adding the [experimentalStudio](/app/references/experiments)
...
- **You can now test file downloads in Cypress** without the download prompt
  ...
  in the [downloadsFolder](/app/references/configuration#Downloads) which is
  set to `cypress/downloads` by default. The `downloadsFolder` will be deleted
  before each run unless
  [trashAssetsBeforeRuns](/app/references/configuration#Downloads) is set to
...
- You can now listen to `before:run`, `after:run`, `before:spec` and
  ...
  [experimentalRunEvents](/app/references/experiments) configuration option
  to `true`. See the [`before:run`](/api/node-events/before-run-api),
  [`after:run`](/api/node-events/after-run-api),
  [`before:spec`](/api/node-events/before-spec-api) and
  [`after:spec`](/api/node-events/after-spec-api) docs for more information.
...
- There's a new `scrollBehavior` configuration option that controls the viewport
  ...
  [global configuration](/app/references/configuration#Actionability),
  [test configuration](/app/core-concepts/writing-and-organizing-tests#Test-Configuration)
  or individual
  [action commands](/app/core-concepts/interacting-with-elements) via
...
[Migrating `cy.route()` to `cy.intercept()`](/app/references/migration-guide).
...
- DOM elements where the CSS style (or ancestors) are `opacity: 0` are no longer
  ...
  [actionable](/app/core-concepts/interacting-with-elements) and
...
- Our [Migration Guide](/app/references/migration-guide) has a new section
  for 6.0 migration.
- Many of our docs examples have been updated to use
  [cy.intercept()](/api/commands/intercept) in place of `cy.route()` including
  our [Network Requests](/app/guides/network-requests) doc.
...
- When [experimentalNetworkStubbing](/app/references/experiments) is enabled,
...
- `cypress.run()` through the [Module API](/app/references/module-api) now has a
...
- You can now run
  [cypress cache prune](/app/references/command-line#cypress-cache-prune) to
...
- There's a new `--size` option for the
  [cypress cache list](/app/references/command-line#cypress-cache-list) command
...
- The [Command Log](/app/core-concepts/open-mode#Command-Log) can be hidden
...
assertions when they fail, test retries will automatically retry a failed test
prior to marking it as failed. Read our new guide on
[Test Retries](/app/guides/test-retries) for more details.
...
- The
  ...
  [test retries](/app/guides/test-retries) built into Cypress. Addresses
...
- The `blacklistHosts` configuration has been renamed to
  [blockHosts](/app/references/configuration#Notes) to more closely reflect
...
- For TypeScript compilation of spec, support, and plugins files, the
  ...
  use `esModuleInterop`, set it in your `tsconfig.json`. Addresses
...
- Cypress [environment variables](/app/guides/environment-variables) that
...
- When [experimentalShadowDomSupport](/app/references/experiments) is
...
- We have a new guide on [Test Retries](/app/guides/test-retries).
- Our [Migration Guide](/app/references/migration-guide) has a new section
  for 5.0 migration.
...
- Now you can control whether screenshots are automatically taken on test
  ...
  [`screenshotOnRunFailure`](/app/references/configuration#Screenshots) in
...
- When [experimentalSourceRewriting](/app/references/experiments) is enabled,
...
- The [Module API](/app/references/module-api) has a new
...
- You can open a `before`, `beforeEach`, `after`, and `afterEach` hook
  ...
  [Command Log](/app/core-concepts/open-mode#Command-Log) by clicking the
...
- `before`, `beforeEach`, `after`, and `afterEach` hook definitions now display
  ...
  [Command Log](/app/core-concepts/open-mode#Command-Log) when defined in
...
- HTTP requests taking longer than the default `responseTimeout` will no longer
  be prematurely cancelled by the Cypress proxy layer. Fixes
...
- An animated progress bar now displays on every command in the
  [Command Log](/app/core-concepts/open-mode#Command-Log) indicating how
...
- There is now an [experimentalFetchPolyfill](/app/references/experiments)
...
- You can now pass a `--quiet` flag to
  [cypress run](/app/references/command-line#cypress-run) to silence any Cypress
...
- Assertion logs now properly display as parent commands in the
  [Command Log](/app/core-concepts/open-mode#Command-Log) regardless of
...
- Highlighting of elements upon hover of a command in the
  [Command Log](/app/core-concepts/open-mode#Command-Log) are now visible
  when targeting absolute positioned elements. Fixes
  [#7762](https://github.com/cypress-io/cypress/issues/7762).
- [cypress run](/app/references/command-line#cypress-run) will no longer crash
...
- There is now a loading state to indicate when tests are loading in the
  [Command Log](/app/core-concepts/open-mode#Command-Log). Addresses
...
- You can now apply certain
  [test configuration](/app/references/configuration#Test-Configuration) for
...
- We now display the currently running file at the top of the Test Runner's
  Command Log.
  [That file can be opened](/app/core-concepts/open-mode#Open-files-in-your-IDE)
  in your
...
- We added experimental shadow DOM support through the
  ...
  option. See the [Experiments page](/app/references/experiments#History)
  for more information. Addresses
...
- [Errors](/app/guides/debugging#Errors) in the Test Runner now display a
...
- Cypress now uses [source maps](/app/guides/debugging#Source-maps) to
...
- Cypress now enables AST-based JS/HTML rewriting when setting the
  [experimentalSourceRewriting](/app/references/experiments) configuration
...
- Number arguments passed to `have.text`, `have.id`, `have.data`, `have.value`,
  and `have.attr`
  [assertions chainers](/app/references/assertions#Chai-jQuery) are now
  automatically cast to strings for comparison. Addresses
  [#7314](https://github.com/cypress-io/cypress/issues/7314).

**Bugfixes:**

- Default [TypeScript](/app/tooling/typescript-support) options are now set
...
- [Custom Mocha reporters](/app/tooling/reporters) will now correctly use the
...
- The typings for run results when using the
  [Module API](/app/references/module-api) now indicate they can be `null` or
  `undefined`. Addresses
...
- [TypeScript](/app/tooling/typescript-support) test files are now supported
...
- [Errors shown in the Test Runner](/app/guides/debugging#Errors) have a new
...
- When you open a Cypress project, clicking on the **Settings** tab and clicking
  ...
  [experimental features](/app/references/configuration#Experiments) that are
...
- Added experimental support for adding `sameSite` values to the objects yielded
  ...
  the
  [experimentalGetCookiesSameSite](/app/references/configuration#Experiments)
...
- [cypress cache list](/app/references/command-line#cypress-cache-list) now
...
- We now more thoroughly ensure the parent of an element exists when recursively
  calling actionability checks to prevent some maximum call stack
...
- [cypress verify](/app/references/command-line#cypress-verify) no longer
...
- The plugins file function's [config](/api/node-events/configuration-api) argument
...
- We now more thoroughly ensure the parent of an element exists when recursively
  calling visibility and existence checks to prevent some maximum call
  stack errors. Fixes
...
[Cross Browser Testing](/app/guides/cross-browser-testing) in Cypress. We've
also updated many of the
[underlying tools](/app/references/bundled-libraries) behind Cypress that
...
- During `before:browser:launch`, we no longer yield the second argument as an
  ...
  `args` property to support more advanced browser launch options.
  Addresses [#6306](https://github.com/cypress-io/cypress/issues/6306).
- We updated the [Cypress browser objects](/api/node-events/browser-launch-api) of
...
- The
  [`--browser` flag](/app/references/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
...
- [cy.writeFile()](/api/commands/writefile) now yields `null` instead of the
  contents written to the file to more align with the behavior of `fs`.
...
- The [`isFinite` assertion](/app/references/assertions) is now supported.
...
- Added [Cross Browser Testing Guide](/app/guides/cross-browser-testing) with
...
- We now pass through errors caused when attempting to read arguments passed
  through [config](/api/node-events/configuration-api). Addresses
...
- You can print debug information about the memory & CPU usage of Cypress, which
  ...
  [`cypress:server:util:process_profiler` debug stream](/app/references/troubleshooting#Log-memory-and-CPU-usage).
...
- Generated reports when using [custom reporters](/app/tooling/reporters) now
...
- We fixed a bug where
  [cypress run --headless](/app/references/command-line#Options)
...
- Cypress now accepts a
  [--tag](/app/references/command-line#cypress-run-tag-lt-tag-gt) argument to be
...
- The Test Runner now shows a non-Chrome logo for
  [browsers passed in via `config.browser`](/app/references/launching-browsers#Customize-available-browsers)
...
- Cypress now includes the full list of browsers found and their properties
  ...
  [editable within plugins by accessing `config.browsers`](/app/references/launching-browsers#Customize-available-browsers)
  via the [Configuration API](/api/node-events/configuration-api). The currently
  available browsers are also displayed within the
  [configuration panel](/app/references/configuration#Resolved-Configuration)
...
- Environment variables within GitHub Actions are now detected and passed to the
  Dashboard to display the proper
  [git information](/app/continuous-integration/overview#Git-information)
...
- Cypress now captures the SHA of builds generated through Travis CI during pull
  ...
  running within
  [GitHub Integration](/cloud/integrations/github). Fixes
...
- Cypress now automatically strips double quotes from
  ...
  and [group](/app/references/command-line#cypress-run-group-lt-name-gt) flag
  arguments during `cypress run`. Fixes
  [#5686](https://github.com/cypress-io/cypress/issues/5686).
- Assertions printed in the
  [Command Log](/app/core-concepts/open-mode#Command-Log) now correctly
...
- Empty spaces are now accurately reflected in the
  [Command Log](/app/core-concepts/open-mode#Command-Log) within
  assertions. Fixes [#1360](https://github.com/cypress-io/cypress/issues/1360).
- Keys printed in the
  [Command Log](/app/core-concepts/open-mode#Command-Log) when using the
...
- Documented
  [how to pass in a custom list of browsers](/app/references/launching-browsers#Customize-available-browsers)
...
- We now record video by default when you run Cypress in Chrome, Chromium, or
  ...
  [cypress run --browser chrome](/app/references/command-line#cypress-run-browser-lt-browser-name-or-path-gt).
...
- Cypress now accepts a
  [`--config-file` argument](/app/references/command-line#cypress-run-config-file-lt-configuration-file-gt)
...
- [.type()](/api/commands/type) now types `-` into number inputs with text
  selection. Fixes
...
- We now throw a better error when passing in an Object to the
  [browser](/api/node-events/browser-launch-api) argument of the Browser Launch API
...
- We added `failures` and `message` types when a run fails using the
  [Module API](/app/references/module-api). Addresses
...
- Added the `--config-file` argument to
  [Command Line](/app/references/command-line) and
  [Module API](/app/references/module-api) doc.
- Added the `nodeVersion` argument to
  [Configuration](/app/references/configuration) doc and mentioned its uses
  in the [How to use Plugins](/app/plugins/plugins-guide) and
  [Node Events Overview](/api/node-events/overview) doc.
...
- Documented new keyboard shortcuts in the
  [Cypress App](/app/core-concepts/open-mode) doc.
- Added a new section on troubleshooting problems with Xvfb to the
  [Continuous Integration](/app/continuous-integration/overview#Xvfb)
  doc.
- Added a section to our Debugging doc about our `cypress-fiddle` plugin used
  for playing around with small test cases.
- Added a section to our [Debugging](/app/references/troubleshooting#Patch-Cypress) doc
...
- We have some newly translated documentation in Portuguese. If you'd like to
  help in translating more, open a pull request in our
...
- We’re excited to announce
  [GitHub integration](/cloud/integrations/github) for the
  ...
  [our blog](https://www.cypress.io/blog/2019/07/29/github-integration-for-the-cypress-dashboard/?utm_source=docs.cypress.io&utm_medium=app-changelog).
...
- We have new instructions on how to set up
  [GitHub Integration](/cloud/integrations/github) with
  the Cypress Dashboard.
- Updated instructions for adding types for custom commands in the
  [TypeScript Support](/app/tooling/typescript-support#Types-for-Custom-Commands)
  doc. Addressed in
...
- Added a section about video encoding and how to speed up the encoding time to
  [Screenshots and Videos](/app/guides/screenshots-and-videos#Video-encoding)
  doc. Addressed in
...
- New error message added about policy settings to our
  [Error Messages](/app/references/error-messages) doc. Addressed in
...
- More content was added to our [Code Coverage](/app/tooling/code-coverage)
  doc.
...
- New [Code Coverage](/app/tooling/code-coverage) doc. Addressed in
  [#1744](https://github.com/cypress-io/cypress-documentation/pull/1744).
- New [Visual Testing](/app/tooling/visual-testing) doc. Addressed in
...
- Added a section about
  [Hacking on Cypress](/app/references/troubleshooting#Hacking-on-Cypress) in the
  Debugging doc. Addressed in
  [#1783](https://github.com/cypress-io/cypress-documentation/pull/1783).
- Added a section on how to
  [opt out of sending exception data to Cypress](/app/references/advanced-installation#Opt-out-of-sending-exception-data-to-Cypress)
...
- We've
  [written a blog post](https://www.cypress.io/blog/2019/05/22/how-we-improved-network-speed-by-300-in-cypress-3-3-0/?utm_source=docs.cypress.io&utm_medium=app-changelog)
...
- Fixed issue where support for `have.focus`, `not.have.focus`, `be.focused` and
  `not.be.focused` [assertions](/app/references/assertions) were not working.
...
- We have some newly translated documentation in
  ...
  If you'd like to help in translating more, open a pull request in our
  [documentation repo](https://github.com/cypress-io/cypress-documentation).
...
- Cypress now automatically reads from your system's `HTTP_PROXY`,
  ...
  accessing 'Runs' within the Test Runner. Read our new
  [Proxy Configuration](/app/references/proxy-configuration) doc to
...
- Your project's current proxy settings now display in the
  [Settings of the Test Runner](/app/references/proxy-configuration#View-proxy-settings-in-Cypress).
...
- Support for `have.focus`, `not.have.focus`, `be.focused` and `not.be.focused`
  [assertions](/app/references/assertions) have been added. Addresses
...
- There's a new 'canceled' XHR status shown on the Command Log to indicate the
  difference between calling `xhr.abort()` and the browser cancelling the request
...
- New [Proxy Configuration](/app/references/proxy-configuration) doc.
  Addressed in
...
- Added newly supported `focus` assertion to
  [Assertions](/app/references/assertions) doc. Addresses
...
- Added new options `retryOnStatusCodeFailure` and `retryOnNetworkFailure` to
  [cy.visit()](/api/commands/visit) and [cy.request()](/api/commands/request).
- Updated
  [install instructions for Windows](/app/references/advanced-installation#Download-URLs)
...
- Added a section about how to request our new
  [Open Source Plan](/cloud/account-management/billing-and-usage#Requesting-OSS-plan-for-an-Organization)
...
- The
  [`--browser` argument](/app/references/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
  of the [Command Line](/app/references/command-line) and the
  [`browser` argument](/app/references/module-api#cypressrun) of the
  [Module API](/app/references/module-api) have been updated to allow passing a
  `<path>`. Addresses
...
- When [cy.wrap()](/api/commands/wrap) is followed directly by a
  ...
  [retry](/app/core-concepts/retry-ability) when its argument evaluates to
...
- New [Retryability](/app/core-concepts/retry-ability) doc
...
- Updated [docs](/app/references/launching-browsers) on explain how to run Chrome
...
- Add new section to
  [Troubleshooting](/app/references/troubleshooting#Allow-the-Cypress-Chrome-extension)
...
- We now pass the Chromium command line switches
  ...
  Chrome by default to better support testing webcam functionality.
...
- Added example on how to use a fake video for webcam testing to
  [Browser API doc](/api/node-events/browser-launch-api#Use-fake-video-for-webcam-testing).

**Dependency Updates**

- We've begun automating our dependency update process to avoid
...
- Removed the word 'already' from the Cypress install message to be
...
- There is a new
  [CYPRESS_DOWNLOAD_MIRROR](/app/references/advanced-installation#Mirroring)
  environment variable for installing Cypress at a mirror url. Fixes
  [#2609](https://github.com/cypress-io/cypress/pull/2609).
- The [Module API](/app/references/module-api) now returns the `runUrl` from
  `cypress.run()`. Fixes
...
- We updated how we handle
  [trashAssetsBeforeRuns](/app/references/configuration#Screenshots)
...
- Added `runUrl` to returned run in [Module API doc](/app/references/module-api).
- Documented new `CYPRESS_DOWNLOAD_MIRROR` flag and rewrote advanced install
  instructions to be clearer in
  [Installing Cypress doc](/app/references/advanced-installation).
...
- `cypress run` accepts a new
  [--parallel](/app/references/command-line#cypress-run-parallel) flag to run
...
- `cypress run` accepts a new
  [--group](/app/references/command-line#cypress-run-group-lt-name-gt) flag to
...
- When using the new
  ...
  [--group](/app/references/command-line#cypress-run-group-lt-name-gt) feature,
...
- [cypress run](/app/references/command-line#cypress-run) now creates a new
  browser profile per run per browser, but keeps the same browser profile when
  run via [cypress open](/app/references/command-line#cypress-open). This means
...
- [Added `Parallelization` doc.](/cloud/features/smart-orchestration/parallelization)
- [Update `Dashboard Service` doc with new screenshots and run sections.](/cloud/get-started/introduction)
- [Update `Command Line` doc with new flags and alphabetized flags.](/app/references/command-line)
...
- We are now properly serializing nested object arguments when using the
  [Module API](/app/references/module-api) via `cypress.open()` or
  ...
  addition to fixing the [Module API](/app/references/module-api), we are now
  also properly handling nested arguments from the
  [Command Line](/app/references/command-line). To pass nested objects
...
- Fixed a bug causing screenshots saved via
  ...
  you ran using [cypress open](/app/references/command-line#cypress-open) vs
  [cypress run](/app/references/command-line#cypress-run) . Fixes
  [#2091](https://github.com/cypress-io/cypress/issues/2091).
- Fixed bugs with Electron losing focus when running headlessly via
  [cypress run](/app/references/command-line#cypress-run) after navigation. Now
...
- We've loosened up the restrictions surrounding what we consider a typeable
  ...
  the browser does still consider them regular `text` inputs and accepts typed
  input. Fixes
...
- [New "Module API" doc](/app/references/module-api)
...
- Fixed an oversight where a specific framebusting check was not being properly
  ...
  [modifyObstructiveCode](/app/references/configuration#Browser). Fixes
...
- There are now several new CLI commands for interacting with the binary cache:
  [cypress cache path](/app/references/command-line#cypress-cache-path),
  [cypress cache list](/app/references/command-line#cypress-cache-list), and
  [cypress cache clear](/app/references/command-line#cypress-cache-clear). Fixes
  [#1856](https://github.com/cypress-io/cypress/issues/1856).
- We've added a new CLI flag: `--no-exit` for
  [cypress run](/app/references/command-line#cypress-run) to keep the browser
  open after the tests finish running. Fixes
  [#1871](https://github.com/cypress-io/cypress/issues/1871).
- The [CLI](/app/references/command-line) now respects config values and
...
- We improved error logging and the user experience around
  [cypress verify](/app/references/command-line#cypress-verify). Fixes
...
- There's now an
  [`after:screenshot` plugin event](/api/node-events/after-screenshot-api) you can
...
- [Updated "Command Line" doc to include new cache commands](/app/references/command-line).
- [Added `Cypress.browser` doc.](/api/cypress-api/browser)
- [Added `Cypress.spec` doc.](/api/cypress-api/spec)
- [Added `after:screenshot` plugin event doc.](/api/node-events/after-screenshot-api)
...
- You can now pass a normal object to `cypress.open()` or `cypress.run()`
  through the [Module API](/app/references/module-api), just as you would from
...
- [Update `Installing Cypress` doc to include new options for Cypress binary caching](/app/get-started/install-cypress)
- [Updated `Writing and Organizing Tests` to include newly seeded `examples` folder](/app/core-concepts/writing-and-organizing-tests)
- [Updated `cypress run` spec args](/app/references/command-line#cypress-run)
- [Updated `Dashboard Service`](/cloud/get-started/introduction)
- [Updated `Launching Browsers` to include our disabling of device discovery notifications](/app/references/launching-browsers)
- [Updated `Preprocessors` to reflect all ways `close` event could be triggered](/cloud/get-started/introduction)
...
- Fixed more edge cases where legitimate JavaScript code was being modified
  unexpectedly by the
  [modifyObstructiveCode](/app/references/configuration#Browser)
...
- We are now detecting and automatically stripping code that obstructs Cypress
  ...
  [modifyObstructiveCode](/app/references/configuration#Browser) that is
...
- [blacklistHosts](/app/references/configuration#Browser) would occasionally
...
- [blacklistHosts](/app/references/configuration#Browser) is now correctly
  accepted via the `--config` CLI flag.
- Spies and Stubs created with [cy.stub()](/api/commands/stub) and
  [cy.spy()](/api/commands/spy) will now retry their assertions when used
...
- The seeded `example_spec.js` file no longer fails in Windows on the
  `cy.exec()` test. It now conditionally runs a command
...
- Plugins now emit a
  [before:browser:launch](/api/node-events/browser-launch-api) event
...
- We have launched a "CSS Selector Playground", a new UI button in the Test
  ...
  use and feedback. Check out issue
...
- [Added `Browser Launch API`](/api/node-events/browser-launch-api)
- [Updated `Plugins Guide`](/app/plugins/plugins-guide)
- [Updated `Writing a Plugin API`](/api/node-events/overview)
...
- There is a new [userAgent](/app/references/configuration#Browser) option in
...
- There is a new [blacklistHosts](/app/references/configuration#Browser)
  option in `cypress.json` that blocks requests made to those hosts. Blocked
  requests will respond with a `503` status code. This is useful
  for blocking 3rd party domains like Google Analytics. We have added a
  [new 'Stubbing Google Analytics' recipe](/app/references/recipes#Stubbing-and-spying)
...
- Updated TypeScript definitions to more accurately match the current Cypress
  API. Fixes [#1050](https://github.com/cypress-io/cypress/issues/1050). If you
  were extending Cypress in TypeScript, the `Chainer` interface changed. Read
  issue #1048 to understand what you will need to update.
...
- [Added `Configuration API`](/api/node-events/configuration-api)
- [Added `Recipes`](/app/references/recipes)
- [Updated `Configuration`](/app/references/configuration)
- [Updated `Environment Variables`](/app/guides/environment-variables)
- [Updated `Using Cypress FAQ`](/app/faq)
- [Updated `Plugins Guide`](/app/plugins/plugins-guide)
- [Updated `Writing a Plugin API`](/api/node-events/overview)
...
- There is now a new
  [`CYPRESS_SKIP_BINARY_INSTALL`](/app/references/advanced-installation#Environment-variables)
...
- [Added ESLint, Docker, and TypeScript to Plugins](/app/plugins/plugins-list)
- [Added recipe for TypeScript with Browserify](/app/references/recipes)
- [Added recipe for TypeScript with webpack](/app/references/recipes)
- [Added recipe for Direct Controlling AngularJS](/app/references/recipes)
- [Added recipe for E2E API Testing](/app/references/recipes)
- Added more video tutorials
- [Improved displaying list of Tutorials, Recipes, Applications, and Docker examples](/app/references/recipes)
...
- We have currently added only
  [one event](/api/node-events/overview#List-of-events) for modifying the
...
- There is now a [Plugins API interface](/app/plugins/plugins-guide).
  Partially addresses [#684](https://github.com/cypress-io/cypress/issues/684).
- We have added the first Plugin API event called:
  [file:preprocessor](/api/node-events/overview#List-of-events). Fixes
...
- There is a new `--global` option useful with
  [cypress open](/app/references/command-line#cypress-open) to force Cypress to
...
- The [module API](/app/references/module-api) accepts `reporterOptions` as an
...
- [Created "Variables and Aliases" core concept](/app/core-concepts/variables-and-aliases)
- [Created "Conditional Testing" core concept](/app/guides/conditional-testing)
- [Created "Plugins" guide](/app/plugins/plugins-guide)
- [Created "List of Plugins"](/app/plugins/plugins-list)
- [Created "Writing a Plugin" API](/api/node-events/overview)
- [Created "Preprocessors API"](/api/node-events/preprocessors-api)
- [Updated "Using Cypress FAQ"](/app/faq)
- [Updated "Best Practices"](/app/core-concepts/best-practices)
...
- You can install the Cypress binary from any URL or file path using the
  [`CYPRESS_BINARY_VERSION` environment variable](/app/references/advanced-installation).
  Closes [#701](https://github.com/cypress-io/cypress/issues/701).

**Documentation Changes:**

- [Created "Conditional Testing" core concept](/app/guides/conditional-testing)
- [Created "Key Differences" core concept](/app/get-started/why-cypress)
- [Updated "Best Practices"](/app/core-concepts/best-practices)
- [Updated "FAQ Usage"](/app/faq)
...
- `1.0.0` marks our transition out of a **private** beta and into a **public**
  ...
  [Dashboard Service](/cloud/get-started/introduction) will continue to be free.
...
- `cy.exec()` now properly sources each time it is invoked
...
- [Updated "Command Line"](/app/references/command-line)
- [Updated "Interacting with Elements"](/app/core-concepts/interacting-with-elements)
...
- You can now
  [install Cypress as a real `npm` module](/app/get-started/install-cypress),
...
- The [`cypress` module can be `required`](/app/references/module-api) in your
...
- Cypress now verifies it can run on your system prior to actually running.
  ...
  for things like
  [missing linux dependencies](/app/continuous-integration/overview#Dependencies)
  instead of absolutely _nothing_ like before.
- We have better error handling and messaging around Xvfb failures.
- There is a new [`--browser` option](/app/references/command-line#cypress-run)
...
- You can use the new environment variable
  [`DEBUG=cypress:cli`](/app/references/command-line#Debugging-commands) to see
...
- [`chai-jQuery` assertions](/app/references/assertions#Chai-jQuery) no
...
- [`chai-jQuery` assertions](/app/references/assertions#Chai-jQuery) have
  ...
  [#605](https://github.com/cypress-io/cypress/issues/605).
- [`chai-jQuery` assertions](/app/references/assertions#Chai-jQuery) now
...
- Input ranges are now more easily testable using the new
  ...
  [new recipe](/app/references/recipes#Testing-the-DOM) for details on how. Fixes
...
- You can now pass a browser option to
  [cypress run](/app/references/command-line#cypress-run) as
...
- [cypress open](/app/references/command-line#cypress-open) no longer opens a
...
- Cypress now scrolls all parent containers (not just `window`) when attempting
  to
  [check an element's actionability](/app/core-concepts/interacting-with-elements#Actionability).
...
- The interface for writing custom commands has been deprecated.
  [Read our docs on the new custom commands interface](/api/cypress-api/custom-commands).
...
- There are no more global, in-app updates. Versioning should be primarily
  [controlled via `npm`](/app/get-started/install-cypress) like every
...
- Fixed issue where `cy.exec()` was failing when running
...
- [The checks used to determine an element's actionability](/app/core-concepts/interacting-with-elements#Actionability)
...
- The example_spec from the
  ...
  [cypress run](/app/references/command-line#cypress-run) on a brand new project.
...
- [cypress install](/app/references/command-line) `-d` option. Fixes
...
- The `CYPRESS_DEBUG` env option has been removed in favor of the `debug` module
  and [`DEBUG=cypress:*`](/app/references/command-line#Debugging-commands) usage.
- Attempting to run [cypress run](/app/references/command-line#cypress-run) on a
...
- [Updated "Command Line"](/app/references/command-line)
- [Updated "Debugging"](/app/guides/debugging)
- [Updated "Assertions"](/app/references/assertions)
- [Updated "Interacting with Elements"](/app/core-concepts/interacting-with-elements)
...
- You can now run your tests in the
  [Electron browser](/app/references/launching-browsers#Electron-Browser) that
...
- We have updated all of the docs related to these changes. The
  [CI Docs](/app/continuous-integration/overview) got a much needed
  facelift.
- There is a new docs section related to the
  [Dashboard](/cloud/get-started/introduction) and the new features.
...
- You will need to download a new [cypress-cli](/app/references/command-line) -
  version `0.13.1`.
...
- [cypress run](/app/references/command-line#cypress-run) now behaves likes
...
- [cypress run](/app/references/command-line#cypress-run) will now download and
...
- `cypress ci` has been deprecated. You now use
  [`cypress run --record --key <record_key>`](/app/references/command-line#cypress-run).
  The key you used to pass to `cypress ci` is the same key. We've consolidated
  these commands into just
  [cypress run](/app/references/command-line#cypress-run) which makes it simpler
  and clearer. Their only difference is that passing `--record` to
  [cypress run](/app/references/command-line#cypress-run) will **record** the
...
- You can now use your regularly installed `node_modules` in your project to do
  things like using [`lodash`](https://lodash.com/) for utility functions or
...
- Previously, we auto-magically included all files within
  `cypress/support`.
  This has now [gone away](/app/references/error-messages#Support-file-missing-or-invalid) and we've
...
- Test files with JS syntax errors are now
  [handled](/app/references/error-messages) and we provide a GUI that points
...
- Improved [cypress run](/app/references/command-line#cypress-run) and
...
- If you do _not_ want these assets to be tracked by Cypress, you need to switch
  to using [cypress run](/app/references/command-line#cypress-run). We will
...
- [cypress run](/app/references/command-line#cypress-run) no longer prompts the
...
- There is a new [configuration](/app/references/configuration) property
...
- There are several new [configuration](/app/references/configuration)
...
- Changed default [`responseTimeout`](/app/references/configuration#Timeouts)
  from `20000` to `30000`.
- Changed default [`pageLoadTimeout`](/app/references/configuration#Timeouts)
  from `30000` to `60000`.
...
- There is now a new
  [`chromeWebSecurity`](/app/references/configuration#Browser) option you can
...
- We've added `JUnit` as a valid
  [built-in reporters](/app/tooling/reporters). Fixes
  [#178](https://github.com/cypress-io/cypress/issues/178).
- You can now
  [add or write your own custom reporters](/app/tooling/reporters). This
...
- [`defaultCommandTimeout`](/app/references/configuration#Timeouts) now works
...
- [cypress run](/app/references/command-line#cypress-run) no longer requires
  being logged in.
- Renamed configuration option `commandTimeout` to
  [`defaultCommandTimeout`](/app/references/configuration). Cypress will
...
- The default value of `port` within a project's global
  [configuration](/app/references/configuration) has changed from `2020` to
  now being a random open port. You can still configure a specific `port` if
  needed within the [configuration](/app/references/configuration).
- We have upgraded the `Chromium` that runs headlessly on
  [cypress run](/app/references/command-line#cypress-run) to version `51`.
...
- Fixed regression caused by [0.16.2](#0-16-2) where a failed
  [`cy.contains()`](/api/commands/contains) would not be cancelled and would
...
- There is now an [`ignoreTestFiles`](/app/references/configuration)
...
- [cypress run](/app/references/command-line#cypress-run) now exits with the
...
- In the [Cypress CLI tool](/app/references/command-line) package version
...
- When running `cypress run` or in CI, Cypress will now automatically take a
  ...
  [`screenshotOnHeadlessFailure`](/app/references/configuration#Screenshots)
  to `false` in your configuration.
- Added new [`screenshotsFolder`](/app/references/configuration#Screenshots)
...
- `cy.route()` can now accept a single function and/or you can pass a function
  to the `response` property. This lazily evaluates routing responses. Great for
  referencing fixtures. Addresses
...
- `cy.exec()` now outputs additional `stderr` and `stdout`
...
- Updating through the Desktop App in **Linux** does not work. To update,
  run [cypress install](/app/references/command-line) from the command line.
...
- `0.16.0` marks a significant change for Cypress. Before this we only issued
  ...
  [browser launch options](/app/references/launching-browsers).
...
- Browser sessions are spawned independently of your existing profiles and we've
  ...
  and other features which get in the way of testing. Read more about
...
- There is a new `cy.exec()` command that can execute any
  arbitrary system command. Additionally there is a new
  [`execTimeout`](/app/references/configuration#Timeouts) configuration
...
- Using [`.its()`](/api/commands/its) on a function now accesses its properties
  instead of automatically calling the function. Fixes
...
- Better error handling of unauthorized users attempting to login to Cypress
  with improved
  [Login documentation](/app/get-started/install-cypress).
...
- Cypress will now
  [display the **resolved** configuration values when you open a project](/app/references/configuration#Resolved-Configuration).
  This tells you the source of all config values.
- The latest version of the [Cypress CLI](/app/references/command-line) now
  accepts passing arguments to
  [cypress open](/app/references/command-line#cypress-open). Example:
...
- [Environment Variables](/app/guides/environment-variables) that match any
  ...
  configuration option. We'll also automatically normalize environment
...
- The `-s` or `--spec` option now works correctly. You now must pass a relative
  or absolute path to your spec file. This is much less confusing, works with
  bash autocompletion, and will support `unitFolder` later
...
- The
  [error message when Cypress detects that a test has ended early](/app/references/error-messages)
  (there are still commands left in the queue) now displays a list of these
  commands with a much improved explanation.
- There is now a new [configuration option](/app/references/configuration):
  ...
  tests.
- You can now set the default [`reporter`](/app/references/configuration) in
  `cypress.json` for use during `cypress run` or in CI.

**Bugfixes:**

- The [--reporter](/app/references/command-line#cypress-run) CLI option is now
  working again.
...
- Renamed [configuration](/app/references/configuration) option
...
- If you choose to install Cypress from the
  [CLI Tool](/app/references/command-line) you must update to the latest version
  `0.9.1`. Just run `npm install -g cypress` and then you can run
  [cypress install](/app/references/command-line). You don't need to do anything
...
- Cypress will now automatically prompt you to add a project when using
  [cypress run](/app/references/command-line#cypress-run) on a project that has
  not yet been added.
...
- Using [cypress run](/app/references/command-line#cypress-run) in OSX now works
  again.
...
- Added `waitForAnimations` and `animationDistanceThreshold`
  [configuration options](/app/references/configuration#Actionability).
...
- Added `responseTimeout` [configuration](/app/references/configuration)
  value.
...
- [Documentation to CLI](/app/references/command-line).
...
- There is now a new [Error Messages Page](/app/references/error-messages)
...
- There are now [Getting Started](/app/get-started/install-cypress)
  docs including [configuration](/app/references/configuration) for
  `cypress.json`.
...
- `Environment Variable` support has been added and can be accessed in your
  ...
  [Environment Variable](/app/guides/environment-variables).
...
- You can now disable the force sending of `404` to all XHR's which are not
  stubbed. This mixes and matches, leaving some requests stubbed and others
  hitting your server and responding normally.
...
- Fixed situation where an `Uncaught Error` was not being properly handled.
  Cypress would incorrectly continue to retry commands instead of cancelling
...
- Stopped sending CI data when
  [cypress run](/app/references/command-line#cypress-run) was issued.
...
- [`.pause()`](/api/commands/pause) is now noop during
  [cypress run](/app/references/command-line#cypress-run) or `cypress ci`.
...
- Created new debugging command: [`.pause()`](/api/commands/pause) which will
  pause executing commands and resume or step into the next command one at a
  time.
...
- Prevented edge case with [`cy.fixture()`](/api/commands/fixture) where it
  would not be able to be cancelled without throwing an unhandled rejection
  error.
...
- Using `cy.server()` across multiple tests before a
...
- Cypress is currently seeking to raise a Series A. This will enable us to grow
  ...
  [send them our way](mailto:support@cypress.io).
...
- Extensive memory profiling has been done and Cypress has implemented several
  ...
  (which walk back in time through DOM snapshots, or access objects from
  previous tests) could exhaust all available memory in previous versions.
...
- All elements are now logged out as real DOM elements instead of jQuery wrapped
  elements. This has several upsides. In Chrome these elements are immediately
  interactive, drilling into their contents, displaying the element box model on
  hover, etc. This prevents you from having to expand the
...
- [`.submit()`](/api/commands/submit) will actually submit the form for real
  ...
  able to be cancelled or have its returnValue set to false and will not submit
...
- [`cy.wait()`](/api/commands/wait) now accepts an alias property called
  ...
  XHR is initially requested, before it is responded to. This tests things while
...
- Added functionality which enables `cy.server()` and `cy.route()` to be created
  ...
  to the next page which is visited. This stubs requests which happen on page
  load. Suggested in
...
- `javascripts` files can now use `snockets`, `browserify`, or `requirejs`.
