...
Cypress 16 makes your tests look more like production and run faster while doing it. In Chrome, Chromium, and Edge, your application now talks to your server over the native browser network, so it negotiates HTTP/2 or HTTP/3 exactly as it does for your users instead of being downgraded to HTTP/1.1. Your own TLS certificate is the one the browser validates. Two performance features that used to be opt-in experiments are now on by default for everyone: the `checkVisibility()`-based visibility algorithm and browser memory management. Typing no longer pays a 10ms tax per keystroke. And reads that used to fail on a race, such as `cy.getCookie()` and `cy.getAllLocalStorage()`, are now retry-able queries, so `cy.getCookie('token').should('exist')` waits for the cookie instead of failing on the first look.

**Breaking Changes:**

:::info

Refer to the [16 Migration Guide](/app/references/migration-guide#Migrating-to-Cypress-160) for help migrating your code.
