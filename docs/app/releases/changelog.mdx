...
- [`blockHosts`](/app/references/configuration#blockHosts) can no longer be set via [`Cypress.config()`](/api/cypress-api/config) during test execution. Setting it this way affected the _next_ test rather than the current one. Set `blockHosts` in the test configuration of a `describe`, `context`, or `it` block instead. See the [Migration Guide](/app/references/migration-guide#blockHosts-can-no-longer-be-set-with-Cypressconfig-during-test-execution).
