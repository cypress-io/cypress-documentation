...
- [Re-run optimization](/cloud/features/smart-orchestration/rerun-optimization) in Cypress Cloud can now re-run only the tests that failed in a previous recorded run, not just the specs that contained failures. Select **Run only failed tests** in project or organization Smart Orchestration settings. Addressed in [#34594](https://github.com/cypress-io/cypress/pull/34594).
