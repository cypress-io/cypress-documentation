...
### <Icon name="angle-right" /> Why doesn't cy.intercept() match my cy.request() calls?
...
### <Icon name="angle-right" /> Why don't cy.request() calls appear in the browser's Network tab?

For the same reason: the request is made from Node, not from the browser, so
the browser has nothing to report. To inspect it, click the `cy.request()` entry
in the [Command Log](/app/core-concepts/open-mode#Command-Log) and Cypress
prints the full request and response to the browser console.
...
### <Icon name="angle-right" /> How do I send an authentication token with cy.request()?

Pass it in the `headers` option, and keep the credential itself in an
...
### <Icon name="angle-right" /> How do I test an endpoint that returns a 4xx or 5xx status code?

By default `cy.request()` fails the test on any response outside `2xx` and
`3xx`. When the error response is what you are testing, set
`failOnStatusCode: false` so the response is yielded instead.
