...
`cypress tap` is a set of Cypress CLI commands that interact with a Cypress
session already running in open mode. From another terminal you can list the
...
poll until it reports `passed` or `failed`, and `cypress tap reporter --test <id>`
...
The session-level commands (`instances`, `status`, `specs`, and `run`) work
...
To list every Cypress session the CLI can reach, run `cypress tap instances`,
and pass a listed pid to `--instance` to target one.
...
tree, and `cypress tap inspect <selector>` returns one element's tag,
