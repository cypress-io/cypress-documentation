...
[Cypress](/app/core-concepts/cypress-app) is a free, downloadable and open
...
You can [install Cypress](/app/get-started/installing-cypress) on Mac,
Linux, and Windows. For additional information, please see our
[System requirements](/app/get-started/installing-cypress#System-requirements).
...
- With Selenium you get either 100% simulated events (with Selenium RC) or 100%
  ...
  support is on our [roadmap](/app/references/roadmap).
...
- [Changelog at Cypress Docs](/app/references/changelog)
...
#### <Icon name="angle-right" /> What information is captured or transmitted when using Cypress?

Cypress runs locally so no data is sent to Cypress aside from exception data,
which can be disabled using the instructions
[here](https://docs.cypress.io/app/references/advanced-installation#Opt-out-of-sending-exception-data-to-Cypress).

#### <Icon name="angle-right" /> Can I write API tests using Cypress? <E2EOnlyBadge />
...
[Conditional Testing Guide](/app/core-concepts/conditional-testing) which
...
Read more about the
[best practices for selecting elements here](/app/references/best-practices#Selecting-Elements).
...
[cypress run](/app/guides/command-line#cypress-run) by
[passing a glob to the `--spec` flag](/app/guides/command-line#cypress-run-spec-lt-spec-gt)
...
[cypress run](/app/guides/command-line#cypress-run).
...
[Read the Environment Variables guide.](/app/guides/environment-variables)
...
[best practice](/app/references/best-practices#Visiting-External-Sites) here.
...
- [`cy.task()`](/api/commands/task) to run code in Node via the
  [setupNodeEvents](/app/tooling/plugins-guide#Using-a-plugin) function
- [`cy.exec()`](/api/commands/exec) to execute a shell command

[Check out the "Node Modules" example recipe.](/examples/recipes#Fundamentals)
...
[here](/app/core-concepts/writing-and-organizing-tests#Hooks).
...
- see
  ...
  blog post and [Redux Testing](/examples/recipes#Blogs) recipe.
- see
  ...
  blog post and [Vue + Vuex + REST Testing](/examples/recipes#Blogs) recipe.
...
#### <Icon name="angle-right" /> How do I prevent the application from opening a new browser window?
...
option described in our [Experiments](/app/references/experiments) page. You
...
[`cypress` npm package](/app/guides/command-line).
...
[select the Electron browser](/app/guides/launching-browsers#Electron-Browser),
...
[Visual Testing guide](/app/tooling/visual-testing) and the following blog
posts.
...
[code coverage](/app/tooling/code-coverage) from tests.

You can also test Next.js apps in component testing. See the
[Framework Configuration Guide on Next.js](/app/component-testing/react/overview#Nextjs)
for more info.
...
For other snapshot plugins, search the [Plugins](/plugins) page.
...
#### <Icon name="angle-right" /> What frameworks does component testing support?

We support [Angular](/app/component-testing/angular/overview),
[React](/app/component-testing/react/overview),
[Svelte](/app/component-testing/svelte/overview), and
[Vue](/app/component-testing/vue/overview) currently for component testing.
...
#### <Icon name="angle-right" /> When should I use component testing vs end-to-end testing?
