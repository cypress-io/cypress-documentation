...
However, **`cy.intercept()` does not intercept WebSocket connections** — it
cannot match the connection (even with `resourceType: 'websocket'`) or stub,
spy on, or wait for individual frames/messages. If your tests need to control
the messages sent over a WebSocket connection, see the
