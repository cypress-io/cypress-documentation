...
sidebar_position: 30
slug: /app/references/experiments
---
...
so your application runs under test with CSP enabled. Valid values for this
...
values and the `modifyObstructiveCode` option. Using this option in combination
with the `experimentalCspAllowList` option can cause a mismatch between the
original hashed directive value, and the modified HTML or JS value.
...
Normally, test retries stop on the first passing attempt. And the final test result of any flaky test is always "passing", regardless of how many prior attempts failed. The following experimental settings for retries give you more control over the retries process.
...
| Version                                      | Changes                                                                                                                                                                |
| -------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [16.0.0](/app/references/changelog#16-0-0)   | Removed `experimentalSourceRewriting`. The default regex-based source rewriting is now the only implementation.                                                        |
| [16.0.0](/app/references/changelog#16-0-0)   | Removed `experimentalFastVisibility`. The modern visibility algorithm is now the default.                                                                              |
| [16.0.0](/app/references/changelog#16-0-0)   | Removed `experimentalMemoryManagement`. Memory management graduated to [`manageBrowserMemory`](/app/references/configuration#manageBrowserMemory), enabled by default. |
...
| [14.0.0](/app/references/changelog#14-0-0)   | Removed `experimentalFetchPolyfill`, `experimentalJustInTimeCompile`, and `experimentalSkipDomainInjection` and made it default behavior.                              |
