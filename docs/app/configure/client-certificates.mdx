...
sidebar_position: 50
slug: /app/references/client-certificates
---
...
To configure certificates that the browser should trust when connecting to a server (such as a self-signed development certificate), see [`trustedCertificates`](/app/references/configuration#trustedCertificates). The `clientCertificates` option is specifically for mutual TLS (mTLS), where Cypress presents client certificates to authenticate against an endpoint.
