...
All you need to do after [recording test runs](/app/cloud/getting-started) is log in to your Cypress Cloud account and leverage these new debugging capabilities. No additional software, configuration or changes to the existing test recording process is required. For example, you do not need to edit your [Cypress configuration](/app/references/configuration) file or pass any flags or new commands when recording via [the command line](/app/guide/command-line#Commands) in order to enable or use Test Replay.
...
The Runner UI is visible during `cypress open` mode, but is also rendered ([headless](/app/guide/command-line#Options)-ly) during `cypress run` so that it displays within captured screenshots or videos, to assist in debugging CI failures.
...
If you'd like to render the Runner UI, you can turn on this behavior by passing `--runner-ui` as a CLI flag. You may encounter a slower performance when turning this on, especially in lower resourced machines. Learn more about the `--runner-ui` [command line option](/app/guide/command-line#cypress-run-runner-ui).
...
- [Cypress Video Artifacts](/app/guide/screenshots-and-videos#Videos)
