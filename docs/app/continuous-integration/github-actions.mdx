...
run Cypress tests. This GitHub Action configuration is placed within
...
- Finally, our Cypress GitHub Action will:
  ...
  - Run the Cypress tests within our GitHub repository. Without a `browser`
    parameter, this uses the bundled Electron browser, which is
    [deprecated](/app/references/migration-guide#Migrating-away-from-the-Electron-browser);
    see
    [Testing on GitHub with Installed Browsers](#Testing-on-GitHub-with-Installed-Browsers)
    to run in Chrome or another installed browser.
...
the above example to select Chrome instead of the deprecated default browser
Electron, add `browser: chrome` as follows.
...
and if you specify a browser other than the deprecated Electron default in parallel mode,
