...
- Finally, our `buildspec.yml` configuration will:
  ...
  - Run the Cypress tests within our GitHub repository. This basic setup uses
    the bundled Electron browser, which is
    [deprecated](/app/references/migration-guide#Migrating-away-from-the-Electron-browser);
    install a browser and pass `--browser` (for example, `--browser chrome`) to
    run in an installed browser instead.
