...
- [cypress/base](https://github.com/cypress-io/cypress-docker-images/tree/master/base) is the entry-level Cypress Docker image. It includes the bundled Electron browser, which is [deprecated](/app/references/migration-guide#Migrating-away-from-the-Electron-browser) as of Cypress 16.0 — to test in an installed browser such as Chrome or Firefox, use the `cypress/browsers` image described below.
