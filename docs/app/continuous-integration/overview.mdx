...
You can also set custom environment variables for use in your tests. Prefix
the variable with `CYPRESS_` in your CI environment:

```shell
export CYPRESS_EXTERNAL_API_SERVER=https://corp.acme.co
```

Cypress strips the prefix, and your tests can read the value with
[`cy.env()`](/api/commands/env):

```javascript
cy.env(['EXTERNAL_API_SERVER']).then(({ EXTERNAL_API_SERVER }) => {
  cy.request({
    method: 'POST',
    url: `${EXTERNAL_API_SERVER}/users/1`,
    body: {
      foo: 'bar',
      baz: 'quux',
    },
...
For public, non-sensitive values that need synchronous access, you can instead
pass them with the `--expose` CLI flag and read them with
