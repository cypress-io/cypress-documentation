...
- [cypress/browsers](https://github.com/cypress-io/cypress-docker-images/tree/master/browsers) builds on the
  ...
  For `Linux/arm64` images it adds **Chrome** browsers from version `151` and above
  and **Firefox** browsers from version `136` and above.
  The Edge browser is currently not available for `Linux/arm64`.
  A corresponding image `<tag>` allows selection of the combined Node.js and browser versions.
  The version tags of browsers which are not available on the `Linux/arm64` platform are empty place-holders only,
