...
to run Cypress tests. This GitLab CI configuration is placed within
`.gitlab-ci.yml`.
...
- Finally, our scripts will:
  ...
  - Run the Cypress tests within the GitLab repository. Because `node:latest`
    has no browser installed, this basic setup uses the bundled Electron
    browser, which is
    [deprecated](/app/references/migration-guide#Migrating-away-from-the-Electron-browser);
    see [Testing with Cypress Docker Images](#Testing-with-Cypress-Docker-Images)
    below to run in an installed browser like Chrome or Firefox.
