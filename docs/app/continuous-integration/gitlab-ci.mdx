---
title: 'Run Cypress in GitLab CI: step-by-step guide'
...
## Displaying test results in GitLab

GitLab can display Cypress results in the
[merge request widget and pipeline test report](https://docs.gitlab.com/ci/testing/unit_test_reports/)
when the job produces a JUnit XML report referenced under
`artifacts:reports:junit`.

Configure Cypress to output JUnit XML (see
[Reporters](/app/tooling/reporters#Reporter-Options)), then upload the report
and the screenshots as artifacts and point GitLab at the generated files.
...
test:
  ...
  script:
    ...
    # start the server in the background
    - npm start &
    # run Cypress tests, writing one JUnit XML file per spec
...
To also show the screenshot of each failed test directly in the GitLab test
report, see
[Including screenshots in JUnit reports](/app/tooling/reporters#Including-screenshots-in-JUnit-reports).
