...
folder—**every** file and **every** nested subfolder, regardless of file type.
It does _not_ only remove screenshots or videos, and it does _not_ limit itself
to the top level of the folder. The folder itself is preserved; only its
...
Because Cypress removes the _entire contents_ of these folders, be careful when
nesting one run's output inside another's. For example, if a daily run writes to
`cypress/screenshots` and a second run writes to `cypress/screenshots/otherRun`,
then the daily run will delete the `otherRun` subfolder (and everything inside
it) when it trashes `cypress/screenshots`.

To keep assets from separate runs, either point each run at a folder that is
**not** nested inside another run's folder (e.g. `cypress/screenshots/daily` and
`cypress/screenshots/otherRun` as siblings, with neither run using the parent
`cypress/screenshots` directly), or set `trashAssetsBeforeRuns` to `false` and
