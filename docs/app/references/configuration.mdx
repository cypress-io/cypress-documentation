...
| `keystrokeDelay`             | `10`    | Time, in milliseconds, between each keystroke when typing with [`cy.type()`](/api/commands/type). Set to `0` to disable the delay. Must be `0` or a positive number.            |
