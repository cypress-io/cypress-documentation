...
| `manageBrowserMemory`  | `true`                                | Whether Cypress clears the browser's heap between tests so memory-heavy suites don't exhaust the browser and crash. Only applies to Chromium-based browsers (Chrome, Edge); no effect in Firefox or WebKit. See [Manage memory accumulation](/app/guides/test-performance#Manage-memory-accumulation).    |
