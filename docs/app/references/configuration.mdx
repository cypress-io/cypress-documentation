...
| `allowCypressEnv`      | `true`                                | Whether to allow the use of `Cypress.env()` in your tests. Please see our [Migration Guide](/app/references/migration-guide#Migrating-away-from-Cypressenv) on migrating from `Cypress.env()` to [`cy.env()`](/api/commands/env) and [`Cypress.expose()`](/api/cypress-api/expose) |
| `env`                  | `{}`                                  | Any secret values to be set as environment variables. Access these values in tests using [`cy.env()`](/api/commands/env).                                                                                                                                                                                                                           |
