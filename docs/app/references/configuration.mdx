...
| `disallowCypressEnv`   | `false`                               | Whether to disallow the use of [`Cypress.env()`](/api/cypress-api/env) in your tests. Please see our [Migration Guide](/app/guides/migration/cypress-env-to-cy-env) on migrating from [`Cypress.env()`](/api/cypress-api/env) to [`cy.env()`](/api/commands/env) and why users should enable this option. |
