...
[migration guide](/app/references/migration-guide#Migrating-to-Cypress-100).
...
| `allowCypressEnv`      | `true`                                | Whether to allow the use of [`Cypress.env()`](/api/cypress-api/env) in your tests. Please see our [Migration Guide](/app/references/migration-guide#Migrating-away-from-Cypressenv) on migrating from [`Cypress.env()`](/api/cypress-api/env) to [`cy.env()`](/api/commands/env) and [`Cypress.expose()`](/api/cypress-api/expose) |
...
[e2e](/app/references/configuration#e2e) testing-type specific object.
