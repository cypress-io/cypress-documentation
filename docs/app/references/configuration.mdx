...
`blockHosts` cannot be overridden with `Cypress.config()` while a test is
executing, because doing so would affect the _next_ test rather than the current
one. Set it in [test-specific configuration](#Test-specific-Configuration)
instead.
...
| Version                                      | Changes                                                                                                                                                                         |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [16.0.0](/app/references/changelog#16-0-0)   | Updated `blockHosts` to no longer be settable with `Cypress.config()` during test execution.                                                                                    |
