...
For `cypress.config.js`, the module syntax used depends on the
[`type`](https://nodejs.org/api/packages.html#type) field in your project's
`package.json`:

| `package.json` `type`   | Config file syntax                    |
| ----------------------- | ------------------------------------- |
| `"module"`              | ESM (`import`/`export default`)       |
| `"commonjs"` or not set | CommonJS (`require`/`module.exports`) |

Cypress additionally supports config files with `.mjs` or `.cjs` extensions to
explicitly set the module format regardless of `package.json`.
...
syntax explicitly. All JavaScript config examples in our docs use the CommonJS
format.
...
The `cypress.config.js` tab below shows CommonJS syntax (`require`/`module.exports`),
which applies when `"type"` in `package.json` is `"commonjs"` or not set. If
your project sets `"type": "module"`, your `cypress.config.js` will instead use
ESM syntax (`import`/`export default`) — the same syntax shown in the
`cypress.config.ts` tab.
