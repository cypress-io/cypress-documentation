...
| `removeSRIAttributes`   | `false` | Whether Cypress will remove [Subresource Integrity (SRI)](https://developer.mozilla.org/en-US/docs/Web/Security/Subresource_Integrity) `integrity` attributes from `<script>` and `<link>` elements so that proxy-rewritten resources are not blocked. [Please read the notes for more information on this setting.](#removeSRIAttributes)            |
...
resources load normally under test. It applies to **both first-party and
third-party** resources, and covers `integrity` defined in any of the following
ways:
...
which only removes SRI for third-party resources. `removeSRIAttributes`
additionally handles first-party resources and the runtime property-assignment
pattern.

The option is additive and backwards-compatible. It defaults to `false`, so
default behavior is unchanged: `experimentalModifyObstructiveThirdPartyCode`
continues to strip SRI for third-party resources, and `modifyObstructiveCode`
alone still strips nothing. Enable `removeSRIAttributes` only if your application
uses SRI and you see resources blocked under test.

:::caution

Removing SRI attributes weakens a security control that protects against tampered
resources. Enable this option only for applications under test.
