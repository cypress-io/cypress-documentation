...
Note that the registry fallback only applies when running your tests. When
downloading Cypress for the first time, the `cypress` command line tool _does
not_ read proxy settings from the Windows registry. If you need to configure a
proxy for the installation to work — which is also the only way to define a
proxy for `cypress install` — you must set the environment variables shown
above.
...
disables the proxy entirely. This can be useful to turn off a proxy that is
...
is **not** applied to `NO_PROXY` at runtime, so set `NO_PROXY` directly if you
need to bypass the proxy for specific domains.

To bypass the proxy for certain domains, a `NO_PROXY` environment variable can
be set to a comma-separated list of domain names to not proxy traffic for.
...
configuration, when **running your tests**, Cypress automatically reads from npm
config's [`cafile`](https://docs.npmjs.com/cli/v8/using-npm/config#cafile) and
...
binary download** (`cypress install`) only reads npm's `cafile` and `ca`
config values (via the `npm_config_cafile` and `npm_config_ca` environment
variables) — it does **not** read `NODE_EXTRA_CA_CERTS`.
...
if you rely on a separate HTTPS proxy it will not appear in this view even though
Cypress still uses it.

:::

You can also view your proxy settings from the command line. Running
`cypress info` prints the `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` values
Cypress detects, making it a useful alternative to the Settings UI — especially
in CI or other headless environments:
