---
title: 'Environment & Runtime Variables'
description: 'Learn how to manage environment and runtime variables in Cypress. Understand when to use cy.env() vs Cypress.expose(), how values flow into tests, and how to configure them safely across environments.'
---

<ProductHeading product="app" />

# Environment & Runtime Variables in Cypress

:::caution

<strong>Difference between OS-level and Cypress environment variables</strong>

In Cypress, "environment variables" are sensitive variables that are accessible via
`cy.env()` in your tests or the browser. These are not the same as OS-level environment variables.
However,
...
<strong>allowCypressEnv</strong>

Starting with Cypress `15.10.0`, users are unable to override test/suite environment variables when the [`allowCypressEnv`](/app/references/configuration#Global) option is set to `false`.

Cypress recommends migrating to using [`cy.env()`](/api/commands/env) instead.
