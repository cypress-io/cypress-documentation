...
The `experimentalSourceRewriting` configuration option has been removed. It enabled an experimental AST-based source rewriter as an alternative to Cypress's default regex-based rewriter, which has now been removed. The default rewriter handles every case the AST-based rewriter did and more, without the performance issues the AST-based approach could introduce, so removing the option does not change how your tests behave.
...
If you enabled `experimentalSourceRewriting` to strip Subresource Integrity (SRI) attributes from rewritten resources, use the [`removeSRIAttributes`](/app/references/configuration#removeSRIAttributes) configuration option instead.
