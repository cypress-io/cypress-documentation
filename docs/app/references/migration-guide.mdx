...
Many community plugins have already adopted the new API. Update to at least the version listed below so the plugin no longer calls `Cypress.env()` and you no longer see the deprecation warning. Some plugins also moved their configuration from the `env` object to the `expose` object, so check each plugin's documentation for its new configuration format.

| Plugin                                                                                              | Version | Migration notes                                                                                                                                                                         |
| --------------------------------------------------------------------------------------------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [@bahmutov/cy-api](https://github.com/bahmutov/cy-api)                                              | 2.3.0+  | Reads its options with `Cypress.expose()`.                                                                                                                                              |
| [cypress-fail-fast](https://github.com/javierbrea/cypress-fail-fast)                                | 8.0.0+  | Breaking change: configuration moves from `env` to `expose`, and option names change (for example, `FAIL_FAST_STRATEGY` becomes `failFastStrategy`).                                    |
| [cypress-firebase](https://github.com/prescottprue/cypress-firebase)                                | 4.4.0+  | Reads values with `cy.env()`, but still falls back to `Cypress.env()` for values it can't find (such as values set at runtime). Setting `allowCypressEnv: false` disables the fallback. |
| [cypress-image-diff-js](https://github.com/uktrade/cypress-image-diff)                              | 2.7.0+  | Uses `Cypress.expose()` and the `expose` config; `Cypress.env()` is only used as a fallback on older Cypress versions.                                                                  |
| [cypress-localstorage-commands](https://github.com/javierbrea/cypress-localstorage-commands)        | 2.3.0+  | Uses `Cypress.expose()` and the `expose` config; `Cypress.env()` is only used as a fallback on older Cypress versions.                                                                  |
| [cypress-maildev](https://github.com/Clebiez/cypress-maildev)                                       | 2.0.0+  | Configuration (`MAILDEV_PROTOCOL`, `MAILDEV_HOST`, `MAILDEV_API_PORT`) moves from `env` to `expose`.                                                                                    |
| [cypress-mailosaur](https://github.com/mailosaur/cypress-mailosaur)                                 | 4.0.0+  | Reads its API key and base URL with `cy.env()`.                                                                                                                                         |
| [cypress-mailpit](https://github.com/pushpak1300/cypress-mailpit)                                   | 2.0.0+  | Reads its server URL with `Cypress.expose()` and its credentials with `cy.env()`.                                                                                                       |
| [cypress-mongodb](https://github.com/Zaista/cypress-mongodb)                                        | 7.0.0+  | Reads its connection settings with `cy.env()`.                                                                                                                                          |
| [cypress-ntlm-auth](https://github.com/bjowes/cypress-ntlm-auth)                                    | 4.2.4+  | Reads its settings with `cy.env()`; `Cypress.env()` is only used as a fallback on older Cypress versions.                                                                               |
| [cypress-plugin-api](https://github.com/filiphric/cypress-plugin-api)                               | 2.12.0+ | Set the plugin options via the `expose` config. Options that aren't exposed still fall back to `Cypress.env()` and will trigger the deprecation warning.                                |
| [cypress-plugin-grep-boxes](https://github.com/dennisbergevin/cypress-plugin-grep-boxes)            | 3.0.0+  | Configuration moves from `env` to `expose`.                                                                                                                                             |
| [cypress-plugin-last-failed](https://github.com/dennisbergevin/cypress-plugin-last-failed)          | 3.0.0+  | Requires Cypress 15.10+. The `cypress run-failed` CLI still passes grep settings through `CYPRESS_`-prefixed OS environment variables, so also update `@cypress/grep` to 6.0.0+.        |
| [cypress-plugin-xhr-toggle](https://github.com/filiphric/cypress-plugin-xhr-toggle)                 | 1.3.0+  | Set `hideXhr` via the `expose` config. If it isn't exposed, the plugin still falls back to `Cypress.env('hideXhr')` and will trigger the deprecation warning.                           |
| [cypress-visual-regression](https://github.com/cypress-visual-regression/cypress-visual-regression) | 6.0.0+  | Breaking change: configuration moves from `env` to `expose`.                                                                                                                            |
| [cypress-voice-plugin](https://github.com/dennisbergevin/cypress-voice-plugin)                      | 2.0.0+  | Configuration moves from `env` to `expose`.                                                                                                                                             |
| [cypress-watch-and-reload](https://github.com/bahmutov/cypress-watch-and-reload)                    | 2.1.0+  | Requires Cypress 15.11+. Uses `Cypress.expose()` and the `expose` config.                                                                                                               |

If a plugin you depend on isn't listed here, search its shipped code and documentation for `Cypress.env()`. If the plugin hasn't migrated yet, check its repository for an open issue or release that adds `Cypress.expose()` or `cy.env()` support before setting `allowCypressEnv: false`.
