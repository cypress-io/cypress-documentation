...
The `experimentalSourceRewriting` configuration option has been removed. The experimental AST-based source rewriting it enabled has been removed in favor of the default regex-based source rewriting, so default behavior is unchanged. If your `cypress.config` still includes it, Cypress will print a warning telling you to remove it — no behavior change, just remove the key.
