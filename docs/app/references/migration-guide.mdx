...
With Test Replay enabled, the Cypress Runner UI is hidden by default when recording a run to the Cloud. If the Runner UI is needed during the run, you can enable it by passing [`--runner-ui`](/app/guides/command-line#cypress-run-runner-ui) to the [`cypress run`](/app/guides/command-line#cypress-run) command.
...
- You may use the `--config-file` command line flag or the `configFile`
  [module API](/app/guides/module-api) option to specify a `.js` or `.ts`
...
See the
[docs for `cypress run --browser`](/app/guides/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
