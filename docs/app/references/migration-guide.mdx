...
[Test Replay](/app/cloud/test-replay) is enabled by default in `v13` of the Cypress App.
...
With Test Replay enabled, the Cypress Runner UI is hidden by default when recording a run to the Cloud. If the Runner UI is needed during the run, you can enable it by passing [`--runner-ui`](/app/guide/command-line#cypress-run-runner-ui) to the [`cypress run`](/app/guide/command-line#cypress-run) command.

You can [opt-out](/app/cloud/test-replay#Opt-out-of-Test-Replay) of this feature in Cloud project-level settings.

### Video updates

#### `video` is set to `false` by default

You can continue recording video by setting `video` to `true` either in your Cypress configuration or via [overriding options](/app/references/configuration#Overriding-Options). This can be useful if you want video locally or want video for some other reason, like in non-Chromium browsers where [Test Replay](/app/cloud/test-replay) is not available.
...
If you want to prevent a passing test from uploading to the Cloud, we recommend deleting the video using our [guide with code examples to discard captured video of passing tests](/app/guide/screenshots-and-videos#Control-which-videos-to-keep-and-upload-to-Cypress-Cloud).
...
[discouraged](/app/references/best-practices#Having-Tests-Rely-On-The-State-Of-Previous-Tests)
...
[Legacy Configuration guide](/app/references/legacy-configuration).
...
- You may use the `--config-file` command line flag or the `configFile`
  [module API](/app/guide/module-api) option to specify a `.js` or `.ts`
...
[Test Retries](/app/guide/test-retries) doc for more information on how this
...
failure. Read the [Test Retries](/app/guide/test-retries) doc for more
...
[test retries](/app/guide/test-retries), the structure of each run's `runs`
...
See the
[docs for `cypress run --browser`](/app/guide/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
