...
separate install, but maintaining it as a test browser has grown costly and the
trade-offs no longer favor keeping it:
...
- **It carries separate maintenance.** The bundled browser needs its own
  security, configuration, and compatibility upkeep that is independent of the
  installed browsers Cypress already supports.
...
warning (error code `BROWSER_ELECTRON_DEPRECATED`) in `cypress run` output and in
the Cypress app. After Electron is removed, runs that target it — whether through
`--browser electron`, a `defaultBrowser: 'electron'` setting, or an implicit
fallback to Electron when no browser is specified — will fail with a clear error.
...
1. **Install a supported browser** in your local and CI environments (for
   ...
   The Cypress [Docker images](/app/continuous-integration/overview#Cypress-Docker-Images)
   include browsers if you don't want to install one yourself.
...
1. **Update your CI configuration and examples.** If your pipeline relied on the
   implicit Electron default, switch it to an installed browser. This includes
   any [`cypress-io/github-action`](https://github.com/cypress-io/github-action)
   steps and Docker-based jobs that previously ran without a `browser` argument.
...
1. ✅ Update CI examples (GitHub Action, Docker) to target an installed browser
1. ✅ Verify your runs no longer emit the `BROWSER_ELECTRON_DEPRECATED` warning
