...
The experimental `experimentalMemoryManagement` configuration option has been removed. Its successor is the new [`manageBrowserMemory`](/app/references/configuration#Global) configuration option, which defaults to **`true`**. Memory management is now on by default for **all** users running Chromium-based browsers, not just those who opted into the experiment.

There is no alias and no value remapping between the two options. If your `cypress.config` still sets `experimentalMemoryManagement`, Cypress prints a warning and otherwise ignores the option.

**If you were setting `experimentalMemoryManagement: true`:** remove it. Memory management is now the default.

**If you were setting `experimentalMemoryManagement: false`:** you must replace it with `manageBrowserMemory: false`. Deleting it without adding the replacement silently opts you **into** memory management, since the removed option is ignored and the new option defaults to `true`.
...
```ts
{
  ...
  // Add this line to stay opted out
...
Unlike `numTestsKeptInMemory`, `manageBrowserMemory` cannot be overridden per test or per suite, or with `Cypress.config()`. It can only be set in your Cypress configuration or at the command line. The underlying behavior is unchanged from the experiment — Cypress samples memory usage and collects garbage when it crosses a threshold, not after every test. See [Manage memory accumulation](/app/guides/test-performance#Manage-memory-accumulation) for the narrow cases where turning it off makes sense.
