...
If your tests relied on the implicit 10ms delay between keystrokes, you can restore the previous behavior using any of these approaches. The more specific the override, the higher its precedence: a per-command `delay` overrides a test/suite-level `keystrokeDelay`, which overrides the global default set via [`Cypress.Keyboard.defaults()`](/api/cypress-api/keyboard-api).

#### Set `keystrokeDelay` globally via `Cypress.Keyboard.defaults()` (recommended)

Set the default in a [support file](/app/core-concepts/writing-and-organizing-tests#Support-file) so it applies to every test:
...
#### Set `keystrokeDelay` for a single test or suite

`keystrokeDelay` is a [test-configuration override](/app/references/configuration#Test-Configuration), so it can be scoped to an individual test or suite:

```javascript
it('types with a delay', { keystrokeDelay: 10 }, () => {
  cy.get('input').type('slow typing')
})
```
