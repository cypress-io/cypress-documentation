...
This deprecation is a precaution to reduce the risk of unintentionally exposing secrets in the browser-accessible state when run within the Cypress browser. There is no requirement to assume a breach. This is about ensuring safer defaults and preventing accidental exposure of sensitive data.
...
- You need the value in support files or plugin-style code outside command chains
- It's acceptable for the value to be accessible in the browser context

### Migrate sensitive values to `cy.env()`

`cy.env()` is read-only and asynchronous (it's a Cypress command).
...
### Migrate public configuration to `Cypress.expose()`
...
Once you've migrated all `Cypress.env()` invocations to either `cy.env()`, `Cypress.expose()`, or `cy.task()`, you should set `allowCypressEnv: false` in your Cypress configuration.
...
2. ✅ Replace each `Cypress.env()` call with either `cy.env()`, `Cypress.expose()`, or `cy.task()`
3. ✅ Update code to handle the asynchronous nature of `cy.env()` and `cy.task()`
