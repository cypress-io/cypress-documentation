...
<CopyPrompt
  ...
  prompt={`Read https://docs.cypress.io/llm/markdown/app/references/migration-guide/migrating-away-from-cypress-env.md, then migrate this project off Cypress.env(). cy.env() and Cypress.expose() require Cypress 15.10.0 or newer, so check my installed cypress version first: if it's on an older 15.x release, detect my package manager and update the cypress dependency to the latest 15.x, and if it's below Cypress 15, stop and tell me; majors should be upgraded one at a time first. Find every Cypress.env() usage, classify each value as sensitive or public, and migrate sensitive reads to cy.env() and public values to Cypress.expose() with a top-level expose block in my Cypress config. Check for --env CLI flags and plugins that rely on Cypress.env(), and show me the proposed changes before applying them. Once every Cypress.env() call is gone, add allowCypressEnv: false to my config and run my Cypress tests to confirm nothing broke, then summarize what changed and anything you couldn't safely automate.`}
/>
...
<CopyPrompt
  ...
  prompt={`Read https://docs.cypress.io/llm/markdown/app/references/migration-guide/migrating-to-cypress-15-0.md, then upgrade this project from Cypress 14 to 15. If the project isn't on Cypress 14.x yet, stop and tell me; majors should be upgraded one at a time. Before installing, verify my environment meets the requirements in that section (Node.js 20, 22, or 24+, a supported Linux distribution, and webpack 5 / Vite 5 / Angular 18+ if I use component testing) and flag anything unsupported. Detect my package manager, update the cypress dependency, and fix every breaking change in that section that affects my code and config. Finish by running npx cypress verify and my Cypress tests, then summarize what changed and anything you couldn't safely automate.`}
/>
...
<CopyPrompt
  ...
  prompt={`Read https://docs.cypress.io/llm/markdown/app/references/migration-guide/migrating-to-cypress-14-0.md, then upgrade this project from Cypress 13 to 14. If the project isn't on Cypress 13.x yet, stop and tell me; majors should be upgraded one at a time. Before installing, verify my environment meets the requirements in that section (Node.js 18+, macOS 11+, a supported Linux distribution, and, if I use component testing, that my framework and bundler versions are still supported) and flag anything unsupported. Detect my package manager, update the cypress dependency, and fix every breaking change in that section that affects my code and config. Finish by running npx cypress verify and my Cypress tests, then summarize what changed and anything you couldn't safely automate.`}
/>
...
<CopyPrompt
  ...
  prompt={`Read https://docs.cypress.io/llm/markdown/app/references/migration-guide/migrating-to-cypress-13-0.md, then upgrade this project from Cypress 12 to 13. If the project isn't on Cypress 12.x yet, stop and tell me; majors should be upgraded one at a time. Before installing, verify my Node.js version is 16+ (18+ recommended) and, if I use TypeScript, that it's 4.x or newer, and flag anything unsupported. Detect my package manager, update the cypress dependency, and fix every breaking change in that section that affects my code and config, calling out behavior changes like video recording being off by default. Finish by running npx cypress verify and my Cypress tests, then summarize what changed and anything you couldn't safely automate.`}
/>
...
<CopyPrompt
  ...
  prompt={`Read https://docs.cypress.io/llm/markdown/app/references/migration-guide/migrating-to-cypress-12-0.md, then upgrade this project from Cypress 11 to 12. If the project isn't on Cypress 11.x yet, stop and tell me; majors should be upgraded one at a time. Before installing, verify my Node.js version is 14, 16, or 18+ and flag anything unsupported. Detect my package manager, update the cypress dependency, and fix every breaking change in that section that affects my code and config, paying special attention to test isolation being enabled by default: audit my tests for state shared between tests and fix them or configure testIsolation as that section describes. Finish by running npx cypress verify and my Cypress tests, then summarize what changed and anything you couldn't safely automate.`}
/>
...
<CopyPrompt
  ...
  prompt={`Read https://docs.cypress.io/llm/markdown/app/references/migration-guide/migrating-to-cypress-11-0.md, then upgrade this project from Cypress 10 to 11. If the project isn't on Cypress 10.x yet, stop and tell me; majors should be upgraded one at a time. Detect my package manager, update the cypress dependency, and fix every breaking change in that section that affects my code and config. If I use component testing, update my devServer configuration and any removed mounting APIs (mountHook, unmount, mountCallback) as that section describes. Finish by running npx cypress verify and my Cypress tests, then summarize what changed and anything you couldn't safely automate.`}
/>
...
<CopyPrompt
  ...
  prompt={`Read https://docs.cypress.io/llm/markdown/app/references/migration-guide/migrating-to-cypress-10-0.md, then upgrade this project from Cypress 9 to 10. If the project isn't on Cypress 9.x yet, stop and tell me; majors should be upgraded one at a time. Detect my package manager and update the cypress dependency, then apply the configuration migration that section describes: convert cypress.json to a cypress.config file, move the plugins file into setupNodeEvents, rename support files and spec file locations to the new defaults, and update any removed config options or APIs my code uses. Finish by running npx cypress verify and my Cypress tests, then summarize what changed and anything you couldn't safely automate.`}
/>
