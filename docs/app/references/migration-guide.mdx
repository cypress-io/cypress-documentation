...
- How `cy.env()` provides a more secure alternative for sensitive values
- How to use `Cypress.expose()` as an alternative migration path for public/non-sensitive configuration
- When to choose `cy.env()` vs `Cypress.expose()` based on your use case
- Step-by-step migration examples for both APIs
- How to set `allowCypressEnv: false` to prevent future use of `Cypress.env()`

:::

Starting with Cypress `15.10.0`, `Cypress.env()` has been deprecated and will be removed in a future major version. This guide will help you migrate to either `cy.env()` (for sensitive values) or `Cypress.expose()` (for public/non-sensitive values), depending on your use case.
...
Whether `Cypress.env()` is used in your test code or not, the API is globally available in the browser, meaning that **all** of these values become accessible, including the sensitive `apiKey` and `dbPassword`.
...
cy.env(['username', 'password']).then((env) => {
  // env = { username: 'MY_USERNAME', password: 'MY_PASSWORD' }
})
```
...
Use this guide to determine which API is right for your use case:

**Use `cy.env()` when:**

- The values are **sensitive** (API keys, passwords, tokens, credentials, secrets)
- You need **privileged access** that doesn't expose values to the browser context
- You're accessing values within Cypress command chains (can use async patterns)
- Security is the primary concern

**Use `Cypress.expose()` when:**

- The values are **public/non-sensitive** (feature flags, API versions, public URLs, plugin configurations)
- You need **synchronous access** to configuration values
- You're working in plugins or support files that run outside of `cy` command contexts
- The values are safe to expose in the browser context

#### Comparison Table

| Feature            | `cy.env()`                                                                              | `Cypress.expose()`                                     |
| ------------------ | --------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| **Access Pattern** | Asynchronous (Cypress command)                                                          | Synchronous (direct access)                            |
| **Security**       | Privileged - values accessible in browser but wrapped in closures, not globally exposed | Public - values directly accessible in browser context |
| **Use Case**       | Sensitive environment variables                                                         | Public/non-sensitive configuration                     |
| **Context**        | Works within Cypress command chains                                                     | Works anywhere, including support files and plugins    |
| **Configuration**  | `env` key in `cypress.config.js`                                                        | `expose` key in `cypress.config.js`                    |
| **CLI Flag**       | `--env` or `-e`                                                                         | `--expose` or `-x`                                     |
| **Setting Values** | Read-only                                                                               | Can set at runtime (scoped to spec file)               |

#### Quick Decision Tree

```
Is the value sensitive (API keys, passwords, tokens)?
├─ YES → Use cy.env()
└─ NO → Does it need synchronous access?
    ├─ YES → Use Cypress.expose()
    └─ NO → Prefer cy.env() for security, but Cypress.expose() is acceptable
```

### How to Migrate to `cy.env()`

Migrating from `Cypress.env()` depends on whether your values are sensitive or public:

- **For sensitive values** (API keys, credentials, tokens): Migrate to `cy.env()` (see examples below)
- **For public/non-sensitive values** (feature flags, API versions, plugin configs): Consider migrating to `Cypress.expose()` for synchronous access (see [Using `Cypress.expose()` for Public Configuration](#Using-Cypressexpose-for-Public-Configuration))

The main difference with `cy.env()` is that it's asynchronous (it's a Cypress command) and requires you to specify which variables you want to retrieve.
...
### Using `Cypress.expose()` for Public Configuration

`Cypress.expose()` is an alternative migration path for **public/non-sensitive** configuration values that need synchronous access. Unlike `cy.env()`, `Cypress.expose()` provides direct, synchronous access to configuration values, making it ideal for plugin initialization and support file setup.

#### When to Use `Cypress.expose()`

Use `Cypress.expose()` when:

1. **Values are public/non-sensitive**: The configuration values are safe to expose in the browser context (e.g., feature flags, API versions, public URLs, plugin settings)
2. **Synchronous access is required**: You need immediate access to values without using `.then()` callbacks
3. **Plugin/support file initialization**: You're setting up plugins or support files that run outside of `cy` command contexts
4. **Browser context access is acceptable**: The values can be safely accessed by application code, third-party scripts, or browser extensions
...
`Cypress.expose()` values are accessible in the browser context. **Never** use `Cypress.expose()` for sensitive data.

:::

**Safe to expose:**

- Feature flags (e.g., `enableNewFeature: true`)
- API versions (e.g., `apiVersion: 'v2'`)
- Public URLs that are documented and don't reveal internal structure (e.g., `publicApiUrl: 'https://api.example.com'`)
- Plugin configurations (e.g., `pluginMode: 'development'`)
- Simple environment identifiers (e.g., `environment: 'staging'` - but only if it doesn't contain URLs or sensitive details)
...
- Secret keys or signing keys
- Any sensitive or confidential data

For sensitive values, always use `cy.env()` instead.

#### Configuration Methods

You can configure exposed values in several ways:

##### Method 1: Configuration File

Set exposed configuration variables in your `cypress.config.js` or `cypress.config.ts`:
...
```shell
cypress run -x FEATURE_FLAG=true,API_VERSION=v2
```
...
Exposed configuration variables set using `Cypress.expose()` are only in scope for the current spec file. Changes do not persist across spec files.
...
3. Runtime changes (via `Cypress.expose(key, value)` or `Cypress.expose(object)`)

#### Migration Examples for `Cypress.expose()`

Here are practical examples showing how to migrate from `Cypress.env()` to `Cypress.expose()` for public/non-sensitive configuration values.

##### Example 1: Migrating Plugin Configuration

Plugin configurations that are non-sensitive can use `Cypress.expose()` for synchronous access:
...
##### Example 2: Migrating Feature Flags

Feature flags are typically public configuration that can be safely exposed:

<Badge type="danger">Before: Cypress.env()</Badge>

```javascript
describe('Feature tests', () => {
  it('tests new feature when enabled', () => {
    const newFeatureEnabled = Cypress.env('NEW_FEATURE_ENABLED')
...
describe('Feature tests', () => {
  it('tests new feature when enabled', () => {
    const newFeatureEnabled = Cypress.expose('NEW_FEATURE_ENABLED')
...
##### Example 3: Migrating Public API URLs

Public API URLs that don't contain sensitive information can use `Cypress.expose()`:

:::caution

**What Makes an API URL "Public"?**

Only use `Cypress.expose()` for API URLs that:

- Are publicly documented and accessible
- Don't reveal internal infrastructure or routing details
- Don't contain sensitive query parameters or paths
- Are safe to expose in browser developer tools

If the URL contains internal endpoints, reveals system architecture, or could be used maliciously, use `cy.env()` instead.
...
export function makePublicRequest(endpoint) {
  return cy.request(`${publicApiUrl}${endpoint}`)
}
```
...
```javascript
// cypress/support/api.js
const publicApiUrl = Cypress.expose('PUBLIC_API_URL')
...
##### Example 4: Support File Initialization

Support files often need synchronous access to configuration during initialization:

:::caution

**Safe Values for `ENVIRONMENT`**

Only expose `ENVIRONMENT` if it contains simple, non-sensitive values like:

- `'staging'`, `'production'`, `'development'` (environment names)
- `'test'`, `'qa'` (test environment identifiers)

**Do NOT expose** `ENVIRONMENT` if it contains:

- Internal URLs or endpoints
- Configuration details that reveal infrastructure
- Any sensitive routing or connection information

If `ENVIRONMENT` contains sensitive information, use `cy.env()` instead.
...
// Configure API client with version
configureApiClient(apiVersion, environment)
```
...
##### Example 5: Getting All Exposed Values

You can get all exposed configuration variables at once:

```javascript
// Get all exposed values
const allExposed = Cypress.expose()
// => { featureFlag: true, apiVersion: 'v2', publicApiUrl: 'https://api.example.com' }

// Use specific values
if (allExposed.featureFlag) {
  // Feature is enabled
}
```

##### Example 6: Setting Values at Runtime

Unlike `cy.env()`, `Cypress.expose()` allows you to set values at runtime (scoped to the current spec file):

<Badge type="danger">Before: Cypress.env()</Badge>

```javascript
describe('Dynamic config tests', () => {
  it('updates configuration', () => {
    Cypress.env('FEATURE_FLAG', true)
    const flag = Cypress.env('FEATURE_FLAG') // => true
  })
})
```
...
describe('Dynamic config tests', () => {
  it('updates configuration', () => {
    Cypress.expose('FEATURE_FLAG', true)
    const flag = Cypress.expose('FEATURE_FLAG') // => true
  })

  it('value persists in same spec', () => {
    const flag = Cypress.expose('FEATURE_FLAG') // => true (from previous test)
  })
})
```
...
Values set with `Cypress.expose(key, value)` only persist within the current spec file. They do not persist across different spec files.
...
#### No Direct Migration Path for Setting Variables

If you were using `Cypress.env()` to **set sensitive** environment variables (not just read them), there is **no direct migration path** to `cy.env()`. The ability to set environment variables at runtime has been intentionally removed for security reasons.
...
**Alternatives for Setting Values at Runtime**

- **For public/non-sensitive values**: Use `Cypress.expose()` instead, which allows setting values at runtime (scoped to the current spec file). See [Using `Cypress.expose()` for Public Configuration](#Using-Cypressexpose-for-Public-Configuration) for details.

- **For sensitive runtime values**: Use `cy.task()` to store state in the Cypress config process. These values are not environment variables and are not exposed to the browser context directly. See [Option 6: Using `cy.task()` for Sensitive Runtime Variables](#Option-6-Using-cytask-for-Sensitive-Runtime-Variables) for details.
...
##### Option 6: Using `cy.task()` for Sensitive Runtime Variables

For **sensitive runtime variables** that need to be set during test execution, you can use `cy.task()` to store state in the Cypress config process. These are **not** environment variables and are **not** accessible through `cy.env()`. This approach is specifically for sensitive values that need to be set and retrieved at runtime.
...
**When to Use `cy.task()` for Runtime Variables**

- Use `cy.task()` when you need to set **sensitive** values at runtime (e.g., tokens obtained during test execution)
- These values are stored in the Node.js config process and are **not** exposed to the browser context directly
- This is **not** a replacement for environment variables—use it only for runtime-sensitive state that cannot be pre-configured
...
### Set `allowCypressEnv: false` Configuration

Once you've migrated all `Cypress.env()` invocations to either `cy.env()` or `Cypress.expose()`, you should set `allowCypressEnv: false` in your Cypress configuration. This will:
...
Add `allowCypressEnv: false` to your Cypress configuration file:
...
#### What Happens When Set to `false`

When `allowCypressEnv` is set to `false`:

- **`Cypress.env()` calls will throw an error**: Any attempt to use `Cypress.env()` in your tests will result in an error with a message directing you to use `cy.env()` or `Cypress.expose()` instead.
...
When `allowCypressEnv` is `false`, you cannot use the `env` option in test or suite configuration:

```javascript
// ❌ This will throw an error when allowCypressEnv is false
...
- **`cy.env()` is secure for sensitive values**: It only retrieves the specific variables you request and doesn't expose them to the browser context
- **`Cypress.expose()` is for public/non-sensitive values**: Use it when you need synchronous access to public configuration that's safe to expose in the browser
- **Choose the right API**: Use `cy.env()` for sensitive values (API keys, credentials, tokens). Use `Cypress.expose()` for public values (feature flags, API versions, plugin configs) that need synchronous access
- **`cy.env()` is read-only**: Unlike `Cypress.env()`, you cannot set environment variables with `cy.env()`. Use configuration files, command-line arguments, or `setupNodeEvents` instead
- **`Cypress.expose()` can set values**: You can set exposed values at runtime, but they only persist within the current spec file
- **Migration is straightforward**:
  - For sensitive values: Replace synchronous `Cypress.env()` calls with asynchronous `cy.env()` calls
  - For public values: Replace `Cypress.env()` with `Cypress.expose()` for synchronous access, or use `cy.env()` if async access is acceptable
- **Set `allowCypressEnv: false`**: Once migrated, set `allowCypressEnv: false` in your Cypress configuration to prevent future use of `Cypress.env()`

For more information, see:

- [`cy.env()` command documentation](/api/commands/env)
- [`Cypress.expose()` API documentation](/api/cypress-api/expose)
