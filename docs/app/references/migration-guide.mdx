...
### `cy.getCookie()` and `cy.getCookies()` are now queries

[`cy.getCookie()`](/api/commands/getcookie) and
[`cy.getCookies()`](/api/commands/getcookies) are now retry-able
...
- **Query commands** — `cy.getCookie()` and `cy.getCookies()` — are now governed
  by [`defaultCommandTimeout`](/app/references/configuration#Timeouts) (default
...
- **Cookie action commands** — [`cy.setCookie()`](/api/commands/setcookie),
  ...
  [`cy.getAllCookies()`](/api/commands/getallcookies), and
...
Because the yielded cookie (or array of cookies) is now re-queried on every
retry, do not treat the value captured inside a
[`.then()`](/api/commands/then) callback as a stable snapshot. If you relied on
the previous one-shot behavior, chain [`.should()`](/api/commands/should)
assertions instead so Cypress retries the entire chain until they pass:

<Badge type="danger">Before</Badge>

```javascript
// Retries until the cookie exists and its value matches or times out
...
If you specifically need the old non-retrying behavior — for example, to assert
synchronously that a cookie does **not** yet exist — set the command `timeout`
to `0`:

```javascript
cy.getCookie('session_id', { timeout: 0 }).should('not.exist')
```

#### `cy.getCookie()` and `cy.getCookies()` can no longer be overwritten with `Cypress.Commands.overwrite()`

Queries must be overwritten using `Cypress.Commands.overwriteQuery()`. If you
were previously overwriting `cy.getCookie()` or `cy.getCookies()`, update your
code to use
...
Because the yielded object is now re-queried on every retry, do not treat the
value captured inside a [`.then()`](/api/commands/then) callback as a stable
snapshot. If you relied on the previous one-shot behavior, chain
[`.should()`](/api/commands/should) assertions instead so Cypress retries the
entire chain until they pass:
...
If you specifically need the old non-retrying behavior — for example, to assert
synchronously that no storage has been written yet — set the command `timeout` to
`0`:

```javascript
cy.getAllLocalStorage({ timeout: 0 }).should('deep.equal', {})
```
