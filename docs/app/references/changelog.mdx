...
- Added [`--runner-ui`](/app/guides/command-line#cypress-run-runner-ui) and [`--no-runner-ui`](https://docs.cypress.io/app/guides/command-line#cypress-run-no-runner-ui) CLI flags to control whether the Cypress Runner UI is visible during a run. Addressed in [#27582](https://github.com/cypress-io/cypress/pull/27582).
...
- Added a new CLI flag, called
  [`--auto-cancel-after-failures`](/app/guides/command-line#Options), that
...
- Fixed an issue where the Cypress migration wizard would fail to run in
  [global mode](/app/guides/command-line#cypress-open-global) on newer
...
- The Chrome web security configuration tooltip will now be shown only if
  [`chromeWebSecurity`](/app/guides/web-security#Disabling-Web-Security) is
...
- [`cy.session()`](/api/commands/session) is now supported when using
  [WebKit (Experimental)](/app/guides/launching-browsers#WebKit-Experimental)
...
- You will no longer be able to run your tests during `cypress run` or
  `cypress open` in
  [browser versions that are not supported by Cypress](/app/guides/launching-browsers#Browser-versions-supported).
...
- Component tests can now be launched via the
  [Module API](/app/guides/module-api) by passing `testingType: 'component'`
...
- `cypress.run()` through the [Module API](/app/guides/module-api) now has a
...
- You can now run
  [cypress cache prune](/app/guides/command-line#cypress-cache-prune) to
...
- There's a new `--size` option for the
  [cypress cache list](/app/guides/command-line#cypress-cache-list) command
...
- The [Module API](/app/guides/module-api) has a new
...
- You can now pass a `--quiet` flag to
  [cypress run](/app/guides/command-line#cypress-run) to silence any Cypress
...
- [cypress run](/app/guides/command-line#cypress-run) will no longer crash
...
- The typings for run results when using the
  [Module API](/app/guides/module-api) now indicate they can be `null` or
  `undefined`. Addresses
...
- [cypress cache list](/app/guides/command-line#cypress-cache-list) now
...
- [cypress verify](/app/guides/command-line#cypress-verify) no longer
...
- The
  [`--browser` flag](/app/guides/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
...
- We fixed a bug where
  [cypress run --headless](/app/guides/command-line#Options)
...
- Cypress now accepts a
  [--tag](/app/guides/command-line#cypress-run-tag-lt-tag-gt) argument to be
...
- The Test Runner now shows a non-Chrome logo for
  [browsers passed in via `config.browser`](/app/guides/launching-browsers#Customize-available-browsers)
...
- Cypress now includes the full list of browsers found and their properties
  ...
  [editable within plugins by accessing `config.browsers`](/app/guides/launching-browsers#Customize-available-browsers)
...
- Cypress now captures the SHA of builds generated through Travis CI during pull
  ...
  running within
  [GitHub Integration](/cloud/integrations/source-control/github). Fixes
  [#5728](https://github.com/cypress-io/cypress/issues/5728).
- Cypress now automatically strips double quotes from
  [--ci-build-id](/app/guides/command-line#cypress-run-ci-build-id-lt-id-gt)
  and [group](/app/guides/command-line#cypress-run-group-lt-name-gt) flag
...
- Documented
  [how to pass in a custom list of browsers](/app/guides/launching-browsers#Customize-available-browsers)
...
- We now record video by default when you run Cypress in Chrome, Chromium, or
  ...
  [cypress run --browser chrome](/app/guides/command-line#cypress-run-browser-lt-browser-name-or-path-gt).
...
- Cypress now accepts a
  [`--config-file` argument](/app/guides/command-line#cypress-run-config-file-lt-configuration-file-gt)
...
- We added `failures` and `message` types when a run fails using the
  [Module API](/app/guides/module-api). Addresses
...
- Added the `--config-file` argument to
  [Command Line](/app/guides/command-line) and
  [Module API](/app/guides/module-api) doc.
- Added the `nodeVersion` argument to
  ...
  [Writing a Plugin](/api/node-events/writing-a-plugin) doc.
...
- We’re excited to announce
  [GitHub integration](/cloud/integrations/source-control/github) for the
...
- We have new instructions on how to set up
  [GitHub Integration](/cloud/integrations/source-control/github) with
...
- The
  [`--browser` argument](/app/guides/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
  of the [Command Line](/app/guides/command-line) and the
  [`browser` argument](/app/guides/module-api#cypressrun) of the
  [Module API](/app/guides/module-api) have been updated to allow passing a
  `<path>`. Addresses
...
- Updated [docs](/app/guides/launching-browsers) on explain how to run Chrome
...
- The [Module API](/app/guides/module-api) now returns the `runUrl` from
  `cypress.run()`. Fixes
...
- Added `runUrl` to returned run in [Module API doc](/app/guides/module-api).
...
- `cypress run` accepts a new
  [--parallel](/app/guides/command-line#cypress-run-parallel) flag to run
...
- `cypress run` accepts a new
  [--group](/app/guides/command-line#cypress-run-group-lt-name-gt) flag to
...
- When using the new
  [--parallel](/app/guides/command-line#cypress-run-parallel) or
  [--group](/app/guides/command-line#cypress-run-group-lt-name-gt) feature,
...
- [cypress run](/app/guides/command-line#cypress-run) now creates a new
  browser profile per run per browser, but keeps the same browser profile when
  run via [cypress open](/app/guides/command-line#cypress-open). This means
...
- [Update `Command Line` doc with new flags and alphabetized flags.](/app/guides/command-line)
...
- We are now properly serializing nested object arguments when using the
  [Module API](/app/guides/module-api) via `cypress.open()` or
  ...
  addition to fixing the [Module API](/app/guides/module-api), we are now
  also properly handling nested arguments from the
  [Command Line](/app/guides/command-line). In order to pass nested objects
...
- Fixed a bug causing screenshots saved via
  ...
  you ran using [cypress open](/app/guides/command-line#cypress-open) vs
  [cypress run](/app/guides/command-line#cypress-run) . Fixes
  [#2091](https://github.com/cypress-io/cypress/issues/2091).
- Fixed bugs with Electron losing focus when running headlessly via
  [cypress run](/app/guides/command-line#cypress-run) after navigation. Now
...
- [New "Module API" doc](/app/guides/module-api)
...
- There are now several new CLI commands for interacting with the binary cache:
  [cypress cache path](/app/guides/command-line#cypress-cache-path),
  [cypress cache list](/app/guides/command-line#cypress-cache-list), and
  [cypress cache clear](/app/guides/command-line#cypress-cache-clear). Fixes
  [#1856](https://github.com/cypress-io/cypress/issues/1856).
- We've added a new CLI flag: `--no-exit` for
  [cypress run](/app/guides/command-line#cypress-run) to keep the browser
  open after the tests finish running. Fixes
  [#1871](https://github.com/cypress-io/cypress/issues/1871).
- The [CLI](/app/guides/command-line) now respects config values and
...
- We improved error logging and the user experience around
  [cypress verify](/app/guides/command-line#cypress-verify). Fixes
...
- [Updated "Command Line" doc to include new cache commands](/app/guides/command-line)
...
- You can now pass a normal object to `cypress.open()` or `cypress.run()`
  through the [Module API](/app/guides/module-api), just as you would from
...
- [Update `Installing Cypress` doc to include new options for Cypress binary caching](/app/get-started/installing-cypress)
...
- [Updated `cypress run` spec args](/app/guides/command-line#cypress-run)
- [Updated `Dashboard Service`](/cloud/get-started/introduction)
- [Updated `Launching Browsers` to include our disabling of device discovery notifications](/app/guides/launching-browsers)
...
- [Updated `Writing a Plugin API`](/api/node-events/writing-a-plugin)
...
- We have currently added only
  [one event](/api/node-events/writing-a-plugin#List-of-events) for modifying the
...
- We have added the first Plugin API event called:
  [file:preprocessor](/api/node-events/writing-a-plugin#List-of-events). Fixes
...
- There is a new `--global` option useful with
  [cypress open](/app/guides/command-line#cypress-open) to force Cypress to
...
- The [module API](/app/guides/module-api) accepts `reporterOptions` as an
...
- [Created "Writing a Plugin" API](/api/node-events/writing-a-plugin)
...
- [Created "Conditional Testing" core concept](/app/core-concepts/conditional-testing)
- [Created "Key Differences" core concept](/app/get-started/key-differences)
- [Updated "Best Practices"](/app/references/best-practices)
...
- [Updated "Installing Cypress"](/app/get-started/installing-cypress)
...
- [Updated "Command Line"](/app/guides/command-line)
...
- You can now
  [install Cypress as a real `npm` module](/app/get-started/installing-cypress),
...
- The [`cypress` module can be `required`](/app/guides/module-api) in your
...
- There is a new [`--browser` option](/app/guides/command-line#cypress-run)
...
- You can use the new environment variable
  [`DEBUG=cypress:cli`](/app/guides/command-line#Debugging-commands) to see
...
- You can now pass a browser option to
  [cypress run](/app/guides/command-line#cypress-run) as
...
- [cypress open](/app/guides/command-line#cypress-open) no longer opens a
...
- The [`cypress` npm package](https://www.npmjs.com/package/cypress) now checks
  ...
  [run](/app/guides/command-line#cypress-run) and does not re-install Cypress
...
- There are no more global, in-app updates. Versioning should be primarily
  [controlled via `npm`](/app/get-started/installing-cypress) like every
...
- The example_spec from the
  ...
  [cypress run](/app/guides/command-line#cypress-run) on a brand new project.
...
- [cypress install](/app/guides/command-line) `-d` option. Fixes
...
- The `CYPRESS_DEBUG` env option has been removed in favor of the `debug` module
  and [`DEBUG=cypress:*`](/app/guides/command-line#Debugging-commands) usage.
- Attempting to run [cypress run](/app/guides/command-line#cypress-run) on a
...
- You can now run your tests in the
  [Electron browser](/app/guides/launching-browsers#Electron-Browser) that
...
- You will need to download a new [cypress-cli](/app/guides/command-line) -
  version `0.13.1`.
- Older CLI versions will continue to work on `0.19.0` except for the
  [cypress open](/app/guides/command-line#cypress-open) command - and will we
...
- [cypress run](/app/guides/command-line#cypress-run) now behaves likes
...
- [cypress run](/app/guides/command-line#cypress-run) will now download and
...
- `cypress ci` has been deprecated. You now use
  [`cypress run --record --key <record_key>`](/app/guides/command-line#cypress-run).
  The key you used to pass to `cypress ci` is the same key. We've consolidated
  these commands into just
  [cypress run](/app/guides/command-line#cypress-run) which makes it simpler
  and clearer. Their only difference is that passing `--record` to
  [cypress run](/app/guides/command-line#cypress-run) will **record** the
...
- Improved [cypress run](/app/guides/command-line#cypress-run) and
...
- If you do _not_ want these assets to be tracked by Cypress, you need to switch
  to using [cypress run](/app/guides/command-line#cypress-run). We will
...
- [cypress run](/app/guides/command-line#cypress-run) no longer prompts the
...
- There is now a new
  ...
  [Cypress Web Security](/app/guides/web-security). This option can be used
...
- [cypress run](/app/guides/command-line#cypress-run) no longer requires
  being logged in.
...
- We have upgraded the `Chromium` that runs headlessly on
  [cypress run](/app/guides/command-line#cypress-run) to version `51`.
...
- [cypress run](/app/guides/command-line#cypress-run) now exits with the
...
- In the [Cypress CLI tool](/app/guides/command-line) package version
...
- Updating through the Desktop App in **Linux** does not work. To update please
  run [cypress install](/app/guides/command-line) from the command line.
...
- `0.16.0` marks a significant change for Cypress. Before this we only issued
  ...
  browser management [here](/app/guides/launching-browsers).
...
- Browser sessions are spawned independently of your existing profiles and we've
  ...
  [here](/app/guides/launching-browsers)
...
- Better error handling of unauthorized users attempting to login to Cypress
  with improved
  [Login documentation](/app/get-started/installing-cypress).
...
- The latest version of the [Cypress CLI](/app/guides/command-line) now
  accepts passing arguments to
  [cypress open](/app/guides/command-line#cypress-open). Example:
...
- The [--reporter](/app/guides/command-line#cypress-run) CLI option is now
  working again.
...
- If you choose to install Cypress from the
  [CLI Tool](/app/guides/command-line) you must update to the latest version
  `0.9.1`. Just run `npm install -g cypress` and then you can run
  [cypress install](/app/guides/command-line). You don't need to do anything
...
- Cypress will now automatically prompt you to add a project when using
  [cypress run](/app/guides/command-line#cypress-run) on a project that has
  not yet been added.
...
- Using [cypress run](/app/guides/command-line#cypress-run) in OSX now works
  again.
...
- [Documentation to CLI](/app/guides/command-line).
...
- There are now [Getting Started](/app/get-started/installing-cypress)
...
- `Environment Variable` support has been added and can be accessed in your
  ...
  [Environment Variable](/app/guides/environment-variables) and
...
- Stopped sending CI data when
  [cypress run](/app/guides/command-line#cypress-run) was issued.
...
- [`.pause()`](/api/commands/pause) is now noop during
  [cypress run](/app/guides/command-line#cypress-run) or `cypress ci`.
