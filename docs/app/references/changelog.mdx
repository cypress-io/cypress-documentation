...
- The default `keystrokeDelay` for [`cy.type()`](/api/commands/type) has been changed from `10` to `0` to improve performance of typing-heavy test runs. Users who relied on the implicit 10ms delay can restore it via [`Cypress.Keyboard.defaults()`](/api/cypress-api/keyboard-api), as a [test-configuration override](/app/references/configuration#Test-Configuration), or per-command with the `delay` option. Addresses [#33523](https://github.com/cypress-io/cypress/issues/33523).
