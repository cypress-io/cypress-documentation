...
Cypress 16 makes your tests look more like production and run faster while doing it. In Chrome, Chromium, and Edge, your application now talks to your server over the native browser network, so it negotiates HTTP/2 or HTTP/3 exactly as it does for your users instead of being downgraded to HTTP/1.1 — and your own TLS certificate is the one the browser validates. Two performance features that used to be opt-in experiments are now on by default for everyone: the `checkVisibility()`-based visibility algorithm and browser memory management. Typing no longer pays a 10ms tax per keystroke. And reads that used to fail on a race — `cy.getCookie()`, `cy.getAllLocalStorage()`, and friends — are now retry-able queries, so `cy.getCookie('token').should('exist')` waits for the cookie instead of failing on the first look.
...
- Cypress now supports HTTP/2/3 in Chrome, Chromium, and Edge. Your application is tested over the same protocol it uses in production — multiplexed requests, no six-connection ceiling — so modern apps load faster under test and behave the way your users actually experience them. Cypress also stops terminating TLS for the application under test, so the browser validates your origin's real certificate instead of one Cypress generates. See [Native network interception](/app/guides/native-network-interception) and the [Migration Guide](/app/references/migration-guide#Chrome-Chromium-and-Edge-use-the-native-browser-network). Addresses [#3708](https://github.com/cypress-io/cypress/issues/3708).
