...
When recording to Cypress Cloud, users now receive our newest feature: [Test Replay](/app/cloud/test-replay). Test Replay brings the debugging experience you know and love from the Cypress app directly into your recorded tests in Cypress Cloud.
...
Test Replay is available in all Cypress Cloud plans. To start using Test Replay, simply record a run to Cypress Cloud. Learn more in the [Test Replay documentation](/app/cloud/test-replay).
...
- Added [`--runner-ui`](/app/guide/command-line#cypress-run-runner-ui) and [`--no-runner-ui`](https://docs.cypress.io/app/guide/command-line#cypress-run-no-runner-ui) CLI flags to control whether the Cypress Runner UI is visible during a run. Addressed in [#27582](https://github.com/cypress-io/cypress/pull/27582).
...
- Cypress Cloud users can now receive [desktop notifications](/app/cloud/recorded-runs#Notifications) about their runs, including when one starts, finishes, or fails. Addresses [#26686](https://github.com/cypress-io/cypress/issues/26686).
...
- Fixed a race condition that was causing a GraphQL error to appear on the [Debug page](/app/cloud/recorded-runs#Debug) when viewing a running Cypress Cloud build. Fixed in [#27134](https://github.com/cypress-io/cypress/pull/27134).
...
- The Cypress Cloud data shown on the
  [Specs](/app/core-concepts/cypress-app#Specs) page and
  [Runs](/app/core-concepts/cypress-app#Runs) page will now reflect Cloud
...
- Ensures Git-related messages on the
  [Runs page](/app/core-concepts/cypress-app#Runs) remain dismissed.
...
- Adds Git-related messages for the
  [Runs page](/app/core-concepts/cypress-app#Runs) and
  [Debug page](/app/cloud/recorded-runs#Debug) when users aren't using Git or
...
- Improved styling on the [Runs Page](/app/core-concepts/cypress-app#Runs).
...
- The [Debug page](/app/cloud/recorded-runs#Debug) now allows for navigating
...
- The [Debug page](/app/cloud/recorded-runs#Debug) is now able to show
...
- Updated the Component Testing
  [community framework](/app/component-testing/third-party-definitions)
...
- Added a Public API for third party component libraries to define a Framework
  ...
  Learn more [here](/app/component-testing/third-party-definitions).
...
- Added a new CLI flag, called
  [`--auto-cancel-after-failures`](/app/guide/command-line#Options), that
...
- When a Cloud user is a part of multiple Cloud organizations, the
  [Connect to Cloud setup](/app/cloud/getting-started#Set-up-a-project-to-record)
...
- Fixed an issue where the Cypress migration wizard would fail to run in
  [global mode](/app/guide/command-line#cypress-open-global) on newer
...
- The Chrome web security configuration tooltip will now be shown only if
  [`chromeWebSecurity`](/app/guide/web-security#Disabling-Web-Security) is
...
- [`cy.session()`](/api/commands/session) is now supported when using
  [WebKit (Experimental)](/app/guide/launching-browsers#WebKit-Experimental)
...
- Re-introduces the experimental
  [Cypress Studio feature](/app/references/cypress-studio). Cypress Studio
...
- You will no longer be able to run your tests during `cypress run` or
  `cypress open` in
  [browser versions that are not supported by Cypress](/app/guide/launching-browsers#Browser-versions-supported).
...
- Added a new Cypress command, called
  ...
  [migration guide](/app/references/migration-guide#Migrating-from-cypress-file-upload-to-selectFile)
...
- When using `experimentalStudio`, you can now generate assertions in
  [Cypress Studio](/app/references/cypress-studio) by right clicking on an
...
- When using the experimental
  [Cypress Studio](/app/references/cypress-studio), there should be a reduced
...
- When using `experimentalStudio`,
  [Cypress Studio](/app/references/cypress-studio) can now copy all commands
...
- Component tests can now be launched via the
  [Module API](/app/guide/module-api) by passing `testingType: 'component'`
...
[Component Testing Guide](/app/component-testing/overview) for more details.
...
- The bundled Node.js version was upgraded from `12.18.3` to `14.16.0`. This
  could change the behavior of code within the `pluginsFile` when using the
  [bundled Node.js version](/app/references/legacy-configuration#Node-version) of
...
- Improved the specs list user interface with a new icon set and accessibility
  features when in [Component Testing](/app/component-testing/overview) mode
  (alpha). Addressed in
...
- When using `experimentalStudio`,
  [Cypress Studio](/app/references/cypress-studio) the icon to add commands
...
- When using `experimentalStudio`,
  [Cypress Studio](/app/references/cypress-studio) now records typing by
...
- There's now a [Command Log](/app/core-concepts/cypress-app#Command-Log)
...
- We addressed several issues with the experimental
  [Cypress Studio](/app/references/cypress-studio). To enable the Cypress
...
- [**Cypress Studio**](/app/references/cypress-studio) provides a visual way
...
[Migrating `cy.route()` to `cy.intercept()`](/app/references/migration-guide#Migrating-cyroute-to-cyintercept).
...
- Many of our docs examples have been updated to use
  ...
  our [Network Requests](/app/guide/network-requests) doc.
...
- `cypress.run()` through the [Module API](/app/guide/module-api) now has a
...
- You can now run
  [cypress cache prune](/app/guide/command-line#cypress-cache-prune) to
...
- There's a new `--size` option for the
  [cypress cache list](/app/guide/command-line#cypress-cache-list) command
...
- The [Command Log](/app/core-concepts/cypress-app#Command-Log) can be hidden
...
[Test Retries](/app/guide/test-retries) for more details.
...
- The
  ...
  [test retries](/app/guide/test-retries) built into Cypress. Addresses
...
- Cypress [environment variables](/app/guide/environment-variables) that
...
- We have a new guide on [Test Retries](/app/guide/test-retries).
- Our [Migration Guide](/app/references/migration-guide) has a new section
  for 5.0 migration.
...
- The [Module API](/app/guide/module-api) has a new
...
- You can open a `before`, `beforeEach`, `after`, and `afterEach` hook
  ...
  [Command Log](/app/core-concepts/cypress-app#Command-Log) by clicking the
...
- `before`, `beforeEach`, `after`, and `afterEach` hook definitions now display
  ...
  [Command Log](/app/core-concepts/cypress-app#Command-Log) when defined in
...
- An animated progress bar now displays on every command in the
  [Command Log](/app/core-concepts/cypress-app#Command-Log) indicating how
...
- You can now pass a `--quiet` flag to
  [cypress run](/app/guide/command-line#cypress-run) to silence any Cypress
...
- Assertion logs now properly display as parent commands in the
  [Command Log](/app/core-concepts/cypress-app#Command-Log) regardless of
...
- Highlighting of elements upon hover of a command in the
  [Command Log](/app/core-concepts/cypress-app#Command-Log) are now visible
  when targeting absolute positioned elements. Fixes
  [#7762](https://github.com/cypress-io/cypress/issues/7762).
- [cypress run](/app/guide/command-line#cypress-run) will no longer crash
...
- There is now a loading state to indicate when tests are loading in the
  [Command Log](/app/core-concepts/cypress-app#Command-Log). Addresses
...
- We now display the currently running file at the top of the Test Runner's
  Command Log.
  [That file can be opened](/app/core-concepts/cypress-app#Open-files-in-your-IDE)
  in your
...
- [Errors](/app/guide/debugging#Errors) in the Test Runner now display a
...
- Cypress now utilizes [source maps](/app/guide/debugging#Source-maps) to
...
- The typings for run results when using the
  [Module API](/app/guide/module-api) now indicate they can be `null` or
  `undefined`. Addresses
...
- [Errors shown in the Test Runner](/app/guide/debugging#Errors) have a new
...
- [cypress cache list](/app/guide/command-line#cypress-cache-list) now
...
- [cypress verify](/app/guide/command-line#cypress-verify) no longer
...
[Cross Browser Testing](/app/guide/cross-browser-testing) in Cypress. We've
...
- The
  [`--browser` flag](/app/guide/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
...
- Added [Cross Browser Testing Guide](/app/guide/cross-browser-testing) with
...
- We fixed a bug where
  [cypress run --headless](/app/guide/command-line#Options)
...
- Cypress now accepts a
  [--tag](/app/guide/command-line#cypress-run-tag-lt-tag-gt) argument to be
...
- The Test Runner now shows a non-Chrome logo for
  [browsers passed in via `config.browser`](/app/guide/launching-browsers#Customize-available-browsers)
...
- Cypress now includes the full list of browsers found and their properties
  ...
  [editable within plugins by accessing `config.browsers`](/app/guide/launching-browsers#Customize-available-browsers)
...
- Environment variables within GitHub Actions are now detected and passed to the
  ...
  [git information](/app/continuous-integration/introduction#Git-information)
...
- Cypress now captures the SHA of builds generated through Travis CI during pull
  ...
  running within
  [GitHub Integration](/app/cloud/integrations/source-control/github). Fixes
  [#5728](https://github.com/cypress-io/cypress/issues/5728).
- Cypress now automatically strips double quotes from
  [--ci-build-id](/app/guide/command-line#cypress-run-ci-build-id-lt-id-gt)
  and [group](/app/guide/command-line#cypress-run-group-lt-name-gt) flag
  arguments during `cypress run`. Fixes
  [#5686](https://github.com/cypress-io/cypress/issues/5686).
- Assertions printed in the
  [Command Log](/app/core-concepts/cypress-app#Command-Log) now correctly
...
- Empty spaces are now accurately reflected in the
  [Command Log](/app/core-concepts/cypress-app#Command-Log) within
  assertions. Fixes [#1360](https://github.com/cypress-io/cypress/issues/1360).
- Keys printed in the
  [Command Log](/app/core-concepts/cypress-app#Command-Log) when using the
...
- Documented
  [how to pass in a custom list of browsers](/app/guide/launching-browsers#Customize-available-browsers)
...
- [testFiles](/app/references/legacy-configuration#Folders--Files) now also accepts
...
- We now record video by default when you run Cypress in Chrome, Chromium, or
  ...
  [cypress run --browser chrome](/app/guide/command-line#cypress-run-browser-lt-browser-name-or-path-gt).
...
- Cypress now accepts a
  [`--config-file` argument](/app/guide/command-line#cypress-run-config-file-lt-configuration-file-gt)
...
- You can now use your system's Node version by setting the
  [nodeVersion](/app/references/legacy-configuration#Node-version) to `system` in
  your configuration. This Node version will be used to build files in your
  [integrationFolder](/app/references/legacy-configuration#Folders--Files) and
  [supportFile](/app/references/legacy-configuration#Folders--Files) and also be used
  to execute code in your
  [pluginsFile](/app/references/legacy-configuration#Folders--Files). If not set,
...
- We added `failures` and `message` types when a run fails using the
  [Module API](/app/guide/module-api). Addresses
...
- Added the `--config-file` argument to
  [Command Line](/app/guide/command-line) and
  [Module API](/app/guide/module-api) doc.
- Added the `nodeVersion` argument to
  [Configuration](/app/references/configuration) doc and mentioned its uses
  in the [Plugins Guide](/app/tooling/plugins-guide) and
...
- Documented new keyboard shortcuts in the
  [Cypress App](/app/core-concepts/cypress-app) doc.
- Added a new section on troubleshooting problems with Xvfb to the
  [Continuous Integration](/app/continuous-integration/introduction#Xvfb)
  doc.
...
- We’re excited to announce
  [GitHub integration](/app/cloud/integrations/source-control/github) for the
...
- We have new instructions on how to set up
  [GitHub Integration](/app/cloud/integrations/source-control/github) with
...
- Added a section about video encoding and how to speed up the encoding time to
  [Screenshots and Videos](/app/guide/screenshots-and-videos#Video-encoding)
  doc. Addressed in
...
- Added a section about how to request our new
  [Open Source Plan](/app/cloud/account-management/billing-and-usage#Requesting-OSS-plan-for-an-Org)
...
- The
  [`--browser` argument](/app/guide/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
  of the [Command Line](/app/guide/command-line) and the
  [`browser` argument](/app/guide/module-api#cypressrun) of the
  [Module API](/app/guide/module-api) have been updated to allow passing a
  `<path>`. Addresses
...
- Updated [docs](/app/guide/launching-browsers) on explain how to run Chrome
...
- The [Module API](/app/guide/module-api) now returns the `runUrl` from
  `cypress.run()`. Fixes
...
- We updated how we handle
  ...
  [screenshotsFolder](/app/references/legacy-configuration#Folders--Files) and
  [videosFolder](/app/references/legacy-configuration#Folders--Files) directories
...
- Added `runUrl` to returned run in [Module API doc](/app/guide/module-api).
...
- `cypress run` accepts a new
  [--parallel](/app/guide/command-line#cypress-run-parallel) flag to run
...
- `cypress run` accepts a new
  [--group](/app/guide/command-line#cypress-run-group-lt-name-gt) flag to
...
- When using the new
  [--parallel](/app/guide/command-line#cypress-run-parallel) or
  [--group](/app/guide/command-line#cypress-run-group-lt-name-gt) feature,
...
- [cypress run](/app/guide/command-line#cypress-run) now creates a new
  browser profile per run per browser, but keeps the same browser profile when
  run via [cypress open](/app/guide/command-line#cypress-open). This means
...
- [Added `Parallelization` doc.](/app/cloud/smart-orchestration/parallelization)
- [Update `Dashboard Service` doc with new screenshots and run sections.](/app/cloud/introduction)
- [Update `Command Line` doc with new flags and alphabetized flags.](/app/guide/command-line)
...
- We are now properly serializing nested object arguments when using the
  [Module API](/app/guide/module-api) via `cypress.open()` or
  ...
  addition to fixing the [Module API](/app/guide/module-api), we are now
  also properly handling nested arguments from the
  [Command Line](/app/guide/command-line). In order to pass nested objects
...
- Fixed a bug causing screenshots saved via
  ...
  you ran using [cypress open](/app/guide/command-line#cypress-open) vs
  [cypress run](/app/guide/command-line#cypress-run) . Fixes
  [#2091](https://github.com/cypress-io/cypress/issues/2091).
- Fixed bugs with Electron losing focus when running headlessly via
  [cypress run](/app/guide/command-line#cypress-run) after navigation. Now
...
- [New "Module API" doc](/app/guide/module-api)
...
- Fixed a bug where changing the
  [integrationFolder](/app/references/legacy-configuration#Folders--Files) in Windows
...
- There are now several new CLI commands for interacting with the binary cache:
  [cypress cache path](/app/guide/command-line#cypress-cache-path),
  [cypress cache list](/app/guide/command-line#cypress-cache-list), and
  [cypress cache clear](/app/guide/command-line#cypress-cache-clear). Fixes
  [#1856](https://github.com/cypress-io/cypress/issues/1856).
- We've added a new CLI flag: `--no-exit` for
  [cypress run](/app/guide/command-line#cypress-run) to keep the browser
  open after the tests finish running. Fixes
  [#1871](https://github.com/cypress-io/cypress/issues/1871).
- The [CLI](/app/guide/command-line) now respects config values and
...
- We improved error logging and the user experience around
  [cypress verify](/app/guide/command-line#cypress-verify). Fixes
...
- [Updated "Command Line" doc to include new cache commands](/app/guide/command-line)
...
- You can now pass a normal object to `cypress.open()` or `cypress.run()`
  through the [Module API](/app/guide/module-api), just as you would from
...
- [Update `Installing Cypress` doc to include new options for Cypress binary caching](/app/getting-started/installing-cypress)
...
- [Updated `cypress run` spec args](/app/guide/command-line#cypress-run)
- [Updated `Dashboard Service`](/app/cloud/introduction)
- [Updated `Launching Browsers` to include our disabling of device discovery notifications](/app/guide/launching-browsers)
- [Updated `Preprocessors` to reflect all ways `close` event could be triggered](/app/cloud/introduction)
...
- The
  [Selector Playground](/app/core-concepts/cypress-app#Selector-Playground)
...
- [Added `Selector Playground Guide`](/app/core-concepts/cypress-app#Selector-Playground)
- [Added `Selector Playground API`](/api/cypress-api/selector-playground-api)
- [Updated `Best Practices`](/app/references/best-practices)
...
- Plugins now emit a
  [before:browser:launch](/app/tooling/plugins-guide#Browser-Launching) event
...
- [Updated `Plugins Guide`](/app/tooling/plugins-guide)
...
- [Updated `Environment Variables`](/app/guide/environment-variables)
...
- There is now a [Plugins API interface](/app/tooling/plugins-guide).
...
- There is a new `--global` option useful with
  [cypress open](/app/guide/command-line#cypress-open) to force Cypress to
...
- The [module API](/app/guide/module-api) accepts `reporterOptions` as an
...
- [Created "Conditional Testing" core concept](/app/core-concepts/conditional-testing)
- [Created "Plugins" guide](/app/tooling/plugins-guide)
...
- [Created "Conditional Testing" core concept](/app/core-concepts/conditional-testing)
- [Created "Key Differences" core concept](/app/overview/key-differences)
- [Updated "Best Practices"](/app/references/best-practices)
...
- `1.0.0` marks our transition out of a **private** beta and into a **public**
  ...
  [Dashboard Service](/app/cloud/introduction) will continue to be free.
...
- [Updated "Installing Cypress"](/app/getting-started/installing-cypress)
...
- [Updated "Command Line"](/app/guide/command-line)
...
- You can now
  [install Cypress as a real `npm` module](/app/getting-started/installing-cypress),
...
- The [`cypress` module can be `required`](/app/guide/module-api) in your
...
- Cypress now verifies it can run on your system prior to actually running.
  ...
  for things like
  [missing linux dependencies](/app/continuous-integration/introduction#Dependencies)
  instead of absolutely _nothing_ like before.
- We have better error handling and messaging around Xvfb failures.
- There is a new [`--browser` option](/app/guide/command-line#cypress-run)
...
- You can use the new environment variable
  [`DEBUG=cypress:cli`](/app/guide/command-line#Debugging-commands) to see
...
- You can now pass a browser option to
  [cypress run](/app/guide/command-line#cypress-run) as
...
- [cypress open](/app/guide/command-line#cypress-open) no longer opens a
...
- The [`cypress` npm package](https://www.npmjs.com/package/cypress) now checks
  ...
  [run](/app/guide/command-line#cypress-run) and does not re-install Cypress
...
- There are no more global, in-app updates. Versioning should be primarily
  [controlled via `npm`](/app/getting-started/installing-cypress) like every
...
- The example_spec from the
  ...
  [cypress run](/app/guide/command-line#cypress-run) on a brand new project.
...
- [cypress install](/app/guide/command-line) `-d` option. Fixes
...
- The `CYPRESS_DEBUG` env option has been removed in favor of the `debug` module
  and [`DEBUG=cypress:*`](/app/guide/command-line#Debugging-commands) usage.
- Attempting to run [cypress run](/app/guide/command-line#cypress-run) on a
...
- [Updated "Command Line"](/app/guide/command-line)
- [Updated "Debugging"](/app/guide/debugging)
...
- You can now run your tests in the
  [Electron browser](/app/guide/launching-browsers#Electron-Browser) that
...
- We have updated all of the docs related to these changes. The
  [CI Docs](/app/continuous-integration/introduction) got a much needed
  facelift.
- There is a new docs section related to the
  [Dashboard](/app/cloud/introduction) and the new features.
...
- You will need to download a new [cypress-cli](/app/guide/command-line) -
  version `0.13.1`.
- Older CLI versions will continue to work on `0.19.0` except for the
  [cypress open](/app/guide/command-line#cypress-open) command - and will we
...
- [cypress run](/app/guide/command-line#cypress-run) now behaves likes
...
- [cypress run](/app/guide/command-line#cypress-run) will now download and
...
- `cypress ci` has been deprecated. You now use
  [`cypress run --record --key <record_key>`](/app/guide/command-line#cypress-run).
  The key you used to pass to `cypress ci` is the same key. We've consolidated
  these commands into just
  [cypress run](/app/guide/command-line#cypress-run) which makes it simpler
  and clearer. Their only difference is that passing `--record` to
  [cypress run](/app/guide/command-line#cypress-run) will **record** the
...
- Previously, we auto-magically included all files within
  [`cypress/support`](/app/references/legacy-configuration#Folders--Files).
  ...
  [`supportFile`](/app/references/legacy-configuration#Folders--Files)
...
- Improved [cypress run](/app/guide/command-line#cypress-run) and
...
- If you do _not_ want these assets to be tracked by Cypress, you need to switch
  to using [cypress run](/app/guide/command-line#cypress-run). We will
...
- [cypress run](/app/guide/command-line#cypress-run) no longer prompts the
...
- There is now a new
  ...
  [Cypress Web Security](/app/guide/web-security). This option can be used
...
- [cypress run](/app/guide/command-line#cypress-run) no longer requires
  being logged in.
...
- We have upgraded the `Chromium` that runs headlessly on
  [cypress run](/app/guide/command-line#cypress-run) to version `51`.
...
- [cypress run](/app/guide/command-line#cypress-run) now exits with the
...
- In the [Cypress CLI tool](/app/guide/command-line) package version
...
- Updating through the Desktop App in **Linux** does not work. To update please
  run [cypress install](/app/guide/command-line) from the command line.
...
- `0.16.0` marks a significant change for Cypress. Before this we only issued
  ...
  browser management [here](/app/guide/launching-browsers).
...
- Browser sessions are spawned independently of your existing profiles and we've
  ...
  [here](/app/guide/launching-browsers)
...
- Better error handling of unauthorized users attempting to login to Cypress
  with improved
  [Login documentation](/app/getting-started/installing-cypress).
...
- The latest version of the [Cypress CLI](/app/guide/command-line) now
  accepts passing arguments to
  [cypress open](/app/guide/command-line#cypress-open). Example:
...
- [Environment Variables](/app/guide/environment-variables) that match any
...
- The [--reporter](/app/guide/command-line#cypress-run) CLI option is now
  working again.
...
- We've removed the configuration option `testFolder` and renamed it to
  [`integrationFolder`](/app/references/legacy-configuration#Folders--Files) inside
  of the `cypress.json`.
...
- Added new [`fileServerFolder`](/app/references/legacy-configuration#Folders--Files)
...
- If you choose to install Cypress from the
  [CLI Tool](/app/guide/command-line) you must update to the latest version
  `0.9.1`. Just run `npm install -g cypress` and then you can run
  [cypress install](/app/guide/command-line). You don't need to do anything
...
- Cypress will now automatically prompt you to add a project when using
  [cypress run](/app/guide/command-line#cypress-run) on a project that has
  not yet been added.
...
- Using [cypress run](/app/guide/command-line#cypress-run) in OSX now works
  again.
...
- [Documentation to CLI](/app/guide/command-line).
...
- There are now [Getting Started](/app/getting-started/installing-cypress)
...
- `Environment Variable` support has been added and can be accessed in your
  ...
  [Environment Variable](/app/guide/environment-variables) and
...
- Stopped sending CI data when
  [cypress run](/app/guide/command-line#cypress-run) was issued.
...
- [`.pause()`](/api/commands/pause) is now noop during
  [cypress run](/app/guide/command-line#cypress-run) or `cypress ci`.
