...
- Added the [`removeSRIAttributes`](/app/references/configuration#removeSRIAttributes) configuration option. When enabled, Cypress removes [Subresource Integrity (SRI)](https://developer.mozilla.org/en-US/docs/Web/Security/Subresource_Integrity) `integrity` attributes from `<script>` and `<link>` elements so that proxy-rewritten first-party and third-party resources are not blocked by the browser. Addresses [#34124](https://github.com/cypress-io/cypress/issues/34124).
...
- `console.log` and `console.error` called within the
  [plugins file](/app/core-concepts/writing-and-organizing-tests#Plugins-file)
