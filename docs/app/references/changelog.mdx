...
- Fixed a regression in [15.18.1](#15-18-1) where action commands (such as [`.click()`](/api/commands/click) and [`.type()`](/api/commands/type)) also scrolled the element's scrollable container horizontally, which could move a container the application positions itself, such as a carousel, even when the element was already visible. The `'top'` and `'bottom'` values of [`scrollBehavior`](/app/references/configuration#Actionability) again align the vertical axis only, matching the behavior of `scrollIntoView(alignToTop)`. To align both axes as `15.18.1` through `15.19.1` did, use `'start'` or `'end'`. Fixes [#34460](https://github.com/cypress-io/cypress/issues/34460). Fixed in [#34474](https://github.com/cypress-io/cypress/pull/34474).
...
- The [`cy.end()`](/api/commands/end) command has been deprecated and will be removed in a future release. Instead of using `.end()` to break a chain, start a new chain of commands off of `cy`. Addressed in [#33707](https://github.com/cypress-io/cypress/pull/33707).
...
[`Cypress.env()`](/api/cypress-api/env) is now deprecated and will be removed in a future major release of Cypress. To understand why, and how to migrate, please read our [Migration Guide](/app/references/migration-guide#Migrating-away-from-Cypressenv). Addressed in [#33181](https://github.com/cypress-io/cypress/pull/33181).
