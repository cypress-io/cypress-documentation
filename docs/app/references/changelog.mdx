...
- Chrome, Chromium, and Edge intercept test traffic on the native browser network instead of the legacy network path, so your application connects to your server directly and negotiates whatever protocol the server supports (HTTP/1.1, HTTP/2, or HTTP/3), exactly as it does in production, rather than always being downgraded to HTTP/1.1. Firefox and WebKit continue to use the legacy network path because Cypress has not implemented a native browser network path for them yet, and Electron stays on the legacy path because it is deprecated as a test browser in Cypress 16. The new [`forceHttp1`](/app/references/configuration#forceHttp1) option is a temporary escape hatch that routes every browser through the legacy network path while you migrate or while an issue you have filed is fixed; if undocumented behavior is what makes your suite pass, [open an issue](https://github.com/cypress-io/cypress/issues/new/choose). See [Native network interception](/app/guides/native-network-interception) and the [Migration Guide](/app/references/migration-guide#Chrome-Chromium-and-Edge-use-the-native-browser-network). Addresses [#3708](https://github.com/cypress-io/cypress/issues/3708).
