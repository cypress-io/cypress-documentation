...
**WebSocket support:** Cypress transparently proxies WebSocket connections, so
your application's WebSocket traffic works as expected during tests. However,
[`cy.intercept()`](/api/commands/intercept) does **not** intercept WebSocket
connections — it cannot match the upgrade request or spy on, stub, or wait for
individual frames/messages. (`resourceType: 'websocket'` appears in the
interception types, but the WebSocket upgrade is proxied outside the
interception layer, so no route will match it.) The strategies described below
are the recommended way to test real-time, message-driven features.
