...
Google publishes native ARM64 Linux packages of Chrome (`.deb` and `.rpm`) from Chrome
`151` onwards, so Chrome can be installed and launched on Linux ARM64 machines such as
AWS Graviton instances or Apple Silicon containers.
[Chrome for Testing](https://github.com/GoogleChromeLabs/chrome-for-testing/) does not
publish `linux-arm64` builds yet, so a fixed browser version cannot be pinned that way on
this platform.
Cypress [Docker images](/app/continuous-integration/overview#Cypress-Docker-Images) for
`Linux/arm64` include Chrome from version `151` and Firefox from version `136`. Edge is
...
- **Versioned, reproducible binaries.** A matching build is published for every
  Chrome version on Windows, macOS and Linux x64, so you can install the exact
  same browser locally and in CI. See
