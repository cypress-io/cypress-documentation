...
and Chromium). To run tests optimally across these browsers in CI, check out the
...
from the CLI, Cypress launches browsers headlessly. To run headed, pass the
`--headed` argument:
...
### Electron Browser (deprecated) {#Electron-Browser}
...
browser** and will be removed in a future version of Cypress. Switch to Chrome
or another installed browser to avoid a breaking change when you upgrade. Set a
[`defaultBrowser`](/app/references/configuration#Browser) (for example,
`'chrome'`) and/or pass [`--browser`](/app/references/command-line#cypress-run)
so your runs don't rely on the bundled Electron default.

After Electron is removed, runs that target it (`--browser electron`, or an
implicit fallback to Electron) will fail with a clear error. See
[Migrating away from the Electron browser](/app/references/migration-guide#Migrating-away-from-the-Electron-browser).

:::

Historically, Electron has been the browser Cypress launches when no
`--browser` is specified, so it is often what runs in CI. Because it is now
deprecated, we recommend launching an installed browser in CI instead — for
example, `cypress run --browser chrome`, or by setting a
[`defaultBrowser`](/app/references/configuration#Browser). If you are seeing
failures in CI, to easily debug them you may want to run locally with the
`--headed` option.
