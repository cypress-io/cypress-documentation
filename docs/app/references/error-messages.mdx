...
2. Use the built-in Electron browser for tests, since it is not affected by
   these policies. Note that the Electron browser is
   [deprecated](/app/references/migration-guide#Migrating-away-from-the-Electron-browser)
   and will be removed in a future major version, so prefer the Chromium option
