...
[Examples](/examples/recipes) or check out our
...
See our [Web Security](/app/guide/web-security#Limitations) documentation.
...
[4.0 migration guide](/app/references/migration-guide#Mocha-upgrade).
...
[Continuous Integration](/app/continuous-integration/introduction). This
means that you did not pass a specific record key to:
[cypress run --record](/app/guide/command-line#cypress-run).
...
[add the key to your config file or as an environment variable](/app/continuous-integration/introduction#Record-tests).
...
[caching the cypress binary in CI](/app/continuous-integration/introduction#Caching),
...
[--group](/app/guide/command-line#cypress-run-group-lt-name-gt) or
[--parallel](/app/guide/command-line#cypress-run-parallel) flag.

The `--ci-build-id` flag is used to either group or parallelize multiple runs
together.

Check out our
[guide on parallelizing runs](/app/cloud/smart-orchestration/parallelization)
and when to use the
...
[--group](/app/guide/command-line#cypress-run-group-lt-name-gt),
[--tag](/app/guide/command-line#cypress-run-tag-lt-tag-gt),
[--parallel](/app/guide/command-line#cypress-run-parallel), or
[--auto-cancel-after-failures](/app/guide/command-line#auto-cancel-after-runs)
flag without also passing the `--record` flag.

These flags can only be used when recording to
[Cypress Cloud](/app/cloud/introduction).

Please review our
[parallelization](/app/cloud/smart-orchestration/parallelization)
...
You passed the
[--group](/app/guide/command-line#cypress-run-group-lt-name-gt) or
[--parallel](/app/guide/command-line#cypress-run-parallel) flag but we could
...
[CI providers](/app/continuous-integration/ci-provider-examples). Please
review the
[natively recognized environment variables](/app/cloud/smart-orchestration/parallelization#CI-Build-ID-environment-variables-by-provider)
...
You passed the
[--group](/app/guide/command-line#cypress-run-group-lt-name-gt) flag, but
this group name has already been used for this run.

If you are trying to parallelize this run, then also pass the
[--parallel](/app/guide/command-line#cypress-run-parallel) flag, else pass a
different group name.

Please review
[grouping test runs](/app/cloud/smart-orchestration/parallelization#Grouping-test-runs)
documentation to learn more.

### <Icon name="exclamation-triangle" color="red" /> Cannot parallelize tests across environments

You passed the [--parallel](/app/guide/command-line#cypress-run-parallel)
...
You cannot use the
[--parallel](/app/guide/command-line#cypress-run-parallel) flag with this
group.
...
You must use the [--parallel](/app/guide/command-line#cypress-run-parallel)
...
[--parallel](/app/guide/command-line#cypress-run-parallel) flag to a run
...
[parallelization doc](/app/cloud/smart-orchestration/parallelization#CI-Build-ID-environment-variables-by-provider).
...
[here](/app/guide/command-line#cypress-run-ci-build-id-lt-id-gt).
...
When a run finishes all of its groups, it waits for a configurable set of time, [a run completion delay](/app/cloud/smart-orchestration/parallelization#Run-completion-delay),
before completing. All `cypress run` calls with any new groups must be executed during that time period.

#### Troubleshooting

- If you are passing [`--ci-build-id`](/app/guide/command-line#cypress-run-ci-build-id-lt-id-gt), make sure it is generating a unique value for the run. If it is not unique and matches a previous run, you may see this error.
- If you are running `cypress run` calls in parallel and they are not completing within the default 60 second run completion delay, you can increase this delay. [See instructions](/app/cloud/account-management/projects#Run-completion-delay).
...
[please read our dedicated guide to it](/app/guide/web-security).
...
1. Don't click `<a>` links in your tests that navigate outside of your
   ...
   [in our "Tab Handling and Links" example recipe](/examples/recipes#Testing-the-DOM).
...
[read about the reasoning here](/app/guide/web-security).
...
- Don't copy the URL you see when launching a Cypress browser from the Cypress
  ...
  [Cross Browser Testing](/app/guide/cross-browser-testing) guide.
...
2. Use the built-in Electron browser for tests, since it is not affected by
   these policies.
   [See the guide to launching browsers for more information.](/app/guide/launching-browsers#Electron-Browser)
