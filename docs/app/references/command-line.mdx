...
<Icon name="exclamation-triangle" /> `cypress tap` is
[experimental](/app/references/experiments). Its commands and output may change
in any release.

:::

Interacts with a running Cypress session. `cypress tap` lets you — or an AI
agent working in your terminal — drive Cypress in open mode: run specs, poll
run status, read test results the way the reporter shows them, and inspect the
app under test. New to `cypress tap`? Start with the
[cypress tap guide](/app/tooling/cypress-tap).
...
directory's project. Reading the browser (`reporter`, `command`, `pin`, `dom`,
`aria`, and `inspect`) requires a Chromium-family browser (Chrome, Chromium,
Edge, or Electron) open in the session; Firefox and WebKit are not supported.
The CLI and the running Cypress must be the same version.
...
Commands exit with `0` on success and `1` on any failure. Failures print a
typed code and a message that names the next command to run, for example:
...
Reports where a running Cypress session is in its lifecycle — built for
polling. The stages are `not connected`, `browser not selected`,
`spec not selected`, `loading`, `running`, `passed`, and `failed`. Only
`passed` and `failed` are verdicts; `loading` is a selected spec waiting on
its own build and stays `loading` for as long as that takes, so a poller
needs its own timeout.

`status` always exits `0` for a determinable stage, including
`not connected` — branch on the `status` field, not the exit code.
...
With `--json`, the output carries `startedAt` (the run's start time, `null`
while loading) from `loading` onwards. Compare `startedAt` before believing a
verdict: a `passed` or `failed` status still carrying the previous run's
`startedAt` describes the run before this one.
...
`results` counts, and a `pinned` object rides along whenever a
[pin](#cypress-tap-pin) is active.

#### `cypress tap specs` {#cypress-tap-specs}

Lists the specs the running Cypress session can run, with each spec's
last-modified time when git history is available. The list covers the testing
type the session has open; to reach the other testing type, open a new Cypress
...
The `--json` result reports `relativePath`, `testingType`, `browser`, and
`previousStartedAt` — the `startedAt` that `status` was reporting when the
request was accepted (`null` on a first run). A verdict still carrying that
same `startedAt` describes the run before this one, so wait for a verdict
whose `startedAt` differs.

A path that matches no runnable spec fails with `SPEC_NOT_FOUND`; requesting a
run while a spec is already running fails with `RUN_IN_PROGRESS`.

#### `cypress tap reporter` {#cypress-tap-reporter}

Shows test results the way the Cypress app's reporter panel does, right in
your terminal.

| Option                | Description                                                     |
| --------------------- | --------------------------------------------------------------- |
| `--test <test>`       | Test id, as listed by the spec overview this command prints     |
| `--attempt <attempt>` | 1-based attempt (attempt 1 = first run); defaults to the latest |
...
With `--test <id>`, it renders that test's full story: its network routes, the
...
| `--attempt <attempt>` | 1-based attempt (attempt 1 = first run); defaults to the latest                                                                 |
| `--depth <depth>`     | How many levels of nested console properties to expand before summarizing the rest: a number or `all` (default 3)               |
| `--path <path>`       | Show one section of the console properties, addressed from the top level as `Response>headers` (case-insensitive)               |
...
Values long enough to bury the rest of the payload — a response body, a long
string — are named by their length instead of printed; pass `--json` for the
complete result with every console property in full.

#### `cypress tap pin` {#cypress-tap-pin}

Pins a command's DOM snapshot into the live app-under-test frame — the same
mechanism as clicking a command in the Cypress app — so the
...
| Option                | Description                                                                                                              |
| --------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `--at <at>`           | Which snapshot to pin: a name like `before`/`after` or a 1-based index; defaults to the last (the command's final state) |
| `--attempt <attempt>` | 1-based attempt (attempt 1 = first run); defaults to the latest                                                          |
| `--clear`             | Release the current pin and restore the app to its pre-pin state                                                         |
...
| Option                    | Description                                     |
| ------------------------- | ----------------------------------------------- |
| `--max-chars <max-chars>` | Cap on returned HTML characters (default 30000) |
...
`dom`, `aria`, and `inspect` read the app between runs. Before any run they
fail with `NO_RUN`, and during a run with `RUN_IN_PROGRESS` — wait for
`status` to report `passed` or `failed`.
...
| Option                    | Description                                                     |
| ------------------------- | --------------------------------------------------------------- |
| `--max-nodes <max-nodes>` | Cap on the number of accessibility nodes returned (default 200) |
...
Inspects the first element matching the selector: its tag, attributes, curated
