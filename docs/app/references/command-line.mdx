...
`cypress tap` is in [beta](/app/references/roadmap#Beta). Its commands and
output can still change in any release.

:::

Drives a Cypress session in open mode from the terminal: run specs, poll run
status, read test results the way the Cypress command log shows them, and
inspect the app under test. It is built for you, or for an AI agent working in
your terminal.

:::note

New to `cypress tap`? Start with the
[guide to driving Cypress with AI agents](/app/tooling/cypress-tap).
...
`cypress tap` attaches to interactive `cypress open` sessions in the current
directory's project.
...
| Option             | Description                                                               |
| ------------------ | ------------------------------------------------------------------------- |
| `--instance <pid>` | Target a specific running Cypress instance by its server process id (pid) |
...
#### `cypress tap instances` {#cypress-tap-instances}

Lists the running Cypress sessions the CLI can reach. Pass a listed pid to
`--instance` to target that session with another `tap` command.
...
Reports where a running Cypress session is in its lifecycle, and is built for
polling.
...
`passed` describes the run, not the tests: it means the run reached the end
without a failure. A spec with no tests at all, or one whose tests were every
one of them skipped or pending, finishes `passed` with `results.passed` at `0`.
Read the `results` counts alongside the stage when it matters whether anything
actually ran.
...
With `--json`, the output carries `startedAt` from `loading` onwards: the run's
...
From `running` onwards the JSON also reports `totalTests` and per-outcome
`results` counts:
...
A `pinned` object rides along whenever a [pin](#cypress-tap-pin) is active:
...
Lists the specs the current session can run. Each spec shows its last-modified
time when git history is available. Only the session's current testing type is
listed, so to see the specs of the other testing type, open a new Cypress
session with that testing type selected.
...
[`specs`](#cypress-tap-specs) command. If no browser is open, Cypress launches
one. The command requests the run and returns immediately; returning does not
mean the run has started, let alone finished. Poll the
[`status`](#cypress-tap-status) command for run progress.
...
With `--json`, `previousStartedAt` is the `startedAt` that `status` was
reporting when the request was accepted (`null` on a first run). A verdict
still carrying that same `startedAt` describes the run before this one, so wait
for a verdict whose `startedAt` differs.
...
The commands below read the browser, so they need a Chromium-family browser
(Chrome, Chromium, Edge, or Electron) open in the session. Firefox and WebKit
are not supported.
...
| --------------------- | ----------------------------------------------------------- | ----------------------- |
| `--test <test>`       | Test id, as listed by the spec overview this command prints | The spec-level overview |
| `--attempt <attempt>` | 1-based attempt, where attempt 1 is the first run           | The latest attempt      |

Without `--test`, it prints the spec-level overview: the run's pass/fail stats
...
✓ 2  ✖ 1  ○ 1  - 1  00:17
   ...
   t1  ✓ redirects an anonymous visitor  20ms

Login
   t2  ✓ shows the sign-in form  10ms
   t4  ○ remembers the last user

Login > Invalid credentials
   t3  ✖ shows an error banner  30ms  (2 retries)
         ✖ attempt 1  4.5s
         ✖ attempt 2  4.4s
         ✖ attempt 3  30ms

Password reset
   t5  - sends a reset email
```

With `--test <id>`, it renders that test's full run: its network routes, the
...
| Option                | Description                                                                                                                                                           | Default            |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| `--test <test>`       | Test id, as listed by the [`reporter`](#cypress-tap-reporter) command                                                                                                 | Required           |
| `--command <command>` | Command id, as listed by the `reporter` command: a row number, an e-prefixed event id, or hook-qualified like `h1:3`                                                  | Required           |
| `--attempt <attempt>` | 1-based attempt, where attempt 1 is the first run                                                                                                                     | The latest attempt |
| `--depth <depth>`     | How many levels of nested console properties to expand before summarizing the rest: a number or `all`. A section over 8 rows folds at any depth unless this is passed | `3`                |
| `--path <path>`       | Show one section of the console properties, addressed from the top level as `Response>headers` (case-insensitive)                                                     | The whole payload  |
...
Console properties render collapsed past the configured depth, with a footer
naming the collapsed sections:
...
is replaced by a note of its length rather than printed. Pass `--json` to get
every console property in full.

#### `cypress tap pin` {#cypress-tap-pin}

Pins a command's DOM snapshot into the live app-under-test frame, using the
same mechanism as clicking a command in the Cypress app, so the
[`dom`](#cypress-tap-dom), [`aria`](#cypress-tap-aria), and
[`inspect`](#cypress-tap-inspect) commands read the page as it was when that
command ran.

```shell
cypress tap pin [test] [command] [options]
```

| Option                | Description                                                             | Default                                      |
| --------------------- | ----------------------------------------------------------------------- | -------------------------------------------- |
| `--at <at>`           | Which snapshot to pin: a name like `before`/`after`, or a 1-based index | The last snapshot, the command's final state |
| `--attempt <attempt>` | 1-based attempt, where attempt 1 is the first run                       | The latest attempt                           |
| `--clear`             | Release the current pin and restore the app to its pre-pin state        | Off                                          |

```shell title="Command"
cypress tap pin r3 h2:1 --at after
```
...
Rerunning `pin` on the pinned command switches snapshots without releasing the
pin. Pins can't be created while a spec is running, and snapshots are kept
...
A pin is the same shared state the Cypress app manipulates when you click a
command in the Command Log, so the app and `cypress tap` can overwrite each
other. Releasing the pin in the app releases the one `cypress tap` set, and
pinning a command in the app changes what [`dom`](#cypress-tap-dom),
[`aria`](#cypress-tap-aria), and [`inspect`](#cypress-tap-inspect) read while
`status` still reports no pin. Avoid interacting with the Cypress window while
`cypress tap` is driving it.

:::

```shell title="Command"
cypress tap pin --clear
```
...
Output is capped so a heavy page never ships megabytes at once.

| Option                    | Description                     | Default |
| ------------------------- | ------------------------------- | ------- |
| `--max-chars <max-chars>` | Cap on returned HTML characters | `30000` |

```shell title="Command"
cypress tap dom .todo-list
```
...
`dom`, `aria`, and `inspect` read the app between runs, not while a spec is
running. Wait for `status` to report `passed` or `failed`.
...
| Option                    | Description                                       | Default |
| ------------------------- | ------------------------------------------------- | ------- |
| `--max-nodes <max-nodes>` | Cap on the number of accessibility nodes returned | `200`   |
...
```shell title="Command"
cypress tap inspect "[data-testid=username]"
```
...
| Version                                      | Changes                                                               |
| -------------------------------------------- | --------------------------------------------------------------------- |
| [TBD](/app/references/changelog)             | Added the `cypress tap` command                                       |
