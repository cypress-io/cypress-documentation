...
| --------------------------------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `experimentalCspAllowList`                    | `false` | Indicates the Content-Security-Policy directives to be permitted during a test run. See [Content-Security-Policy](/app/guides/content-security-policy) for more information.                                                                                                                                                                |
...
| `experimentalWebKitSupport`                   | `false` | Enable experimental support for running tests in WebKit. When set, installs of `playwright-webkit` will be detected and available in Cypress. See [Launching Browsers](/app/guides/launching-browsers#WebKit-Experimental) for more information.                                                                                            |
...
[Test retries](/app/guides/test-retries) is a Cypress [Flake Detection](/app/cloud/flaky-test-management#Flake-Detection) feature that enables you to re-attempt any tests that initially fail. The failure may not be a "true" failure, i.e. flaky. The only way to determine this is to retry the test.
...
[injects document.domain](/app/guides/web-security#Examples-of-what-Cypress-does-under-the-hood)
...
and
[`experimentalModifyObstructiveThirdPartyCode`](/app/guides/web-security#Modifying-Obstructive-Third-Party-Code)
flags.
...
- Inability to leverage [Smart orchestration](/app/cloud/features/smart-orchestration/overview) effectively to scale resources due to all assets being compiled into the dev server regardless of what specs are run.
...
This also significantly reduces the memory requirements needed to run your tests in `run` mode, with a tradeoff being that your tests might run slightly slower since the dev server needs to recompile. If time is a factor, [Smart orchestration](/app/cloud/features/smart-orchestration/overview) can now be effectively used to parallelize your component testing runs on smaller infrastructure since the dev server only compiles what is needed to run the spec.
