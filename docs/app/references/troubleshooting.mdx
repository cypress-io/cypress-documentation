...
We always recommend using the latest version of Cypress. If you're not using the
...
- Look at the
  [video recordings and screenshots](/app/guides/screenshots-and-videos).
- Split large spec files into smaller ones.
- Split long tests into smaller tests.
- Run the same test using
  [--browser chrome](/app/guides/command-line#cypress-run-browser-lt-browser-name-or-path-gt).
...
Cypress attempts to
[automatically find installed Chrome versions for you](/app/guides/launching-browsers).
...
[See 'Launching Browsers' for more information](/app/guides/launching-browsers#Launching-by-a-path)
...
**Tip:** use the [cypress info](/app/guides/command-line#cypress-info)
...
You can change `run` to `open` in the above to capture debug logs when starting your tests from the [Cypress Specs](../core-concepts/cypress-app#Specs) UI.
...
[How to run Cypress CLI commands](/app/guides/command-line#How-to-run-commands) provides background information on running Cypress from the command line and includes a reference of additional command line options.

Read more: [Good Logging](https://glebbahmutov.com/blog/good-logging/) blog post.
...
In some cases the [Command Log](/app/core-concepts/cypress-app#Command-Log),
...
You can also hide the entire Cypress Runner UI in `cypress run` by passing the [`--no-runner-ui`](/app/guides/command-line#cypress-run-no-runner-ui) command line flag.
...
[cypress cache path](/app/guides/command-line#cypress-cache-path) and
[cypress cache list](/app/guides/command-line#cypress-cache-list) commands.
...
referring to [Linux Prerequisites](/app/get-started/installing-cypress#Linux-Prerequisites).
...
**Tip:** If you are having problems with dependencies, use a [Cypress Docker image](/app/get-started/installing-cypress#Docker-Prerequisites). These have all Cypress dependencies pre-installed.
