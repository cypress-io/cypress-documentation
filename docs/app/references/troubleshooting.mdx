---
title: 'Troubleshooting Cypress: Fix Common Errors & Issues'
description: 'Troubleshoot common Cypress errors like failed installs, missing binary, browsers that will not launch, read ECONNRESET, and slow tests, with step-by-step fixes.'
...
Something not working the way you expect? Most Cypress problems fall into a
handful of categories: a failed install or missing binary, a browser that
won't launch, a dropped connection, or tests that run slowly or crash. This
guide explains _why_ each happens and walks you through the fastest fix.

:::info

##### <Icon name="question-circle" color="#4BBFD2" /> What you'll learn

- How to diagnose and fix the most common Cypress errors
- Where to look first when an install, browser, or test fails
- How to capture debug logs to investigate deeper problems

:::

## Find your problem fast

Jump straight to the section that matches what you're seeing:

| Symptom you're seeing                                          | Where to go                                               |
| -------------------------------------------------------------- | --------------------------------------------------------- |
| Cypress won't install, or `"the Cypress binary was not found"` | [Manage the Cypress cache](#Manage-the-Cypress-cache)     |
| A browser is missing or won't launch                           | [Launching browsers](#Launching-browsers)                 |
| `Error: read ECONNRESET` crashes Cypress or the browser        | [Error: read ECONNRESET](#Error-read-ECONNRESET)          |
| Tests are slow, crashing, or using too much memory             | [Log memory and CPU usage](#Log-memory-and-CPU-usage)     |
| Cypress behaves oddly, especially after an upgrade             | [Clear App Data](#Clear-App-Data)                         |
| You need more detail to diagnose an issue                      | [Print DEBUG logs](#Print-DEBUG-logs)                     |
| You're on a corporate network and requests are blocked         | [Allow Cypress URLs on VPNs](#Allow-Cypress-URLs-on-VPNs) |

## Start here

Before digging into a specific fix, these first steps resolve a large share of
issues, so try them first.
...
- Ask an AI assistant (such as ChatGPT, Claude, GitHub Copilot, or Cursor) and
  point it at our docs so it answers from official Cypress content instead of
  guessing. See [Ask an AI assistant](#Ask-an-AI-assistant) below.
...
AI coding assistants are great at answering Cypress questions, but only if they
read the right source material. This documentation is published in
machine-readable formats so an assistant can ground its answers in our official
content rather than relying on (potentially outdated) memory.

To get accurate help, point your assistant at the docs and give it context:

- **Share the docs manifest.** Give the assistant
  [`https://docs.cypress.io/llms.txt`](https://docs.cypress.io/llms.txt). This
  is an [llms.txt](https://llmstxt.org/) manifest that describes the site and
  links to full Markdown and JSON exports of every page, so the assistant knows
  where to read.
- **Link the exact page.** If your question maps to a specific guide, give the
  assistant that page's URL. Every page is also available as plain Markdown
  under `https://docs.cypress.io/llm/markdown/`, which is easier for an
  assistant to parse than the HTML version.
- **Ask it to cite the docs.** Request that it link the specific pages it used
  so you can verify the answer against the source.
- **Give it your context.** Include your Cypress version, the full error text,
  your config, and a minimal reproducible example so the assistant can give a
  precise answer instead of a generic one.

:::caution

Always verify AI-generated answers against the linked documentation. Assistants
can confidently invent commands or options that don't exist, so confirm any
suggested API or configuration on the page it cites.

:::

### Isolate the problem

When a test is failing and you're not sure why, narrow down the cause before
trying fixes. These principles help you find the smallest change that
reproduces the problem:

- Start with [Test Replay](/cloud/features/test-replay), which shows you the
  exact steps that were taken during the test, including console logs and
  requests. You can also review the screenshots and video of the test.
...
- Comment out or remove your Cypress plugins to isolate whether the problem is caused by Cypress itself or by one of your plugins.

## Common fixes

These are the fixes that resolve the most frequently reported Cypress issues:
installation and binary problems, blocked extensions and URLs, corrupted app
data, and capturing the logs you need to diagnose anything else.

### Manage the Cypress cache

Cypress downloads its binary to a global cache folder, separate from the npm
package, so the same binary can be reused across every project and install on
your machine. When you're troubleshooting installation or startup problems, the
[`cypress cache`](/app/references/command-line#cypress-cache-command) commands
let you inspect and clean up that cache. Each subcommand is useful in a
different situation.

#### Find the cache location

When an error message references the binary path, or you need to manually
inspect, back up, or delete cached files, use `cypress cache path` to print the
exact folder Cypress is using instead of guessing at a default like
`~/.cache/Cypress`. The location varies by operating system and can be
overridden with the
[`CYPRESS_CACHE_FOLDER`](/app/references/advanced-installation#Binary-cache)
environment variable, so printing it is the only reliable way to know where
your binaries actually live, especially in CI, where the cache folder is often
relocated for caching between runs.
...
Use `cypress cache list` when you suspect a stale or duplicate install is
causing trouble. For example, a mismatch between the version in your
`package.json` and the version Cypress actually runs, or a CI cache that never
seems to update. It prints a table of every cached version alongside the last
time each binary was used, which makes it easy to spot old versions left behind
...
Add the `--size` flag to also calculate how much disk space each version is
using. This is helpful when the cache has grown large enough to fill up disk
space on a developer machine or CI runner. Note that calculating size can be
slow.
...
If `cypress cache list` reveals several old versions piling up, use
`cypress cache prune` to delete every cached version _except_ the one currently
installed. This is the safe way to reclaim disk space without forcing a full
re-download of the binary you're actively using. It is ideal for trimming a
bloated CI cache or freeing up space locally while keeping your current install
intact.
...
If you're having an issue during installation of Cypress, or you suspect a
corrupted or partially-downloaded binary, use `cypress cache clear` to remove
the contents of the Cypress cache entirely.

This will clear out all installed versions of Cypress that may be cached on your
machine, giving you a clean slate to reinstall from.
...
#### Override the cache location with `CYPRESS_CACHE_FOLDER`

By default Cypress stores the binary in a per-user cache folder, but the
location can be changed with the
[`CYPRESS_CACHE_FOLDER`](/app/references/advanced-installation#Binary-cache)
environment variable. This is a frequent source of confusing
`"the Cypress binary was not found"` errors and CI caching problems, so it's
worth checking whenever the install and the run disagree about where the binary
lives.

Common situations where it helps to verify this variable:

- **CI caches the wrong folder.** Many CI configurations cache
  `CYPRESS_CACHE_FOLDER` to avoid re-downloading the binary on every run. If the
  variable is set during `cypress install` but missing (or different) when the
  tests run (or vice versa), Cypress installs to one folder and looks in
  another, and reports the binary as missing.
- **The binary "disappears" between steps.** If different jobs, containers, or
  shells set `CYPRESS_CACHE_FOLDER` inconsistently, each one resolves a
  different cache path. Run `cypress cache path` in the failing step to see
  exactly where Cypress is looking.
- **Permission or disk-space issues with the default location.** Pointing
  `CYPRESS_CACHE_FOLDER` at a writable, persistent directory resolves installs
  that fail because the default home-directory cache is read-only or ephemeral.

Run `npx cypress cache path` in the same environment as the failing command to
confirm which folder is actually in use.
...
When the on-screen error isn't enough to explain a failure, debug logs show you
exactly what Cypress is doing under the hood: how it parsed your arguments,
which browsers it found, and where a request or process stalled. They're the
single most useful thing to capture before opening an issue or asking for help.

Cypress uses the [debug](https://github.com/debug-js/debug) module, so you can
turn this output on by setting the `DEBUG` environment variable to `cypress:*`
before running `cypress run` or `cypress open`.
...
| `cypress:network:*`              | Low-level networking (proxy-aware agents, CA certificates)    |
...
| `cypress:server:socket-base`     | WebSocket messages between the browser and the Cypress server |
...
Unlike the `cypress:*` namespaces above, which are defined by Cypress,
`DEBUG_DEPTH` is not a Cypress-specific option. It is provided by the upstream
[debug](https://github.com/debug-js/debug) module and controls how deeply nested
objects are expanded when logged (via Node's `util.inspect`). It works with any
tool that uses `debug`, not just Cypress.
...
The sections below cover specific, frequently-reported problems, a browser
Cypress can't find and the `read ECONNRESET` error, with the cause and the
steps to fix each one.

### Launching browsers

Cypress tries to
[automatically find installed browsers for you](/app/references/launching-browsers),
but probing across different operating systems and environments can be
error-prone. If Cypress can't find a browser you know is installed, the steps
below help Cypress "see" it.
...
<strong>WebKit (experimental) is not auto-detected</strong>

The per-platform tables below cover the Chromium- and Firefox-family browsers
that Cypress discovers by scanning your system. The experimental `webkit`
browser is the exception: it is not detected from a bundle identifier, `PATH`
binary, or executable path. Instead, it is provided by the
[`playwright-webkit`](https://www.npmjs.com/package/playwright-webkit) npm
package that you install into your project, and it becomes available once you
opt in to `experimentalWebKitSupport`. So if `webkit` is missing from
`cypress info`, check that the package is installed and the experiment is
enabled rather than looking for a system install. See
...
| `firefox`            | `C:/Program Files/Mozilla Firefox/firefox.exe`, `C:/Program Files (x86)/Mozilla Firefox/firefox.exe`, `%LOCALAPPDATA%/Mozilla Firefox/firefox.exe`                                                           |
| `firefox:dev`        | `C:/Program Files/Firefox Developer Edition/firefox.exe`, `C:/Program Files (x86)/Firefox Developer Edition/firefox.exe`, `%LOCALAPPDATA%/Firefox Developer Edition/firefox.exe`                             |
| `firefox:nightly`    | `C:/Program Files/Firefox Nightly/firefox.exe`, `C:/Program Files (x86)/Firefox Nightly/firefox.exe`, `%LOCALAPPDATA%/Firefox Nightly/firefox.exe`                                                           |
