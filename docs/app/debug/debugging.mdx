...
This pauses after each command, so you can inspect the web application, the DOM,
the network, and any storage to make sure everything happens as expected.

## Debug with an AI agent

Everything above puts you in front of the Cypress app. If you are working with
an AI coding agent, it can read the same information from the terminal with the
[`cypress tap`](/app/tooling/cypress-tap) CLI: run the spec you just changed,
poll the run's status, read the failing test's Command Log and error, and
inspect the DOM of the app under test at the moment a command ran.

That closes the loop without you relaying failures back and forth. Start
Cypress with `cypress open`, then let your agent drive it from another
terminal.
...
modern ecosystem, opting to use these tools wherever possible.
...
It replays the test exactly as it ran in CI.
:::
...
- [When Can The Test Start?](https://www.cypress.io/blog/2018/02/05/when-can-the-test-start/?utm_source=docs.cypress.io&utm_medium=debugging)
- [When Can The Test Stop?](https://www.cypress.io/blog/2020/01/16/when-can-the-test-stop/?utm_source=docs.cypress.io&utm_medium=debugging)
- [When Can The Test Click?](https://www.cypress.io/blog/2019/01/22/when-can-the-test-click/?utm_source=docs.cypress.io&utm_medium=debugging)
- [When Can The Test Log Out?](https://www.cypress.io/blog/2020/06/25/when-can-the-test-log-out/?utm_source=docs.cypress.io&utm_medium=debugging)
- [Do Not Get Too Detached](https://www.cypress.io/blog/2020/07/22/do-not-get-too-detached/?utm_source=docs.cypress.io&utm_medium=debugging)
