...
sidebar_position: 30
slug: /app/references/error-messages
...
Cypress defaults or using custom Cypress commands, instead of needing to
...
There have been situations where Cypress incorrectly blocks interaction with an
element that should be interactable. If that's the case,
...
same-origin within a single test. You can read more about the
[same-origin policy on MDN](https://developer.mozilla.org/en-US/docs/Web/Security/Same-origin_policy).
...
To fix this, our `cy.get()` command **must** be wrapped with the
...
To use either of these parameters a `ciBuildId` must be determined.

The `ciBuildId` is automatically detected if you are running Cypress in most
[CI providers](/app/continuous-integration/overview#CI-Examples). Review the
...
To run in parallel mode each machine must send identical environment
parameters such as:
...
To uniquely identify each run during `cypress run`, Cypress attempts to
...
your own unique CI Build ID per run as described in
[`cypress run --ci-build-id <id>`](/app/references/command-line#cypress-run-ci-build-id-lt-id-gt).
...
When a run finishes all of its groups, it waits for a configurable amount of time, [a run completion delay](/cloud/features/smart-orchestration/parallelization#Run-Completion-Delay),
...
This should get rid of the error. If you are still receiving this error,
...
service, visit your billing and upgrade to another plan with Auto
Cancellation.
...
[read our dedicated guide to it](/app/guides/cross-origin-testing).
...
For Chromium-based browsers, Cypress manages browser memory by default through
[manageBrowserMemory](/app/references/configuration#Global), so confirm you
...
- [Web Security](/app/guides/cross-origin-testing)
...
1. You do not have internet. Ensure you have connectivity then try again.
2. You are a developer that has forked our codebase and do not have access to
   run our API locally. Read more about this in our
...
`uncaught:exception` event. See the
