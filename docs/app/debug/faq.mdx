...
See our [Pricing Page](https://www.cypress.io/pricing?utm_source=docs.cypress.io&utm_medium=app-faq) for more details.
...
Linux, and Windows. For additional information, see our
...
[end-to-end testing mobile apps with Ionic and Cypress](https://www.cypress.io/blog/2020/07/08/end-to-end-testing-mobile-apps-with-ionic-and-cypress/?utm_source=docs.cypress.io&utm_medium=app-faq)
...
Cypress does _not_ use WebDriver for testing, so it has no notion of driver
bindings. If your language can be somehow transpiled to
...
which you can disable by following
...
Yes. Use [`cy.request()`](/api/commands/request) to call an endpoint directly
and assert on the response. No page needs to be visited first.
...
Because API calls and UI commands share the same command chain, you can also
write tests that span both layers, driving the application through its interface
and then confirming over HTTP that the backend persisted the change:
...
Our <Icon name="github" inline="true" contentType="rwa" /> was built API-first
and still runs
[API test suites](https://github.com/cypress-io/cypress-realworld-app/tree/develop/cypress/tests/api)
alongside its UI tests.
...
code using [code coverage](/app/tooling/code-coverage) as a guide.

For a full walkthrough covering authentication, CRUD workflows, error responses,
GraphQL, and polling, see [API Testing](/app/guides/api-testing).
...
For examples how to do this, read our
...
No. The Command API is not designed in a way that makes this possible currently. To understand how Cypress commands work, read:
...
The
[Uncaught Exceptions examples](/api/cypress-api/catalog-of-events#Uncaught-Exceptions)
on the Catalog of Events page document this in detail.
...
in your application, and thus does not fail the test. Register your own listener
to fail the test instead:
...
The machine running Cypress must already have a valid Kerberos ticket
...
See [polling an asynchronous job](/app/guides/api-testing#Poll-an-asynchronous-job)
for a version with a bounded number of attempts.

### <Icon name="angle-right" /> Why doesn't cy.intercept() match cy.request() calls in Cypress?

[`cy.intercept()`](/api/commands/intercept) matches traffic your application
makes from the browser, which flows through the Cypress proxy.
[`cy.request()`](/api/commands/request) is issued from the Cypress Node process
instead, so it never enters that proxy and cannot be spied on or stubbed.

The two commands answer different questions. Use `cy.request()` to call an
endpoint and assert on its response. Use `cy.intercept()` to assert on, wait
for, or stub the requests your app makes. See
[choose the right tool](/app/guides/api-testing#Choose-the-right-tool).

### <Icon name="angle-right" /> Why don't Cypress cy.request() calls appear in the browser's Network tab?

Cypress makes [`cy.request()`](/api/commands/request) calls from its Node
process rather than from the browser, so the browser never sees the request and
has nothing to show in its Network tab.

To inspect one, click the `cy.request()` entry in the
[Command Log](/app/core-concepts/open-mode#Command-Log) and Cypress prints the
full request and response to the browser console.

This also works after the fact on a CI run. Open the run in
[Test Replay](/cloud/features/test-replay), click the `cy.request()` command,
and the same detail is printed to your console. Open DevTools before clicking,
since that is where the output goes.

### <Icon name="angle-right" /> How do I send an authentication token with cy.request() in Cypress?

Pass the token in the `headers` option of
[`cy.request()`](/api/commands/request), and keep the credential itself in an
[environment variable](/app/guides/environment-variables) rather than in the
spec file. Read it with [`cy.env()`](/api/commands/env), which resolves the
value in the Node process so the token is never hydrated into the browser.
...
When you need to log in to obtain the token first, wrap the login request in
[`cy.session()`](/api/commands/session) so it is cached and restored instead of
re-run for every test. See
[authenticate once and reuse the session](/app/guides/api-testing#Authenticate-once-and-reuse-the-session).

### <Icon name="angle-right" /> How do I test an API endpoint that returns a 4xx or 5xx status code in Cypress?

By default [`cy.request()`](/api/commands/request) fails the test on any
response outside `2xx` and `3xx`. When the error response is what you are
testing, set `failOnStatusCode: false` so the response is yielded instead.
...
### <Icon name="angle-right" /> Where should I put my spec files, and how does Cypress find them?

Cypress discovers spec files using the
[`specPattern`](/app/references/configuration#e2e) configuration option, which
defaults to `cypress/e2e/**/*.cy.{js,jsx,ts,tsx}` for end-to-end testing and
`**/*.cy.{js,jsx,ts,tsx}` for component testing. Any file anywhere in your
project that matches `specPattern` is picked up, so you're free to organize
specs into whatever folder structure suits your team. See
[How Cypress finds your specs](/app/core-concepts/writing-and-organizing-tests#How-Cypress-finds-your-specs)
for details.

### <Icon name="angle-right" /> What hooks does Cypress support, and in what order do they run?

Cypress uses [Mocha](https://mochajs.org/)'s hooks: `before`, `beforeEach`,
`afterEach`, and `after`. `before` and `after` run once per suite, while
`beforeEach` and `afterEach` run around every test. For the full execution order,
including how hooks interleave with tests, see
[Hooks](/app/core-concepts/writing-and-organizing-tests#Hooks).

### <Icon name="angle-right" /> What's the difference between a pending test and a skipped test?

A **pending** test is one you intentionally left out of the run. It has no body,
it's marked with `.skip` / `xit`, or it's restricted to a browser you aren't
currently running. A **skipped** test is one Cypress _meant_ to run but couldn't
because a `before`, `beforeEach`, or `afterEach` hook failed. See
[Pending](/app/core-concepts/writing-and-organizing-tests#Pending) and
[Skipped](/app/core-concepts/writing-and-organizing-tests#Skipped) for examples.

### <Icon name="angle-right" /> Why does Cypress skip the rest of my tests when a hook fails?

If a hook shared by a block of tests (such as a `beforeEach`) fails, Cypress
marks the first test as **failed** and **skips** the remaining tests in that
block. Because the same hook would fail identically for every test, re-running it
would only produce the same failure. Fix the hook and the rest of the tests will
run. See
[Skipped](/app/core-concepts/writing-and-organizing-tests#Skipped).

### <Icon name="angle-right" /> How do I stop Cypress from re-running my tests every time I save a file?

When running with [cypress open](/app/references/command-line#cypress-open),
Cypress watches your spec files and re-runs them on save. To turn this off, set
[`watchForFileChanges`](/app/references/configuration#Global) to `false`:
...
[cypress run](/app/references/command-line#cypress-run), so this option only
affects `cypress open`. See
[Disable file watching](/app/core-concepts/writing-and-organizing-tests#Disable-file-watching).

### <Icon name="angle-right" /> What's the difference between `describe`, `context`, `it`, and `specify`?

Nothing functional. They're aliases provided by Mocha. `context` is an alias of
`describe` (used to group tests into a suite), and `specify` is an alias of `it`
(used to define an individual test). Use whichever reads best for your tests. See
[Test Structure](/app/core-concepts/writing-and-organizing-tests#Test-Structure).
...
1. If your application is running locally and is sending the emails directly
   ...
   ["Testing HTML Emails using Cypress"](https://www.cypress.io/blog/2021/05/11/testing-html-emails-using-cypress/?utm_source=docs.cypress.io&utm_medium=app-faq)
   for details.
2. If your application is using a 3rd party email service, or you cannot stub
   ...
   the blog post
   ["Full Testing of HTML Emails using SendGrid and Ethereal Accounts"](https://www.cypress.io/blog/2021/05/24/full-testing-of-html-emails-using-ethereal-accounts/?utm_source=docs.cypress.io&utm_medium=app-faq)
   for details.
...
We recommend using [`cy.task()`](/api/commands/task) to run code in Node via the
[setupNodeEvents](/app/plugins/plugins-guide#Using-a-plugin) function. This
includes running external commands, which a task can spawn directly from Node.

[Spec files](/app/core-concepts/writing-and-organizing-tests#Spec-files) shows an
example spec that imports an npm package and a local module.
...
- You can also check the
  [latest released version](https://download.cypress.io/desktop.json).
...
- see
  [Testing Redux Store](https://www.cypress.io/blog/2018/11/14/testing-redux-store/?utm_source=docs.cypress.io&utm_medium=app-faq)
...
`cy.$$.escapeSelector()` doesn't work. `cy.$$` doesn't refer to
...
- There is a problem isolated to the Electron browser (without a `--browser`
  flag or a `defaultBrowser` set, `cypress run` falls back to the Electron
  browser)
...
- Remove time-sensitive variability in your tests. For example, ensure a network
  ...
  data from that network request. You can use
...
[Split a very long Cypress test into shorter ones using App Actions](https://www.cypress.io/blog/2019/10/29/split-a-very-long-cypress-test-into-shorter-ones-using-app-actions/?utm_source=docs.cypress.io&utm_medium=app-faq).

### <Icon name="angle-right" /> How can I parallelize my runs?

You can read more about
[parallelizing test runs in Cypress Cloud](/cloud/features/smart-orchestration/parallelization).
...
Yes. You can use visual testing tools to test that charts and graphs are
...
Standard, Chrome-branded browsers version 137 and above can no longer
...
stubbing events from Electron. These tests are open source so you can check
out the
[Launchpad end-to-end tests](https://github.com/cypress-io/cypress/tree/develop/packages/launchpad/cypress/e2e).
...
[End-to-End Snapshot Testing](https://www.cypress.io/blog/2018/01/16/end-to-end-snapshot-testing/?utm_source=docs.cypress.io&utm_medium=app-faq).
...
simulated environment like jsdom. This renders the real component in front of
you to see and interact with as you work on it. You can use the same
browser-based developer tools that you are used to when building web applications, such as
...
(like [Cypress Cloud](https://www.cypress.io/cloud?utm_source=docs.cypress.io&utm_medium=app-faq)) already available to
...
This registers `cy.mount()` for any component test without having to `import`
the framework-specific mount command, and it can be customized to fit your
needs. See the examples guide for each framework for info on creating a
...
Cypress App's command log. It gives you the code for any prompt and saves it.
This represents an author-only use of `cy.prompt`.
...
The `cypress tap` CLI commands let you, or an AI coding agent, drive a running
Cypress session from the terminal. See the
[cypress tap guide](/app/tooling/cypress-tap) and the
[command line reference](/app/tooling/cypress-tap#Available-commands).
...
It is built for AI coding agents. Every command prints stable JSON with
`--json`, so an agent can run the Cypress test it just changed and read the
failure itself instead of asking you to paste results back to it.

### <Icon name="angle-right" /> How do I let an AI agent run my Cypress tests and read the results?

Start Cypress in open mode with `cypress open`, then point your agent at the
`cypress tap` commands in another terminal. A typical loop is
`cypress tap run <spec>` to start a Cypress run, `cypress tap status --json` to
...
To teach the same tools how to write good Cypress tests in the first place,
see [Cypress AI Skills](/app/tooling/ai-skills). The two work well together:
the skills cover how to write Cypress tests, and `cypress tap` reports what
happened when those tests ran.

### <Icon name="angle-right" /> Can an AI agent fix my failing Cypress tests automatically?

It can get much closer than it could without a way to run them. With
[`cypress tap`](/app/tooling/cypress-tap), an agent can run the Cypress spec
it just changed, poll for a verdict, read the failing test's Command Log and
error, inspect the DOM of the application under test, then fix and rerun until
the spec passes, all without you pasting results back into the conversation.

Review what it changed before you commit it. Cypress reports the run honestly,
but a passing Cypress test only proves the assertions the agent wrote are
satisfied, and `passed` on a spec whose tests were all skipped still reports
`passed`.

### <Icon name="angle-right" /> Does `cypress tap` work with `cypress run` in CI?

No. `cypress tap` attaches to interactive `cypress open` sessions only.
Headless `cypress run` sessions, including the ones in your CI pipeline, are
not discoverable. `cypress tap` is a local development tool for tightening the
loop while you and your agent write code.
...
Run `cypress tap status`. It finds the Cypress session for you and reports
where it is in its run lifecycle: `not connected`, `browser not selected`,
`spec not selected`, `loading`, `running`, `passed`, or `failed`. It exits `0`
for any stage it can determine, including `not connected`, so branch on the
`status` field rather than the exit code.

To list every Cypress session the CLI can reach, run `cypress tap sessions`,
and pass a listed pid to `--session` to target one.

### <Icon name="angle-right" /> Can my AI agent read the DOM of the application under test in Cypress?

Yes. `cypress tap dom` returns the application under test's HTML,
`cypress tap aria` returns its accessibility tree as a role, name, and state
tree, and `cypress tap inspect --selector <selector>` returns one element's tag,
attributes, computed styles, box model, and accessibility node.

To read the page as it was at an earlier point in the Cypress test rather than
its final state, pin a command's DOM snapshot first with `cypress tap pin`.
These commands read a completed run, so wait for `status` to report `passed`
or `failed` before calling them.

### <Icon name="angle-right" /> What is the difference between `cypress tap` and Cypress Cloud MCP?

They cover different halves of the same loop. Cypress
[Cloud MCP](/cloud/integrations/cloud-mcp) and the
[Cloud CLI](/cloud/integrations/cloud-cli) give an agent your **recorded**
Cypress runs from Cypress Cloud, so it can see what failed in CI.
`cypress tap` gives it the **live** Cypress session on your machine, so it can
rerun a spec and confirm a fix before you push.

Many teams use both: diagnose a CI failure through Cloud, then verify the fix
locally with `cypress tap`. See
[Verify a Cloud failure locally](/app/tooling/cypress-tap#Verify-a-Cloud-failure-locally).

### <Icon name="angle-right" /> Should I use the Cypress window while `cypress tap` is running?

No. `cypress tap` drives the same Cypress session you are looking at, and
there is no lock between the two. Starting a run from the Cypress app,
switching spec, or pinning and unpinning commands all change what your agent's
next command reads. Treat a Cypress session an agent is driving as one you
watch rather than touch.
