...
sidebar_position: 20
slug: /app/references/troubleshooting
---
...
[paid plans](https://www.cypress.io/pricing/?utm_source=docs.cypress.io&utm_medium=troubleshooting), you can get dedicated email
...
Cypress uses a Chrome extension to run properly. If you or your
...
Cypress maintains some local application data to save user preferences
...
Cypress uses the [debug](https://github.com/debug-js/debug) module.
...
You can change `run` to `open` in the above to capture debug logs when starting your tests from the [Cypress Specs](/app/core-concepts/open-mode#Specs) UI.
...
This isolates the problem a little better
...
To isolate these issues, you can hide the Command Log by passing the
...
<CypressCommandTabs command="run --no-runner-ui" />
...
add a comment to the relevant Cypress GitHub issue explaining your workaround.
