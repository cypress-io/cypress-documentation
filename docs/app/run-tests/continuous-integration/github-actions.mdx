---
title: 'Run Cypress tests in GitHub Actions: step-by-step guide'
...
sidebar_position: 50
slug: /app/continuous-integration/github-actions
---
...
- Finally, our Cypress GitHub Action will:
  ...
  - Run the Cypress tests within our GitHub repository. To run in a specific
    browser such as Chrome, see
...
the above example to select Chrome, add `browser: chrome` as follows.
...
To work around this issue, we recommend using a
[cypress/browsers](#Testing-with-Cypress-Docker-Images) Docker image, which uses one consistent browser version.
...
are using three useful features of
