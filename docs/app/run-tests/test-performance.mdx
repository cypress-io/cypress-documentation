...
sidebar_label: Optimize test performance
sidebar_position: 70
slug: /app/guides/test-performance
---
...
<CypressCommandTabs
  env="DEBUG=cypress:server:util:process_profiler"
  command="run"
/>

### Available system resources

<CypressCommandTabs command="info" />
...
1. [Choose the right test level](#Choose-the-right-test-level) — moving tests from E2E to component or API level is often the single largest gain
1. [cy.session() for authentication caching](#Cache-authentication) — eliminates 2-5 seconds per test
1. [Stub network requests](#Stub-network-requests-strategically) — replaces 100-2000ms real calls with < 20ms stubs
1. [Programmatic state setup](#Set-up-state-programmatically-not-through-the-UI) — replaces minutes of UI navigation with seconds of API calls
1. [Parallelization](#Parallelization) — linear scaling with the number of machines
1. [Spec prioritization and auto cancellation](#Spec-prioritization) — surfaces failures faster; reduces cost of broken builds
1. [Disable video, use Test Replay](#Disable-video-use-Test-Replay-instead) — removes per-spec encoding overhead from CI; Test Replay provides better debugging at lower cost
1. [Tag tests with @cypress/grep for CI tiers](#Run-the-right-tests-at-the-right-time) — run a fast smoke or critical subset on every PR; `grepFilterSpecs` skips loading specs that contain no matching tests
1. [Cache the Cypress binary in CI](#Cache-the-Cypress-binary-in-CI) — prevents a 100 MB+ binary download on every run
1. [Right-size CI machine resources](#Allocate-enough-CI-machine-resources) — resource starvation presents as slow or flaky tests, not a clear error; fix this before other CI tuning
...
1. [UI Coverage to identify over-tested elements](#Find-and-eliminate-over-tested-UI-with-UI-Coverage) — interaction count data identifies UI your tests repeatedly pass through without intending to test
1. [cy.prompt for selector maintenance](#Reduce-selector-maintenance-overhead-with-cyprompt) — reduces recurring fix time when the UI changes; cached commands run without AI overhead in CI

## Strategies to speed up tests

### Keep Cypress up to date

Before tuning configuration or restructuring tests, make sure you're running the latest version of Cypress. The Cypress team continuously ships performance improvements. Older versions don't get these improvements.

For example, if you're using an older version of Cypress, you can upgrade to 16.0.0 to get automatic garbage collection of browser memory between tests (see [manageBrowserMemory](/app/references/configuration#manageBrowserMemory)), faster visibility checks, faster default keystroke typing, and other performance improvements.
...
### Use the modern visibility algorithm
...
As of Cypress 16, the [modern visibility algorithm](/app/core-concepts/interacting-with-elements#Visibility-Strategy) is the default. It delegates to the browser's native [`Element.checkVisibility()`](https://developer.mozilla.org/en-US/docs/Web/API/Element/checkVisibility) API, which is faster than the legacy DOM-walking approach and aligns Cypress's notion of "visible" with the browser's own definition.

If you were previously enabling [`experimentalFastVisibility`](/app/references/migration-guide#experimentalFastVisibility-replaced-by-visibilityStrategy) on Cypress 15, remove it — the modern algorithm is now always on by default.

If a test fails after upgrading because it depended on legacy-only visibility semantics (ancestor `overflow` clipping, transform-based hiding, or fixed/sticky coverage checks), prefer rewriting the assertion to verify the same user-visible behavior. As a temporary migration path, opt back into the legacy algorithm with [`visibilityStrategy`](/app/references/configuration#Actionability):
...
You can also set `visibilityStrategy` per suite or per test while migrating incrementally:
...
The modern algorithm is most impactful on suites with many visibility assertions or suites that run against applications with large, deeply nested component trees.
...
Also see [manageBrowserMemory](/app/references/configuration#manageBrowserMemory) for how Cypress manages browser memory on your behalf and when it makes sense to turn that off.
...
<PackageManagerTabs install="@cypress/grep" dev />
...
<CypressCommandTabs command='run --expose grepTags="@smoke @critical",grepFilterSpecs=true,grepOmitFiltered=true' />

Pre-merge: full suite minus known slow tests.

<CypressCommandTabs command='run --expose grepTags="-@slow",grepFilterSpecs=true' />

Nightly: everything.

<CypressCommandTabs command="run" />
...
- **[UI Coverage](/ui-coverage/get-started/introduction)**: UI Coverage scores are derived from the set of tests that ran. A filtered run covers a narrower portion of your UI, producing a lower score than a full run would. Comparing a `@smoke`-only PR run against a full nightly run makes coverage appear to regress when it hasn't; the numbers reflect different test scopes.
