...
- Cypress features that replace common Protractor patterns, like
...
executable Cypress commands. It's a useful way to stand up coverage quickly
while you port your Protractor suite. See the
...
sequences. Pressing a key against the page itself, like tabbing through
focusable elements, uses [`cy.press()`](/api/commands/press), which
dispatches a native keyboard event. This makes it a direct replacement for
Protractor's `Key.TAB` when testing focus management and keyboard
navigation.
...
| `browser.wait(EC..., timeout)`                         | A retrying [`.should()`](/api/commands/should) assertion, or [`cy.wait('@alias')`](/api/commands/wait) for network waits |
...
A note on `browser.executeScript()`: Protractor specs run in Node, so
`executeScript` exists to reach into the browser and run code there. Cypress
tests already run in the browser, so that code needs no bridge. Use
[`cy.window()`](/api/commands/window), [`cy.document()`](/api/commands/document),
or [`.invoke()`](/api/commands/invoke) to work with the page directly. The
reverse also applies: work your Protractor specs did in Node, like reading
files or seeding a database, moves behind [`cy.task()`](/api/commands/task),
and shell commands behind [`cy.exec()`](/api/commands/exec).

`browser.wait()` deserves the same care. Its condition-based form
(`browser.wait(EC.visibilityOf(el), 5000)`) should become a retrying
assertion like `cy.get(el).should('be.visible')`, not a
[`cy.wait(ms)`](/api/commands/wait) pause. Hard-coded `cy.wait(<number>)`
calls are an
[anti-pattern](/app/core-concepts/best-practices#Unnecessary-Waiting) that
`eslint-plugin-cypress` flags. The one wait worth keeping is on the network:
`cy.wait('@alias')` with [`cy.intercept()`](/api/commands/intercept), covered
in [Network Handling](#Network-Handling).
...
If your interest is in which parts of your app's UI your tests exercise rather
than which lines of code they run,
[UI Coverage](/ui-coverage/get-started/introduction) in Cypress Cloud answers
the same question from the user's perspective: it inspects every page your
tests visit and reports the interactive elements that were and were not tested,
with no instrumentation to set up.

## Next Steps

For more information on how to create end-to-end and
[component tests](/app/component-testing/get-started) with Cypress, be sure to
