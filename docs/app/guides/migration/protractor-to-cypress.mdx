...
Both of these commands launch Cypress in the bundled Electron browser, which is
[deprecated](/app/references/migration-guide#Migrating-away-from-the-Electron-browser)
as of Cypress 16.0. Specify an installed browser with `--browser` (for example,
`--browser chrome`) or set a
