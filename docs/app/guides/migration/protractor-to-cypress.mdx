...
Cypress runs your test alongside your app and limits each test to a single
origin by default. Commands that need to run on a different origin, like a
single sign-on provider, must be wrapped in
[`cy.origin()`](/api/commands/origin).

```js title="sso-login.cy.js"
...
[Cypress Component Testing](/app/component-testing/get-started). Instead of
visiting a page, you mount an Angular component by itself, in a real browser
with the same Cypress API, using the `cy.mount()` command from our first-class
[Angular support](/app/component-testing/angular/overview).

```ts title="button.cy.ts"
