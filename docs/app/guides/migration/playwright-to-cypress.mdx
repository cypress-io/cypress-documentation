...
See [Cypress Configuration](/app/references/configuration).
...
# Multiple files
cypress run --spec "cypress/e2e/login.cy.ts,cypress/e2e/signup.cy.ts"
```
...
<Badge type="danger">Before: Playwright</Badge>
```sh
npx playwright test --grep "signs up"
```

<Badge type="success">After: Cypress</Badge>
```sh
cypress run --env grep="signs up"
```

Cypress test title filtering requires the 
[`@cypress/grep`](https://github.com/cypress-io/cypress/tree/develop/npm/grep) 
plugin. Install it once and the `--env grep` flag works across your suite.

### Override config values

<Badge type="danger">Before: Playwright</Badge>
```sh
npx playwright test --timeout=60000 --retries=3
```

<Badge type="success">After: Cypress</Badge>
```sh
cypress run --config defaultCommandTimeout=60000,retries=3
```

### Flags with no direct equivalent

A few Playwright flags do not map one-to-one to a Cypress CLI flag.

**`--last-failed`**

Playwright re-runs only the tests that failed in the previous run. Cypress 
does not have a CLI flag for this. In Cypress Cloud, 
[Spec Prioritization](/cloud/features/smart-orchestration/spec-prioritization) 
automatically runs previously failed specs first on every CI run.

**`--forbid-only`**

Playwright prevents commits that contain `test.only` from running in CI. 
Cypress does not have an equivalent flag, but `cy.only` is ignored in 
`cypress run` by default. To enforce this in CI, use a lint rule or a 
pre-commit hook to catch `.only` calls before they reach your pipeline.

**`--repeat-each`**

Playwright can run each test N times to check for flakiness. Cypress does 
not have a direct CLI equivalent. To detect flakiness before committing, 
run your spec multiple times by calling the command repeatedly, or use 
Cypress Cloud's [flaky test management](/cloud/features/flaky-test-management) 
to track flake rates automatically over time.

See [Command Line](/app/references/command-line) for a full reference of 
Cypress CLI flags.
