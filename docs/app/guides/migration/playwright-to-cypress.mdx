...
Unlike Playwright, Cypress does not download its own browser binaries. It uses browsers already installed on your machine. A bundled Electron browser is also available, but it is [deprecated](/app/references/migration-guide#Migrating-away-from-the-Electron-browser) as of Cypress 16.0 — we recommend running your tests in an installed browser such as Chrome.
