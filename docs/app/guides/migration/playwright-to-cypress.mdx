...
| `reporter`              | `reporter`                               | See [Reporters]('/app/references/reporters').                                                 |
...
| `page.clock.setFixedTime(date)`                                    | `cy.clock(now)`                                                              | Playwright's `setFixedTime` leaves timers running; `cy.clock()` stops them |
| `page.clock.install({ time })`                                     | `cy.clock(now)`                                                              | Direct equivalent                                                          |
| `page.clock.fastForward(duration)`                                 | `cy.tick(ms)`                                                                | Cypress only accepts milliseconds                                          |
| `page.clock.runFor(duration)`                                      | `cy.tick(ms)`                                                                | Cypress only accepts milliseconds                                          |
| `page.clock.pauseAt(date)`                                         | No equivalent                                                                | Calculate the delta and use `cy.tick()`                                    |
| `page.clock.resume()`                                              | No equivalent                                                                | In Cypress, time only moves when `cy.tick()` is called                     |
| `page.clock.setSystemTime(date)`                                   | `clock.setSystemTime(date)`                                                  | Both change the time without firing timers                                 |
| `page.on('dialog', dialog => dialog.accept())`                     | Auto-accepted by default                                                     | No handler needed to accept `alert()` or `confirm()`                       |
| `page.on('dialog', dialog => dialog.dismiss())`                    | `cy.on('window:confirm', () => false)`                                       | `alert()` cannot be dismissed in Cypress                                   |
| `page.on('dialog', dialog => dialog.accept('text'))`               | `cy.stub(win, 'prompt').returns('text')` in `onBeforeLoad`                   | Cypress requires a stub; there is no `window:prompt` event                 |
| `dialog.message()`                                                 | `cy.on('window:alert', msg => ...)` or `cy.on('window:confirm', msg => ...)` | Inspect dialog text inside the event handler                               |
| `dialog.type()`                                                    | Separate events per dialog type                                              | `window:alert`, `window:confirm`; `prompt` requires a stub                 |
| `page.on('dialog')` with `beforeunload`                            | `cy.on('window:before:unload', event => ...)`                                | Cypress cannot intercept or cancel the `beforeunload` dialog               |
| Evaluate + `waitForFunction` for `window.print`                    | `cy.stub(win, 'print')` in `onBeforeLoad`                                    | Cypress approach is simpler and uses the same stub API                     |
