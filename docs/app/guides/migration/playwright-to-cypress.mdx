...
Playwright commonly runs browsers via projects (`--project=chromium|firefox|webkit`). Cypress runs browsers via the `--browser` flag.
...
See [Launching Browsers](/app/references/launching-browsers) and [Cross Browser Testing](/app/guides/cross-browser-testing).
