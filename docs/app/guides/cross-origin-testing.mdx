...
but not in the _same_ test. (ED: should this be removed, as it links to itself?) Please read our
...
3. Issue a JavaScript redirect in your application, such as
   `window.location.href = '...'`, to a different origin where additional
...
```javascript
// this will error because https://docs.cypress.io doesn't match the https://www.cypress.io origin
it('navigates', () => {
  cy.visit('https://www.cypress.io')
