...
- Parts of a URL that make up the same superdomain or origin
- How to use `cy.origin()` to interact with different origin URLs
...
- [Different superdomain per test requires cy.origin command](#Different-superdomain-per-test-requires-cy.origin-command)
