...
By default, when running [cypress run](/app/guides/command-line#cypress-run)
...
[See the Command Line guide for more information about the `--browser` arguments](/app/guides/command-line#cypress-run-browser-lt-browser-name-or-path-gt)
