...
[Installing Cypress](/app/getting-started/installing-cypress) guide and
...
[Module API](/app/guide/module-api).
...
Set Cypress [environment variables](/app/guide/environment-variables).
...
[Read more about grouping.](/app/cloud/smart-orchestration/parallelization#Grouping-test-runs)
...
[parallel](/app/cloud/smart-orchestration/parallelization) across multiple
machines.
...
[group](/app/cloud/smart-orchestration/parallelization#Grouping-test-runs).

```shell
cypress run --record --parallel --group e2e-staging-specs
```

Read our [parallelization](/app/cloud/smart-orchestration/parallelization)
documentation to learn more.
...
[set up your project to record](/app/cloud/getting-started#Setup), make sure
...
when running in
[Continuous Integration](/app/continuous-integration/introduction).
...
[Cypress Cloud setup instructions](/app/cloud/getting-started#Setup). For an
in-depth explanation of how Cypress uses your record key and `projectId` to save
your test results to Cypress Cloud, see the
[Identification section](/app/cloud/account-management/projects#Identification).
...
Useful for when [Test Replay](/app/cloud/test-replay) is enabled and you would still like the Runner UI to be rendered for screenshots and video.
...
<DocsImage
  src="/img/app/cloud/runs/dashboard-run-with-tags.png"
...
- Any environment variables that start with the `CYPRESS` prefix (with sensitive
  variables like
  [record key](/app/cloud/account-management/projects#Record-key) masked for
  security).
