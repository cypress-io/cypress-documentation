...
{/* VIDEO: Overview of the full Studio AI workflow — record, see recommendations appear, accept, save */}
...
| Browser | Chromium |
| Tests | E2E only |
| Sourcemaps | Must be configured (see [Enable sourcemaps](#sourcemaps)) |
| Internet access | Required for Studio |
| **For AI features only** | Cypress Cloud account and linked project |
...
| Snapshots of the DOM for AI recommendations | — | ✓ |
| Undo / redo / reset | ✓ | ✓ |
...
{/* VIDEO: Clicking 'Edit in Studio' on a test in the Command Log */}

{/* VIDEO: Clicking 'New Test' on a suite */}
...
Studio creates the test definition and opens the recording interface.

{/* VIDEO: Creating a new test — naming it, entering the URL, Studio loading the app */}
...
{/* VIDEO: Extending an existing test — hovering, clicking 'Edit in Studio', Studio running the test and stopping */}

You can also edit any portion of the test code directly in the Studio code panel. There's no need to switch to your IDE.
...
{/* VIDEO: Recording a sequence of interactions — click, type, select — watching code appear in the panel */}
...
Studio automatically picks a unique selector for each element you interact with, add an assertion for, or for smart recommendations using a priority strategy designed for stability.

<DefaultSelectorPriority />

To customize selector priority for Studio and `cy.prompt()`, use the [`Cypress.ElementSelector`](/api/cypress-api/element-selector-api) API.

## Add assertions manually

Right-click any element in your app to see available assertions based on that element's current state — visible, hidden, a specific value, a CSS class, and more. Assertions are appended to the end of your test.

{/* VIDEO: Right-clicking an element and selecting an assertion from the context menu */}

## AI smart recommendations

As you record, Studio AI observes what changes in the DOM between interactions and suggests meaningful assertions for each step.
...
{/* IMAGE: Studio panel showing a thinking indicator, a generated recommendation with comment, and the expand toggle */}
...
Each recommendation includes **Accept** and **Reject (×)** buttons that remain available even after saving. You can:
...
- Undo, redo, or reset any changes in the Studio panel editor (including acceptances and rejections).
...
- The exact state of the DOM at the time the assertion was generated. You'll see a before and after snapshot of the DOM for each recommendation that represents the state of the DOM before and after the interaction that triggered the recommendation.
...
{/* IMAGE: Hovering over a recommendation to reveal the DOM state and element highlight */}

### Failing tests

Studio AI does not generate recommendations for failing tests. If a test is in a failing state, recommendations and recording are both disabled. To recover:

1. Edit the failing code directly in the Studio panel or in your IDE.
2. Click **Save** to write the changes.
...
{/* IMAGE: Failing line highlighted in the Studio code panel */}
...
{/* IMAGE: The AI status bar toggle at the bottom of the Studio panel */}
...
For more information, see the [Cloud FAQ](/cloud/faq#Are-all-Cloud-AI-capabilities-enabled-by-default-How-can-I-turn-them-off).
