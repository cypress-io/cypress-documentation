---
title: 'Network requests in Cypress'
...
`cy.intercept()` does **not** intercept WebSocket connections. It cannot match
the WebSocket upgrade request (even with `resourceType: 'websocket'`) and cannot
spy on, stub, or wait for individual messages/frames — that traffic is proxied
outside the interception layer. If your application relies on receiving specific
