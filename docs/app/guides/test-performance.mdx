...
18. [manageBrowserMemory](#Manage-memory-accumulation) — targeted remedy for suites that slow down over time or crash; measure before committing, as the cleanup cost can exceed the benefit on lighter suites
...
[`manageBrowserMemory`](/app/references/configuration#Global) enables improved memory management for Chromium-based browsers, actively clearing memory between tests. It does this by clearing the browser's heap after each test.

:::cypress-config-example

```js
{
  manageBrowserMemory: true // ✅ Faster approach: enable memory management
}
```
...
`manageBrowserMemory` only applies to Chromium-based browsers (Chrome, Edge, Electron). It has no effect in Firefox or WebKit.

`manageBrowserMemory` replaces the deprecated `experimentalMemoryManagement` option, which continues to work as an alias but will be removed in a future major version.
...
Also see [manageBrowserMemory](#Manage-memory-accumulation) for information on how to improve memory management.
