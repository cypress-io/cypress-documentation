...
If your tests are in the 10-30 second range, start with [choosing the right test level](#choose-the-right-test-level) and [caching authentication with `cy.session()`](#cache-authentication-with-cysession).
...
3. [cy.session() for authentication caching](#cache-authentication-with-cysession) — eliminates 2-5 seconds per test
...
9. [Tag tests with @cypress/grep for CI tiers](#run-the-right-tests-at-the-right-time-with-cypressgrep) — run a fast smoke or critical subset on every PR; `grepFilterSpecs` skips loading specs that contain no matching tests
...
17. [experimentalFastVisibility](#enable-faster-visibility-checks-with-experimentalfastvisibility) — high impact on complex DOMs; replaces the layout-thrashing legacy visibility algorithm
18. [experimentalMemoryManagement](#manage-memory-accumulation-with-experimentalmemorymanagement) — targeted remedy for suites that slow down over time or crash; measure before committing, as the cleanup cost can exceed the benefit on lighter suites
