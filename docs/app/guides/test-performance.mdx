...
17. [Modern visibility algorithm](#Use-the-modern-visibility-algorithm) — high impact on complex DOMs; Cypress 16 defaults to the faster `checkVisibility()`-based algorithm
18. [manageBrowserMemory](#Manage-memory-accumulation) — on by default in Cypress 16, protecting suites that slow down over time or crash; disable it only if your suite is demonstrably not memory-constrained and you can measure a win
...
As of Cypress 16, [`manageBrowserMemory`](/app/references/configuration#Global) defaults to `true`, so Cypress clears the browser's heap after each test without any configuration on your part. Memory-heavy suites get this protection automatically.
...
Memory management is not free. Clearing the heap between tests takes time, paid on every test transition, and a suite that was never under memory pressure pays that cost for no benefit. On a large suite of short tests, the per-test overhead is the part you'll notice.

Consider setting `manageBrowserMemory: false` when **all** of the following hold:

- Your suite runs at a consistent pace from start to finish — it doesn't get progressively slower
- The browser never [crashes mid-run](/app/references/error-messages#Browser-Errors)
- Your CI machines have memory headroom to spare, rather than sitting near their limit (see [Allocate enough CI machine resources](#Allocate-enough-CI-machine-resources))

:::cypress-config-example

```js
{
  manageBrowserMemory: false // opt out if your suite isn't memory-constrained
}
```

:::

Measure before and after, the same way you would for any other change in this guide: run the suite both ways and compare total duration. If disabling it doesn't produce a measurable win, leave it on — the crash protection is worth more than a marginal timing difference.

:::caution

`manageBrowserMemory` cannot be overridden per test or per suite, or with `Cypress.config()`. Set it in your Cypress configuration.
