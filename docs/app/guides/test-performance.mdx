...
18. [manageBrowserMemory](#Manage-memory-accumulation) — on by default in Cypress 16; collections are threshold-gated rather than per-test, so most suites should leave it enabled
...
As of Cypress 16, [`manageBrowserMemory`](/app/references/configuration#Global) defaults to `true`, so this is handled for you without any configuration. Cypress samples the browser's memory usage on an interval while your tests run, and forces a garbage collection before the next test only when a sample has crossed a memory threshold. It is not a collection after every test.

That gating is what keeps the cost proportional. A suite that never approaches the threshold pays only for the periodic sampling and never pays for a collection; a suite under real memory pressure gets collections exactly when it needs them. In practice this means most suites should simply leave the option enabled.
...
If you have measured your own suite and want to reclaim the sampling overhead — and your suite stays well clear of the threshold, runs at a consistent pace from start to finish, and never [crashes mid-run](/app/references/error-messages#Browser-Errors) — you can opt out:
...
Weigh that against what you give up: disabling memory management removes the protection against the browser running out of memory mid-run, which costs far more time than the sampling ever will. If turning it off doesn't produce a measurable win on your suite, leave it on.
