...
flakiness](/cloud/flaky-test-management#Flake-Detection) and continuous integration (CI) build failures. By doing so, this will
...
You may want to use the [experimental test retries](/app/references/experiments#Experimental-Test-Retries) feature which offers more options for [detecting flaky tests](/cloud/flaky-test-management#Flake-Detection). This experimental feature is available as of Cypress `13.4.0`.
...
failed test when run via [cypress run](/app/guides/command-line#cypress-run).

<DocsImage src="/img/app/test-retries/cli-error-message.png" />

During [cypress open](/app/guides/command-line#cypress-open) you will be able
...
If you are using [Cypress Cloud](/cloud/introduction), information
...
can review the
[Flake Detection](/cloud/flaky-test-management#Flake-Detection) and
[Flake Alerting](/cloud/flaky-test-management#Flake-Alerting) features
