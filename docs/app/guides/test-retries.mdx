...
flakiness](/app/cloud/flaky-test-management#Flake-Detection) and continuous integration (CI) build failures. By doing so, this will
...
You may want to use the [experimental test retries](/app/references/experiments#Experimental-Test-Retries) feature which offers more options for [detecting flaky tests](/app/cloud/flaky-test-management#Flake-Detection). This experimental feature is available as of Cypress `13.4.0`.
...
1. A test runs for the first time. If the
   ...
   <Icon name="check-circle" color="green" /> test passes, Cypress will move forward
...
<DocsImage src="/img/app/test-retries/v10/attempt-2-start.png" />
...
<DocsImage src="/img/app/test-retries/v10/attempt-3-start.png" />
...
<DocsImage src="/img/app/test-retries/v10/attempt-3-fail.png" />

The following is a screen capture of what test retries looks like on the same
failed test when run via [cypress run](/app/guide/command-line#cypress-run).

<DocsImage src="/img/app/test-retries/cli-error-message.png" />

During [cypress open](/app/guide/command-line#cypress-open) you will be able
to see the number of attempts made in the
[Command Log](/app/core-concepts/cypress-app#Command-Log) and expand each
...
If you are using [Cypress Cloud](/app/cloud/introduction), information
...
can review the
[Flake Detection](/app/cloud/flaky-test-management#Flake-Detection) and
[Flake Alerting](/app/cloud/flaky-test-management#Flake-Alerting) features
