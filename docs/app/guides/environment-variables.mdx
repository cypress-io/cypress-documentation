...
This guide helps you choose the right API for your use case and points you to the correct documentation.
...
Pass environment variables via the command line:
...
Multiple values must be separated by a comma, not a space. In some shells, like Windows PowerShell, you may need to surround the key/value pair with quotes.

:::

### 5. Dynamic Configuration in `setupNodeEvents`
...
export default defineConfig({
  e2e: {
    setupNodeEvents(on, config) {
      config.env.apiUrl = process.env.API_URL || 'https://api.example.com'
...
- [`cy.env()`](/api/commands/env) - Secure command for accessing sensitive environment variables
- [`Cypress.expose()`](/api/cypress-api/expose) - API for public/non-sensitive configuration
- [Configuration](/app/references/configuration) - Setting environment variables and exposed configuration
- [Best Practices: Handling Secrets](/app/core-concepts/best-practices#Handling-Secrets-and-Sensitive-Data) - Comprehensive security guidance
