...
sidebar_position: 90
slug: /app/core-concepts/best-practices
---
...
[read our Introduction to Cypress guide](/app/core-concepts/introduction-to-cypress#Chains-of-Commands).
...
For working with either of these patterns, read our
...
However, you will only want to use these commands for resources in your
...
1. Use another platform that you control to log in with username and password
   ...
   requests by using [`cy.session()`](/api/commands/session).
...
1. If your application is running locally and is sending the emails directly
   ...
   ["Testing HTML Emails using Cypress"](https://www.cypress.io/blog/2021/05/11/testing-html-emails-using-cypress/?utm_source=docs.cypress.io&utm_medium=best-practices)
   for details.
2. If your application is using a 3rd party email service, or you cannot stub
   ...
   the blog post
   ["Full Testing of HTML Emails using SendGrid and Ethereal Accounts"](https://www.cypress.io/blog/2021/05/24/full-testing-of-html-emails-using-ethereal-accounts/?utm_source=docs.cypress.io&utm_medium=best-practices)
   for details.
...
always leave you with the same login page at the end of the test. To
...
interacting with your database, you can use [`cy.task`](/api/commands/task) to
run them from Node, including
[spawning an external command or CLI](/api/commands/task#Run-an-external-command-or-CLI).
...
This removes the need to hard-code fully qualified domain name (FQDN) URLs in
