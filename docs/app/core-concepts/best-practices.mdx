...
<Icon name="exclamation-triangle" color="red" /> **Anti-Pattern:** Using `then()` unnecessarily or re-querying the same element multiple times when chaining would work.

:::

:::tip

<Icon name="check-circle" color="green" /> **Best Practice:** Use command chaining when possible, and only use `then()` when you need to perform complex operations or work with the actual DOM element.
