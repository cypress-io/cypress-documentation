...
<Icon name="exclamation-triangle" color="red" /> **Anti-Pattern:** Hardcoding secrets in test files, using `Cypress.expose()` for sensitive values, or exposing secrets to the browser context.

:::

:::tip

<Icon name="check-circle" color="green" /> **Best Practice:** Use the right API for the right purpose: `cy.env()` for sensitive values in tests, `cy.task()` for Node.js-only secrets, and `Cypress.expose()` only for public configuration.
...
| [`cy.task()`](/api/commands/task) | Browser (privileged) & Node.js | No | Secrets needed in tests andNode.js |
...
### Use `cy.task()` for support file or plugin secrets

`cy.task()` executes code in the Node.js process and is a great way to access secrets in your support files or plugins.

**Example: Accessing secrets in a support file**
...
on('task', {
  seedDatabase() {
    // Access secrets from Node.js environment
    const dbPassword = process.env.DB_PASSWORD
    const connectionString = `postgresql://user:${dbPassword}@localhost/testdb`

    // Seed database using secure connection
...
**Example: Retrieving secrets for test use**

If you need a secret in your support file, plugin, or test:

```javascript
// In test
cy.task('getApiKey').then((apiKey) => {
  cy.request({
    url: 'https://api.example.com/data',
...
on('task', {
  getApiKey() {
    // Access secret from Node.js environment
