...
<Icon name="exclamation-triangle" color="red" /> **Anti-Pattern:** Hardcoding secrets
in test files, using `Cypress.expose()` for sensitive values, or exposing secrets
...
<Icon name="check-circle" color="green" /> **Best Practice:** Use the right API for
the right purpose: `cy.env()` for sensitive values in tests, `cy.task()` for Node.js-only
secrets, and `Cypress.expose()` only for public configuration.
