...
sidebar_position: 70
slug: /app/core-concepts/variables-and-aliases
---
...
asynchronous nature of our APIs. There are many ways to reference, compare, and use the objects that Cypress
...
Under the hood, aliasing basic objects and primitives uses Mocha's shared
...
Often times you may load a fixture in a `beforeEach` hook but want to use
...
`<input />` text field for editing the todo. The previous `<li>` has
