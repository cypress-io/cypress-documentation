...
sidebar_position: 80
slug: /app/core-concepts/open-mode
---
...
**Reading the Command Log from the terminal**

An AI coding agent can read everything on this page without the GUI. The [`cypress tap`](/app/tooling/cypress-tap) CLI attaches to an open mode session and prints the Command Log, pins a command's snapshot the same way clicking it here does, and reads the resulting DOM and accessibility tree.
