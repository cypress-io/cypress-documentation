...
[Cypress Cloud](/cloud/introduction), you'll see data from your latest
...
[recorded](/cloud/getting-started#Set-up-a-project-to-record).
...
[you can run `cypress open` with the `--browser` and `--component` or `--e2e` options](/app/guides/command-line#cypress-open)
...
A test is considered to be [flaky](/cloud/flaky-test-management) when it
...
[test runs](/cloud/recorded-runs#Latest-Runs) to Cypress Cloud, showing
the latest first.
...
[Test Replay](/cloud/test-replay) and other artifacts like screenshots,
videos, and logs from your tests.

The Debug page shows the latest completed
[test run](/cloud/recorded-runs#Latest-Runs) that matches the `HEAD` commit
...
Cypress Cloud. Learn more about [Cloud Run Notifications](/cloud/recorded-runs#Notifications).
