...
doesn't match `specPattern` — that's the first thing to check. You can override
`specPattern` in your [configuration](/app/references/configuration#e2e) if you
...
Your tests run in the browser, but some things — touching the file system,
talking to a database, controlling how specs are bundled — can only happen in
...
project — a specific viewport, a particular browser, or a different retry count.
