...
Node. The plugins file is where that Node-side code lives. It executes before
the project is loaded, before the browser launches, and during your test run,
and your tests reach it through the [cy.task()](/api/commands/task) command.

This makes it the right place to define how spec files are bundled via
[preprocessors](/api/node-events/preprocessors-api), how browsers are found and
launched via the [browser launch API](/api/node-events/browser-launch-api), and
other tasks that need access to the operating system. Read our
