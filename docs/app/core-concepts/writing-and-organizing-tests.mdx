...
[save them to the cloud with Cypress Cloud](/cloud/recorded-runs#Run-Details).
...
After a test fails, the [Test Replay](/cloud/test-replay) or screenshots and videos with Cypress Cloud can help find the problem so it
can be fixed.
