...
[`cypress.env.json`](/app/guide/environment-variables#Option-2-cypressenvjson),
...
[Network Requests](/app/guide/network-requests).
...
[Screenshots and Videos](/app/guide/screenshots-and-videos#Screenshots)
...
[save them to the cloud with Cypress Cloud](/app/cloud/recorded-runs#Run-Details).
...
other cool things. Read our [plugins guide](/app/tooling/plugins-guide) for
...
[Component](/app/component-testing/overview) or
...
[thoughts on the anti-pattern of cleaning up state with `after()` or `afterEach()`](/app/references/best-practices#Using-after-Or-afterEach-Hooks).
...
`cypress open`. See [Test Retries](/app/guide/test-retries) for more
information.
...
[test retries](/app/guide/test-retries). In that case the Command Log shows
...
After a test fails, the [Test Replay](/app/cloud/test-replay) or screenshots and videos with Cypress Cloud can help find the problem so it
can be fixed.
...
When running in using [cypress open](/app/guide/command-line#cypress-open),
...
- [cypress.env.json](/app/guide/environment-variables)
...
[cypress open](/app/guide/command-line#cypress-open).
