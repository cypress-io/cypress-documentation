...
By default, Cypress only discovers spec files that include the `.cy.`
infix in their filename. This is determined by the
[`specPattern`](/app/references/configuration#e2e) configuration option,
whose defaults are:

- **E2E:** `cypress/e2e/**/*.cy.{js,jsx,ts,tsx}`
- **Component:** `**/*.cy.{js,jsx,ts,tsx}`

So a file named `cypress/e2e/login.js` is **not** picked up by default&mdash;it
needs to be named `cypress/e2e/login.cy.js`. If your test isn't showing up,
check that the filename matches `specPattern`. You can override `specPattern`
in your [Cypress configuration](/app/references/configuration#e2e) to use a
...
only to end-to-end testing&mdash;it is not a configurable option for component
testing. It is a global configuration and can be overridden for end-to-end
testing at the `describe` level with the
...
Cypress watches your entire project root for spec files&mdash;not just the
`cypress/e2e/` and `cypress/support/` directories. Any file that matches your
[`specPattern`](/app/references/configuration#e2e), wherever it lives in the
project, is detected. When a change is made, the active spec file, the support
file, and any files they `import` (or `require`) are recompiled through the
[preprocessor](/api/node-events/preprocessors-api) and the browser reloads.
...
and bundles the loaders and configuration needed to give you TypeScript,
CoffeeScript, and JSX support out of the box with zero configuration.
