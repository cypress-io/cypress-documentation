...
testing type, with several commented-out examples to copy from. By default
Cypress includes the type-specific file: `cypress/support/e2e.{js,jsx,ts,tsx}`
for E2E and `cypress/support/component.{js,jsx,ts,tsx}` for Component Testing.

You can point `supportFile` at a different file or turn it off completely using
the
