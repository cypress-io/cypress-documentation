...
As of Cypress 16, the default visibility algorithm delegates to the browser's native [`Element.checkVisibility()`](https://developer.mozilla.org/en-US/docs/Web/API/Element/checkVisibility) API and aligns Cypress's notion of "visible" with the browser's own definition. See [Visibility Strategy](#Visibility-Strategy) below to configure or opt back into the previous algorithm.
