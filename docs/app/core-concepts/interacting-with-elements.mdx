...
- Its computed CSS properties prevent it from being clicked on or visible:
  ...
  - `display: none`
  - `pointer-events: none`
- It is positioned outside the scrollable document bounds (e.g. `position: absolute; top: -100px`) and cannot be scrolled into view. Elements that are simply below the fold are scrolled into view per the [`scrollBehavior`](/app/references/configuration#Global) config before visibility is checked.
...
Elements that are simply below the fold are automatically scrolled into view per the [`scrollBehavior`](/app/references/configuration#Global) config before visibility is checked, matching the behavior of action commands like [`.click()`](/api/commands/click). When `scrollBehavior` is set to `false`, off-screen elements remain hidden.
