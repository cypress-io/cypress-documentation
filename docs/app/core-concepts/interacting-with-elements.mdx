...
In Cypress `15.18.1` through `15.19.1`, `'top'` and `'bottom'` aligned both axes.
That could scroll a horizontally scrollable container, such as a carousel that
positions itself, even when the element was already visible. Cypress `15.20.0`
restored the pre-`15.18.1` behavior described above.
