...
As of Cypress 16, the default visibility algorithm delegates to the browser's native [`Element.checkVisibility()`](https://developer.mozilla.org/en-US/docs/Web/API/Element/checkVisibility) API. This algorithm is faster and aligns Cypress's notion of "visible" with the browser's own definition.

Configure which algorithm Cypress uses with the [`visibilityStrategy`](/app/references/configuration#Actionability) option, which accepts either `'modern'` (the default) or `'legacy'`.

When `visibilityStrategy` is set to `'modern'`, an element is considered hidden if:
...
- Its computed CSS properties hide it (per `checkVisibility()`):
  ...
  - `opacity: 0`
- It is an `<option>` or `<optgroup>` inside a hidden `<select>`
...
- Fully covered by a sibling element
- Rotated past 90 degrees via `transform` with `backface-visibility: hidden`
- Clipped by a `clip-path` or legacy `clip` property
...
**Collapsed containers that hide via `overflow: hidden` + `max-height: 0`**

Many UI libraries hide collapsible content by setting `max-height: 0` and `overflow: hidden`. The modern algorithm doesn't detect this. Most libraries also set `aria-hidden="true"` on the closed container — assert on that instead:

```javascript
// Before
cy.get('.collapsible-content').should('not.be.visible')
