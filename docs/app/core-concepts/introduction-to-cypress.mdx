...
You query for elements with [`cy.get()`](/api/commands/get), using the same CSS
selectors you already use in the browser:

```js
cy.get('.my-selector')
```

Cypress supports the full range of CSS selectors, and you can chain DOM traversal
methods to drill into complex HTML structures:
...
Cypress includes a powerful
[selector engine](/app/references/bundled-libraries#Other-Library-Utilities) and
many DOM traversal helpers, so selecting elements feels familiar and stays
readable.
...
There's one thing to know up front: a query like
[`cy.get()`](/api/commands/get) does **not** return the element synchronously.
...
Instead, Cypress yields the element to the next command in the chain. That
difference is the key to everything Cypress does for you. Let's look at why.

### Cypress retries queries automatically {#Cypress-Retries-Queries-Automatically}

What happens when a query can't find any matching elements yet? In many testing
tools, the query returns immediately with nothing, leaving you to add conditional
checks and retry by hand, which quickly leads to flaky tests.

Cypress takes a different approach. When an element isn't there yet, it's _no big
deal_: Cypress automatically retries the query until either:
...
### Implicit assertions {#Implicit-Assertions}
...
## Chaining commands and subjects {#Chaining-Commands-and-Subjects}
...
### Commands are asynchronous {#Commands-Are-Asynchronous}
...
### Commands run serially {#Commands-Run-Serially}
...
### The Cypress command queue {#The-Cypress-Command-Queue}
