...
sidebar_label: How Cypress works
sidebar_position: 10
slug: /app/core-concepts/introduction-to-cypress
---
...
Cypress uses jQuery's powerful selector engine to help make tests familiar
...
As we saw in the initial example, Cypress clicks on and types into elements on
the page with [`.click()`](/api/commands/click) and
...
In each of these examples, Cypress will
...
Below is one way the code above could be rewritten to ensure the
...
about handling asynchronous Cypress Commands, read our
...
manner to create consistency. Because integration and e2e tests
...
`expect` asserts on any JavaScript object, not just the current subject. This is
probably how you're used to seeing assertions written in unit tests:
...
You _never_ change the timeout inside the assertion. The `timeout`
...
- [`cy.task()`](/api/commands/task) runs code in Node such as _seeding a
