...
Simplicity is about getting more done with less code. Let's look at an
example:
...
example work. You'll learn how to write tests that behave as much like a real
user as possible, and when it makes sense to take shortcuts.
...
[customizable timeouts](/app/references/configuration#Timeouts), Cypress handles
all of this for you, so the flake simply never appears.
...
As we saw above, Cypress anticipates the asynchronous nature of web applications
and doesn't give up the first time an element isn't found. Instead, it gives your
app a window of time to finish whatever it's doing before failing.

That window is called a `timeout`, and most commands let you customize it
([the default is 4 seconds](/app/references/configuration#Timeouts)). Any command
that supports this lists a `timeout` option in its API documentation, telling you
how to set the number of milliseconds Cypress should keep retrying before it
gives up.
...
Understanding how Cypress chains commands together is key to using it well.
Behind the scenes, Cypress manages a Promise chain on your behalf: each command
yields a _subject_ to the next command, and the chain continues until it ends or
an error is encountered. You never need to work with Promises directly, but
knowing what's happening under the hood will make your tests easier to reason
about.

### Interacting With Elements

As we saw in the first example, Cypress lets you click on and type into elements
by chaining the [`.click()`](/api/commands/click) and
[`.type()`](/api/commands/type) action commands onto a
[`cy.get()`](/api/commands/get) or [`cy.contains()`](/api/commands/contains)
query. This is chaining in action. Let's look at it again:
...
Assertions let you describe the _desired_ state of your application: that an
element is visible, or has a particular attribute, CSS class, or value. Rather
than checking a condition once, Cypress automatically waits until your elements
reach the state you describe, and only fails the test if they never do. Here's a
quick look at assertions in action:
...
it reaches that state, or fail the test if it never does.
...
Almost every command can time out in some way, and retrying can't continue
forever. A timeout is the upper bound on how long Cypress will wait and retry
