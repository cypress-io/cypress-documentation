...
### Cypress retries queries automatically

Web apps are asynchronous: an element you're looking for may not be in the DOM
at the instant your code runs. Without built-in retrying, a query returns
whatever exists at that moment, which is nothing if your app hasn't rendered yet,
leaving you to add conditional checks and manual waits that quickly lead to flaky
tests.
