...
Writing component tests with an AI coding tool? Use the `/cypress-author` skill — it reads your existing component tests and matches your conventions. Note that Studio AI and `cy.prompt()` don't currently support component testing, so AI Skills are the best way to accelerate this workflow. See [Cypress AI Skills](/app/tooling/ai-skills).
