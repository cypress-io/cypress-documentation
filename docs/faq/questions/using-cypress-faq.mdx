...
- You can override this with `userAgent` in your
  [Cypress configuration](/guides/references/configuration#Browser).
- You can override the config value for a **single** `cy.visit()` by setting the
  `user-agent` header in the
  [options to `cy.visit()`](../../api/commands/visit#User-agent)
