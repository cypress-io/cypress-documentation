...
<DocsImage
  src="/img/app/cloud/runs/run-details-page.png"
...
You can read more [here](/app/cloud/introduction).
...
[delete videos or screenshots](/app/guide/screenshots-and-videos#Control-which-videos-to-keep-and-upload-to-Cypress-Cloud)
...
[disable capturing](/app/cloud/test-replay#Opt-out-of-Test-Replay) this data.
...
[CI provider](/app/continuous-integration/introduction), and plays a
completely different role.

It doesn't replace or change anything related to CI. You will still run Cypress
tests in your CI provider.

The difference between [Cypress Cloud](/app/cloud/introduction) and your CI
...
[Cypress Cloud](/app/cloud/introduction) provides you with the low level
...
Tests are recorded when [cypress run](/app/guide/command-line#cypress-run)
is called with the `--record` flag while supplying the record `--key`. This
means your [test run data](/app/cloud/recorded-runs#Latest-Runs) is being
...
[Billing & Usage](/app/cloud/account-management/billing-and-usage) page
within Cypress Cloud.

### <Icon name="angle-right" /> What counts as a user?

A [user](/app/cloud/account-management/users) is anyone with a login to
...
[users](/app/cloud/account-management/users) you explicitly invite to your
[organization](/app/cloud/account-management/organizations) can see its
...
[cypress run](/app/guide/command-line#cypress-run) without the `--record`
...
you've [set up your tests to record](/app/cloud/recorded-runs#Latest-Runs).
...
[Identification](/app/cloud/account-management/projects#Identification)
section of the [Cypress Cloud](/app/cloud/introduction) docs.

### <Icon name="angle-right" /> What is a Record Key?

A _Record Key_ is a GUID that's generated automatically by Cypress once you've
[set up your tests to record](/app/cloud/recorded-runs#Latest-Runs). It helps
...
<DocsImage
  src="/img/app/cloud/projects/record-key-desktop-gui.jpg"
...
1. First [set up the project to record](/app/cloud/getting-started#Setup).
2. Then [record your runs](/app/cloud/recorded-runs#Latest-Runs).

After recording your tests, you will see them in
[Cypress Cloud](https://on.cypress.io/cloud) and in the Cypress App [Runs](/app/core-concepts/cypress-app#Runs) tab.

### <Icon name="angle-right" /> Can I delete a run from Cypress Cloud?

You can [archive a run](/app/cloud/recorded-runs#Archive-run) so that it does
...
Cypress
[test parallelization](/app/cloud/smart-orchestration/parallelization) is
...
[CI examples we have](/app/continuous-integration/ci-provider-examples) and
...
[Cypress Cloud Guide.](/app/cloud/account-management/organizations#Organization-ID)

### <Icon name="angle-right" /> What if I can't access my Cypress Cloud account?

1. If you haven't signed up, see the
   [Sign Up with an Invitation](/app/cloud/account-management/users#Sign-Up-with-an-Invitation)
   instructions.
...
[Test Replay](/app/cloud/test-replay) enables you and your team to troubleshoot and debug failed tests faster. After updating to Cypress `v13`, the new feature will automatically record all browser events and allow you to “replay” what your application under test looked like.
...
<DocsImage
  src="/img/app/cloud/test-replay/test-replay-ui.png"
...
Test Replay would be disabled, with a message that it's only available on Chromium, for tests run in Firefox or WebKit (Safari). You can still record and capture test [artifacts](/app/cloud/recorded-runs#Artifacts) (screenshots, videos and CI logs) via other browsers in separate [run groups](/app/cloud/smart-orchestration/parallelization#Grouping-test-runs).
...
At this time, users can only opt-out of Test Replay via [project-level settings](/app/cloud/test-replay#Opt-out-of-Test-Replay) in Cypress Cloud. There is no local option for opting out at this time.
