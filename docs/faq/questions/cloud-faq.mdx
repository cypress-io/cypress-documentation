...
You can read more [here](/cloud/introduction).
...
[disable capturing](/cloud/test-replay#Opt-out-of-Test-Replay) this data.
...
The difference between [Cypress Cloud](/cloud/introduction) and your CI
...
[Cypress Cloud](/cloud/introduction) provides you with the low level
...
means your [test run data](/cloud/recorded-runs#Latest-Runs) is being
...
you've [set up your tests to record](/cloud/recorded-runs#Latest-Runs).
...
section of the [Cypress Cloud](/cloud/introduction) docs.

### <Icon name="angle-right" /> What is a Record Key?

A _Record Key_ is a GUID that's generated automatically by Cypress once you've
[set up your tests to record](/cloud/recorded-runs#Latest-Runs). It helps
...
1. First [set up the project to record](/cloud/getting-started#Setup).
2. Then [record your runs](/cloud/recorded-runs#Latest-Runs).
...
You can [archive a run](/cloud/recorded-runs#Archive-run) so that it does
...
Cypress
[test parallelization](/cloud/smart-orchestration/parallelization) is
...
[Test Replay](/cloud/test-replay) enables you and your team to troubleshoot and debug failed tests faster. After updating to Cypress `v13`, the new feature will automatically record all browser events and allow you to “replay” what your application under test looked like.
...
<DocsImage
  src="/img/cloud/test-replay/test-replay-ui.png"
...
Test Replay would be disabled, with a message that it's only available on Chromium, for tests run in Firefox or WebKit (Safari). You can still record and capture test [artifacts](/cloud/recorded-runs#Artifacts) (screenshots, videos and CI logs) via other browsers in separate [run groups](/cloud/smart-orchestration/parallelization#Grouping-test-runs).
...
At this time, users can only opt-out of Test Replay via [project-level settings](/cloud/test-replay#Opt-out-of-Test-Replay) in Cypress Cloud. There is no local option for opting out at this time.
