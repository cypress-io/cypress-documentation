...
| Run & Debug Tests | `cypress tap` CLI                          | Lets AI agents drive a live Cypress session: run specs, read failures, and inspect the app under test            |
...
:::note
**Best for:** AI coding agents that verify their own changes against a live Cypress session while they code.
:::

`cypress tap` is an extension of the `cypress` CLI that enables driving and inspecting an open-mode Cypress
session from the terminal. An agent can run a spec and poll its status, then
read the failing test's Command Log and error the way the reporter shows them.
It can also inspect the DOM of the app under test at the moment a command ran.
Every command outputs JSON built for automation.

Where the Cloud CLI and Cloud MCP give agents your recorded run data in
Cypress Cloud, `cypress tap` gives your AI agent the live session on your
machine, so failures are found and fixed before the code ever reaches CI.
...
- [cypress tap: drive a live Cypress session from the terminal](/app/tooling/cypress-tap)
