...
`cypress tap` is an experimental set of CLI commands that drive a running
`cypress open` session from the terminal. An agent can run a spec, poll the
run's status, read the failing test's Command Log and error the way the
reporter shows them, and inspect the DOM of the app under test at the moment a
command ran — all with JSON output built for automation.

Where the Cloud CLI and Cloud MCP give agents your recorded run data in
Cypress Cloud, `cypress tap` gives them the live session on your machine, so
failures are found and fixed before the code ever reaches CI.
