...
- How many Cypress tests are you running across all projects in your organization?
- Where are those tests concentrated?
- How has test usage changed over time?

These are a few of the questions that can be answered by the cards shown on the **Overall Usage** tab of Enterprise
Reporting. Use the `Start Date` and `End Date` filters if you need to drill into a specific time period. The `Projects`
...
of Enterprise Reporting shows the status rates of your tests at all three levels of granularity.
...
The **Flaky Tests** tab on Enterprise Reporting shows flaky test occurrences and rates for your entire organization or
specific projects. This can be a single snapshot based on the start and end date filters, or can be viewed over time. Flake
_rate_ can be helpful because it reports what % of runs in a project are flaky. This allows easy comparison of flaky
...
All of the charts shown in Enterprise Reporting offer a download of the raw data used to
...
chart, and a menu will be displayed that delivers the data in the following formats:
...
Cypress Cloud API. To download, hover over the chart, select `Download results`, and choose the
