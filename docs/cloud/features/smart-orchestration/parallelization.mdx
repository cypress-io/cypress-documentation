...
that buffer often has to be long so that pauses in between test groups do not cause the run to complete early.

Without keeping the run open to wait for new groups, a late group
attempting to join an already-completed Cloud Run would cause an error.
...
The end result: **no tradeoff needed** between the potential recording gaps in your CI setup,
and having Cypress Cloud to resolve your runs as quickly as possible with correct grouping.
...
For that reason, if you do not pass the `expectedGroupCount`, the most reliable place to make the call is a **dedicated
...
:::tip
**Tip:** Hard-coding an expected group count in your config file is an easy way to try this API out with **no CI updates**,
and in some cases may fully meet your needs.
:::
...
environment variable that CI it to the run's fixed number of groups.
