...
## Run Completion API <Badge type="caution">Experimental</Badge>

The run completion delay is a safety buffer: it protects against late CI machines
racing past a run that Cypress Cloud has already marked complete. For teams with
non-deterministic build timing, that buffer often has to be long, which taxes
_every_ run — including the ones where CI already knows all the work is done.

The **Run Completion API** gives CI and tooling an explicit "we're done" signal.
When you know a parallelized or grouped run has finished, you can call the API to
complete the run immediately and skip the remaining completion delay.

:::caution

This API is **experimental**. Its request and response contract may change in a
backward-incompatible way, and the endpoint may be altered or removed, without a
major version bump. Pin your usage to the behavior documented here and check back
for updates before relying on it in critical pipelines.

:::

:::info

This is an imperative signal, not a force. The API never seals a run mid-flight —
...
<strong>Where to call the API in a parallelized run</strong>

On a parallelized or grouped run, `after:run` fires **once per machine** — so
every machine sees the same `runUrl` and would each call the endpoint. Firing a
bare "complete now" request from all of them races: whichever machine finishes
its own specs first would ask to complete a run whose other groups are still
going.

For that reason, the most reliable place to make the call is a **dedicated
...
groups have finished (and absorbs any still joining), it doesn't matter which
machine's handler wins the race — the run only completes once all expected groups
...
environment variable that CI sets to the run's fixed number of groups: each
machine's call is deferred until that many groups have finished, so the run
completes exactly once — no matter which handler runs first.
...
Without `expectedGroupCount`, this same handler would be unsafe: the first
machine to finish could complete the run while other groups are still recording.
If you can't provide a reliable group count, call the API from a
[dedicated post-run CI step](#Finding-the-run-URL-or-run-number) instead.
...
(not merely registered). A request that arrives slightly early waits for the
remaining in-flight groups — and absorbs any that join in the meantime — rather
than sealing the run partway through.
