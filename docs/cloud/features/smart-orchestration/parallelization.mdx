...
Cypress' parallelization strategy is file-based, so to use
  ...
  build:
    jobs:
      - cypress/run:
          ...
          cypress-command: 'npx cypress run --record --browser chrome'
```
  ...
  build:
    jobs:
      - cypress/run:
          parallelism: 2
          start-command: 'npm run start'
          cypress-command: 'npx cypress run --record --parallel --browser chrome'
```

It finished in **59 seconds**.

```text
2x-chrome, Machine #1, 9 specs            2x-chrome, Machine #2, 10 specs
--------------------------------          -----------------------------------
...
runs can be used independently of Cypress parallelization.
...
The third group can be called `Linux/Firefox`.

```shell
cypress run --record --group Linux/Firefox --browser firefox
```
...
group to test against Firefox with 4 machines, and another group to test
against Firefox again, but only with one machine:

```shell
cypress run --record --group 1x-firefox --browser firefox
```
...
```shell
cypress run --record --group 4x-firefox --browser firefox --parallel
```
...
- **From the [`after:run`](/api/node-events/after-run-api) or [`before:run`](/api/node-events/before-run-api) Node event:** the
...
- [Blog: Run Your End-to-end Tests 10 Times Faster with Automatic Test Parallelization](https://www.cypress.io/blog/2018/09/05/run-end-to-end-tests-on-ci-faster/?utm_source=docs.cypress.io&utm_medium=parallelization)
