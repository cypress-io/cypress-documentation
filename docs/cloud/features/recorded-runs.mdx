...
The Errors tab aggregates any errors thrown during tests in this run.
...
The diff is based on the state of the code **after** transpilation,
...
You can cancel a run currently in progress from Cypress Cloud's UI or through the [Run Cancellation API](/cloud/features/smart-orchestration/run-cancellation-api). From the UI, runs can only be cancelled by members of the project. Through the API, cancellation requires your project's Record Key.

### To cancel a run through Cypress Cloud's UI
...
- A message will appear on the run details page showing the time of cancellation
  and the user that cancelled the run.
...
- Any tests recorded to completion will still count towards your monthly test
  results limit.
- If you have Analytics enabled, cancelled runs will appear in the
...
- Any incomplete calls to
  ...
  will be marked as cancelled and not run.
- Any existing calls to
  ...
  <DocsImage
    ...
    alt="Error shown when a recorded run is cancelled"
  />
...
Runs that have been cancelled or are in an errored state can be archived from
Cypress Cloud.
...
The [Debug](/app/core-concepts/open-mode#Debug) page brings failed CI test runs
into the Cypress app. From this page, you can
