...
You can cancel a run currently in progress from Cypress Cloud's UI or through the [Run Cancellation API](/cloud/features/smart-orchestration/run-cancellation-api). From the UI, runs can only be canceled by members of the project. Through the API, cancellation requires your project's Record Key.
