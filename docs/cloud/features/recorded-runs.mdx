...
- The test's **hooks**: the `before`, `beforeEach`, `afterEach`, and `after`
  blocks that run around it.
...
A test is matched by its **spec file path** and its **full title**. Renaming the
