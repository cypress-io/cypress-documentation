...
- **Network or HTTP error**: These errors indicate that Cypress could not establish a connection to upload. To fix:
  ...
  - Ensure that the network configuration of the machine recording the spec allows for [Cypress URLs](/guides/references/troubleshooting#Allow-Cypress-URLs-on-VPNs).
