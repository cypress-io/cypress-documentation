...
| CI storage footprint & cost                 | Written to the file system; when kept as CI artifacts, consumes billed artifact storage and retention quota | Written to a temp directory that is deleted after the run; no local or CI artifact storage cost |
...
Unlike video files, which teams often retain as CI artifacts and which accrue billed artifact storage
over their retention window, Test Replay leaves nothing behind on your CI runners, so it adds no local
or CI artifact storage cost.
