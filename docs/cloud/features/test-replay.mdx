...
account to use Test Replay's debugging capabilities. No additional configuration
...
## Access Test Replay from the command line

You can also query a test's Test Replay from your terminal with the
[Cloud CLI](/cloud/integrations/cloud-cli) (`cy-cloud`), without opening a
browser. This is handy for scripting and for AI agents that work through a
terminal.

Get a summary of a replay, including how many command, network, and log events
it holds and where the failure occurred:

```bash
cy-cloud replay info --testId <testId>
```

Query the timeline itself as JSON. Choose event types with `--commands`,
`--network`, and `--logs`, and narrow to the failure with `--aroundFailure` or
`--failedOnly`:
...
captured data as the Test Replay interface, so it works on any run where Test
Replay was recorded and is within your data retention window. See the
[Cloud CLI documentation](/cloud/integrations/cloud-cli#cy-cloud-replay-timeline)
...
### Network data redaction in Test Replay

By default, Cypress redacts sensitive values from captured network requests and responses before Test Replay data is uploaded to Cypress Cloud. Redaction runs in the Cypress app, on your machine. Redacted values appear as `(redacted)` in the network log.

To learn more about what is redacted and how to change this, see [Network data redaction](/cloud/account-management/data-storage-and-masking#Network-data-redaction).

### Sensitive form input masking in Test Replay

By default, Cypress replaces values in password and payment fields with
asterisks before creating Test Replay artifacts. Masking runs in the Cypress App
on your machine before Cypress uploads any data to Cypress Cloud.

To learn which fields Cypress masks and how to change this setting, see
[Sensitive form input masking](/cloud/account-management/data-storage-and-masking#Sensitive-form-input-masking).
...
Refer to the [Cypress Cloud Terms of Use](https://cloud.cypress.io/terms-of-use) and our [Security & Compliance](https://www.cypress.io/security/?utm_source=docs.cypress.io&utm_medium=test-replay) guide for specifics around reasonable use of Cypress Cloud, data storage, and security at Cypress.

Test Replays, including the test data used within your tests, are visible to everyone with access to your project.
...
**Tip:** Before troubleshooting further, ensure you're using the latest version of Cypress. We fix bugs with Test Replay often,
...
2. Ensure tests were recorded using a Chromium-based browser (Chrome, Edge, or the deprecated Electron browser).
...
- [Branch Review](/cloud/features/branch-review) - compare test results and Test Replays side by side between branches
- [Cloud CLI](/cloud/integrations/cloud-cli) - query a test's replay timeline of commands, network requests, and logs from your terminal
- [Test Replay FAQ](/cloud/faq#Test-Replay)
- [Data storage and controls](/cloud/account-management/data-storage-and-masking#Test-Replay-controls) - network redaction, sensitive input masking, and other Test Replay controls
