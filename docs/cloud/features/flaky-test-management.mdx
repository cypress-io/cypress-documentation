...
Once you've identified a flaky test, use [Test Replay](/cloud/features/test-replay)
to replay the failing attempt exactly as it ran in CI, inspecting the DOM, network
requests, console logs, and JavaScript errors, so you can pin down the root cause without
...
- [Test Replay](/cloud/features/test-replay): debug detected flake by replaying the run as it happened in CI
- [Test Retries](/app/guides/test-retries): the mechanism that powers flake detection
