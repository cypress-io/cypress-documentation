...
- **The Cloud CLI** (`cy-cloud`) is a command-line tool for humans, scripts, and agents that run shell commands. It returns JSON, which makes it the best fit for automation, custom tooling, and agents that work through a terminal.

Many teams use both. If your assistant speaks MCP, connect it to Cloud MCP. If it works through a terminal, or you are scripting, reach for the Cloud CLI.
