...
To connect with a personal access token instead, or to set up any other client, follow the steps for your client below.
...
1. Add the Cloud MCP server:
   ...
   ```bash
   claude mcp add --transport http cypress-cloud https://mcp.cypress.io/mcp
   ```
...
Use the [Quick install](#Quick-install) button above to add the server in one step, or configure it yourself:
...
You can also add the server to your user profile from the command line:

```bash
code --add-mcp "{\"name\":\"cypress-cloud\",\"type\":\"http\",\"url\":\"https://mcp.cypress.io/mcp\"}"
```
