...
| `cypress_get_tests`                       | Returns per-test results for a run at any status, including skipped and pending. Includes each test's duration, flaky status, a Test Replay link, and a full per-attempt breakdown. Filter by test status, or by the status of the spec it lives in. | "List every skipped and pending test in this run, grouped by spec."     |
...
By design, these tools return compact responses to keep token usage and context costs low — just the essentials for the question at hand. When your prompt calls for more, the agent pulls the additional detail it needs, so you get depth on demand rather than paying for it on every call.
...
<CopyPrompt
  ...
  prompt={`For the latest run on main in Cypress Cloud, give me a release report: who authored the commit, what the run status was, which specs failed, and which tests inside them failed.`}
>
...
### The Suite Time Audit

</CopyPrompt>

<CopyPrompt
  title="The Retry Tax"
  subtext="Separate genuinely slow tests from fast ones burning time on retries."
  prompt={`For run [Run URL] in Cypress Cloud, find tests that took more than 20 seconds and show me each attempt's duration. Tell me how much of that time was retries.`}
>
...
### The Skipped Test Audit

</CopyPrompt>

<CopyPrompt
  title="The Accessibility Triage"
