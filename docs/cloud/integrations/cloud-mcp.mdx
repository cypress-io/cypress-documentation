...
<CopyPrompt
  ...
  defaultExpanded
  prompt={`Check Cypress Cloud for the latest run on this branch.\nGive me a high-level summary of any failures.`}
/>
...
<CopyPrompt
  ...
  defaultExpanded
  prompt={`Find the failing tests for [Run URL].\nFor each failure, look at the stack trace, check my local code, and propose a fix to stop the regression.`}
/>
...
<CopyPrompt
  ...
  defaultExpanded
  prompt={`List all flaky tests from the last 5 runs in Cypress Cloud.\nAre there any common patterns in the error messages?`}
/>
...
<CopyPrompt
  ...
  defaultExpanded
  prompt={`Pull the Cypress Cloud Accessibility report for the latest run on this branch.\nList any critical or serious violations grouped by rule, and tell me which views are most impacted.`}
/>
...
<CopyPrompt
  ...
  defaultExpanded
  prompt={`Using Cypress Cloud, pull the UI Coverage report for the latest run on this branch.\nTell me the overall coverage score, list the views with the most untested elements, and show me the specific untested elements on the riskiest view.`}
/>
