...
Some tools are relevant for all Cypress Cloud plans (including free), but some others are for **plan-specific data** like flaky test reporting, or **product-specific data** like Cypress Accessibility violation details. View and component names in those accessibility responses follow your [`accessibility.components`](/accessibility/configuration/components) configuration. These are only available if your organization has subscriptions for the plans and products that would provide the underlying data.
...
To keep MCP output aligned with how your app is structured, configure [`accessibility.components`](/accessibility/configuration/components) in your Cypress project - while not required, this can make it easier and faster to trace issues to their source, consuming minimum resources. 

In general, it's a good idea to look carefully at the _inputs_ for your accessibility reports, to make sure the contents are things you are currently interested in having your agent triage. You can learn how exclude pages or elements, mute known issues for certain elements, and reduce duplication in the [Cypress Accessibility configuration overview](/accessibility/configuration/overview).
