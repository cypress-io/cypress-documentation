...
If your AI client does not support OAuth, use a [personal access token](#authenticate-with-a-personal-access-token-alternative) instead.

### Authenticate with a Personal Access Token (Alternative)
...
3. Add the following to your `claude_desktop_config.json`:
   ...
   ```
4. Restart Claude Desktop after saving. It will open a browser window to sign in to Cypress Cloud. You should see a tools indicator in the chat input after authenticating.
...
2. Claude Code will open a browser window to sign in to Cypress Cloud when the server is first used.
