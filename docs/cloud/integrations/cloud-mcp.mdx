...
| `cypress_get_runs`                        | Returns run summaries for a project, run URL, or git branch, and can filter by run status. Summaries include run number, status, run URL, tags, test status counts, the commit author, duration, and CI build ID. | "Check the status of the last CI run on main"                           |
| `cypress_get_specs`                       | Returns per-spec results for a run, filterable by status, including each spec's duration, group, and test counts.                                                                                                 | "Find my ten slowest specs and tell me which ones to split."            |
| `cypress_get_tests`                       | Returns per-test results for a run at any status, including skipped and pending, with per-attempt durations and a Test Replay link. Filter by test or spec status.                                                | "List every skipped and pending test in this run, grouped by spec."     |
| `cypress_get_failed_tests`                | Retrieves attempt details, like the error, stack trace, per-attempt duration, and a Test Replay link for debugging. Does not return test artifacts.                                                               | "Analyze why the checkout test failed in the latest run."               |
| `cypress_get_flaky_tests`                 | Identifies the flaky tests in a run, with per-attempt durations.                                                                                                                                                  | "Are these failures regressions or known flakiness?"                    |
...
| `cypress_get_accessibility_views`         | Returns paginated views from an accessibility report, sorted by number of failing rules.                                                                                                                          | "Which pages have the most accessibility failures in this run?"         |
...
| `cypress_get_ui_coverage_views`           | Returns paginated views from a UI Coverage report, sorted by most untested elements and links.                                                                                                                    | "Which pages have the most untested elements in this run?"              |
...
By design, these tools return compact responses to keep token usage and context costs low so just the essentials for the question at hand. When your prompt calls for more, the agent pulls the additional detail it needs, so you get depth on demand rather than paying for it on every call.

## Writing effective prompts

To get the most out of Cloud MCP, follow these four principles:
...
- **Scope the Ask:** Ask for only what you need — a run status, a branch, a specific run. Narrower prompts return smaller responses and keep the agent's context free for the actual debugging.
...
<CopyPrompt
  title="Investigate failing tests and propose fixes"
  subtext="Investigate each failing test and propose a fix for the regression."
  prompt={`Find the failing tests for [Run URL]. For each failure, look at the stack trace, check my local code, and propose a fix to stop the regression.`}
>
...
<CopyPrompt
  title="Find flaky tests and patterns across recent runs"
  subtext="Surface flaky tests across recent runs and look for common patterns."
  prompt={`List all flaky tests from the last 5 runs in Cypress Cloud. Are there any common patterns in the error messages?`}
>

### Find flaky tests and patterns across recent runs

</CopyPrompt>

<CopyPrompt
  title="Build a release report of Cypress runs"
  subtext="Tie a run back to the change and the person behind it, down to the failing tests."
  prompt={`For the latest run on main in Cypress Cloud, give me a release report: who authored the commit, what the run status was, which specs ran, and which tests were skipped.`}
>

### Build a release report of Cypress runs

</CopyPrompt>

<CopyPrompt
  title="Find the slowest specs and break one down by test"
  subtext="Find where your suite time goes, from run to spec to individual test."
  prompt={`For run [Run URL] in Cypress Cloud, pull spec durations and tell me which specs are the slowest. Then break the slowest one down by test so I know whether to split it.`}
>

### Find the slowest specs and break one down by test

</CopyPrompt>

<CopyPrompt
  title="Audit skipped and pending tests in a run"
  subtext="Catch coverage erosion by listing what a run has stopped testing."
  prompt={`List every skipped and pending test in run [Run URL] in Cypress Cloud, grouped by spec.`}
>

### Audit skipped and pending tests in a run

</CopyPrompt>

<CopyPrompt
  title="Triage Cypress Accessibility violations by rule and view"
  subtext="Summarize accessibility violations for the latest run, grouped by rule and view."
  prompt={`Pull the Cypress Cloud Accessibility report for the latest run on this branch. List any critical or serious violations grouped by rule, and tell me which views are most impacted.`}
>

### Triage Cypress Accessibility violations by rule and view
...
<CopyPrompt
  title="Review UI Coverage and find untested elements"
  subtext="Review UI Coverage for the latest run and drill into the riskiest view."
  prompt={`Using Cypress Cloud, pull the UI Coverage report for the latest run on this branch. Tell me the overall coverage score, list the views with the most untested elements, and show me the specific untested elements on the riskiest view.`}
>

### Review UI Coverage and find untested elements
...
Cloud MCP and the [Cloud CLI](/cloud/integrations/cloud-cli) both give programmatic access to your Cypress Cloud data. They solve related problems in different ways:
...
Both are available on every Cypress Cloud plan at no additional cost. See the [Cloud CLI documentation](/cloud/integrations/cloud-cli) for its setup and command reference.
...
Cloud MCP implements the standard MCP authorization specification with OAuth 2.0 and uses streamable HTTP transport. It falls under our [AI security practices →](https://on.cypress.io/ai-security?utm_source=docs.cypress.io&utm_medium=cloud-mcp)
...
<CopyPrompt
  hideTitle
  title="Send feedback"
  subtext="Share feedback, use cases, or issues with Cypress from your agent."
  prompt={`Send Cypress the following feedback: [Your message here]`}
/>
