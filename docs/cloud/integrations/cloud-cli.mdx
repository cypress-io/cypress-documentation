...
Older versions are eventually unsupported. When your version falls behind, commands print a warning to upgrade, and versions older than the minimum stop working. Check your version and its support status with [`cy-cloud version`](#cy-cloud-version), and upgrade to the latest release when needed:
...
`cy-cloud logout` clears the credentials stored on your machine, but the underlying session or token stays valid until it expires. To revoke access, open your [Cypress Cloud profile](https://cloud.cypress.io/profile) and, under **Cloud CLI access**, remove the OAuth connection or delete the personal access token. Revoking there invalidates the credential everywhere, including anywhere it is still stored locally.
