...
**6. Verify the fix before you push.** Rerun the same spec against a live `cypress open` session with [`cypress tap`](/app/tooling/cypress-tap). Pass the `specFilepath` from [`test list`](#cy-cloud-test-list) to [`cypress tap run`](/app/references/command-line#cypress-tap-run) to run it locally. Then, run [`cypress tap status`](/app/references/command-line#cypress-tap-reporter) to verify the fix is correct; if it isn't, your agent can use `cypress tap` to inspect the full report, dig into the command logs, and inspect the DOM to make corrections before recording to the Cloud.
