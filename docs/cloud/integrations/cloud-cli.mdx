...
<PackageManagerTabs install="@cypress/cloud@latest" global />
...
| [`cy-cloud replay view`](#cy-cloud-replay-view)         | Open a test replay in the browser to inspect your app. Requires `cy-cloud` `0.4.0`+. |
...
Each spec includes its id, status, whether Cypress Cloud has flagged it as flaky, and its file path:
...
Filter to only flaky specs with `--onlyFlaky`. This requires a Cypress Cloud plan with [Flaky Test Management](/cloud/features/flaky-test-management); without one, the command returns an error.

```bash
cy-cloud spec list --projectId <projectId> --runNumber <runNumber> --onlyFlaky
```
...
Each test includes its title parts, spec, status, whether Cypress Cloud has flagged it as flaky, a Test Replay URL (when available), and, for failures, the error and stack trace per attempt:
...
Filter to only flaky tests with `--onlyFlaky`. This requires a Cypress Cloud plan with [Flaky Test Management](/cloud/features/flaky-test-management); without one, the command returns an error.

```bash
cy-cloud test list --projectId <projectId> --runNumber <runNumber> --onlyFlaky
```
...
`replay view` requires `cy-cloud` version `0.4.0` or later. Check your version with [`cy-cloud version`](#cy-cloud-version) and [upgrade](#Keep-the-CLI-updated) if you are on an older release.
...
**6. Verify the fix before you push.** The best way to confirm the fix is to run the test locally. Start Cypress with `cypress open`, then use [`cypress tap`](/app/tooling/cypress-tap) to start the spec: pass the `specFilepath` returned from [`cy-cloud test list`](#cy-cloud-test-list) to [`cypress tap run`](/app/tooling/cypress-tap#cypress-tap-run). Then run [`cypress tap status`](/app/tooling/cypress-tap#cypress-tap-status) to verify the updated test passed; if it isn't, your agent can use `cypress tap` to inspect the full report, dig into the command logs, and inspect the DOM to make corrections before recording to the Cloud.
...
A [flaky test](/cloud/features/flaky-test-management) failed and then passed on retry, so its status is `passed` but it has more than one attempt. This requires [test retries](/app/guides/test-retries).

**1. Find the flaky tests in a run.** If your plan includes Flaky Test Management, list only the tests Cypress Cloud has flagged as flaky with `--onlyFlaky`:

```bash
cy-cloud test list --projectId <projectId> --runNumber <runNumber> --onlyFlaky
```

Without that plan, spot them by attempt count instead: list the passed tests and keep only those with more than one attempt:
