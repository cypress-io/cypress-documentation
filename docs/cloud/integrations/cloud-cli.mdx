...
If your agent runs shell commands (for example, Claude Code, Codex CLI, or Copilot CLI), point it at `cy-cloud` and describe the goal in plain language. Copy one of the prompts below to get started, swapping in your own project ID, branch, and test ID.

<CopyPrompt
  title="Summarize the latest failure"
  subtext="Find the most recent failed run on a branch and explain why each test failed."
  prompt={`Using the cy-cloud CLI, find the most recent failed run on main for project abc123 and summarize why each failing test failed.`}
>
...
<CopyPrompt
  title="Debug from Test Replay"
  subtext="Pull the Test Replay timeline around a failure and propose a fix in your code."
  prompt={`With cy-cloud, pull the replay timeline around the failure for test <testId>, including commands, network requests, and console logs, then look at my code and propose a fix.`}
>
...
<CopyPrompt
  title="Triage a specific run"
  subtext="Confirm you are signed in, list the failing specs in a run, and save a failure screenshot."
  prompt={`Run cy-cloud status to confirm I am signed in, then list the failing specs in run 4021 of project abc123 and save the first failure's screenshot to ./screenshots.`}
>
...
<CopyPrompt
  title="Daily failure check"
  subtext="Check the latest run each day and summarize any failures."
  prompt={`Every morning, run cy-cloud to check the latest run on main for project abc123. If it failed, open a summary of the failing tests and their errors.`}
>
