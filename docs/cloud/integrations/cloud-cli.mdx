...
<CopyPrompt
  ...
  prompt={`Using the cy-cloud CLI, find the most recent failed run on <branch> for project <projectId> and summarize why each failing test failed.`}
>
...
<CopyPrompt
  ...
  prompt={`Run cy-cloud status to confirm I am signed in, then list the failing specs in run <runNumber> of project <projectId> and save the first failure's screenshot to ./screenshots.`}
>
...
<CopyPrompt
  ...
  prompt={`Every morning, run cy-cloud to check the latest run on <branch> for project <projectId>. If it failed, open a summary of the failing tests and their errors.`}
>
