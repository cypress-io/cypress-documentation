...
A [flaky test](/cloud/features/flaky-test-management) is one that failed and then passed on retry within the same run. The CLI has no `--status flaky` filter, but a flaky test is easy to spot: its `status` is `passed` and its `attempts` array holds more than one attempt, because it retried after a failure. A test that passes the first time has a single attempt. This workflow assumes [test retries](/app/guides/test-retries) are enabled, since that is what lets a test recover and be marked flaky.
