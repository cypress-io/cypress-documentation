...
**6. Verify the fix before you push.** Rerun the same spec against a live `cypress open` session with [`cypress tap`](/app/tooling/cypress-tap). The `specFilepath` that [`test list`](#cy-cloud-test-list) reports is exactly the path [`cypress tap run`](/app/references/command-line#cypress-tap-run) takes, and [`cypress tap reporter`](/app/references/command-line#cypress-tap-reporter) prints a local command log you can compare against the replay timeline above.
