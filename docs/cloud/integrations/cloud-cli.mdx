...
A [flaky test](/cloud/features/flaky-test-management) is one that failed and then passed on retry within the same run. The CLI has no `--status flaky` filter, but a flaky test is easy to spot: its `status` is `passed` and it still carries an `attempts` array, because at least one earlier attempt failed. This workflow assumes [test retries](/app/guides/test-retries) are enabled, since that is what lets a test recover and be marked flaky.

**1. Find the flaky tests in a run.** List the passed tests and keep only those that retried after a failure:

```bash
cy-cloud test list --projectId <projectId> --runNumber <runNumber> --status passed \
  | jq '.tests[] | select(.attempts != null) | {testId, testName, attempts}'
```
