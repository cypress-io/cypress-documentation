---
title: 'Cypress Cloud API for enterprise reporting'
...
formats that drop into your BI reporting platform, as well as a JSON file that can be
programmatically parsed. This API returns your test data at a variety of levels, including
...
This data helps you understand which projects in your organization are using the
...
This data provides consolidated run status at the project level.
...
This data covers all spec results regardless of spec result status. The maximum number of records
...
This report extracts all test result details for failed tests. The maximum number of records
returned is 500,000. Make multiple requests with different start_date and end_date parameter values if your volume of records will
exceed 500k for a single request.
...
This report extracts all test result details regardless of test result status. The maximum number of records
returned is 500,000. Make multiple requests with different start_date and end_date parameter values if your volume of records will
exceed 500k for a single request.
...
This report returns the average, median, min, and max durations of passing runs. This data is reported over time.
...
This report returns the average, median, min, and max durations of specs. This data is reported over time. The maximum
...
This report shows the flaky _rate_ occurring in your projects, not just the _count_ of flaky tests.
...
This report extracts all test result details of flaky tests. The maximum number of records
returned is 500,000. Make multiple requests with different start_date and end_date parameter values if your volume of records will
exceed 500k for a single request.
...
This report identifies which tests have the highest flaky count and provides a URL to an example of a recent
...
This report identifies which tests have the highest number of failures and provides a URL to an example of a recent
...
This report identifies which tests have the highest number of errors and provides a URL to an example of a recent
...
Learn more about [Cypress UI Coverage](/ui-coverage/get-started/introduction).
...
This report extracts the UI Coverage % for each run along with other attributes of the run. The maximum number of records
returned is 500,000. Make multiple requests with different start_date and end_date parameter values if your volume of records will
exceed 500k for a single request.
...
This report extracts Cypress Accessibility scores for each run along with other attributes of the run. The
