...
## Why use the Data Extract API

The Data Extract API turns the test results captured in Cypress Cloud into a data source you fully control,
so your team can answer questions and demonstrate progress that the built-in dashboards alone can't. Common ways
teams put it to work include:

- **Bring test data into your own BI and analytics tools.** Export results as CSV or XLSX to feed dashboards in
  tools like Power BI, Tableau, or Looker, or pull JSON directly into a data pipeline or warehouse. This lets you
  blend Cypress data with other engineering and business metrics and build the exact views your stakeholders need.
- **Build custom, organization-wide reports.** Aggregate data across every project to report on test health,
  adoption, and quality trends to leadership, instead of reviewing projects one at a time.
- **Track quality and reliability over time.** Many endpoints return data "over time," making it easy to monitor
  trends in pass/fail rates, flake, run durations, and coverage so you can measure whether changes are actually
  moving the needle.
- **Find and prioritize the work that matters most.** Reports such as top failures, top errors, and top flaky
  tests surface your highest-impact problems and include Test Replay links so engineers can jump straight into
  troubleshooting the root cause.
- **Govern and standardize your test suite.** Track which Cypress versions, browsers, and testing types are in use
  across the organization to drive upgrades, enforce coverage standards, and understand how your suite is growing.
- **Automate reporting on your schedule.** Because everything is available through a single API key and a simple
  URL, you can script recurring exports and refresh your reports automatically rather than pulling data by hand.
...
- [Analytics overview](/cloud/features/analytics/overview)
