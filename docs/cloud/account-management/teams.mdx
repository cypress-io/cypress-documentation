...
Teams in Cypress Cloud let you shape your organization so that every person sees the
projects relevant to their work — and nothing else. A team is a reusable grouping of
users that is granted access to a specific set of projects, so you can manage access
once at the team level instead of project by project, user by user.
