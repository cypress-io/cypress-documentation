...
[Start a free trial of Cypress Cloud](/cloud/getting-started) now, or
...
current state of your app on the [Latest Runs](/cloud/recorded-runs) page.

Replay the test as it executed during the recorded run with full debug capability using [<Logo src="/img/cloud/test-replay/replay-icon.svg" /> Test Replay](/cloud/test-replay).

Or view each test's
[command logs, screenshots, video replays, stack traces, and CI logs](/cloud/recorded-runs#Test-detail-sidebar).
...
View each [run's overview](/cloud/recorded-runs#Overview-tab) to see and compare
...
tests with [Flaky Test Management](/cloud/flaky-test-management).

<DocsVideo src="https://vimeo.com/806900039" />

The [analytics](/cloud/analytics/overview) capabilities show test metrics over time,
...
<DocsImage
  src="/img/cloud/analytics/analytics-runstatus.png"
  alt="Analytics run status"
/>

<DocsImage
  src="/img/cloud/analytics/enterprise-reporting-usage-report.png"
  alt="Analytics organization usage"
/>

### Reduce test time & CI costs

[Smart Orchestration](/cloud/smart-orchestration/overview) enables you to run
tests across multiple machines simultaneously in your CI environment while
Cypress Cloud coordinates runners and
[balances test loads](/cloud/smart-orchestration/load-balancing#Balance-strategy) -
no setup required! You can prioritize recently failed specs with
[Spec Prioritization](/cloud/smart-orchestration/spec-prioritization) to
surface problems earlier, and cancel whole test runs on failure with
[Auto Cancellation](/cloud/smart-orchestration/run-cancellation) to save
on resource usage. You can also
[cancel in-progress runs](/cloud/recorded-runs#Run-cancellation) manually
from Cypress Cloud if you need to.

<DocsImage
  src="/img/cloud/introduction/orchestration-diagram.png"
...
The [Auto Cancellation](/cloud/smart-orchestration/run-cancellation)
feature will cancel a whole test run on failure, saving time and resource usage.
You can also [cancel in-progress runs](/cloud/recorded-runs#Run-cancellation)
...
Cypress Cloud integrates with [GitHub](/cloud/integrations/source-control/github),
[GitLab](/cloud/integrations/source-control/gitlab), and
[Bitbucket](/cloud/integrations/source-control/bitbucket) to provide rock-solid
...
integration. You can
[create bidirectionally linked Jira tickets](/cloud/integrations/jira#Creating-a-Jira-issue-for-a-test-case)
directly from specific test failures.

### Cypress app integration

The [Cypress app](/cloud/recorded-runs#Cypress-App) integrates directly with
