...
- [Connect the Cypress app to Cypress Cloud](#Connect-the-Cypress-app-to-Cypress-Cloud) to view CI runs and test health without leaving the app.
...
[Business and Enterprise plans](https://www.cypress.io/pricing/?utm_source=docs.cypress.io&utm_medium=cloud-intro) also include secure
...
Query your Cypress Cloud data from the terminal with the [Cloud CLI](/cloud/integrations/cloud-cli). Check run status, read failing tests, and pull [Test Replay](/cloud/features/test-replay) timelines as JSON, so you can build custom tooling, script automation, or hand the same context to a terminal-based agent.

### Connect the Cypress app to Cypress Cloud

Log in to Cypress Cloud from the [Cypress app](/cloud/features/recorded-runs#Cypress-App)
to see your latest CI test results without switching tools. The app shows run
history alongside your specs, notifies you when new tests fail in CI, and lets
you re-run those failures locally and open [Test Replay](/cloud/features/test-replay)
to debug them. See
[Open the Cypress app](/app/get-started/open-the-app#Connect-the-Cypress-app-to-Cypress-Cloud)
...
for teams on [Business or Enterprise plans](https://www.cypress.io/pricing/?utm_source=docs.cypress.io&utm_medium=cloud-intro).
