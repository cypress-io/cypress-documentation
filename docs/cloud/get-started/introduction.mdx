...
Query your Cypress Cloud data from the terminal with the [Cloud CLI](/cloud/integrations/cloud-cli) (`cy-cloud`). Check run status, read failing tests, and pull [Test Replay](/cloud/features/test-replay) timelines as JSON, so you can build custom tooling, script automation, or hand the same context to a terminal-based agent.
