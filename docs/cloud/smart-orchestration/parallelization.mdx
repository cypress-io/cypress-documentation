...
2. Once multiple machines are available within your CI environment, you can pass
   the [--parallel](/app/guides/command-line#cypress-run-parallel) key to
   [cypress run](/app/guides/command-line#cypress-run) to have your recorded
   tests parallelized.
...
[`--record` flag](/app/guides/command-line#cypress-run) be passed. This
...
Multiple [cypress run](/app/guides/command-line#cypress-run) calls can be
labeled and associated to a **single** run by passing in the
[`--group <name>` flag](/app/guides/command-line#cypress-run-group-lt-name-gt),
...
[`--ci-build-id` flag](/app/guides/command-line#cypress-run-ci-build-id-lt-id-gt).
