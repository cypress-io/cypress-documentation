...
- **In CI**, [Re-run optimization](/cloud/features/smart-orchestration/rerun-optimization)
  re-runs only the spec files that contained failures when you trigger a rerun of a
  recorded build. It operates at the **spec** level: if any test in a spec failed,
  the whole spec is re-executed.
...
only the spec files that contained failures. You can use both together: test
...
No, not currently. A rerun-optimized run shows only the specs that were re-executed. Specs that
passed in the anchor run are skipped and do not appear in the rerun's results.
...
specs, so a rerun-optimized run reports fewer specs and tests than a full run.
...
### <Icon name="angle-right" /> What happens if the rerun has no failed specs?

If the anchor run contained no failing specs, there is nothing to re-execute and
...
from, and the run is labeled with a **Re-run** tag in Cypress Cloud.
