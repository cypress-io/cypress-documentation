...
An empty `commands` array (`"commands": []`) passes validation but makes matching
elements impossible to cover. In rare cases, you may intend this behavior, but to remove elements from
the report instead, use [`elementFilters`](/ui-coverage/configuration/elementfilters)
with `include: false`.
