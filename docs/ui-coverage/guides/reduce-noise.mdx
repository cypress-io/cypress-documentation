...
But sometimes what appears as an "untested element" by default isn't a gap your team would care about. It might
...
and [Ignore views and links](/ui-coverage/guides/ignore-views-and-links). This guide assumes you have already removed unwanted elements from the report. If you haven't, do that first, as it will make the rest of the work much easier.
:::
...
- **Elements identified by unstable attributes.** Selectors in the report are
  ...
  team recognizes. This can produce noise between pages, but also between runs when using Branch Review to [compare reports](/ui-coverage/guides/compare-reports).
...
The report now shows a single entry instead of three, named
`.cart [id^='remove-item-']` with a badge showing the number of grouped elements.
Hovering the badge reads _3 instances grouped by configuration_.
