...
For thresholds and rules you define in **App Quality** configuration, use [`getPolicyResults`](/ui-coverage/results-api#policy-results) from `@cypress/extract-cloud-results` so Cypress Cloud evaluates [policies](/ui-coverage/configuration/policies) consistently.

The examples below use [`getUICoverageResults`](/ui-coverage/results-api) for **custom** checks in your script (for example per-view rules or baseline files) that go beyond Cloud-defined policies.
