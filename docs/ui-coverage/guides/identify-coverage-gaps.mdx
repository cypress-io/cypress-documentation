...
The table is sorted by **Views** by default. Sort by **Coverage %** to bring your lowest-covered views to the top, then
...
- **Test Replay**: a menu of the tests that reached the snapshot, each opening [Test Replay](/cloud/features/test-replay) at that point, where you can see the element within the full test run instead of a single captured state.
...
Expanding an untested element shows its selector and navigates to an example snapshot if needed. It lists any views where the element appears without any interaction, so you can trace the same
element back to every page it shows up on.

For tested elements, the individual interactions
are shown and can be traced through to the tests themselves.
