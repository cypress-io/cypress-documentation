...
runs. Branch Review shows the delta as a badge — a positive value when the score
rose and a negative one when it fell, comparing the changed run's score against
the base run's.

This gives you a good sense of whether the overall ratio of tested to untested elements has changed.
It should help you understand if things are going off-track at a high level,
but it doesn't tell you anything about changes to the actual nature of what is tested.

Even if the score has not changed, it is important to review any new untested elements,
as they may represent missing coverage on new features.

For example, if a code change adds 2 buttons and only test one of them, that would introduce
one new tested element and one new untested at the same time - keeping the ratio, and the score, flat.
...
**Tip:** Reviewing the element diffs on a routine basis is incredibly useful. It will help your team:

- **Spread out maintenance** of your coverage into small increments
- **Take ownership** over changes in the test suite by explicitly deciding to ignore new elements or not
...
Clicking any new or resolved item opens a detail view of that item, where you see
every example of that element or link on a full-page, inspectable DOM snapshot,
with tested elements highlighted green and untested elements red. Items with the
same status on both runs are removed here, so you focus only on what changed.
...
differ only by numeric IDs or UUIDs automatically (`/orders/123` and `/orders/456`
become `/orders/*`), but other dynamic segments, such as slugs, are not grouped by
