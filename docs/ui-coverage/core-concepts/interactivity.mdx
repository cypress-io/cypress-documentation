...
`scrollIntoView` is a special case: it credits the element's nearest **scrollable ancestor**, not the element you called it on. That container must have a non-negative `tabindex` or be otherwise interactive to appear in the report.
