...
**Contact links** are the one deliberate exception: anchors whose `href` uses `tel:`, `mailto:`, or `sms:` are not treated as interactive.
...
An interactive element only counts toward your score if it was **visible in at least one snapshot** during the run. 

:::note

A control that only ever sits inside a collapsed menu your tests never open is not counted at all. It neither raises nor lowers your score. This is why your total element count can be lower than the number of controls you know exist in your markup. UI Coverage will flag the _trigger_ element as untested, as an unopened door.
:::
...
Only the closest `data-cy-ui-interactive` ancestor applies.
...
`scrollIntoView` is a special case: it credits the element's nearest **scrollable ancestor**, not the element you called it on. That container must have a non-negative `tabindex` or be otherwise interactive in order to appear in the report.
