...
1. **An element is counted when it is interactive and has been visible in the run**, which forms the denominator of your score.
2. **An interactive element is marked as tested when a recognized Cypress command targets it**, which forms the numerator. Links are also marked tested when a test visits their destination.
3. **A link is flagged as untested when its destination is never visited** during your run.

In short, your score is the share of interactive elements your tests exercise: **tested ÷ total**. Links share this total: each distinct destination counts as one unit. See [how the score is calculated](/ui-coverage/faq#How-is-the-UI-Coverage-score-calculated) for the details.
...
<DocsImage
  ...
  alt="Flow diagram: Test Replay captures DOM snapshots, UI Coverage analyzes each one to produce scoring points from every visible interactive element including links (the denominator), to credit interactions and link visits (the numerator), and to list untested elements and untested links separately as coverage gaps, all feeding the coverage score and report."
...
- **Natively interactive elements**: elements the browser already treats as interactive, such as `<a>` links, `<button>`, form controls like `<input>`, `<select>`, and `<textarea>`, `<label>`, and any element that is `contenteditable`. A `<label>` counts on its own and is [grouped with the control it identifies](/ui-coverage/core-concepts/element-grouping#Automatic-grouping).
- **Elements with an interactive ARIA role**: an explicit `role` such as `button`, `checkbox`, `tab`, or `menuitem`, or an `aria-haspopup` value that opens a menu or dialog. `role="textbox"` and `role="searchbox"` count only when the element is also `contenteditable`.
- **Tab-navigable elements**: any element made keyboard-focusable with a `tabindex` of `0` or greater.

An element with a widget role is skipped when it contains another interactive element, so a `role="button"` wrapper around a real `<button>` is counted once rather than twice.

**Contact links** are the one deliberate exception: anchors whose `href` uses `tel:`, `mailto:`, or `sms:` are not treated as interactive. This applies to the anchor rule only. A `mailto:` link that independently qualifies, for example by carrying `tabindex="0"` or an interactive `role`, is still tracked. See [Untested Links](#Untested-Links) for how the rest of an anchor's `href` is handled.

Because detection is based on semantics rather than event handlers, a real `<button>` is recognized automatically, while a `<div>` wired up with a click handler is not, until you give it interactive semantics or use the `data-cy-ui-interactive` attribute, described below.

### Visibility

An interactive element only counts toward your score if it was **visible in at least one snapshot** during the run. Visibility means the element passes a CSS visibility check and is the topmost element at its own center point once scrolled into view. Elements with `opacity: 0` still count as visible for this purpose, since they remain clickable (and teams sometimes use them as an invisible "interaction target" area for a card or similar control).

:::note

A control that only ever sits inside a collapsed menu your tests never open is not counted at all. It neither raises nor lowers your score. This is why your total element count can be lower than the number of controls you know exist in your markup.
:::
...
Only the closest `data-cy-ui-interactive` ancestor applies, which is what makes `reset` work: it sits between an `exclude-all` container and the subtree you want back. `reset` isn't a special keyword (any value other than `exclude` or `exclude-all` has the same effect), but use `reset` so the intent reads clearly.
...
`scrollIntoView` is a special case: it credits the element's nearest **scrollable ancestor**, not the element you called it on. If that ancestor isn't itself an interactive element, the command marks nothing as tested. Use `click`, `focus`, or `trigger` when you need to mark a specific element tested.
