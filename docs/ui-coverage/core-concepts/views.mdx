...
Views turn a run's raw coverage data into a page-by-page breakdown you can act on. If your report shows `/checkout` at 95% but `/settings/*` at 40% with 12 untested elements, you know exactly where the next test belongs. Views show you:
...
<DocsImage
  ...
  alt="UI Coverage tab in Cypress Cloud showing an 11% coverage score and a list of views named by URL path with columns for snapshots, tested elements, and coverage percentage, alongside navigation for untested links, tested elements, and untested elements"
/>
