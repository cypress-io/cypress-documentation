...
`getPolicyResults` resolves to an object with the following data:
...
```javascript
{
  ...
  policies: [{
    ...
    passed: boolean
    // The individual checks that make up this policy (see below).
    validationResults: ValidationResult[]
  }] | null
}
```

Each entry in `validationResults` reports:

- the metric it checked — one of `accessibility.score`, `uiCoverage.score`, `accessibility.failedElements`, or
  `uiCoverage.untestedElements`
- whether the check passed
- a human-readable message describing the result
- the measured value and the configured threshold it was compared against
- a link to the relevant report
- where relevant, the specific views that failed the check
