...
The package is distributed from the Cypress CDN rather than the public npm registry. Install the tarball for
the major version you want. The version is specified by the path segment in the URL (for example, `v1`):

```shell
npm install --force https://cdn.cypress.io/extract-cloud-results/v1/extract-cloud-results.tgz
```

The package exports three async functions with identical call signatures; `getUICoverageResults`, documented
...
```javascript
{
  ...
  runStatus: 'errored' | 'failed' | 'passed' | 'timedOut' | 'noTests' | 'running' | 'overLimit' | 'cancelled'
  ...
  summary: {
    ...
    // The number of links that were tested.
    testedLinksCount: number
    // The number of links that were not tested.
  ...
  views: [{
    ...
    untestedElementsCount: number
    // The number of links that were tested on this view.
    testedLinksCount: number
    // The number of links that were not tested on this view.
    untestedLinksCount: number
    // The url that links the report for this view.
    uiCoverageReportUrl: 'https://cloud.cypress.io/[...]'
  }]
  // The resolved UI Coverage configuration that was applied to this run.
  // Optional - omitted when no configuration was applied.
  config?: {
    // When the configuration was last updated.
    updatedAt: string
    // The configuration value that was applied.
    value: any
  }
}
```
...
The package also exports `getPolicyResults`, which evaluates the policies configured for your project against a
run. It uses the same call signature as `getUICoverageResults` and is product-agnostic. A single policy can
check both UI Coverage and Accessibility metrics.

```javascript
const { getPolicyResults } = require('@cypress/extract-cloud-results')

getPolicyResults().then((results) => {
  console.log(results)
})
```

`getPolicyResults` resolves to a `PolicyResults` object:

```typescript
type RunStatus =
  | 'errored' | 'failed' | 'passed' | 'timedOut'
  | 'noTests' | 'running' | 'overLimit' | 'cancelled'

type PolicyResults = {
  runNumber: number
  runUrl: string
  runStatus: RunStatus
  // 'evaluated' when the project has policies that were checked, or
  // 'not-configured' when the project has no policies (in which case `policies` is null).
  policiesStatus: 'evaluated' | 'not-configured'
  // The evaluated policies, or null when policiesStatus is 'not-configured'.
  policies: PolicyResult[] | null
}

type PolicyResult = {
  // The name of the policy.
  name: string
  // true only if every validation result for this policy passed.
  passed: boolean
  validationResults: PolicyValidationResult[]
}

// Every validation result shares these base fields.
type PolicyValidationResultBase = {
  // Whether this check passed.
  passed: boolean
  // A human-readable description of the result.
  message: string
  // A deep link to the relevant report.
  reportUrl: string
  // The view pattern the policy was scoped to, if any.
  viewMatch?: string
  // The number of views the policy matched, if applicable.
  matchedViewCount?: number
}

// A validation result is one of the following, discriminated by `metric`.
type PolicyValidationResult =
  // Score-based metrics compare a score against a minimum.
  | (PolicyValidationResultBase & {
      metric: 'accessibility.score' | 'uiCoverage.score'
      score: number
      minScore: number
      failedViews?: { viewName: string; score: number; viewUrl: string }[]
    })
  // Count-based metrics compare a count against a maximum.
  | (PolicyValidationResultBase & {
      metric: 'accessibility.failedElements' | 'uiCoverage.untestedElements'
      count: number
      maxCount: number
      failedViews?: { viewName: string; count: number; viewUrl: string }[]
    })
```

## Waiting for results

Reports are generated asynchronously after a run finishes. If you call `getUICoverageResults` before the report
has finished processing, the package does not fail. It polls for you automatically:

- It checks the report status, and while the report is still processing it waits and retries.
- Polling runs for up to 30 attempts at 30-second intervals, i.e. up to ~15 minutes.
- While polling, it logs progress (how many specs of the total have been processed) so your CI log shows it's
  still working.
- If the report is still processing after the maximum number of attempts, it throws a polling-exhausted error
  that includes the processed/total spec counts and a link to check the report, advising you to run the script
  again.

This means you can safely call `getUICoverageResults` immediately after your Cypress run completes, so you don't
need to add your own sleep/wait logic.

## Errors

All errors are thrown to the caller as plain `Error` objects with a clear, actionable message (stack traces are
stripped to avoid leaking internal paths). The errors you may encounter include:
...
[tarball for the correct major version](#installation) to resolve this.
