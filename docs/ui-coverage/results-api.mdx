...
If you use config [profiles](/ui-coverage/configuration/profiles) to produce a different config based on run tags, resolve the effective config first by matching the run's tags against profile names:
...
getUICoverageResults({ runTags: RUN_TAGS }).then(({ config }) => {
  ...
  // The first profile whose name matches a run tag wins. Its config replaces
  // the base value for the properties it defines, so check it first.
  const activeProfile = (config.value.profiles ?? []).find((profile) =>
    RUN_TAGS.includes(profile.name)
  )
  const effective = activeProfile?.config ?? config.value
