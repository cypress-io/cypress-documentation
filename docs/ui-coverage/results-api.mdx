...
- **Read and assert the configuration** that was in place when this UI Coverage report was processed.
...
getUICoverageResults({
  ...
  // Optional if the CYPRESS_RUN_TAG env var is set. That variable holds a
  // single tag only (it is not comma-separated), so pass runTags directly
  // when a run carries more than one tag.
  runTags: string[]
})
```
...
`getUICoverageResults` resolves with the following data. A **link** is a navigable
anchor that points at another view; **elements** are the other interactive
controls. Both count toward the coverage score, but the counts below report them
separately, so `testedElementsCount` plus `untestedElementsCount` is **not** the
score's denominator. Add the link counts to reconstruct it.
...
```javascript
{
  ...
  // The App Quality configuration used for this report.
  // Present when configuration exists for the project.
  config: {
    // ISO 8601 timestamp of when the configuration was last updated.
    updatedAt: string
    // The App Quality configuration saved for the project when this report was processed.
    value: object
  }
}
```
  ...
  views: [
    {
      ...
      uiCoverageReportUrl:
        'https://cloud.cypress.io/projects/ypt4pf/runs/68086/ui-coverage/view/a1b2c3d4e5f60718',
    },
    {
      ...
      uiCoverageReportUrl:
        'https://cloud.cypress.io/projects/ypt4pf/runs/68086/ui-coverage/view/9f8e7d6c5b4a3021',
    },
  ],
  config: {
...
The `config` property returns the **App Quality Config** saved for your project when this report was processed, including the full `profiles` array.

:::caution

`config.value` is **not** resolved against the run's tags. If a
[Profile](/ui-coverage/configuration/profiles) applied to this run, `config.value`
still shows the base configuration plus the profile definitions, not the merged
result. If you need to determine which profile applied, match the run's tags against the
`name` of each entry in `config.value.profiles`. Reach out to your Cypress point-of-contact
or use the in-product feedback buttons if you need more than the current functionality.
...
UI Coverage reports links (navigable anchors that point at another view)
separately from other interactive elements, though both count toward your score. Use the link counts to catch pages that have user-facing links rendered in Cypress tests, but are never visited:
...
A run can be canceled, time out, or produce a partial report. Check the run
...
The `config` property returns the App Quality Config that was used to process this run. You can assert that required rules are present so a configuration change doesn't
silently alter what's measured.

If you use config [profiles](/ui-coverage/configuration/profiles) to produce a different config based on run tags, resolve the effective config first: match the run's tags against profile names, then layer the matching profile's `config` over the base configuration, since a profile replaces only the properties it defines.

```javascript title="scripts/assertCoverageConfig.js"
const { getUICoverageResults } = require('@cypress/extract-cloud-results')

// Tags this run was recorded with. CYPRESS_RUN_TAG holds a single tag only;
// pass runTags directly when a run carries more than one.
const RUN_TAGS = process.env.CYPRESS_RUN_TAG
  ? [process.env.CYPRESS_RUN_TAG]
  : []

getUICoverageResults({ runTags: RUN_TAGS }).then(({ config }) => {
  if (!config) {
    throw new Error('No App Quality Config was saved for this run.')
  }

  // The first profile whose name matches a run tag wins.
  const base = config.value
  const activeProfile = (base.profiles ?? []).find((profile) =>
    RUN_TAGS.includes(profile.name)
  )

  // A profile replaces only the properties it defines, and `uiCoverage` and
  // `accessibility` merge one level deep, so layer the profile over the base
  // config rather than reading it on its own.
  const overrides = activeProfile?.config ?? {}
  const effective = {
    ...base,
    ...overrides,
    uiCoverage: { ...base.uiCoverage, ...overrides.uiCoverage },
  }

  const elementFilters = effective.uiCoverage.elementFilters ?? []
