...
When you define [policies](/ui-coverage/configuration/policies) in App Quality configuration, Cypress Cloud evaluates them after **UI Coverage** and **Accessibility** reports are processed. The `@cypress/extract-cloud-results` package exposes **`getPolicyResults`** so your CI job can retrieve that evaluation without re-implementing threshold logic.

For what you can configure, see [policies](/ui-coverage/configuration/policies) (shared with Accessibility).

### Installation and CI context

Use the same installation steps and CI provider support as elsewhere on this page. `getPolicyResults` resolves the Cypress run from the current CI environment and polls until policy evaluation finishes (or surfaces an error), analogous to `getUICoverageResults` / `getAccessibilityResults`.
...
Use `getUICoverageResults` when you need custom logic (for example comparing against an external baseline file) that goes beyond fixed numeric thresholds, or you want to log more details based on a policy failure.

For end-to-end examples of failing CI when standards are not met, see [Block pull requests and set policies](/ui-coverage/guides/block-pull-requests).
