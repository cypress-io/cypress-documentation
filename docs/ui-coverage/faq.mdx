...
- **[Grouped elements](/ui-coverage/core-concepts/element-grouping) count as a single unit.** The group counts once toward the total, and an interaction with any element in the group marks the whole group as tested. Fifty identical buttons are one unit, not fifty.
- **Links share the same total.** Each distinct link destination counts as one unit, and counts as tested when a test either interacts with the link or visits its destination. Untested links lower your score. They're reported in a separate list from untested elements, so `testedElementsCount` plus `untestedElementsCount` won't add up to the score's total on its own — add the link counts to get the true total.
- **Only [visible](/ui-coverage/core-concepts/interactivity#Visibility) elements count.** An element that was never visible in any snapshot is excluded from both the tested and total counts, so it neither raises nor lowers your score. If the element
...
Yes. The result object includes a `config` property containing the [App Quality Config](/ui-coverage/configuration/overview) as saved for the project when the run was processed. Read `config.value` for the rules and `config.updatedAt` for when that configuration last changed. Note that `config.value` is not resolved against the run's tags, so if a [Profile](/ui-coverage/configuration/profiles) applied you match the run's tags against profile names yourself — see [Assert the saved configuration](/ui-coverage/results-api#Assert-the-saved-configuration).
