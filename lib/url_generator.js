...
function getLocalFilePath (sidebar, href, source, fullUrl) {
...
function validateLocalFile (sidebar, href, source) {
  const hash = extractHash(href)

  const pathToFile = getLocalFilePath(sidebar, stripHash(href), source, href)

  return Promise.resolve(`/${pathToFile}${hash}`)
}

function validateAndGetUrl (sidebar, href, source, text) {
  if (!href) {
  ...
  return keyv.get(href).then((cachedValue) => {
    ...
    return validateLocalFile(sidebar, href, source)
    .then((pathToFile) => {
