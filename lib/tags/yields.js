...
module.exports = function yields (hexo, args) {
  ...
  // {% yields changesDomSubjectOrSubjects .children %}
  ...
  const changesDomSubject = () => {
    return `<ul>
      <li><p>${cmd} yields the new DOM element it found.</p></li>
    </ul>`
  }

  const changesDomSubjectOrSubjects = () => {
    return `<ul>
  ...
  switch (type) {
    ...
    case 'changes_dom_subject_or_subjects':
      return changesDomSubjectOrSubjects()
