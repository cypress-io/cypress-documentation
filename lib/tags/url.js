...
function getUrlProps (hexo, href, text) {
  ...
  return urlGenerator.validateAndGetUrl(sidebar, href, this.full_source, text)
}
...
function url (hexo, args) {
  ...
  return Promise.resolve(hexo.render.render({ text: props.text, engine: 'markdown' }))
  .then((markdown) => {
    // remove <p> and </p> and \n
    markdown = markdown
    .split('<p>').join('')
    ...
    .catch(() => {
      /* eslint-disable no-console */
      console.error(`Request to: ${props.url} failed.`)
      /* eslint-enable no-console */
