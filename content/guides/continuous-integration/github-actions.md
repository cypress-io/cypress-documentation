...
The Cypress team maintains the official [Cypress GitHub Action](https://github.com/marketplace/actions/cypress-io) for running Cypress tests. This action provides npm installation, custom caching and additional configuration options &mdash; simplifying the setup of advanced workflows with Cypress and GitHub Actions.

## Basic Setup

The example below is a basic CI setup and job using the [Cypress GitHub Action](https://github.com/marketplace/actions/cypress-io) to run Cypress tests within the Electron browser. This configuration is placed within `.github/workflows/main.yml`.
...
- On `git push` to this repository, this job will provision and start the GitHub-hosted Ubuntu Linux instance. It will then run the outlined `steps` for the declared `cypress-run` job within the `jobs` section of the configuration.
...
- Finally, our Cypress GitHub Action will:
  ...
  - Run the Cypress tests in Electron.
...
Dependencies and build artifacts can be cached between jobs using the [cache](https://github.com/marketplace/actions/cache) GitHub Action.

The job below includes a cache of:

- `node_modules`
- `~/.cache/Cypress` - the Cypress binary
- The `build` directory

In addition, the `build` attribute is added to the Cypress GitHub Action to generate the build artifacts prior to the test run.

<Alert type="info">

Caching of dependencies and build artifacts can be accomplished with the GitHub Actions [ Upload/Download Artifact Actions](https://docs.github.com/en/actions/guides/storing-workflow-data-as-artifacts). However, uploading takes more time when using the [cache GitHub Action](https://github.com/marketplace/actions/cache).
...
GitHub Actions offers a [matrix strategy](https://docs.github.com/en/actions/reference/workflow-syntax-for-github-actions#jobsjob_idstrategymatrix) for declaring different job configurations for a single job definition. Jobs declared within a matrix strategy can run in parallel &mdash; enabling us to run multiples instances of Cypress at same time.
...
It is important to separate the install job from the worker job when running in parallel. By doing this, we can reuse the cached artifacts from the install job in the subsequent worker jobs &mdash; speeding up our pipelines.
...
Using our [Cypress GitHub Action](https://github.com/marketplace/actions/cypress-io) we specify `install: false` since our dependencies and build were cached during the `install` job.
...
1. [Recording test results with the `record: true` option](https://on.cypress.io/how-do-i-record-runs) to the [Cypress Dashboard](https://on.cypress.io/dashboard):
   ...
   - Visibility into test failures to quickly access error messages, stack traces, screenshots, videos and contextual details.
