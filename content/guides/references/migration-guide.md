...
- "Run all specs"
- "Run filtered specs"
- the experimental "Cypress Studio"
...
Because of this, support for `cypress.json` has been fully removed.
Documentation for `cypress.json` is now available in the
...
- Cypress now requires a config file, so specifying `--config-file false` on the
  ...
  valid.
- You can not have both `cypress.config.js` _and_ `cypress.config.ts` files.
  This will result in an error when Cypress loads.
- A
...
Because of this, support for the plugins file has been removed. It has been
...
- The `cypress/plugins/index.js` plugins file is no longer automatically loaded
  by Cypress.
- `setupNodeEvents` is functionally equivalent to the function exported from the
  plugins file; it takes the same `on` and `config` arguments, and should return
  the same value.
- The
  [`devServer` and `devServerConfig`](/guides/references/configuration#devServer-devServerConfig)
  options are specific to component testing, and offer a much more streamlined
  and consistent way to configure a dev server than using the plugins file.
- Many pages and examples throughout the documentation have been updated to show
