...
Note, that Cypress allows you to optionally specify CA / client certificate information for use with HTTPS sites. See [Configuring security certificates](/guides/references/certificates). If for a configured URL, the remote server requests a client certificate, Cypress will present it.
