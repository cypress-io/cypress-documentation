...
- When `any` type value is returned from the last function, then the argument
  will no longer be wrapped with `JQuery`. Fixes
...
- You can now configure certificate authority (CA) and client certificates to
  use within tests on a per-URL basis via a `clientCertificates` . See
