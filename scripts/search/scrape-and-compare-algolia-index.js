...
const ALGOLIA_INDEX = require('./config.json').index_name
...
const scrape = async () => {
  try {
    ...
    console.log('Scraper will not log any output until it completes. Please wait...')
    console.time('scraper')
    const { stdout, stderr } = await exec(`API_KEY=${API_KEY} APPLICATION_ID=${APPLICATION_ID} sh run-scraper.sh`)
    if (stdout) {
      console.log(stdout)
    }
    if (stderr) {
      console.log('Output from stderr (this may or may not contain actual errors): ')
      console.log(stderr)
    }
  } finally {
...
const main = async () => {
  console.log(`Comparing entries for index: ${ALGOLIA_INDEX}`)
	const countBeforeScraper = await algoliaClient.getEntriesForIndex(ALGOLIA_INDEX)
  console.log(`Entries count before scraper: ${countBeforeScraper}`)
  await scrape()
  const countAfterScraper = await algoliaClient.getEntriesForIndex(ALGOLIA_INDEX)
  ...
  if (delta > ACCEPTABLE_DELTA) {
    console.error(`Delta after scrape is higher than the acceptable delta of ${ACCEPTABLE_DELTA}. Delta: ${delta}`)
    console.error('Check the config.json file for any changes and check the scraper logs for any errors.')
    process.exit(1)
  }

  console.log(`Delta after scrape was found to be within the acceptable delta of ${ACCEPTABLE_DELTA}. Delta: ${delta}`)
  process.exit(0)
}
