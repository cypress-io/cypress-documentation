...
const createFocusListItem = cmd => `${codify(cmd)} requires the element to be able to receive focus.`
const createChildCommandListItem = cmd => `${codify(cmd)} requires being chained off a previous command.`
const createChildCommandDOMListItem = cmd => `${codify(cmd)} requires being chained off a command that yields DOM element(s).`
const createDualCommandListItem = cmd => `${codify(cmd)} can bechained off of ${codify('cy')} or off another command.`
const createParentCommandListItem = cmd => `${codify(cmd)} requires being chained off of ${codify('cy')}.`
...
const createChildListItems = cmd => [
  createChildCommandListItem(cmd)
]

const createCheckabilityListItems = cmd => [
  createChildCommandDOMListItem(cmd),
  `${codify(cmd)} requires the element to have type ${codify('checkbox')} or ${codify('radio')}.`
]

const createClearabilityListItems = cmd => [
  createChildCommandDOMListItem(cmd),
  `${codify(cmd)} requires the element to be an ${codify('input')} or ${codify('textarea')}.`
]

const createDOMListItems = cmd => [
  (cmd === 'cy.get' || cmd === 'cy.focused') ? createParentCommandListItem(cmd) : createChildCommandDOMListItem(cmd)
]

const createDualListItems = cmd => [
  createDualCommandListItem(cmd)
]

const createDualExistenceListItems = cmd => [
  `${codify(cmd)} can be chained off of ${codify('cy')} or off a command that yields DOM element(s).`
]

const createDualExistenceSingleDOMListItems = cmd => [
  `${codify(cmd)} can be chained off of ${codify('cy')} or off a command that yields a single DOM element.`
]
...
const createExecListItems = cmd => [
  ...
  `${codify(cmd)} requires that the exit code be ${codify('0')} when ${codify('failOnNonZeroExit')} is ${codify('true')}.`
]
...
const createParentListItems = cmd => [
  createParentCommandListItem(cmd)
]

const createPageListItems = cmd => [
  createParentCommandListItem(cmd),
  `${codify(cmd)} requires the response to be ${codify('content-type: text/html')}.`,
  `${codify(cmd)} requires the response code to be ${codify('2xx')} after following redirects.`,
  `${codify(cmd)} requires the load ${codify('load')} event to eventually fire.`
]
...
const createRequestListItems = cmd => [
  ...
  `${codify(cmd)} requires that the response status code be ${codify('2xx')} or ${codify('3xx')} or ${codify('failOnStatusCode')} is ${codify('true')}.`
]
...
const createShadowDOMListItems = cmd => [
  `${codify(cmd)} requires being chained off a command that yields a DOM element that is a shadow host (i.e. has a shadow root directly attached to it).`
]

const createSpreadListItems = cmd => [
  createChildCommandListItem(cmd),
  `${codify(cmd)} requires being chained off a command that yields an array-like structure.`
]
...
const createWaitListItems = cmd => [
  ...
  `When passed an ${codify('alias')} argument ${createParentCommandListItem(cmd)}.`
]

const createWriteFileListItems = cmd => [
  createParentCommandListItem(cmd),
  `${codify(cmd)} requires the file be successfully written to disk. Anything preventing this such as OS permission issues will cause it to fail.`
]
