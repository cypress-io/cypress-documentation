...
<strong className="alert-header">Best Practices</strong>
...
<strong className="alert-header">Example Recipe</strong>
...
<Icon name="exclamation-triangle"></Icon> Keep in mind, when clicking "Run all specs"
after [cypress open](/guides/guides/command-line#cypress-open), the code in the support
file is executed once before all spec files, instead of once before each spec file.
See [Execution](#Execution) for more details.
...
<DocsImage
  src="/img/guides/global-hooks.png"
...
<Icon name="exclamation-triangle"></Icon> Before writing `after()` or `afterEach()`
...
<Icon name="exclamation-triangle"></Icon> Be wary of root-level hooks, as they could
execute in a surprising order when clicking the "Run all specs" button. Instead place
them inside `describe` or `context` suites for isolation. Read [Be careful when running
all specs together](https://glebbahmutov.com/blog/run-all-specs/).
...
<Icon name="exclamation-triangle" color="red"></Icon> **Note:** Some configuration
values are readonly and cannot be changed via test configuration. The following configuration
values **can be changed** via per test configuration:
...
<DocsImage
  src="/img/guides/core-concepts/run-all-specs.gif"
