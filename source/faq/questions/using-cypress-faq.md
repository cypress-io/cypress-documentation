...
Once you {% url "set up your tests to record" runs-dashboard %}, we generate a unique `projectId` for your project and automatically insert it into your `cypress.json` file.
...
Once you're {% url "set up to record test runs" runs-dashboard %}, we automatically generate a *Record Key* for the project.
