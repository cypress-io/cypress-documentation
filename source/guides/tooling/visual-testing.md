...
Below we stub network calls using {% url "`cy.http()`" http %} to return the same response data for each XHR request. This ensures that the data displayed in our application images does not change.

```js
cy.http('/api/items', { fixture: 'items' }).as('getItems')
// ... action
cy.wait('@getUsers')
cy.mySnapshotCommand()
```
