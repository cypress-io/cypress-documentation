...
典型情况下，你一般需要起一个本地服务来运行Cypress。当你启动完你的服务的时候，它会像一个**长期运行进程**那样运行，永不退出。因为这样，你需要它运行在**后台**。否则你的CI平台将永远不会继续执行下一条指令。
...
你需要参考CI提供商的文档，了解如何设置在CI环境中配置多台计算机。

在CI环境中有多台计算机可用后，你可以传递{% url "`--parallel`" command-line#cypress-run-parallel %}参数以使你的测试并行运行。
