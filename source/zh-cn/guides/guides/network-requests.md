...
Cypress使在应用程序内测试Ajax/XHR请求的整个生命周期变得更加容易。Cypress提供了对XHR对象的直接访问，允许你使用其属性进行断言。此外，你甚至可以打桩和模拟请求的响应。
...
在Cypress中，你可以选择是打桩响应，还是允许它们实际访问你的服务器。你还可以在同一个测试中通过选择打桩某些请求，同时允许其他请求访问你的服务器来进行混合搭配。

让我们来研究一下这两种策略：为什么你会使用其中一种而不是另一种，以及为什么你要经常使用这两种策略。

## 不打桩响应

没有打桩的请求实际上会到达服务器。通过*不*打桩你的响应，你编写的才是真正的“端到端”测试。这意味着你以与真实用户相同的方式驱动应用程序。

> 当请求没有打桩时，这将确保客户端和服务器之间的*契约*正常工作。

换句话说，你可以确信你的服务器正在以正确的结构发送正确的数据供你的客户端使用。围绕应用程序的*关键路径*进行*端到端*测试是一个好主意。这些通常包含登录、注册，或其他关键路径，如计费。

***你应该意识到不打桩响应的不足之处：***

- 因为没有打桩响应，这意味着 **你的服务器必须实际发送真实的响应**。这可能会有问题，因为你可能必须在每一个测试前*播种一个数据库*才能生成状态。例如，如果你要测试*分页*，你必须要为用于复制应用程序中此功能的每一个对象播种数据库。
- 因为真实的响应要经过服务器的每一层（控制器、模型、视图等）所以测试通常比打桩响应*慢得多*。

如果你正在写一个传统的服务器端应用程序，多数的响应是`HTML`，可能很少有桩响应。可是，大多数提供`JSON`服务的现代应用程序都可以利用打桩。
...
打桩响应使你能够控制响应的各个方面，包括响应`body`、`status`、`headers`，甚至网络`delay`。桩速度非常快，大多数响应将在小于20毫秒内返回。

> 打桩响应是一个控制返回客户端数据的好方法。

你不需要在服务器上做任何操作。你的应用程序将不知道它的请求被打桩，因此*不需要更改代码*。
...
{% note danger 缺点 %}
- 不能保证你的打桩响应与服务器发送的实际数据相同
- 某些服务器端点没有测试覆盖
- 如果你正在使用传统的服务器端HTML渲染，则不太适用
{% endnote %}
...
Cypress自动收集请求的`headers`和请求的`body`并将这些提供给你。
...
当你启动一个{% url `cy.server()` server %}并定义{% url `cy.route()` route %}命令时，Cypress会在命令日志中的“Routes”下显示该命令。
...
当打桩响应时，通常需要管理潜在的大型复杂JSON对象。Cypress允许你将固件语法直接集成到响应中。
...
你还可以在响应中引用{% url '别名' variables-and-aliases %}。这些别名不必指向固件，但这是一个常见的用例。分离固件使你能够在将该对象传递给响应之前对其进行操作和修改。
...
Cypress会自动搭建一个推荐的文件夹结构，用于在每个新项目中组织固件。默认情况下，当你将项目添加到Cypress时，会创建一个`example.json`文件。

```text
/cypress/fixtures/example.json
```

固件可以在其他文件夹中进一步组织。例如，你可以创建另一个名为`images`的文件夹并添加图像：
...
无论你是否选择打桩响应，Cypress都允许你声明性地使用{% url `cy.wait()` wait %}来处理请求及其响应。

{% note info %}
下面的部分使用了一个{% url '别名' variables-and-aliases %}的概念。如果你是Cypress的新手，你最好先看看这个。
{% endnote %}
...
声明式等待响应的一个优点是它减少了测试不稳定性。你可以将{% url `cy.wait()` wait %}看作一个警卫，当你希望发出一个匹配特定路由别名的请求时，它会通知Cypress。这将阻止下一个命令在响应返回之前运行，并防止出现请求最初被延迟的情况。

***自动完成的例子：***

下面这个例子之所以如此强大，是因为Cypress将自动等待匹配`getSearch`别名的请求。你可以测试结果的实际原因，而不是强制Cypress测试成功请求的副作用(显示Book结果)。
...
使用Cypress，通过添加一个{% url `cy.wait()` wait %}，你可以更容易地确定你的特定问题。如果响应没有返回，你将收到如下错误：
...
对请求使用{% url `cy.wait()` wait %}的另一个好处是，它允许你访问实际的`XHR`对象。当你想对这个对象进行断言时，这是非常有用的。
