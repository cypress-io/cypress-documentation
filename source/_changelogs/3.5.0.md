...
- We now record video during `cypress run` in Chrome, Chromium, and Canary browsers. This requires passing the the name or path of the Chrome browser to the {% url "`--browser` flag" command-line#cypress-run-browser-lt-browser-name-or-path-gt %}. Addresses {% issue 4608 %}.
...
- {% url "`.dblclick()` dblclick %} now accepts `position`, `x`, and `y` arguments.
- {% url "`.dblclick()` dblclick %} now accepts options `force` and `multiple` in it's `options`. Addresses {% issue 116 %}.
- {% url "`.screenshot()`" screenshot %} now accepts a `padding` argument for element screenshots that will alter the dimensions of the screenshot of the element. Addresses {% issue 4440 %}.
...
- {% url "`.dblclick()` dblclick %}, {% url "`.click()`" click %} now fire all appropriate mouse events including `mouseover`, `mousemove`, `mouseout`, `pointerdown`, `pointerup`, and `pointermove`. Fixes {% issue 1016 %}, {% issue 1847 %}, {% issue 2045 %}, {% issue 2521 %}, {% issue 2768 %}, {% issue 2928 %}, {% issue 2957 %}, and {% issue 3224 %}.
...
- Cypress now send `screenX` and `screenY` properties to events triggered using {% url "`.trigger()`" trigger %}. Fixes {% issue 3187 %}.
...
- We fixed a bug where Cypress would exit with an `ENOTCONN` error at the end of a test run when run using Node >12.11.0 on Windows OS. Fixes {% issue 5241 %}.
...
- We improved the way that cookies are handled for Chrome browsers. This fixes an issue when parsing set cookies with a domain beginning with a `.` in Chrome 77. Fixes {% issue 5142 %} and {% issue 5206 %}.
...
- We now throw a more helpful error message during `.screenshot()` that would result in a screenshot with 0 width or height. Addresses {% issue 5149 %}.
- We've added more flags to send to Chrome upon launch that should help alleviate issues encountered during browser automation including `--disable-backgrounding-occluded-window`, `--disable-breakpad`, `--disable-ipc-flooding-protection`, `--password-store=basic`, and `--use-mock-keychain`. Addresses {% issue 5132 %} and {% issue 3633 %}.
...
- Updated typing for {% url "`Cypress.Commands.overwrite()`" custom-commands %} to not allow an `options` object as an argument. Addresses {% issue 5341 %}.
