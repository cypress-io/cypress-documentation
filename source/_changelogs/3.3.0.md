...
- The {% url "`cy.clock()`" clock %} type definition now allows for type `Date`. Fixes {% PR 4106 %}.
...
- Updated {% url "`.its()`" its#Assertions %} and {% url "`.invoke()`" invoke#Assertions %} docs to better explain how default assertions are handled. Addresses {% url "#1643" https://github.com/cypress-io/cypress-documentation/issues/1643 %}.
