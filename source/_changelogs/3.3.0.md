...
- Assertions running outside of a test are not properly handled in Cypress instead of throwing internal errors. Fixes {% issue 3926 %}.
...
- We downgraded `parse-domain` from 2.1.7 to 2.0.0 to avoid a regression released in `parse-domain`. Addresses {% issue 3717 %}.
